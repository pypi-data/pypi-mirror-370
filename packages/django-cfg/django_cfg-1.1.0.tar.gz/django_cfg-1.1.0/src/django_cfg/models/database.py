...
class DatabaseConnection(BaseModel):
    ...
    def validate_connection_consistency(self) -> 'DatabaseConnection':
        ...
        if self.engine == 'django.db.backends.sqlite3':
            if self.name not in [':memory:', ''] and not (
                self.name.endswith(('.db', '.sqlite', '.sqlite3')) or 
    ...
    def to_django_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            raise DatabaseError(
                ...
                database_alias=getattr(self, '_alias', 'unknown'),
                context={'config': self.model_dump()}
            ) from e
...
class DatabaseRoutingRule(BaseModel):
    """
    Type-safe database routing rule for multi-database setups.
    
    Defines which Django apps should use which database for different operations.
    """
    ...
    apps: List[str] = Field(
        ...,
        description="List of Django app labels for this rule",
        min_length=1,
    )
    ...
    migrate_to: Optional[str] = Field(
        default=None,
        description="Override database for migrations (if different from main database)",
    )
    ...
    def validate_apps(cls, v: List[str]) -> List[str]:
        ...
        if not v:
            raise ValueError("At least one app must be specified")
    ...
    def matches_app(self, app_label: str) -> bool:
        """
        Check if this rule matches the given app label.
        
        Args:
            app_label: Django app label to check
            
        Returns:
            True if this rule applies to the app
        """
    ...
    def get_migration_database(self) -> str:
        ...
        Returns:
            Database alias for migrations
        """
        return self.migrate_to or self.database
    
    def to_router_config(self) -> Dict[str, Any]:
        """
        Convert to database router configuration format.
        
        Returns:
            Configuration for Django database router
        """
        return {
            'apps': self.apps,
