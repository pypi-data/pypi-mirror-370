...
class SettingsGenerator:
    ...
    def _generate_database_settings(cls, config: 'DjangoConfig') -> Dict[str, Any]:
        ...
        try:
            ...
            if config.database_routing:
                ...
                # Convert rules to simple app_label -> database mapping
                routing_rules = {}
                for rule in config.database_routing:
                    for app in rule.apps:
                        routing_rules[app] = rule.database
        ...
        try:
            ...
            if config.cache_sessions:
                caches['sessions'] = config.cache_sessions.to_django_config(
                    config._environment, config.debug, 'sessions'
                )
            ...
            for attr_name in dir(config):
                if attr_name.startswith('cache_') and attr_name not in ['cache_default', 'cache_sessions']:
                    ...
                    if hasattr(cache_obj, 'to_django_config'):
                        ...
                        caches[cache_alias] = cache_obj.to_django_config(
                            config._environment, config.debug, cache_alias
                        )
        ...
        try:
            ...
            if config.security_domains:
                security_defaults = SmartDefaults.get_security_defaults(
                    config.security_domains, config._environment, config.debug
                )
    ...
    def _generate_email_settings(cls, config: 'DjangoConfig') -> Dict[str, Any]:
        ...
        try:
            ...
            if config.email:
                email_settings = config.email.to_django_config(
                    config._environment, config.debug
                )
                ...
                revolution_settings = {
                    "DJANGO_REVOLUTION": {
                        ...
                        "zones": {
                            ...
                            for zone_name, zone_config in config.revolution.zones.items()
                        } if hasattr(config.revolution, 'zones') else {},
                    }
                }
