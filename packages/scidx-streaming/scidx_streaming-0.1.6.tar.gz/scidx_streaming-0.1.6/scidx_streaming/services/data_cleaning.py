        ...
        try:
            if condition.startswith("window_filter"):
                ...
                if match:
                    ...
                    affected_column = parse_condition(condition_str)[0]
                ...
                # Try to parse; if it’s a simple condition we can branch on operator
                try:
                    field_for_parse, op_for_parse, _ = parse_condition(condition)
                    if op_for_parse == 'IN':
                        # Drop non-matching rows (row filtering for IN)
                        filtered_df = filtered_df[matches]
                    else:
                        # Column-wise masking for other operators
                        filtered_df.loc[~matches, field_for_parse] = placeholder_value
                except Exception:
                    # If it's a complex expression we can't parse, keep prior behavior (no row drop)
                    pass

            # Keep your typing pass
