...
async def process_url_stream(stream, filter_semantics, buffer_lock, send_data, loop, stop_event):
    ...
    while retries < RETRY_LIMIT:
        try:
            if file_type == 'TXT':
            ...
            elif file_type.upper() == "RSS":
                logger.info(f"Fetching RSS feed from {resource_url}...")
                ...
                logger.info(f"Finished processing RSS stream.")
                return
...
async def process_rss_stream(stream, processing, mapping, send_data, buffer_lock, loop, filter_semantics, stop_event):
    url = stream["resources"][0]["url"]
