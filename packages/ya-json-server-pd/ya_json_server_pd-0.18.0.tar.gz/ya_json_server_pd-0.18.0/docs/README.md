...
(e.g.: `my-db-data.json`)  
<details open>
<summary>Case with one "resources" (`posts`).
</summary>
...
<details open>
<summary>Multiple "resources" (`posts`, `comments`, `products`, and `stoke-exchange`).
</summary>
...
```shell
docker run --detach -p 80:8000 -v ./my-db-data.json:/home/worker/data/db.json registry.gitlab.com/adrianovieira/ya-json-server-pd:v1.0
```
