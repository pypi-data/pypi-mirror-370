...
impl StreamManager {
    ...
    async fn add_stream(&self, session_id: uuid::Uuid, stream: Box<dyn SttStream>) {
        self.streams.lock().await.insert(session_id, stream);
    }
...
impl PyStreamingResultIterator {
    ...
    fn __anext__<'py>(slf: PyRef<'_, Self>, py: Python<'py>) -> PyResult<Option<&'py PyAny>> {
        ...
        let fut = pyo3_asyncio::tokio::future_into_py(py, async move {
            ...
            let mut stream = match stream_manager.take_stream(&session_id).await {
                Some(s) => s,
                None => {
            ...
            match result {
                Ok(Some(streaming_result)) => {
                    // Put the stream back for next iteration
...
impl PyVoiceProcessor {
    ...
    fn stream_audio<'py>(
        ...
        future_into_py(py, async move {
            // Get the stream temporarily to send audio
            let mut stream = match stream_manager.take_stream(&session_uuid).await {
                Some(s) => s,
                None => {
            ...
            match stream.send_audio(&audio_chunk).await {
                Ok(_) => {
                    // Put the stream back
                    stream_manager.add_stream(session_uuid, stream).await;
                    Ok(Python::with_gil(|py| py.None()))
                }
                Err(e) => {
                    // Put the stream back even on error
                    stream_manager.add_stream(session_uuid, stream).await;
    ...
    fn stop_streaming_transcription<'py>(
        ...
        future_into_py(py, async move {
            ...
            if let Some(mut stream) = stream_manager.remove_stream(&session_uuid).await {
