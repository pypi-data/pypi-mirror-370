...
# 导入依赖包
try:
    from icrawler.builtin import BingImageCrawler, BaiduImageCrawler, GoogleImageCrawler, FlickrImageCrawler, GreedyImageCrawler, UrlListCrawler
    from PIL import Image
...
class ImageDownloader:
    ...
    def download_from_flickr(self, keyword, num_images=20, api_key=None, api_secret=None, tag_mode="any"):
        """
        从Flickr搜索并下载图片
        ...
        # 转换所有图片为jpg格式并使用哈希文件名，保存到对应关键词目录
        converted = self._convert_images_to_jpg_with_hash(temp_dir, keyword, keyword_dir, "flickr", url_mappings)
    ...
    def download_from_website(self, urls, keyword="website_images", num_images=20, allowed_domains=None):
        """
        从指定网站抓取所有图片（贪婪模式）
        ...
        url_mappings = []
        if self.save_mapping:
            url_mappings = self._extract_urls_from_temp_dir(temp_dir, keyword, "website")
        
        # 转换所有图片为jpg格式并使用哈希文件名，保存到对应关键词目录
        converted = self._convert_images_to_jpg_with_hash(temp_dir, keyword, keyword_dir, "website", url_mappings)
    ...
    def download_from_urls(self, url_list, keyword="url_images", num_images=None):
        ...
        # 转换所有图片为jpg格式并使用哈希文件名，保存到对应关键词目录
        converted = self._convert_images_to_jpg_with_hash(temp_dir, keyword, keyword_dir, "urls", url_mappings)
    ...
    def download_from_unsplash(self, keyword, num_images=20, per_page=30):
            ...
            while downloaded_count < num_images:
                try:
                    ...
                    for img in img_elements:
                        src = img.get('src', '')
                        # 过滤出Unsplash的图片URL
                        if 'images.unsplash.com' in src and ('photo-' in src or 'unsplash-' in src):
                            # 尝试获取更高质量版本
                            if '?ixlib=' in src:
                                # 修改URL参数获取更大尺寸
                    ...
                    # 下载图片
                    for img_url in found_images[:min(20, num_images - downloaded_count)]:
                        ...
                        try:
                            ...
                            if img_response.status_code == 200:
                                ...
                                downloaded_count += 1
                                print(f"    下载图片 {downloaded_count}/{num_images}: {filename}")
        ...
        url_mappings = []
        if self.save_mapping:
            url_mappings = self._extract_urls_from_temp_dir(temp_dir, keyword, "unsplash")
        
        converted = self._convert_images_to_jpg_with_hash(temp_dir, keyword, keyword_dir, "unsplash", url_mappings)
    ...
    def download_from_pixabay(self, keyword, num_images=20, category="all"):
        ...
        url_mappings = []
        if self.save_mapping:
            url_mappings = self._extract_urls_from_temp_dir(temp_dir, keyword, "pixabay")
        
        converted = self._convert_images_to_jpg_with_hash(temp_dir, keyword, keyword_dir, "pixabay", url_mappings)
    ...
    def download_from_pexels(self, keyword, num_images=20, per_page=20):
        ...
        try:
            ...
            while downloaded_count < num_images:
                try:
                    ...
                    for img in img_elements:
                        ...
                        if 'images.pexels.com' in src:
                            ...
                            if '?auto=compress' in src and 'w=' in src:
                                ...
                                src = base_url + '?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1'
                            found_images.append(src)
                    ...
                    # 下载图片
                    for img_url in found_images[:min(15, num_images - downloaded_count)]:
                        if downloaded_count >= num_images:
                            break
        ...
        converted = self._convert_images_to_jpg_with_hash(temp_dir, keyword, keyword_dir, "pexels", url_mappings)
    ...
    def download_images(self, keyword, num_images=20, engine="bing", **kwargs):
        ...
        if engine == "baidu":
            ...
            raise ValueError(f"不支持的搜索引擎: {engine}，请使用 'baidu', 'bing', 'google', 'flickr', 'unsplash', 'pixabay', 'pexels', 'website' 或 'urls'")
...
def download_images_cli(keywords, num_images=50, engines=None, save_dir="downloaded_images", save_mapping=True, flickr_api_key=None, flickr_api_secret=None, website_urls=None, url_list_file=None):
    """
    CLI友好的图片下载函数
