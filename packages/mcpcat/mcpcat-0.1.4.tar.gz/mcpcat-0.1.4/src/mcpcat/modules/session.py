...
def get_headers_from_request_context(request_context: RequestContext) -> dict[str, str] | None:
...
def get_client_info_from_request_context(server: Server, request_context: RequestContext | None) -> None:
        ...
        try:
            ...
            if headers:
                ...
                if headers.get('x-mcp-client-version'):
                    data.session_info.client_version = headers.get('x-mcp-client-version')

                if data.session_info.client_name or data.session_info.client_version:
                    set_server_tracking_data(server, data)
                    write_to_log(f"Extracted client info from headers: {data.session_info.client_name} v{data.session_info.client_version}")
        except Exception as e:
