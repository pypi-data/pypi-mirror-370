...
from skipper_tool.omniparser.ocr import check_ocr_box, is_paddleocr_available
...
def predict_ui_elements(
    ...
    ocr_elements = []
    if use_ocr:
        is_paddle = is_paddleocr_available()
        if not is_paddle:
    ...
    filtered_boxes = remove_overlap_new(
        boxes=xyxy_elem, iou_threshold=iou_threshold, ocr_bbox=ocr_elements
    )
