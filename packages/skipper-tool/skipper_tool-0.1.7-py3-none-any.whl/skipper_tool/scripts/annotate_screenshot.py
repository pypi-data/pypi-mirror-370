...
Examples:
  ...
  %(prog)s input.png --output-dir ./results
    ...
    if OmniParser is None:
        logger.error(
            "OmniParser dependencies are not installed. Install with: pip install 'skipper[local]'"
        )
        return 1
    ...
    try:
        ...
        predictions = omni_parser.predict(args.image_path, return_annotated_image=not args.no_regular)
        ...
        if not args.no_regular and "annotated_image" in predictions:
            omni_parser.save_annotated_image(predictions["annotated_image"], str(annotated_path))
