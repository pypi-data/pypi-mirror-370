...
class OmniParserClient:
    ...
    def predict(
        ...
        **annotation_kwargs,
        ...
        """
        if self.mode == "local":
            return self._predict_local(image, return_annotated_image, **annotation_kwargs)
        else:
            return self._predict_remote(
                image, return_annotated_image, conf, imgsz, iou_threshold, scale_img, output_coord_in_ratio, **annotation_kwargs
            )



    def _predict_local(self, image: str | Image.Image | np.ndarray, return_annotated_image: bool = True, **annotation_kwargs) -> dict:
        """Execute prediction using local OmniParser."""
        return self.local_parser.predict(image, return_annotated_image=return_annotated_image, **annotation_kwargs)
    ...
    def _predict_remote(
        ...
        **annotation_kwargs,
        ...
        # Choose endpoint based on whether we need annotated image
        endpoint = "detect" if return_annotated_image else "predict"
        result = _make_api_request(self.api_url, endpoint, self.api_key, files, params)
        ...
        if return_annotated_image and "annotated_image" in result:
            import base64
            import io
            try:
