...
Examples:
  ...
  %(prog)s input.png --output-dir ./results
    ...
    try:
        ...
        # Create regular annotated image
        if not args.no_regular:
            annotated_img = omni_parser.annotate(args.image_path, predictions)
            omni_parser.save_annotated_image(annotated_img, str(annotated_path))
