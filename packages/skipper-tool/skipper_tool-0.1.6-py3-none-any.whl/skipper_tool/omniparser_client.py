...
class OmniParserClient:
    ...
    def detect(
        ...
        """
        Detect UI elements in the image using either local or remote execution.
        
        This method provides detailed detection results including annotated images.
        ...
        Returns:
            ...
            - 'parsed_content': List of parsed content descriptions
            - 'image_size': Tuple of (width, height)
            - 'annotated_image': Base64 encoded annotated image (remote mode only)
        """
        if self.mode == "local":
            # For local mode, detect is the same as predict
            return self._predict_local(image)
        else:
    ...
    def _detect_remote(
        ...
        """Execute detection using remote API."""
        # Prepare image for upload
        files, pil_image = _prepare_image_for_upload(image)
        ...
        # Make API request to detect endpoint
        result = _make_api_request(self.api_url, "detect", self.api_key, files, params)

        # Ensure the result has the expected format including annotated image
        return {
            "boxes": result.get("boxes", []),
            "labels": result.get("labels", []),
            "coordinates": result.get("coordinates", {}),
