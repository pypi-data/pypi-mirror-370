...
class OmniParser:
    ...
    def _initialize_parser(self):
        ...
        if not self.force_remote and self.yolo_model_path:
            try:
                ...
                local_config = {"som_model_path": self.yolo_model_path, **self.config}
                self._parser = LocalOmniParser(config=local_config)
                self._mode = "local"
                logger.info(f"OmniParser initialized in local mode with model: {self.yolo_model_path}")
                return
            except ImportError:
                logger.warning("Local OmniParser dependencies not available. Falling back to remote API.")
            except Exception as e:
                logger.warning(f"Failed to initialize local OmniParser: {e}. Falling back to remote API.")
    ...
    def predict(
        ...
        **annotation_kwargs,
        ...
        """
        logger.info(
            f"Predicting with {self._mode} mode, imgsz: {imgsz}, iou_threshold: {iou_threshold}, scale_img: {scale_img}, output_coord_in_ratio: {output_coord_in_ratio}"
        )
        
        if self._mode == "local":
            # Local mode - different parameter handling
            return self._parser.predict(image, **annotation_kwargs)
        else:
            # Remote mode - pass all parameters
