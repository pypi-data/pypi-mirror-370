...
def handle_move_longest_path_card(game, action):
    random_player_idx = action.legal_action.player_idx
    if not game or random_player_idx is None:
        return game

    player = game.players[random_player_idx]
    if not player:
        return game
...
def getprivatestate(s, player_idx):
    ...
    print(f"get_goal_completions: {goal_completions}")
    return PrivateState(
        player=s.players[player_idx],
