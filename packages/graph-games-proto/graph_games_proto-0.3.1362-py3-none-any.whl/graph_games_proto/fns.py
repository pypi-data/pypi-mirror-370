...
def find_player_with_longest_path(game):
    ...
    print("game.player_graphs[0]: ", game.player_graphs[longest_path_player_idx])
    print(f"longest_path_winner [{longest_path_length}]: ", longest_path_player_idx)
    return game.players[longest_path_player_idx]


def calc_path_len_from_edges(edges):
    if edges is None:
        return 0
    return len(edges)


def longest_path_length(game, player_idx):
    longest_path_length = 0
    ...
    for _ in range(500):
        ...
        if not walk:
            return 0
        path_len = calc_path_len_from_edges(walk[1])
