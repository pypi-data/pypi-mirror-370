...
class SlidingWindowAttention(nn.Module):
    ...
    def forward(self, x, rope=None, causal=True):
        ...
        if not self.use_flex_attn:
            ...
            if causal:
                attn_mask = self.causal_windowed_mask(src_len, self.window_len, dilation=self.dilation, to_bias=True, device=x.device)  # (1, tgt_len, src_len)
            else:
                attn_mask = self.symmetric_windowed_mask(src_len, self.window_len, dilation=self.dilation, to_bias=True, device=x.device)  # (1, tgt_len, src_len)
            ...
            if causal:
                block_mask = self.causal_windowed_block_mask(src_len, tgt_len, device=x.device)
            else:
                block_mask = self.symmetric_windowed_block_mask(src_len, tgt_len, device=x.device)
            
            attn_device = "cuda" if x.device == "cuda" else "cpu"
            ...
            attn_output = rearrange(attn_output, 'b h s d -> s b (h d)', h=self.n_heads).to(x.device)
