...
# (참고) 아래 certifi/urllib 설정은 requests/urllib 용입니다.
# Selenium(ChromeDriver) HTTPS에는 영향이 거의 없습니다.
try:
    import certifi
...
class SeleniumClient(CrawlerClient):
    def __init__(self):
        ...
        # 불안정: --headless=new  -> 일반 headless 권장
        self.options.add_argument('--headless')
        self.options.add_argument('--disable-dev-shm-usage')

        # --- 단일 스레드 전용 실행기 & 이벤트 루프 참조 ---
    ...
    async def _recreate_driver(self):
        """드라이버 재생성 (NoSuchWindow 등에서 1회 사용)"""
