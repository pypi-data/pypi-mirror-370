...
This also doubles-up as an MCP server that MCP clients (such as Cursor, Windsurf, Claude Desktop etc.) can integrate with for tool usage. Refer to MCP server setup section below for more details.
...
testchimp-local as an MCP server as well, enabling the following capabilities:

1. Provide access to your webapps' *runtime behaviour context* - capturing the different screens, pathways, states and detailed down to individual element level granularity, giving your IDEs deep contextual awareness about the product being built.

2. Visual Vibe Coding: Point and just "Say the change you  want to see" in the browser.

3. Fetch recent console logs from browser (for context-aware troubleshooting, checking fixes implemented are working etc.)

4. Run agentic exploratory testing

### Prerequisites

1. Install TestChimp Chrome extension

TestChimp MCP server communicates with the TestChimp chrome extension to fetch current browser state information for quick feedback visual vibe coding sessions.

2. Install TestChimp VSCode extension

The VSCode extension acts as a communication bridge between VSCode and the browser extension.

### Installation and usage

1. Install the server as instructed above.
...
It starts in port: 43449 by default. You can change to a different one by passing --port argument.


4. Configure Cursor / Windsurf to use this as an MCP server by adding following to mcpServers config file:
...
5. Your MCP client (Cursor, Claude Desktop etc.) can now use the TestChimp MCP server to leverage the above detailed capabilities. To see it in action, open your webapp in the browser, and open the TestChimp chrome extension for visual vibe coding, capturing bugs, resolving bugs right from the browser and more...
