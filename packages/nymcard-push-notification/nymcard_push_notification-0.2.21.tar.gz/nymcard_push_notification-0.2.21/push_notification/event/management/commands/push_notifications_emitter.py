...
from push_notification.event.models import PushNotificationEvent, LS_EVENTS
    ...
    async def emit(self, session: aiohttp.ClientSession, event: PushNotificationEvent):
        ...
        try:
            ...
            try:
                ...
                text = await res.text()
                self.stdout.write("Response is not JSON: {}".format(text))
                json_res = {"status": -1, "message": "Response is not JSON"}

            if event.event.event_type in LS_EVENTS and json_res.get("status") != 0:
                raise Exception("Failed to deliver event, json_response: {}".format(json_res))
