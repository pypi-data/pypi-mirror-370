...
class ExchangeMonitor:
    ...
    def __init__(self, host: str = None, port: int = None,
        ...
        # Statistics storage (in-memory for current session)
    ...
    async def connect(self) -> tuple[bool, str]:
        ...
        """
        try:
            # Initialize database and session manager if not done
    ...
    async def disconnect(self):
        ...
        # End current session when disconnecting
    ...
    async def process_trace_message(
        ...
        try:
            ...
            # Store in exchange-specific message history (in-memory for current session)
            ...
            # Store in routing key-specific history for hierarchical viewing (in-memory)
            ...
            try:
                await self.session_manager.store_message(message_info)
                logger.debug(f"📀 Message stored in database for session {self.session_manager.get_current_session_id()}")
    ...
    async def start_new_session(self, name: str = None) -> str:
        """Start a new monitoring session."""
        if not name:
            name = f"Session_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            
        session_id = await self.session_manager.start_session(
            name, self.host, self.port, self.username, self.trace_queue_name
        )
        
        # Clear current in-memory stats when starting new session
        self.reset_stats()
        
        logger.info(f"Started new monitoring session: {name}")
        return session_id
    
    async def end_current_session(self):
        """End the current monitoring session."""
        await self.session_manager.end_current_session()
        
    async def save_connection_settings(self, name: str):
        """Save current connection settings."""
        await self.session_manager.save_connection_settings(
            name, self.host, self.port, self.username, 
            self.password, self.trace_queue_name
        )
        
    async def get_connection_settings(self) -> List[Dict]:
        """Get saved connection settings."""
        return await self.session_manager.get_connection_settings()
        
    async def get_sessions(self, limit: int = 50) -> List[Dict]:
        """Get list of monitoring sessions."""
        return await self.session_manager.get_sessions(limit)
        
    async def load_session_data(self, session_id: str) -> Dict:
        """Load data from a specific session."""
        return await self.session_manager.load_session(session_id)
        
    async def get_session_exchange_messages(self, session_id: str, 
                                          exchange_name: str,
                                          limit: int = 1000) -> List[Dict]:
        """Get messages for a specific exchange from a session."""
        return await self.session_manager.get_session_exchange_messages(
            session_id, exchange_name, limit
        )
    ...
    async def get_session_routing_key_messages(self, session_id: str,
                                             ...
                                             routing_key: str,
                                             limit: int = 1000) -> List[Dict]:
        """Get messages for a specific routing key from a session."""
        return await self.session_manager.get_session_routing_key_messages(
            session_id, exchange_name, routing_key, limit
        )
        
    async def get_session_routing_keys(self, session_id: str,
                                     exchange_name: str) -> List[str]:
        """Get routing keys for an exchange from a session."""
        return await self.session_manager.get_session_routing_keys(
