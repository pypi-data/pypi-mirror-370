                ...
                if isinstance(hook_group, dict) and "hooks" in hook_group:
                    for hook in hook_group["hooks"]:
                        if (
                            ...
                            and hook.get("command") == "claudeguard-hook"
                        ):
                            return True
...
def _load_template_data(
    template_file: Path, template_name: str
) -> dict[str, Any] | None:
        ...
        for profile_path in profiles_dir.glob("*.yaml"):
            try:
                ...
                profiles_found.append(
                    (profile_path.stem, "Profile file (corrupted)", "⚠")
                )
...
def switch_profile(profile_name: str) -> None:
    ...
    if not target_profile_file.exists():
        ...
        if available_profiles:
            ...
            click.echo(
                "   No profiles found. Run 'claudeguard install' to create default profile"
            )
        sys.exit(1)
    try:
    ...
    *,
    ...
    try:
        ...
        if switch:
            ...
            click.echo(
                f"📝 Run 'claudeguard switch-profile {profile_name}' to use this profile"
            )
...
@click.argument("profile_name")
@click.option(
    "--force", is_flag=True, help="Delete profile even if it's currently active"
)
