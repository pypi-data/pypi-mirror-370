...
class ProfileLoader:
    def __init__(
        self, project_root: Path | None = None, home_directory: Path | None = None
    ):
        ...
        try:
            ...
            for i, rule in enumerate(rules_data):
                if not isinstance(rule, dict):
                    raise ProfileLoadError(
                        f"Invalid rule structure: rule {i} must be a dictionary"
                    )
                ...
                if not pattern:
                    raise ProfileLoadError(
                        f"Missing required field: pattern in rule {i}"
                    )
                if not action:
                    raise ProfileLoadError(
                        f"Missing required field: action in rule {i}"
                    )

                if action not in ("allow", "deny", "ask"):
                    raise ProfileLoadError(
                        f"Invalid action value: {action} in rule {i}"
                    )

                rules.append(
                    ProfileRule(
                        pattern=pattern, action=action, comment=rule.get("comment", "")
                    )
                )
