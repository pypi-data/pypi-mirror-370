...
class TestInitExistingProjects:
    ...
    def test_preserves_existing_profiles_directory(self, temp_dir) -> None:
        ...
        custom_data = make_yaml_profile_data(name="custom", description="Existing custom profile")
    ...
    def test_creates_active_profile_if_missing_from_existing_project(self, temp_dir) -> None:
    ...
    def test_creates_default_profile_if_missing_from_profiles_directory(self, temp_dir) -> None:
        ...
        with runner.isolated_filesystem():
            with patch("pathlib.Path.cwd", return_value=project_dir):
                with patch("pathlib.Path.mkdir", side_effect=OSError("Disk full")):
                    result = runner.invoke(install, [])
        assert result.exit_code != 0
        assert "Error initializing claudeguard" in result.output or "Disk full" in result.output
...
class TestInitProfileContent:
    ...
    def test_created_profile_rules_have_required_fields(self, temp_dir) -> None:
        ...
        for i, rule in enumerate(rules):
            ...
            assert rule["action"] in ["allow", "deny", "ask"], f"Rule {i} invalid action"
