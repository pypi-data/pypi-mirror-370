...
class TestAllClaudeToolsSupport:
    ...
    def test_bash_extracts_command(self, pattern_matcher: PatternMatcher):
        ...
        tool_call = ToolCall(
            name="Bash", input=ToolInput(data={"command": "git status"})
        )

        resource = pattern_matcher.extract_resource(tool_call)
        assert resource == "git status"

    def test_search_tools_extract_path_or_pattern(
        self, pattern_matcher: PatternMatcher
    ):
    ...
    def test_no_resource_tools_return_empty_string(
        self, pattern_matcher: PatternMatcher
    ):
        ...
        for tool_name in no_resource_tools:
            tool_call = ToolCall(
                name=tool_name, input=ToolInput(data={"some_param": "value"})
            )
    ...
    def test_unknown_tools_return_empty_string(self, pattern_matcher: PatternMatcher):
        ...
        tool_call = ToolCall(
            name="UnknownTool", input=ToolInput(data={"any_param": "value"})
        )

        resource = pattern_matcher.extract_resource(tool_call)
        assert resource == ""

    def test_no_resource_tools_match_bare_patterns(
        self, pattern_matcher: PatternMatcher
    ):
        """Tools with no resources should match bare tool name patterns."""
        pattern = ToolPattern(pattern="Task", action="allow")
        tool_call = ToolCall(
            name="Task", input=ToolInput(data={"description": "some task"})
        )
