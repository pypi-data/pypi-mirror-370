...
class TestActiveProfileMetadataLoading:
    ...
    def test_loads_different_profile_when_active_profile_changes(
        self, project_dir
    ) -> None:
        ...
        git_push_rule = next(
            rule for rule in profile1.rules if "git push" in rule.pattern
        )
        ...
        # Find the git push rule
        git_push_rule2 = next(
            rule for rule in profile2.rules if "git push" in rule.pattern
        )
        assert git_push_rule2.action == "ask"

    def test_defaults_to_default_profile_when_no_active_profile_file(
        self, project_dir
    ) -> None:
    ...
    def test_falls_back_to_generated_default_when_no_profiles_directory(
        self, project_dir
    ) -> None:
...
class TestActiveProfileErrorHandling:
    ...
    def test_handles_corrupted_active_profile_file_gracefully(
        self, project_dir
    ) -> None:
    ...
    def test_raises_error_when_referenced_profile_yaml_invalid(
        self, project_dir
    ) -> None:
...
class TestActiveProfileWithHomeDirectory:
    ...
    def test_falls_back_to_home_when_project_has_no_active_profile(
        self, temp_dir
    ) -> None:
