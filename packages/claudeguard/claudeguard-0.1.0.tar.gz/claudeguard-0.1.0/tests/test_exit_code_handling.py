...
class TestExitCodeWithOutputBehavior:
    ...
    @patch("sys.stdout", new_callable=StringIO)
    @patch("sys.stderr", new_callable=StringIO)
    def test_deny_decision_outputs_to_stderr_with_exit_2(
        self, mock_stderr, mock_stdout
    ) -> None:
...
class TestExitCodeErrorHandling:
    ...
    @patch("sys.stdout", new_callable=StringIO)
    @patch("sys.stderr", new_callable=StringIO)
    def test_exits_with_0_on_json_parse_error_fail_safe(
        self, mock_stderr, mock_stdout
    ) -> None:
    ...
    @patch("sys.stdout", new_callable=StringIO)
    @patch("sys.stderr", new_callable=StringIO)
    def test_exits_with_0_on_profile_load_error_fail_safe(
        self, mock_stderr, mock_stdout
    ) -> None:
