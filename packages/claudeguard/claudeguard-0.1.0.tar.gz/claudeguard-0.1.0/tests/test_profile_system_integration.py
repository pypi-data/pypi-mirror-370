...
""")
    ...
    def test_falls_back_to_default_when_no_claudeguard_found(self) -> None:
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            assert (
                profile.metadata.description == "Default security profile for claudeguard"
            )
...
""")
            ...
            with pytest.raises(
                ProfileLoadError, match="Missing required field: pattern"
            ):
                loader.load_profile()
...
""")
    ...
    def test_handles_missing_profile_gracefully(self) -> None:
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            with pytest.raises(
                ProfileLoadError, match="Profile 'nonexistent' not found"
            ):
                loader.load_profile()
