...
# Use GPT-5 (recommended)
grit commit --model gpt-5 --commit
...
# Model selection
grit commit --model gpt-5           # Recommended (default)
...
# Combined examples
grit commit --lang ko --model gpt-5 --commit  # Using recommended GPT-5
...
| `core.model` | `gpt-5` (recommended), `gpt-4o`, `gpt-4`, `gpt-3.5-turbo`, `gpt-4o-mini` | Default AI model (⚠️ gpt-4o-mini may include markdown formatting) |
...
[core]
lang = en
model = gpt-5  # Recommended default
commit = false
```
...
```bash
# GPT-5 사용 (권장)
grit commit --model gpt-5

# 또는 기본 모델을 GPT-5로 변경
grit config core.model gpt-5
