...
- Python 3.9+
- NoneBot2 2.0+
- nonebot-plugin-localstore（自动安装）
...
| 命令 | 英文别名 | 功能 | 参数 |
|------|----------|------|------|
| `世界 <集群>` | `world <集群>` | 获取世界信息 | 集群名称（必需） |
| `房间 <集群>` | `room <集群>` | 获取房间信息 | 集群名称（必需） |
| `系统` | `sys` | 获取系统信息 | 无 |
| `玩家 <集群>` | `players <集群>` | 获取在线玩家列表 | 集群名称（必需） |
| `直连 <集群>` | `connection <集群>` | 获取服务器直连信息 | 集群名称（必需） |
| `菜单` | `help` | 显示帮助信息 | 无 |
...
| `重置世界 <集群> <世界>` | `reset <集群> <世界>` | 重置世界 | 集群名称（必需）、世界名称（必需） |
| `聊天历史 <集群> <世界> <行数>` | `chathistory <集群> <世界> <行数>` | 获取聊天历史 | 集群名称（必需）、世界名称（必需）、行数（必需） |
| `聊天统计 <集群>` | `chatstats <集群>` | 获取聊天统计信息 | 集群名称（必需） |
...
| `最新消息` | `latestmessages` | 获取游戏内最新消息 | 自动选择集群和世界 |
...
- [📖 详细配置说明](./CONFIGURATION.md) - 插件配置详解和故障排除
- [🚀 完整安装教程](./INSTALLATION.md) - 从零开始的部署指南
