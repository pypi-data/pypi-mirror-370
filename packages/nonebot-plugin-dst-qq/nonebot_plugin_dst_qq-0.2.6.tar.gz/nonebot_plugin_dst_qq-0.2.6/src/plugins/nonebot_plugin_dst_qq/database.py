...
class ChatHistoryDatabase:
    ...
    def __init__(self, db_path: str = None):
        if db_path is None:
            # 使用 localstore 插件获取存储目录
            try:
                from nonebot_plugin_localstore import get_plugin_data_dir
                plugin_data_dir = get_plugin_data_dir("nonebot_plugin_dst_qq")
                self.db_path = str(plugin_data_dir / "chat_history.db")
            except ImportError:
                # 如果 localstore 插件不可用，使用默认路径
    ...
    async def get_player_chat_history(self, player_name: str, limit: int) -> List[Dict[str, Any]]:
        """获取指定玩家的聊天历史"""
        try:
            await self.init_database()
            async with aiosqlite.connect(self.db_path) as db:
    ...
    async def get_qq_messages(self, user_id: int, limit: int) -> List[Dict[str, Any]]:
        """获取QQ消息历史"""
        try:
            await self.init_database()
            async with aiosqlite.connect(self.db_path) as db:
    ...
    async def sync_chat_logs(self, cluster_name: str, world_name: str, lines: int) -> dict:
        """同步聊天日志到数据库"""
        try:
            # 导入API模块
