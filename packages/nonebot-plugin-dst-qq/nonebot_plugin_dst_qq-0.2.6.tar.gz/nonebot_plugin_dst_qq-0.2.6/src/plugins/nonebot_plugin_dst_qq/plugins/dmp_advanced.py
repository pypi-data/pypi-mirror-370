...
class DMPAdvancedAPI:
    ...
    async def get_backup_list(self, cluster_name: str) -> dict:
        """获取备份列表"""
        if not cluster_name:
    ...
    async def create_backup(self, cluster_name: str) -> dict:
        """创建备份"""
        if not cluster_name:
            cluster_name = await self.get_first_available_cluster()
            if not cluster_name:
    ...
    async def rollback_world(self, days: int, cluster_name: str) -> dict:
        """回档世界"""
        if not cluster_name:
            cluster_name = await self.get_first_available_cluster()
            if not cluster_name:
    ...
    async def reset_world(self, cluster_name: str, world_name: str) -> dict:
        """重置世界"""
        if not cluster_name:
            cluster_name = await self.get_first_available_cluster()
            if not cluster_name:
    ...
    async def get_chat_history(self, cluster_name: str, world_name: str, lines: int) -> dict:
        """获取聊天历史"""
        if not cluster_name:
            cluster_name = await self.get_first_available_cluster()
            if not cluster_name:
    ...
    async def get_chat_statistics(self, cluster_name: str) -> dict:
        """获取聊天统计"""
        if not cluster_name:
            config = get_config()
...
📋 备份管理:
• /查看备份 - 查看可用的世界备份（自动选择集群）
• /回滚世界 <天数> - 将世界回滚指定天数（1-5天）
...
async def handle_backup_cmd(bot: Bot, event: Event):
    ...
    try:
        ...
        cluster_name = await dmp_advanced_api.get_first_available_cluster()
        if not cluster_name:
            await bot.send(event, "❌ 没有可用的集群")
            return
        result = await dmp_advanced_api.get_backup_list(cluster_name)
