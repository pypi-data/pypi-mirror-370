...
class MessageExchangeManager:
        ...
        try:
            ...
            for cluster in clusters:
                ...
                for world in worlds:
                    ...
                    # 获取当前聊天日志
                    chat_logs = await self._fetch_latest_chat_logs(cluster_name, world_name, 50)
    ...
    async def _fetch_latest_chat_logs(self, cluster_name: str, world_name: str, lines: int) -> List[Dict]:
        """直接从API获取最新的聊天日志"""
        try:
            headers = {
                "Authorization": config.dmp_token,
    ...
    async def get_chat_logs(self, cluster_name: str, world_name: str, lines: int) -> dict:
        """获取聊天日志"""
        try:
            headers = {
                "Authorization": config.dmp_token,
    ...
    async def send_message_to_game(self, message: str, cluster_name: str, world_name: str):
        """发送消息到游戏"""
        if not cluster_name:
            # 获取第一个可用的集群名称
            config = get_config()
...
async def handle_push_latest(bot: Bot, event: Event, state: T_State):
    ...
    try:
        ...
        # 直接从API获取最新聊天记录
        chat_logs = await message_manager._fetch_latest_chat_logs(cluster_name, world_name, 20)
