...
def llm_parser(entries, provider_config, task_info):
    ...
    # Cerca un prompt custom nel task, altrimenti usa quello di default
    task_P = task_info.get("prompt")
    #if not task_prompt:
    task_prompt = f"""
    ...
    # Formatta il prompt con le variabili dinamiche
    ...
    analysis_result = analyze_log_messages(messages, final_prompt, provider_config)
