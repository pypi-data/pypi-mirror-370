        ...
        try:
            ...
            if hasattr(e, 'response') and e.response is not None and e.response.status_code == 429:
                if attempt < max_retries - 1:
                    ...
                    console.print(f"[yellow]Rate limit raggiunto. Riprovo tra {sleep_time:.2f} secondi... (Tentativo {attempt + 1}/{max_retries})[/yellow]")
                    time.sleep(sleep_time)
                else:
                    console.print(f"[bold red]Rate limit raggiunto. Tutti i {max_retries} tentativi sono falliti.[/bold red]")
                    return None
            else:
