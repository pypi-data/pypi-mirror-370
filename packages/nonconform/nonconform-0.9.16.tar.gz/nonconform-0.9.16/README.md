...
> **Note:** The following examples use the built-in datasets. Install with `uv pip install nonconform[data]` (UV) or `pip install nonconform[data]` (pip) to run these examples. (see [Optional Dependencies](#optional-dependencies))
...
**Using UV (recommended):**
- `uv pip install nonconform[data]` - Includes pyarrow for loading example data (via remote download)
- `uv pip install nonconform[deep]` - Includes deep learning dependencies (PyTorch)
- `uv pip install nonconform[fdr]` - Includes advanced FDR control methods (online-fdr)
- `uv pip install nonconform[dev]` - Includes development tools (black, ruff, pre-commit)
- `uv pip install nonconform[docs]` - Includes documentation building tools (sphinx, furo, etc.)
- `uv pip install nonconform[all]` - Includes all optional dependencies
