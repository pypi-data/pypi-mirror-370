...
### 1. UV-Specific Issues

#### UV Command Not Found

**Problem**: Getting "command not found" error when trying to use `uv`.
...
**Problem**: Dependencies are inconsistent or installation fails with UV.

**Solutions**:
```bash
# Regenerate lock file
make lock  # or: uv lock
...
#### UV vs Pip Installation Issues

**Problem**: Mixing UV and pip installations causing conflicts.

**Solution**: Use one package manager consistently:
```bash
# If using UV, stick with UV commands
uv add nonconform --extra data
make install-dev

# If using pip, avoid UV commands
...
**Problem**: `make` commands not recognized or failing.
...
- **Alternative**: Use UV commands directly instead of Makefile
...
### 2. ImportError: Cannot import DetectorConfig
