...
from typing import (
    ...
    _SpecialForm as _SpecialForm # 0.3.52
)
...
class _SpecialFormMeta(type):
    ...
    def __instancecheck__(self, obj: object):
        try:
            return isinstance(obj, _SpecialForm)
        except:
            return type(obj) is _SpecialForm
...
class SpecialForm(metaclass = _SpecialFormMeta):
    ...
    Use this class to find, if a type is actually a special form
    """
...
class TypingGenericType(Protocol):
    ...
    def __mro_entries__(self, bases: AVT_Iterable[object]) -> AVT_Tuple[type, ...]: ... # overriden
...
import _hashlib, hmac as _hmac, typing as _typing, typing_extensions as _typing_ext
...
Hash = _hashlib.HASH # >= 0.3.44; 0.3.52: deduce correctly
Hashxof = _hashlib.HASHXOF # >= 0.3.44; 0.3.52: deduce correctly
...
if hasattr(_typing_ext, "TypeForm"): # typing >= Py3.14?; typing_extensions >= 4.13.0
    TypeForm = _typing_ext.TypeForm # >= 0.3.52
...
TypingConcatenateType = cast(TypingConcatenateType, type(Concatenate[int, ...])) # >= 0.3.52
TypingGenericType = cast(TypingGenericType, _typing._GenericAlias) # >= 0.3.52
...
del _hashlib, _hmac, _typing, _typing_ext # not for export!
