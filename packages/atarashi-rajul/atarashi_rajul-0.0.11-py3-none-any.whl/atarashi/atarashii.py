...
def atarashii_runner(inputFile, processedLicense, agent_name,
  ...
  return run_scan(scanner, inputFile, verbose)
...
def run_scan(scanner, inputFile, verbose):
  '''
  ...
  if verbose > 0:
    print(f"[Atarashi DEBUG] File {inputFile} scanned in {elapsed:.2f} ms")
  return results
...
def main():
  ...
  if os.path.isfile(inputPath):
    ...
    if not skip_keyword:
      keyword_results = keyword_scanner.scan(inputPath)
    if not skip_keyword and keyword_results[0]['shortname'] != 'No_license_found':
      keyword_ok = True
    kw_time = (time.perf_counter() - t_kw) * 1000
    if verbose > 0:
      print(f"[Atarashi DEBUG] KeywordAgent check on {inputPath} took {kw_time:.2f} ms (result={keyword_ok})")
    if not keyword_ok:
      files_skipped += 1
      if verbose > 0:
      ...
      try:
        result = run_scan(scanner_obj, inputPath, verbose)
        result = list(result)
    ...
    for dirpath, _, filenames in os.walk(inputPath):
      ...
      for file in filenames:
        ...
        if not args.skip_keyword:
          keyword_results = keyword_scanner.scan(fpath)
        if not skip_keyword and keyword_results[0]['shortname'] != 'No_license_found':
          keyword_ok = True
        kw_time = time.perf_counter()
        if verbose > 0:
          print(f"[Atarashi DEBUG] KeywordAgent check on {inputPath} took {kw_time:.2f} ms (result={keyword_ok})")
        if not keyword_ok:
          files_skipped += 1
          continue
        try:
          result = run_scan(scanner_obj, fpat, verbose)
          result = list(result)
        except FileNotFoundError as e:
          result = [f"Error: {e.strerror}: '{e.filename}'"]
          return_code |= 2
    ...
    print("]")
    if verbose > 0:
      print(f"[Atarashi DEBUG] Total scanned files: {files_scanned}")
      print(f"[Atarashi DEBUG] Total skipped by KeywordAgent: {files_skipped}")
