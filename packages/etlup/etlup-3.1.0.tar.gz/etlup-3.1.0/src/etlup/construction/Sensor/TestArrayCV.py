...
from .data_models import TestArrayCVData

from ... import jinja_env

class TestArrayCVV0(bm.ConstructionBaseV0):
    model_config = ConfigDict(json_schema_extra={
        'examples': [
            {
                "component": "FBK_LF1_ROL_054",
                "type": "Sensor Test Array Vendor CV",
    ...
    type: Literal['Sensor Test Array CV - Vendor', 'Sensor Test Array CV - ETL Site']
    version: Literal["0.0"]
    component: str
    data: TestArrayCVData.TestArrayCVDataV0

    @classmethod
    def html_display(cls, row_data: dict) -> str:
        display_data = row_data['data']
        fig, ax = plt.subplots(figsize=(10, 6))
        if display_data["voltage"] is not None and display_data["capacitance"] is not None:
            ax.plot(display_data["voltage"], display_data["capacitance"], label='CV Curve', marker='o', color='orange')
            ax.set_xlabel('Voltage (V)')
            ax.set_ylabel('Capacitance (pF)')

            ax.set_title(f'CV Curve')
            if display_data["gain_category"] is not None:
                ax.set_title(f'CV Curve - Gain Category: {display_data["gain_category"]}')
