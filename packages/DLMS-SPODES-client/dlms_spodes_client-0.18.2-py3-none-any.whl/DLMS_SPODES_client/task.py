...
class MatchLDN(Base):
    ...
    async def exchange(self, c: Client) -> result.Result:
        ...
        if c.objects.LDN.value is None:
            ...
            raise exc.IDError(F"got {res.value=}, expected {c._objects.LDN.value}")
        return res
...
class Par2Data[T: cdt.CommonDataType](CDT):
    ...
    async def exchange(self, c: Client) -> result.Simple[T] | result.Error:
        obj = c._objects.par2obj(self.par)
...
class Write(Base):
    ...
    async def exchange(self, c: Client) -> result.Result:
        obj = c._objects.par2obj(self.par_data.par)
...
class AccessValidate(Base[result.Ok | result.Simple[list[Parameter]]]):
    ...
    async def exchange(self, c: Client) -> result.Ok | result.Simple[list[Parameter]] | result.Error:
        ...
        if c._objects is None:
            ...
            return result.Error(exc.EmptyObj(F"empty object_list for {c._objects.sap2association(c.SAP)}"))
...
class WriteParTranscript(Base):
    ...
    async def exchange(self, c: Client) -> result.Simple:
        ...
        if (data := c._objects.par2obj(par).get_attr(par.i)) is None:
