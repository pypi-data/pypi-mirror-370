...
RELATEDSECTION_ANY_OF_SCHEMAS = ["int", "str"]
...
class RelatedSection(BaseModel):
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = RelatedSection.model_construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when setting the actual_instance in RelatedSection with anyOf schemas: int, str. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into RelatedSection with anyOf schemas: int, str. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
