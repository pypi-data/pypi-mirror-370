...
NETAMOUNT_ANY_OF_SCHEMAS = ["float", "int"]
...
class NetAmount(BaseModel):
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = NetAmount.model_construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when setting the actual_instance in NetAmount with anyOf schemas: float, int. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into NetAmount with anyOf schemas: float, int. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
