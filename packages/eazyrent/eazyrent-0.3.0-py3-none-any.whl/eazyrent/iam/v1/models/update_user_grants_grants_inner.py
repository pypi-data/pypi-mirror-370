...
UPDATEUSERGRANTSGRANTSINNER_ANY_OF_SCHEMAS = ["str"]


class UpdateUserGrantsGrantsInner(BaseModel):
    """
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = UpdateUserGrantsGrantsInner.model_construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when setting the actual_instance in UpdateUserGrantsGrantsInner with anyOf schemas: str. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into UpdateUserGrantsGrantsInner with anyOf schemas: str. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
