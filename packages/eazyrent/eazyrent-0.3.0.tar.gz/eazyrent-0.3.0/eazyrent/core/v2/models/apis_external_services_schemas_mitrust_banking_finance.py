...
class ApisExternalServicesSchemasMitrustBankingFinance(BaseModel):
    """
    ApisExternalServicesSchemasMitrustBankingFinance
    """  # noqa: E501

    income: ApisExternalServicesSchemasMitrustBankingFinanceIncome
    credit_scoring: CreditScoring
    __properties: ClassVar[List[str]] = ["income", "credit_scoring"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of credit_scoring
        if self.credit_scoring:
            _dict["credit_scoring"] = self.credit_scoring.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {
                "income": ApisExternalServicesSchemasMitrustBankingFinanceIncome.from_dict(
                    obj["income"]
                )
                if obj.get("income") is not None
                else None,
                "credit_scoring": CreditScoring.from_dict(obj["credit_scoring"])
                if obj.get("credit_scoring") is not None
                else None,
            }
        )
        return _obj
