...
class ResponseValidateDescriptor(BaseModel):
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when setting the actual_instance in ResponseValidateDescriptor with anyOf schemas: CsvDescriptor, ExcelDescriptor, JsonDescriptor, XmlDescriptor, ZipDescriptor. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into ResponseValidateDescriptor with anyOf schemas: CsvDescriptor, ExcelDescriptor, JsonDescriptor, XmlDescriptor, ZipDescriptor. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
