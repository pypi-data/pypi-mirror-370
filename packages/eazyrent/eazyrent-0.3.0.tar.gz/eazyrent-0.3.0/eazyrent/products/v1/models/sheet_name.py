...
SHEETNAME_ANY_OF_SCHEMAS = ["List[str]", "int", "str"]
...
class SheetName(BaseModel):
    ...
    any_of_schemas: Set[str] = {"List[str]", "int", "str"}
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        instance = SheetName.model_construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when setting the actual_instance in SheetName with anyOf schemas: List[str], int, str. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into SheetName with anyOf schemas: List[str], int, str. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
