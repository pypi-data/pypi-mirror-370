    ...
    def _apply_text_fusion(self, image: Image.Image, draw: ImageDraw.Draw,
        ...
        if orientation and orientation != '0':
            try:
                ...
                # Create a temporary image for the text with rotation
                bbox = draw.textbbox((0, 0), text, font=font)
                text_width = bbox[2] - bbox[0]
                text_height = bbox[3] - bbox[1]
                
                # Calculate better padding for Windows compatibility
                # Use moderate padding - increased from 0.3 to 0.4 to prevent bottom cutoff
                font_size = getattr(font, 'size', 50)  # Get font size or default
                padding = max(25, int(font_size * 0.4))  # Slightly increased padding to prevent cutoff
                
                # Create a temporary image for the text with generous padding
                ...
                # Adjust position based on alignment for rotated text
                rotated_width, rotated_height = rotated_img.size
                
                # Better positioning calculation for Windows
                if alignment == 'center':
                    x = x - rotated_width // 2
                elif alignment == 'right':
                    x = x - rotated_width
                
                # For Windows compatibility, adjust Y position slightly up for rotated text
                import platform
                if platform.system() == 'Windows' and abs(angle) > 0:
                    y = y - int(font_size * 0.1)  # Slight upward adjustment for Windows
                
                # Paste the rotated text onto the main image
                image.paste(rotated_img, (x, y), rotated_img)
