...
class IvyBloomTUI(App):
    ...
    def __init__(self, config: Config, auth_manager: AuthManager, initial_project_id: Optional[str] = None, show_header: bool = False, show_footer: bool = False) -> None:
        ...
        # Services
        # Route debug to a file by default to avoid overlay; respect --debug flag
        log_file = str(self.config.config_dir / "tui_debug.log") if bool(self.config.get("debug", False)) else None
        self._debug_logger = DebugLogger(enabled=bool(self.config.get("debug", False)), prefix="TUI", log_path=log_file)
