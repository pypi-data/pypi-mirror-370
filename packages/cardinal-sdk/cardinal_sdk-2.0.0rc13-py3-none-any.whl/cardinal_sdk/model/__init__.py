...
@dataclass
class PublicAgendasAndCalendarItemTypes:
	agendas: list['Agenda']
	calendar_item_types: list['CalendarItemType']

	def __serialize__(self) -> Any:
		return {
			"agendas": [x0.__serialize__() for x0 in self.agendas],
			"calendarItemTypes": [x0.__serialize__() for x0 in self.calendar_item_types],
		}

	@classmethod
	def _deserialize(cls, data: Union[str, dict[str, JsonElement]]) -> 'PublicAgendasAndCalendarItemTypes':
		deserialized_dict: dict[str, JsonElement]
		if isinstance(data, str):
			deserialized_dict = json.loads(data)
		else:
			deserialized_dict = data
		return cls(
			agendas=[Agenda._deserialize(x0) for x0 in deserialized_dict["agendas"]],
			calendar_item_types=[CalendarItemType._deserialize(x0) for x0 in deserialized_dict["calendarItemTypes"]],
		)
