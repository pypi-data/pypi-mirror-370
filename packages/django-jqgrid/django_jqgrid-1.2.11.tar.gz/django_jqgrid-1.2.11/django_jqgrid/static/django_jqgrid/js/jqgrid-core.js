...
function initFormatters() {
    ...
    // Register custom formatters if they exist
    if (window.jqGridConfig && window.jqGridConfig.formatters) {
 ...
 * Helper utilities
 */
window.jqGridUtils = window.jqGridUtils || {
    ...
    return $.ajax({
        ...
        success: function(response) {
            if (!response || !response.data) {
                window.jqGridUtils.notify('error', 'Invalid grid configuration received from server.', tableInstance);
                return;
            }
            ...
            gridOptions.colModel.forEach(function(item) {
                ...
                if (item.searchoptions && item.searchoptions.dataUrl) {
                    item.searchoptions['buildSelect'] = function(data) {
                        try {
                            data = window.jqGridUtils.safeJsonParse(data);
        ...
        error: function(xhr, status, error) {
            window.jqGridUtils.notify('error', 'Error loading grid configuration: ' + error, tableInstance);
...
function initializeToolbarForTable(tableInstance, gridConfig) {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (!data || !data.data || !data.data.data || !data.data.data[0]) {
                    window.jqGridUtils.notify('error', 'Content type data not found', tableInstance);
                    return;
                }
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        window.jqGridUtils.notify('success', 'Filter saved successfully', tableInstance);
                    },
                    error: function(xhr, status, error) {
                        window.jqGridUtils.notify('error', 'Error saving filter: ' + error, tableInstance);
                    }
                });
            },
            error: function(xhr, status, error) {
                window.jqGridUtils.notify('error', 'Error getting content type: ' + error, tableInstance);
            }
        });
    });
    ...
    $(`#${tableInstance.toolbarSettings.deleteFiltersButtonId}`).on('click', function() {
        ...
        if (!tableInstance['searchOptions'] ||
            ...
            window.jqGridUtils.notify('warning', 'No saved filters found', tableInstance);
            return;
        }
        ...
        tableInstance['searchOptions']['tmplFilters'].forEach(function(item, index) {
            if (item === filterParam) {
                ...
                $.ajax({
                    ...
                    // CSRF token is handled by $.ajaxSetup in jqgrid-csrf.js
                    success: function(data) {
                        window.jqGridUtils.notify('success', 'Filter deleted successfully', tableInstance);
                        initializeGridForTable(tableInstance);
                    },
                    error: function(xhr, status, error) {
                        window.jqGridUtils.notify('error', 'Error deleting filter: ' + error, tableInstance);
                    }
                });
            }
        });
        ...
        if (!filterDeleted) {
            window.jqGridUtils.notify('warning', 'No matching filter found to delete', tableInstance);
        }
    });
        ...
        bulkActionConfig.actions.forEach(action => {
            ...
            button.addEventListener('click', function() {
                ...
                if (selectedIds && selectedIds.length > 0) {
                    ...
                    window.jqGridUtils.notify('warning', 'Please select at least one row', tableInstance);
                }
            });
...
function submitBulkUpdate(tableInstance) {
    ...
    if (!ids || ids.length === 0) {
        window.jqGridUtils.notify('warning', 'No items selected', tableInstance);
        return;
    }
    ...
    if (Object.keys(action).length === 0) {
        window.jqGridUtils.notify('warning', 'Please enable at least one field to update', tableInstance);
        return;
    }
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            window.jqGridUtils.notify('success', response.message || 'Bulk update successful', tableInstance);
        ...
        error: function(xhr) {
            ...
            window.jqGridUtils.notify('error', errorMessage, tableInstance);
        },
        complete: function() {
...
function performBulkDelete(tableInstance) {
    ...
    $.ajax({
        ...
        // CSRF token is handled by $.ajaxSetup in jqgrid-csrf.js
        success: function(response) {
            window.jqGridUtils.notify('success', response.message || 'Items deleted successfully', tableInstance);
...
function showMessage(tableInstance, type, message) {
    ...
    if (window.jqGridUtils.notify(type, message, tableInstance)) {
        return;
    }
