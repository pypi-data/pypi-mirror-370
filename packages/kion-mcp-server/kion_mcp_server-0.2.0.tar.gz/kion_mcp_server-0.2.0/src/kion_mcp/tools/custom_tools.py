...
def register_custom_tools(mcp: FastMCP, auth_state):
    ...
    async def get_spend_report(
        ...
        spend_type: Annotated[str, Field(description="How spend is calculated, amortized and attributed which shows spend spread across the time-period it occurred attributed to the correct accounts (`attributed` default), billed what's invoiced from the provider, amortized spend spread across the time it occurs but not attributed and shown at the billing source level (`unattributed`), or list the public list price"), Literal["attributed", "billed", "unattributed", "list"]] = "attributed",
