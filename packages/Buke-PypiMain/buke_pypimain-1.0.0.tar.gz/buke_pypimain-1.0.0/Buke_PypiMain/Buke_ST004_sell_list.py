...
from Buke_PypiMain.indicator import sma, pivot_standard, stochastic_fast_k, stochastic_fast_d, pivot_standard_for_sellprice, RSI, ibs
from Buke_PypiMain.function import rank, ts_min, ts_max
from Buke_PypiMain.parameter import Market, MovingAverage
from Buke_PypiMain.tools import get_ask_price, get_stock_exchange_tax, get_bid_price
...
def add_indicator(day_price: DataFrame, index_day_price: DataFrame) -> DataFrame:
    ...
    for key, group in grouped:
        ...
        group['순매수'] = group['외국인순거래대금'] + group['기관순거래대금'] + group['연기금순거래대금']
        group['순매수_이평선_10'] = sma(group['순매수'], 10)
    ...
    for key, group in grouped:
        ...
        increase_condition2 = group['변동폭_저가'] / group['변동폭_저가_이평선_10'] <  0.99
        increase_condition3 = group['RSI_5'] / group['RSI_5_이평선_5'] <  0.97
        increase_condition4 = group['변동폭_이평선_3'] / group['변동폭_이평선_10'] < 1.0
        increase_condition5 = group['ibs_이평선_10'] >  0.4
        increase_condition6 = group['스토캐스틱_fast_10'] / group['스토캐스틱_slow_10_3'] <  0.9
        increase_condition7 = group['변동폭_저가'] <  1.02
        ...
        group['#priority'] = -1 * group['순매수_이평선_10']
        group = group.set_index('날짜')
        day_price_dict[key] = group

    # Market Timing
    index_day_price['종가_최솟값_20'] = ts_min(index_day_price['종가'], 20)
    index_day_price['#market_timing'] = index_day_price['종가'] > index_day_price['종가_최솟값_20']
...
def Buke_ST004_sell_list(dir, today):
