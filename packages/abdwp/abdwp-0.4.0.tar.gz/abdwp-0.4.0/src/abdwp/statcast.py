...
def load_season(year, force_download=False, regular_season=False, data_dir="statcast"):
    ...
    if data_dir == "statcast":
        parquet_path_in_data = os.path.join("data", data_dir, f"statcast-{year}.parquet")
    if force_download:
...
# NOTE: currently defaulting to ascending=False to match pybaseball
...
def add_names(df, remove_player=True):
    df["batter_name"] = get_name_from_id(df["batter"])
    df["pitcher_name"] = get_name_from_id(df["pitcher"])
    if remove_player:
...
def get_name_from_id(mlbam_id):
    ...
    # build fast look mapping
    if not hasattr(get_name_from_id, "_id_to_name"):
        get_name_from_id._id_to_name = {
            int(row["key_mlbam"]): name_for_row(row)
            for _, row in people.iterrows()
            if pd.notna(row["key_mlbam"])
        }
    id_to_name = get_name_from_id._id_to_name

    # handle different input types
    if isinstance(mlbam_id, int):
        return id_to_name.get(mlbam_id, None)
    elif isinstance(mlbam_id, list):
        return [id_to_name.get(mid, None) for mid in mlbam_id]
    elif isinstance(mlbam_id, pd.Series) and pd.api.types.is_integer_dtype(mlbam_id):
        return [id_to_name.get(mid, None) for mid in mlbam_id.to_list()]
    else:
        raise TypeError("mlbam_id must be an integer, a list of integers, or an integer Series.")


# TODO*: different versions: search within full name versus by element?
def get_id_from_name(name_last, name_first):
    people = _get_chadwick_people()
    mask = (people["name_first"].fillna("").str.lower() == str(name_first).strip().lower()) & (
        people["name_last"].fillna("").str.lower() == str(name_last).strip().lower()
    )
    return people.loc[mask]
