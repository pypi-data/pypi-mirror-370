...
def createTable(
    ...
    *,
    ...
    from .helpers import connect_signals
    ...
    if headers_config:
        for col, name in enumerate(headers or []):
            ...
            if mode is not None:
                table.horizontalHeader().setSectionResizeMode(col,getattr(QHeaderView.ResizeMode, mode))
            else:
                table.horizontalHeader().setSectionResizeMode(col, resize_mode_default)
            # explicit width
            colWidth = cfg.get("ColumnWidth")
            
            if colWidth != None and is_number(colWidth):
