...
def createTable(
    ...
    **kwargs
    ...
    """
    from abstract_utilities import make_list
    from .properties_resolver import apply_properties  # your resolve_attr-based applier
    from .auto_signals import connect_signals, default_signals  # helpers with prepend_widget support
    ...
    if table.horizontalHeader():
        ...
        if horizontalHeaders:
            horizontalHeaders = make_list(horizontalHeaders)
            for i,horizontalHeader in enumerate(horizontalHeaders):
                if horizontalHeader != None:
                    table.horizontalHeader().setSectionResizeMode(i,setattr(QHeaderView.ResizeMode, horizontalHeader, None))
