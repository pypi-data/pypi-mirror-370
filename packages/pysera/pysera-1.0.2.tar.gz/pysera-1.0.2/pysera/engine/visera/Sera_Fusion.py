...
def RGB2HSI(rgb_img):
    ...
    for i in range(row):
        ...
        h = np.zeros(col, dtype=np.float32)
...
def Wavelet_Fusion(ct_image, pet_image, FUSION_METHOD, wavelet, MODE, level):
    ...
    for jj in range(0, ct_image.shape[2]):
        ...
        fusedImage = np.multiply(
            np.divide(fusedImage - np.min(fusedImage), (np.max(fusedImage) - np.min(fusedImage))),
            255)
...
def pca_fusion_folder(ct_path, pet_path, ss, nc, destfolder):
    ...
    fixed = [i for i in Fixed_datas if (
                i.endswith(".nrrd") | i.endswith(".dcm") | i.endswith(".dicom") | i.endswith(
            ".nii") | i.endswith(".nii.gz"))]
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=Num_worker) as executor:
            ...
            for co in fixed:
                if len(moved) > 0:
                    ...
                    fixed_filename = co.replace('.nii.gz', '').replace('.nii', '').replace('.nrrd',
                                                                                           '').replace(
                        '.dcm', '').replace('.dicom', '')
                    ...
                    for co_moved in moved:
                        moved_filename = co_moved.replace('.nii.gz', '').replace('.nii',
                                                                                 '').replace(
                            '.nrrd', '').replace('.dcm', '').replace('.dicom', '')
                    ...
                    if s_ratio > 0:
                        ...
                        try:
                            if isinstance(moved_img[0], np.ndarray) & isinstance(fixed_img[0],
                                                                                 np.ndarray):
                                ...
                                if len(ct_image.shape) == 3 & len(pet_image.shape) == 3:
                                    ...
                                    if (dim01[0] == dim02[0]):
                                        ...
                                        futures.append(
                                            executor.submit(PCA_fusion_folder_Thread, fixed_img,
                                                            moved_img, ct_image, pet_image, nc, ss,
                                                            destfolder))
                            ...
                            raise (
                            'Out of Memory or the parameters of image fusion tool should be selected properly:',
                            e)
                    # else:
...
def PCA_fusion_folder_Thread(fixed_img, moved_img, ct_image, pet_image, nc, ss, destfolder):
    ...
    convert_modalities(registered, header, DatatypeFrom, Datatype, OUTPUT_DIR=destfolder,
                       filename=_filename, createfolder='False')
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=Num_worker) as executor:
        ...
        for co in fixed:
            if len(moved) > 0:
                ...
                selected_index = 0
                for co_moved in moved:
                    moved_filename = co_moved.replace('.nii.gz', '').replace('.nii', '').replace(
                        '.nrrd', '').replace('.dcm', '').replace('.dicom', '')
                    sim_ratio = similar(fixed_filename, moved_filename)
                        ...
                        if isinstance(moved_img[0], np.ndarray) & isinstance(fixed_img[0],
                            ...
                            if len(ct_image.shape) == 3 & len(pet_image.shape) == 3:
                                ...
                                if (dim01[0] == dim02[0]):
                                    ...
                                    cont = True
                                    wavelet_family_list = ['gaus', 'mexh', 'morl', 'shan', 'fbsp',
                                                           'cmor', 'cgau']
                                    ...
                                    if cont:
                                        ...
                                        futures.append(
                                            executor.submit(wavelet_fusion_folder_Thread, fixed_img,
                                                            moved_img, ct_image, pet_image,
                                                            FUSION_METHOD, wavelet, MODE, level,
                                                            destfolder))
...
def wavelet_fusion_folder_Thread(fixed_img, moved_img, ct_image, pet_image, FUSION_METHOD, wavelet,
                                 MODE, level, destfolder):
    # ct_max_or = np.max(ct_image)
    ct_min_or = np.min(ct_image)
    we_fusion_final = Wavelet_Fusion(ct_image, pet_image, FUSION_METHOD, wavelet, MODE, level)
    _filename = fixed_img[3]
                    ...
                    try:
                        if isinstance(moved_img[0], np.ndarray) & isinstance(fixed_img[0],
                            ...
                            if len(ct_image.shape) == 3 & len(pet_image.shape) == 3:
                                ...
                                if (dim01[0] == dim02[0]):
                                    futures.append(
                                        executor.submit(weighted_fusion_folder_Thread, fixed_img,
                                                        moved_img, ct_image, pet_image, w1, w2,
                                                        interpolation, destfolder))
...
def weighted_fusion_folder_Thread(fixed_img, moved_img, ct_image, pet_image, w1, w2, interpolation,
                                  destfolder):
    # ct_max_or = np.max(ct_image)
    ct_min_or = np.min(ct_image)
    we_fusion_final = weighted_Fusion(ct_image, pet_image, float(w1), float(w2), interpolation)
    ...
    convert_modalities(registered, header, DatatypeFrom, Datatype, OUTPUT_DIR, filename,
                       createfolder)
    return ""
            ...
            if len(moved) > 0:
                ...
                if s_ratio > 0:
                    ...
                    try:
                        if isinstance(moved_img[0], np.ndarray) & isinstance(fixed_img[0],
                            ...
                            if len(ct_image.shape) == 3 & len(pet_image.shape) == 3:
                                ...
                                if (dim01[0] == dim02[0]):
                                    ...
                                    futures.append(
                                        executor.submit(HSI_fusion_folder_Thread, fixed_img,
                                                        moved_img, ct_image, pet_image, destfolder))
                        ...
                        raise (
                        'Out of Memory or the parameters of registration tool should be selected properly:',
                        e)
                # else:
                # raise('There is no image with the same name of',co,'in another folder.')
    executor.shutdown(wait=True)
