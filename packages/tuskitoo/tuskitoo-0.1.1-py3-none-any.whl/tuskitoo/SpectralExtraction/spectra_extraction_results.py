...
class spectral_extraction_results_handler:
    def __init__(self,spectral_extraction_results,conditions={"rsquared":0.7},header=None,names=None,band="NoK",name="noname",nsigmas=10,wavelength=None,relevant_keywords_header=None):
        """
        ...
        nsigmas : Number of sigmas of the sigma cliping to make nan outlayers
        """
        self.name = name
        ...
        ####################
        self.spectras1d_raw = {i:spectral_extraction_results_handler.interpolate_1d(self.pandas_results[i].values,nsigmas=100) for i in self.cleaned_panda.columns if "flux" in i}
        self.spectras1d = {i:spectral_extraction_results_handler.interpolate_1d(self.cleaned_panda[i].values,nsigmas=nsigmas) for i in self.cleaned_panda.columns if "flux" in i}
    ...
    def array_to_pandas(self):
        ...
        #superM =  pd.DataFrame(self.full_fit[:,2*self.parameter_number*self.source_number:3*(self.parameter_number*self.source_number)]).dropna().values[1]
        #data[~np.isnan(data).any(axis=1)]
        df = pd.DataFrame(self.full_fit[:,2*self.parameter_number*self.source_number:3*(self.parameter_number*self.source_number)])
        df_cleaned = df.replace("nan", np.nan)
        # Drop all rows that contain only NaN values
        df_non_nan = df_cleaned.dropna(how='all')
        # Drop all columns that contain only NaN values
        #df_non_nan = df_non_nan.dropna(axis=1, how='all')
        # Reset index if needed
        columns_model = df_non_nan.values[0]

        #columns_model = pd.DataFrame(self.full_fit[:,2*self.parameter_number*self.source_number:3*(self.parameter_number*self.source_number)]).dropna().values[1] 
        #[i for i in np.unique(deepcopy(self.full_fit[:,2*self.parameter_number*self.source_number:3*(self.parameter_number*self.source_number)]),axis=0) if "nan" not in i][0]
    ...
    @staticmethod
    def interpolate_1d(flux,nsigmas=10):
        """
        ...
        """
        clip_,lower,upper = sigma_clip(flux,sigma=nsigmas, cenfunc='median', return_bounds=True)
    ...
    def plot_column(self,column_name="",**kwargs):
        if column_name not in list(self.cleaned_panda.columns):
    ...
    def plot_spectra(self,obj=None,xlim=None,ylim=None,save=False,add_lines=False,xlabel=None):
        #it will be interesting can change between clear and not clear in this routine to check the diferences
        if isinstance(obj,str):
            obj = [obj]
        wavelength = None
        if xlabel=="pixel":
            wavelength = np.arange(len(self.cleaned_panda))
        else:
            try:
