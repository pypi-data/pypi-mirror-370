...
def do_periodic_uploads(uploadsecs):
    with open("upload.bat", "w") as f:
        f.write("git add -f ./plots/*.png\n")
        f.write("git commit -m 'upload local data'\n")
        f.write("git pull\n")
        f.write("git clean -f\n")
        f.write("git push -f\n")
    while (True):
        subprocess.Popen("upload.bat", creationflags=subprocess.CREATE_NO_WINDOW)
        time.sleep(uploadsecs)
...
def get_args_and_run():
    ...
    parser.add_argument('action', type=str, help = "start_rx|start_tx|do_plots|start_all|--runsecs nsecs|--plotsecs nsecs|--uploadsecs nsecs")
    ...
    parser.add_argument('--uploadsecs', type=str, default = "600", help = "Upload to github every uploadsecs seconds")
    parser.add_argument('--repo', type=str, default = "600", help = "Upload to github every uploadsecs seconds")
    ...
    if(args.action == "start_uploader"):
        do_periodic_uploads(int(args.uploadsecs))
    ...
    if(args.action == "start_all"):
        ...
        uploadsecs = int(args.uploadsecs)
        print("Starting Uploader with period {uploadsecs}")
        subprocess.Popen(f"hamplots start_uploader --uploadsecs {uploadsecs}", creationflags=subprocess.CREATE_NO_WINDOW)
