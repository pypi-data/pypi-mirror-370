...
def orchestrator(state):
    try:
        ...
        # Prevent infinite loops - if all agents for current file are done, force move to next file
        if (state['pattern_complete'] and 
            ((state['context_complete'] and state['compliance_complete']) or 
             (not state['context_complete'] and not state['compliance_complete'] and len(state.get('vulnerabilities', [])) == 0))):
            console.print(f"[yellow]🔄 All analysis complete for current file, forcing move to next...[/yellow]")
            if state['current_file_index'] + 1 < len(state['all_files']):
                state['current_file_index'] += 1
                state['file_complete'] = True
                updated_state['current_file_index'] = state['current_file_index']
                updated_state['file_complete'] = True
            else:
                state['all_files_complete'] = True
                updated_state['all_files_complete'] = True
