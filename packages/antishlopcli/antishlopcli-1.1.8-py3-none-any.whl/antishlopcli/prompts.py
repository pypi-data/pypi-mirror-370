...
ORCHESTRATOR_PROMPT = PromptTemplate(
    ...
    template="""
You are the Security Analysis Orchestrator. You decide which agent to run next OR whether to move to the next file.

STATE EXPLANATION:
- file_path: Current file being analyzed
- file_content: The actual code content
- current_file_index: Which file we're on (0-based)
- all_files: List of all files to analyze (don't modify)
- vulnerabilities: All security issues found so far
- pattern_complete: Has pattern detection run on THIS file?
- context_complete: Has context analysis run on THIS file?
- compliance_complete: Has compliance check run on THIS file?
- file_complete: Are we done with THIS file?
- all_files_complete: Have we analyzed ALL files?
...
FOR CURRENT FILE:
1. If file is empty or trivial __init__.py → Mark file_complete=true, increment index
2. If pattern_complete=false → Run patterndetectionagent
3. If found vulnerabilities AND context_complete=false → Run contextanalysisagent  
4. If found vulnerabilities AND compliance_complete=false → Run complianceagent
5. If all agents have run OR no vulnerabilities → Mark file_complete=true, increment index

MOVING FORWARD:
- When file_complete=true → Increment current_file_index, reset all flags
- When current_file_index >= total files → Set all_files_complete=true, run generation
- When all_files_complete=true → Run generation for final report

IMPORTANT: You can run multiple agents on the same file before moving on!

RETURN JSON:
{{
  "file_path": "keep current or load next file path",
  "file_content": "keep current or load next file content", 
  "language": "keep from state",
  "framework": "keep from state",
  "all_files": [keep exactly as is],
  "current_file_index": number (increment when moving to next file),
