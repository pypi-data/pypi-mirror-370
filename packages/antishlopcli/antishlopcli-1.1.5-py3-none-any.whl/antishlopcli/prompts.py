...
- patterndetectionagent: Fast vulnerability scanning
- contextanalysisagent: Deep code analysis (expensive - only for complex/risky files)
...
1. SKIP FILES ENTIRELY (set file_complete=true, move to next):
- Empty files or files with only comments
- __init__.py files with no code
- Pure CSS/SCSS files
- Image/asset files
- README/documentation files
- Package-lock.json, yarn.lock files
- .gitignore, .env.example files
- Migration files with only schema definitions
- Test files with only assertions (no security-relevant code)

2. MINIMAL SCAN (pattern only, then skip to next):
- Simple utility functions (math, formatting, helpers)
- Pure type definitions/interfaces
- Constants and configuration without secrets
- Simple getters/setters
- Pure UI components without user input handling

3. STANDARD SCAN (pattern + maybe context):
- Files with user input but low complexity
- Configuration files that might have secrets
- Simple API endpoints
- Basic CRUD operations

4. FULL ANALYSIS (all agents):
...
- Any file with obvious security implications

SMART DECISION LOGIC:

For __init__.py or empty files:
→ Set file_complete=true, current_file_index++, skip all agents

For simple utility files with no vulnerabilities:
→ Run pattern only, then set file_complete=true, move to next

For files with user input or database operations:
→ Run pattern, if vulnerabilities found run context, then compliance

For authentication/payment files:
→ Always run full analysis regardless of initial findings

EFFICIENCY EXAMPLES:
File: "__init__.py" → SKIP ENTIRELY → Move to next file
File: "constants.py" → Pattern only → If clean, skip to next
File: "utils/formatter.py" → Pattern only → Skip rest if no issues
File: "auth/login.py" → Full analysis → All agents
File: "api/payment.js" → Full analysis → All agents
File: "tests/test_utils.py" → Pattern only → Skip if just test assertions
File: "styles.css" → SKIP ENTIRELY → Not relevant for security

DECISION PRIORITY:
1. Check if file should be skipped entirely (empty, __init__, CSS, etc.)
2. If not skipped, run pattern detection
3. Based on findings + file risk, decide on further analysis
4. Skip to next file as soon as sufficient analysis is done
5. Only run expensive agents when truly needed
