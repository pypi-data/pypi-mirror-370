...
def show_agent_pipeline_start():
    """Show the start of the agent pipeline with animation"""
    console.print()
    console.rule("[bold #00FFFF]🤖 MULTI-AGENT SECURITY PIPELINE 🤖[/bold #00FFFF]", style="#00FFFF")
    console.print()
    
    # Show the pipeline flow
    pipeline_panel = Panel(
        "[bold #FFD700]🎯 ORCHESTRATOR[/bold #FFD700] → [bold #FFD700]🔍 PATTERN DETECTION[/bold #FFD700] → [bold #00FFFF]🧠 CONTEXT ANALYSIS[/bold #00FFFF] → [bold #FF00FF]📋 COMPLIANCE[/bold #FF00FF] → [bold #00FF00]📄 REPORT[/bold #00FF00]",
        title="[bold #FFFFFF]🚀 ANALYSIS PIPELINE[/bold #FFFFFF]",
        border_style="#00FFFF",
        box=ROUNDED,
        padding=(0, 1),
        expand=False
    )
    console.print(Align.center(pipeline_panel))
    console.print()
