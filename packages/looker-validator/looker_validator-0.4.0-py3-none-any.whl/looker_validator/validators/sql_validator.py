...
class AsyncSQLValidator(AsyncBaseValidator):
    ...
    # NEW METHOD: Fetch schema information without executing SQL queries
    async def _fetch_explore_schema_only(self, explore: LookMLExplore):
        """Fetch schema information for a single explore without executing SQL queries."""
        model_name = explore.model_name
        explore_name = explore.name
        logger.debug(f"Fetching schema for explore '{model_name}.{explore_name}' (no SQL execution)...")
        
        try:
            # Use schema-only method that doesn't execute SQL
            dimensions_data = await self.client.get_lookml_dimensions_schema_only(model_name, explore_name)
            ...
            explore.dimensions = valid_dimensions
            logger.debug(f"Found {len(valid_dimensions)} valid dimensions in schema for '{model_name}.{explore_name}'.")

        except Exception as e:
            logger.warning(f"Could not fetch schema for explore '{model_name}.{explore_name}': {e}")
            # Don't fail completely - just set empty dimensions for comparison
            explore.dimensions = []

    # UPDATED METHOD: Use schema-only fetch for target explores
    async def _get_target_explores(self) -> List[LookMLExplore]:
        """Get explores from the target branch/commit for schema comparison only."""
        if not self.target:
        ...
        try:
            ...
            # Get models from target branch
            logger.debug(f"Getting LookML models for project '{self.project}' on target branch...")
            try:
                all_models = await self.client.get_lookml_models(fields=["name", "project_name", "explores"])
                logger.debug(f"Retrieved {len(all_models)} models from target")
            except Exception as e:
                raise LookerValidatorException(f"Failed to retrieve LookML models from target: {e}")
            ...
            if not project_models:
                logger.warning(f"No LookML models found for project '{self.project}' on target.")
                return []
            ...
            for model in project_models:
                ...
                for explore_data in model["explores"]:
                    ...
                    explore = LookMLExplore(name=explore_name, model_name=model_name)
                    explores_list.append(explore)
                    # Use schema-only fetch instead of full dimension fetch
                    schema_fetch_tasks.append(self._fetch_explore_schema_only(explore))

            logger.info(f"Fetching schemas for {len(schema_fetch_tasks)} selected explores from target...")
            await asyncio.gather(*schema_fetch_tasks)

            final_explores = []
            for explore in explores_list:
                if explore.errors:
                    logger.error(f"Skipping explore '{explore.model_name}.{explore.name}' due to errors fetching schema.")
                    final_explores.append(explore)
                elif not explore.dimensions and not explore.skipped:
                    logger.warning(f"Explore '{explore.model_name}.{explore.name}' has no dimensions in target, skipping.")
                    explore.skipped = SkipReason.NO_DIMENSIONS
                    final_explores.append(explore)
                elif explore.dimensions:
                    final_explores.append(explore)

            logger.info(f"Successfully fetched {len(final_explores)} explore schemas from target '{self.branch or self.commit_ref}'.")
            return final_explores

        except Exception as e:
            logger.exception(f"Failed to get explore schemas from target '{original_target}': {e}")
            raise LookerValidatorException(
                title="Target branch error",
                detail=f"Could not retrieve explore schemas from target state '{original_target}'."
            )
    ...
    def _is_refactoring_error(self, error_message: str, query: Query) -> bool:
        """Check if an error indicates a refactoring/column rename rather than a true validation error.
        
        Common refactoring error patterns:
        - "Name trans_purchase_date not found"
        - "Column not found: trans_purchase_date"
        - References to old column names that were renamed
        """
        # Common column-not-found patterns
        not_found_patterns = [
            "not found",
            "does not exist",
            "cannot be found",
        ...
        # Check if this is a "column not found" type error
        is_column_error = any(pattern in error_lower for pattern in not_found_patterns)
        
        if not is_column_error:
            return False
        
        # Extract the problematic column name from the error
        import re
        
        # Look for column names in error messages like "Name trans_purchase_date not found"
        column_matches = re.findall(r'(?:name|column)\s+(\w+)\s+not found', error_lower)
        if not column_matches:
            # Try broader pattern
            column_matches = re.findall(r'(\w+)\s+not found', error_lower)
        
        if column_matches:
            missing_column = column_matches[0]
            
            # Check if any dimension in this query references the missing column
            for dimension in query.dimensions:
                if dimension.sql and missing_column in dimension.sql:
                    # Check if this looks like a refactored column name
                    if self._is_refactored_column(missing_column, dimension.name):
                        logger.info(f"🔍 Column '{missing_column}' in dimension '{dimension.name}' appears to be refactored - not a validation error")
                        return True
        
        return False
    
    def _is_refactored_column(self, missing_column: str, dimension_name: str) -> bool:
        """Check if a missing column appears to be from refactoring."""
        
        # Common refactoring patterns
        refactoring_indicators = [
            # Old view names in dimension names
            'cma_nps_responses' in dimension_name,
            # Date suffix added/removed
            missing_column.endswith('_date') and missing_column[:-5] in dimension_name,
            missing_column.replace('_date', '') in dimension_name,
            # General column renaming patterns
            missing_column.replace('_', '') in dimension_name.replace('_', ''),
        ]
        
        return any(refactoring_indicators)
            ...
            try:
                ...
                for task_id, result_data in results.items():
                    ...
                    if status == "complete":
                        ...
                        # INTELLIGENT ERROR HANDLING: Check if this is a refactoring/column-not-found error
                        if self._is_refactoring_error(error_message, query):
                            logger.info(f"🧹 Detected refactoring error for {query.explore.model_name}.{query.explore.name} - treating as expected change, not validation failure")
                            # Mark dimensions as queried but don't add errors
                        ...
                        if len(query.dimensions) > 1:
                             ...
                             # Check if this is a refactoring error for a single dimension
                             if self._is_refactoring_error(error_message, query):
                                 logger.info(f"🧹 Dimension '{dimension.name}' has refactoring error - treating as expected change, not validation failure")
                                 dimension.queried = True
                                 # Don't add to errors - this is expected due to refactoring
                             else:
