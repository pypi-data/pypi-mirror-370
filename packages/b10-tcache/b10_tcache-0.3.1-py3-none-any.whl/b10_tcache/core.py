    ...
    with cache_operation("Load"):
        ...
        try:
            with temp_file_cleanup(temp_path):
                # Make a queue using the file system to limit concurrency on copy operation
                copy_out_file = b10fs_dir / "copy_out.lock"

                # Wait until no other copy is in progress
                while copy_out_file.exists():
                    time.sleep(1)

                # Create the lock file (empty file)
                copy_out_file.touch()
                ...
                try:
                    ...
                    copy_out_file.unlink(missing_ok=True)
    ...
    with cache_operation("Save"):
        ...
        try:
            with temp_file_cleanup(local_temp):
                ...
                # Make a queue using the file system to limit concurrency on copy operation
                copy_in_file = b10fs_dir / "copy_in.lock"

                # Wait while another copy is in progress
                while copy_in_file.exists():
                    time.sleep(1)
                ...
                try:
                    ...
                    # Always delete the lock, even on errors
                    copy_in_file.unlink(missing_ok=True)
