...
def test_get_empyt_suite_name(testrail_reporter):
    """Test fetching suite name"""
    mock_response = MagicMock()
    mock_response.status_code = 400
    mock_response.ok = False
...
def test_add_run(testrail_reporter: TestrailReporter):
    """Test if add_run() makes the correct API request and sets run_id."""

    # Mock response for session.post()
    mock_response = MagicMock()
    mock_response.status_code = 200
    mock_response.json.return_value = {"id": 999}  # Fake run ID

    # Set the mock response for session.post
    with patch.object(testrail_reporter, "session", create=True) as mock_session:
        mock_session.post.return_value = mock_response

    # Call the method under test
    run_id = testrail_reporter.add_run([1234])

    assert run_id == 999, "Run ID should be set to the mocked response ID."


def test_add_run_with_error(testrail_reporter: TestrailReporter):
    """Test if add_run() makes the correct API request and sets run_id."""
    ...
    mock_response.json.return_value = {
        "error": "error while creating run"
    }  # Fake run ID
    ...
    assert not run_id, "Run ID should be None incase failure"
...
def test_add_result_to_run(testrail_reporter: TestrailReporter):
    """Test if add_run() makes the correct API request and sets run_id."""
    ...
    # Call the method under test
    response = testrail_reporter.add_test_case_result_to_run(
        case_id="1234",
    ...
    assert response.json()["id"] == 1234, "response shouldn't be empty"


def test_error_add_result_to_run(testrail_reporter: TestrailReporter):
    """Test if add_run() makes the correct API request and sets run_id."""
    ...
    mock_response.status_code = 400
    mock_response.json.return_value = {
        "error": "Field 'status_id' is a required field."
    }
    ...
    # Call the method under test
    response = testrail_reporter.add_test_case_result_to_run(
        case_id="1234", message="Test executed", status=1, elapsed_time=100.02
    )
    assert response.json()["error"] == "Field 'status_id' is a required field.", (
        "error response should be returned"
    )


def test_error_response_add_result_to_run(testrail_reporter: TestrailReporter):
    """Test if add_run() makes the correct API request and sets run_id."""
    ...
    mock_response.ok = False
    mock_response.json.return_value = {
        "error": "error adding testcase result to run",
    }
    # Set the mock response for session.post
    with patch.object(testrail_reporter, "session", create=True) as mock_session:
        mock_session.post.return_value = mock_response
    ...
    assert response.json()["error"] == "error adding testcase result to run", (
        "error response should be returned"
    )
...
def test_pytest_collection_finish_existing_run(testrail_reporter, mock_session):
    """Test when a test run already exists (should return early)"""
    testrail_reporter.run_id = 1234  # Simulate existing run

    with (
        patch("liveramp_automation.utils.log.Logger.info") as mock_logger,
        patch.object(testrail_reporter, "add_run", MagicMock()) as mock_add_run,
    ):
        testrail_reporter.pytest_collection_finish(mock_session)

        # Verify log message
        mock_logger.assert_any_call(
            "Already run exisits, using run: 1234 for updating the test results..."
        )

        # Ensure the function exits early (no further calls to add_run)
        mock_add_run.assert_not_called()


def test_pytest_collection_finish_extract_project_suite_ids_and_case_ids(
    testrail_reporter, mock_session
):
    """Test extracting project_id, suite_id, and handling case_ids correctly"""

    # Mock test items
    mock_item1 = MagicMock()
    mock_item2 = MagicMock()
    mock_session.items = [mock_item1, mock_item2]
    testrail_reporter.suite_id = None
    testrail_reporter.project_id = None

    # Mock method calls
    with patch.object(testrail_reporter, "get_id_from_marker", side_effect=["101", "202", "C123", "C456"]), \
         patch.object(testrail_reporter, "add_run", return_value=7890):
        testrail_reporter.pytest_collection_finish(mock_session)

    # Ensure the run_id is set correctly
    assert testrail_reporter.run_id == 7890


def test_pytest_collection_finish_no_case_ids(testrail_reporter, mock_session):
    """Test handling items without case IDs gracefully"""
    mock_item = MagicMock()
    mock_session.items = [mock_item]

    with patch.object(testrail_reporter, "get_id_from_marker", side_effect=["101", "202", None]), \
         patch.object(testrail_reporter, "add_run") as mock_add_run, \
         patch("liveramp_automation.utils.log.Logger.info") as mock_logger:
        testrail_reporter.pytest_collection_finish(mock_session)

    mock_add_run.assert_called_once_with(
        case_ids=["101"]
    )  # Should be empty
