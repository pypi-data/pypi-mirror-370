...
def main():
    cdp_port_proxy = getenv("CDP_PORT_PROXY")
    cdp_port_direct = getenv("CDP_PORT_DIRECT")
    proxy = getenv("GOOGLE_PROXY")
    cmd1 = 'google-chrome --remote-debugging-port="' + str(cdp_port_proxy) + '" --proxy-server="' + str(proxy) + '" --user-data-dir=/tmp/chrome-profile-proxy &'
    cmd2 = 'google-chrome --remote-debugging-port="' + str(cdp_port_direct) + '" --user-data-dir=/tmp/chrome-profile-direct &'
    system(cmd1)
    system(cmd2)
    # sleep(2)
