...
from ..models.tools.salesforce_models import (
    ...
    RetrieveNoteWithConditionsInput,
...
def retrieve_note_with_conditions(
    *, fields: str | None = DEFAULT, id: str
) -> ComposioResult:
    """Execute Salesforce: Retrieve Note With Conditions"""
    kwargs = locals()
    kwargs = {k: v for k, v in kwargs.items() if v is not DEFAULT}
    input_model = RetrieveNoteWithConditionsInput.model_validate(kwargs)

    client = A79Client()
    output_model = client.execute_tool(
        package="salesforce",
        name="retrieve_note_with_conditions",
        input=input_model.model_dump(),
    )
    return ComposioResult.model_validate(output_model)
