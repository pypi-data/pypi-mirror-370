...
class ThreadSafeQueueHandler(logging.Handler):
            ...
            if hasattr(handler, 'shouldRollover') and hasattr(handler, 'doRollover'):
                try:
                    ...
                    if handler.shouldRollover(dummy_record):
                        handler.doRollover()
                        print(f"🔄 Log rotation performed for {getattr(handler, 'baseFilename', 'unknown')}")
                except Exception as e:
                    print(f"⚠️  Error during rotation check: {e}")
    ...
    def _worker_loop(self):
        ...
        while self.running:
            try:
                ...
                print(f"⚠️  Error processing log record: {e}")
    ...
    def _process_record(self, record: logging.LogRecord):
        ...
        try:
            ...
            print(f"⚠️  Error emitting log record: {e}")
    ...
    def emit(self, record: logging.LogRecord):
        ...
        try:
            ...
            try:
                ...
                print(f"⚠️  Log queue is full, dropping log record: {record.getMessage()}")
        except Exception as e:
            print(f"⚠️  Error queuing log record: {e}")
...
def load_config(config_file: str = "app_config.ini") -> configparser.ConfigParser:
    """Load configuration from app_config.ini file with fallback defaults."""
    config = configparser.ConfigParser()
    
    config['app'] = {
        'name': 'powerlogger_app'
    }
...
def get_logger(name: str = None, level: int = None, config_file: str = "app_config.ini") -> logging.Logger:
...
def get_logger_with_queue(name: str = None, level: int = None, 
                         config_file: str = "app_config.ini") -> logging.Logger:
    ...
    queue_enabled = config.get('logging', 'queue_enabled', fallback='true').lower() == 'true'
    ...
    queue_handler = ThreadSafeQueueHandler(queue_size=queue_size, flush_interval=flush_interval)
...
def get_logger_with_queue_and_file(name: str = None, level: int = None, 
                                 log_file: str = None, config_file: str = "app_config.ini") -> logging.Logger:
    ...
    try:
        ...
        file_format = config.get('logging', 'format', fallback='%(asctime)s - %(name)s - %(levelname)s - %(message)s')
        ...
        print(f"⚠️  Warning: Could not add file handler to queue: {e}")
...
def get_logger_with_file_handler(name: str = None, level: int = None, 
                               log_file: str = None, config_file: str = "app_config.ini") -> logging.Logger:
    ...
    try:
        ...
        combined_logger.file_handler = file_handler
        
        return combined_logger
        
    except Exception as e:
        print(f"⚠️  Warning: Could not add file handler: {e}")
        return get_logger(name, level, config_file)
...
def cleanup_loggers():
            ...
            for logger in loggers:
                for handler in logger.handlers[:]:
                    try:
                        ...
                        print(f"⚠️  Warning: Could not close handler {handler}: {e}")
                
                if hasattr(logger, 'file_handler'):
                    try:
                        logger.file_handler.close()
                    except Exception as e:
                        print(f"⚠️  Warning: Could not close file handler: {e}")
    except Exception as e:
        print(f"⚠️  Warning: Could not access logger manager: {e}")
    ...
    for handler in logging.root.handlers[:]:
        try:
            ...
            print(f"⚠️  Warning: Could not close root handler: {e}")
