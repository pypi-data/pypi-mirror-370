...
class BaseClient(ABC):
    ...
    def _validate_criteria(self, criteria: Optional[Union[str, List[str]]]) -> None:
        """Validate that criteria is provided and follows the required format"""
        if criteria is None:
            raise ValueError(
                "criteria parameter is required. Please provide evaluation criteria."
            )
        ...
        all_valid_prefixes = reward_prefixes + binary_prefixes

        # Convert to list if single string
        criteria_list = [criteria] if isinstance(criteria, str) else criteria

        # Validate each criterion
        for i, criterion in enumerate(criteria_list):
            if not criterion or not isinstance(criterion, str):
                raise ValueError(f"Criteria {i+1} must be a non-empty string")

            # Check if criterion starts with any valid prefix (case-insensitive)
            if not any(
                criterion.lower().startswith(prefix.lower())
                for prefix in all_valid_prefixes
            ):
                raise ValueError(
                    f"Criteria {i+1} must start with one of the following prefixes: "
                    f"{', '.join(all_valid_prefixes)}"
                )
