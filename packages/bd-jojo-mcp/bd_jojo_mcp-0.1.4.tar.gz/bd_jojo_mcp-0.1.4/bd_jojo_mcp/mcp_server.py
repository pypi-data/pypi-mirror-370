...
def get_error_repo_name(workspace_dir: str) -> str:
    ...
    error_lines = basic_tool.get_all_error_lines(build_log_content)
    if error_lines:
        # 找到所有的repo_name，不包括空值
        repo_names = [basic_tool.extract_repo_name(line) for line in error_lines if basic_tool.extract_repo_name(line)]
