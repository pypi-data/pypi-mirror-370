...
def main():
    ...
    id2label = {idx: label for label, idx in label2id.items()}  # 反向映射
    ...
    # model = ElectraForSequenceClassification.from_pretrained("hfl/chinese-roberta-wwm-ext", num_labels=num_labels)  # num_labels 根据任务调
    ...
    #中文模型
    # tokenizer = BertTokenizer.from_pretrained("hfl/chinese-roberta-wwm-ext", mirror="tuna")
    # model1 = BertForSequenceClassification.from_pretrained("hfl/chinese-roberta-wwm-ext", num_labels=num_labels, mirror="tuna")

    tokenizer = BertTokenizer.from_pretrained("hfl/chinese-roberta-wwm-ext-large")
    model1 = BertForSequenceClassification.from_pretrained("hfl/chinese-roberta-wwm-ext-large", num_labels=num_labels)
    
    # tokenizer = ElectraTokenizer.from_pretrained("hfl/chinese-electra-180g-large-discriminator")
    # model1 = ElectraForSequenceClassification.from_pretrained("hfl/chinese-electra-180g-large-discriminator", num_labels=num_labels)
    ...
    test(model1,test_dataset,train_dataset,id2label)

# ======================== Test ========================
def test(model, test_dataset, train_dataset, id2label):
    ...
    # 反向映射数字ID到标签文本
    train_labels_text = [id2label[train_dataset[i]["labels"].item()] for i in range(len(train_dataset))]
    train_preds_text = [id2label[p] for p in train_preds]

    test_labels_text = [id2label[test_dataset[i]["labels"].item()] for i in range(len(test_dataset))]
    test_preds_text = [id2label[p] for p in test_preds]

    # 保存 CSV
    train_df = pd.DataFrame({
        "label": train_labels_text,
        "pred": train_preds_text
    })
    test_df = pd.DataFrame({
        "label": test_labels_text,
        "pred": test_preds_text
    })
