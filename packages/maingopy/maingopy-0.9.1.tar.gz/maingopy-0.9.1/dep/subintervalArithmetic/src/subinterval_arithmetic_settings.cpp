...
namespace subinterval_arithmetic {
	...
	// Only branch more when pow(minBranchFacPerDim, dim) < numSubIntervals
	// Where dim = numBranchDims
	int subinterval_arithmetic_settings::calc_branchFacPerDim() {
		...
		// Get the new number of subintervals, determine how many dimensions to perform adaptive branching
		// Do pow(branch_per_dim+1, numBranchMoreDims)*pow(branch_per_dim, numBranchDims-numBranchMoreDims) instead of pow(branch_per_dim, numBranchDims)
	...
	subinterval_arithmetic_settings::subinterval_arithmetic_settings(int dim, int maxNumSubintervals, int intervalArithmetic, int branchingStrategy, int centerStrategy, int minBranchFacPerDim) {
