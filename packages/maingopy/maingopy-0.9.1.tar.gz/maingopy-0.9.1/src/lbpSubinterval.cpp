...
#include "dagLibrary.h"
#include "subinterval_arithmetic.h"
...
/////////////////////////////////////////////////////////////////////////
// constructor for the lower bounding solver, update the constructor to the latest version of MAiNGO (referring lbpinterval class)
    ...
    LowerBoundingSolver(DAG, DAGvars, DAGfunctions, variables, variableIsLinear, nineqIn, neqIn, nineqRelaxationOnlyIn, neqRelaxationOnlyIn, nineqSquashIn, settingsIn, loggerIn, constraintPropertiesIn)
{
    ...
    _svtDag = std::make_shared<SVT_DAG::Dag>();
    SVT_DAG::construct_dag_from_MCdag(*_svtDag, _DAGobj->DAG, _DAGobj);

    int inputDim = _svtDag->numIndependentVars;
    ...
    _subintervalArithmeticSettings = std::make_shared<subinterval_arithmetic::subinterval_arithmetic_settings>(inputDim, numSubintervals, intervalArithmetic, branchingStrategy, centerStrategy, minBranchingPerDim);
    ...
    _subintervalArithmeticMemory = std::make_shared <subinterval_arithmetic::subinterval_arithmetic_memory<I_cpu>>(inputDim, numSubintervals, intervalArithmetic, *_svtDag, numBranchMoreDims, adaptiveBranching);

    _dagVarValues.clear();
    _dagVarValues.resize(_svtDag->numVars);
...
LbpSubinterval::~LbpSubinterval(){
    _subintervalArithmeticMemory -> clear();
}
...
LbpSubinterval::_update_LP(const babBase::BabNode& currentNode)
{
    ...
    _subintervalArithmeticMemory->copy_domain_into_memory(_Intervals.data());
    ...
    subinterval_arithmetic::perform_subinterval_arithmetic_seriel(*_subintervalArithmeticMemory, *_subintervalArithmeticSettings); 

    // Store results
    _subintervalArithmeticMemory->update_results(); 
...
/////////////////////////////////////////////////////////////////////////////////////////////
// solves the current linear program, update the return type (to the latest version of MAiNGO)
LP_RETCODE 
...
/////////////////////////////////////////////////////////////////////////////////////////////
// check need for options in preprocessing, update the inputs to print_message function (the latest version of MAiNGO), refering to lbpinterval class.
void
