...
def causal_pipeline(output_path: Path) -> ExecutableGraph:
    """Create a comprehensive causal inference pipeline.
    
    This is the main entry point for PyAutoCausal. It creates a complete
    pipeline that automatically selects appropriate causal methods based
    ...
    Returns:
        Configured ExecutableGraph ready for execution with data
    ...
    Example:
        ...
        >>> import pandas as pd
        >>> 
        >>> pipeline = causal_pipeline(Path("./output"))
        >>> results = pipeline.fit(df=your_data)
    """
    graph = ExecutableGraph()
    ...
    # Add both panel and cross-sectional decision structures
    create_panel_decision_structure(graph)
    create_cross_sectional_decision_structure(graph)
    
    # Add all analysis branches
    from .branches import (
        create_did_branch,
        create_event_study_branch, 
        create_synthetic_did_branch,
