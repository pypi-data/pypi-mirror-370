...
def get_llm_feedback(question, user_answer, correct_solution):
    ...
    if not model:
        return "INFO: Set GEMINI_API_KEY or OPENAI_API_KEY environment variables to get AI-powered feedback."
    ...
    try:
        ...
        if llm_type == "gemini":
            ...
            response = model.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "You are a Kubernetes expert helping a student study for the CKAD exam."},
...
@cli.group()
def config():
    """Manage global configuration and secrets."""
    pass

@config.command()
def list():
    """List all global configuration key-value pairs."""
    clear_screen()
    print(f"{Style.BRIGHT}{Fore.CYAN}--- Global Configuration ---{Style.RESET_ALL}")
...
def set(key, value):
    ...
    set_key(os.path.join(os.getcwd(), '.env'), key.upper(), value)
    print(f"Configuration key '{key.upper()}' set successfully.")

@config.command()
@click.argument('key')
def remove(key):
    """Remove a configuration key."""
    # set_key with empty string effectively removes it from dotenv_values
    set_key(os.path.join(os.getcwd(), '.env'), key.upper(), "")
    # Also remove from os.environ if it was loaded
    if key.upper() in os.environ:
        del os.environ[key.upper()]
    print(f"Configuration key '{key.upper()}' removed successfully.")
