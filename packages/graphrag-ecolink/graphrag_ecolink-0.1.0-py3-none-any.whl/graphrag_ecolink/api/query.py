...
from graphrag_ecolink.query.factory import (
    get_basic_search_engine,
    get_drift_search_engine,
    get_global_search_engine,
    get_local_search_engine, get_local_search_engine_for_ecolink,
)
from graphrag_ecolink.query.indexer_adapters import (
...
from graphrag_ecolink.utils.api import (
    get_embedding_store,
    load_search_prompt,
    update_context_data,
)
...
def global_search_streaming(
    ...
    """
    """
    read_indexer_communities 这段代码的主要功能是：
    - 确保社区与社区报告匹配，
    - 过滤掉没有社区报告的社区，
    - 转成Community
    """
    communities_ = read_indexer_communities(communities, community_reports)
    logging.info(f"Communities整理后的数据: {communities_}")
    """
    read_indexer_reports 这段代码的主要功能是：
    ...
    logging.info(f"reports整理后的数据: {reports}")
    """
    read_indexer_entities 这段代码的主要功能是：
    ...
    logging.info(f"entities_整理后的数据: {entities_}")
    # 用来回答分数的，在最后一步给大模型的回答中应用的 {
    #     "points": [
    #         {"description": "关键点 1 的描述 [数据：报告 (报告 ID)]", "score": 重要性分值},
    #         {"description": "关键点 2 的描述 [数据：报告 (报告 ID)]", "score": 重要性分值}
    #     ]
    # }
    map_prompt = load_search_prompt(config.root_dir, config.global_search.map_prompt)
    # “X 先生是 Y 公司的所有者，并且面临许多不当行为的指控 [数据：报告 (2, 7, 34, 46, 64, +more)]。他还是 X 公司的首席执行官 [数据：报告 (1, 3)]。”  其中 1、2、3、7、34、46 和 64 表示相关数据记录的 ID（而不是索引）。
    reduce_prompt = load_search_prompt(
        config.root_dir, config.global_search.reduce_prompt
    )
    # 该响应可能包括数据集之外的相关真实世界知识，但必须明确标注验证标签 [LLM: verify]。例如： “这是一个由真实世界知识支持的示例句子 [LLM: verify]。”
...
def local_search_streaming(
    ...
    #     callbacks=callbacks,
    # )
    search_engine = get_local_search_engine_for_ecolink(
        config=config,
        # response_type=response_type,
        system_prompt=prompt,
    )
    # return search_engine.stream_search(query=query)
    return search_engine.stream_search_for_ecolink(query=query,config=config)
