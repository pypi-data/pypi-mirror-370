...
from graphrag_ecolink.language_model.providers.fnllm.events import FNLLMEvents
...
from graphrag_ecolink.language_model.response.base import (
    BaseModelOutput,
    BaseModelResponse,
    ModelResponse,
)
...
if TYPE_CHECKING:
    ...
    from graphrag_ecolink.config.models.language_model_config import (
        LanguageModelConfig,
    )
...
class OpenAIChatFNLLM:
    ...
    async def achat_stream(
        ...
        try:
            ...
            logging.info("调用 结束，prompt：%s, response: %s", prompt, response)
            logging.info("调用耗时: %.2fs", time.time() - start)
            async for chunk in response.output.content:
                if chunk is not None:
                    yield chunk
        except Exception as e:
            logging.info("调用 出错，prompt：%s", prompt, exc_info=True)
