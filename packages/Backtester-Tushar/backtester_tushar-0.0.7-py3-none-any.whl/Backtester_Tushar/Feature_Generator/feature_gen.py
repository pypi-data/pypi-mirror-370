...
class FeatureGenerator:
    ...
    def compute_cross_sectional_features(self, df_all, rank_method="dense"):
        """
    ...
    def compute_sector_spreads(self, data, mapping_file,
        ...
        returns_file : str
            Path to a CSV file containing daily returns of all companies.
        ...
        sector_df = _compute_daily_sector_median_returns(mapping_file, df,
        ...
        for w in windows:
            ...
            df[f"Company_CumRet_{w}D"] = (
                    1 + df.groupby("Symbol")[returns_column_name].transform(lambda x: x.rolling(w).sum())
            )
        ...
        for w in windows:
            df[f"Company_Vol_{w}D"] = (
                df.groupby("Symbol")[returns_column_name].transform(lambda x: x.rolling(w).std())
            )
