    ...
    def compute_sector_spreads(self, data, mapping_df,
        ...
        for w in windows:
            ...
            df[f"Sector_CumRet_{w}D"] = (
                    1 + df.groupby(sector_column_name)["Sector_Return"].transform(lambda x: x.rolling(w).sum())
            )
        ...
        for w in windows:
            ...
            df[f"Sector_Vol_{w}D"] = (
                df.groupby(sector_column_name)["Sector_Return"].transform(lambda x: x.rolling(w).std())
            )
