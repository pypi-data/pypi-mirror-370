...
class FeatureGenerator:
    ...
    def compute_cross_sectional_features(self, parquet_dir, return_periods=[1], rank_method="dense"):
        """
        ...
        ----------
        parquet_dir : str
            Path to the directory containing parquet files for each ticker.
            Each parquet file must contain 'Close' prices and any features from
            `compute_features` or `price_action_transformations`.
        return_periods : list of int, optional (default=[1])
            List of lookback periods (in days) for which to compute returns and ranks.
        ...
        -------
        >>> fg = FeatureGenerator()
        >>> df_cs = fg.compute_cross_sectional_features("data/parquet", return_periods=[1, 5])
        >>> df_cs.xs("AAPL", level="Ticker").tail()
        """
        all_dfs = []
        ...
        df_all = df_all.set_index(["Ticker", "Date"]).sort_index()


        zscore_cols = [col for col in df_all.columns if "Z_Score" in col]
        for col in zscore_cols:
            rank_name = f"{col}_Cross_Sectional_Rank"
            df_all[rank_name] = df_all.groupby(level="Date")[col].rank(method=rank_method, ascending=False)
    ...
    def compute_sector_spreads(self, data, mapping_file, sector_returns_file, windows=None):
        """
        Compute multi-horizon spreads between company and sector returns/volatilities.

        Parameters
        ----------
        mapping_file : str
            CSV path containing company-to-sector mapping.
        ...
        sector_returns_file : str
            CSV path containing daily sector median returns.
            Must contain: ['Date', 'Sector', 'Median_Return'].

        windows : list of int, optional
            List of lookback periods (in days) for spreads.
            Default: [1, 3, 5, 10, 20, 40, 60, 120, 252].
        ...
        pd.DataFrame
            ...
            ['Date', 'Symbol', 'Sector', 'Return', 'Sector_Return',
             'Return_Spread_{window}D', 'Vol_Spread_{window}D', ...].
        """
        if windows is None:
        ...
        sector_df = pd.read_csv(sector_returns_file)

        # Normalize column names
        mapping_df.columns = mapping_df.columns.str.strip().str.capitalize()
        sector_df.columns = sector_df.columns.str.strip().str.capitalize()
        df = data
        df.columns = df.columns.str.strip().str.capitalize()

        # Merge mapping
        df = df.merge(mapping_df, on="Symbol", how="left")

        # Merge sector returns
        df = df.merge(sector_df, on=["Date", "Sector"], how="left")
        ...
        for w in windows:
            ...
            df[f"Company_CumRet_{w}D"] = (
                    1 + df.groupby("Symbol")["Return"].transform(lambda x: x.rolling(w).sum())
            )
            df[f"Sector_CumRet_{w}D"] = (
                    1 + df.groupby("Sector")["Sector_Return"].transform(lambda x: x.rolling(w).sum())
            )
        ...
        for w in windows:
            df[f"Company_Vol_{w}D"] = (
                df.groupby("Symbol")["Return"].transform(lambda x: x.rolling(w).std())
            )
            df[f"Sector_Vol_{w}D"] = (
                df.groupby("Sector")["Sector_Return"].transform(lambda x: x.rolling(w).std())
            )
