...
class AkumuliTester:
    ...
    def __init__(self,tcp_port=None,http_port=None):
        if tcp_port is None:
        ...
        self.URL = f"http://localhost:{http_port}/api/query"

    @asynccontextmanager
    async def _daemon(self, http=None, tcp=None):
        if http is None:
            http = self.HTTP_PORT
        if tcp is None:
            tcp = self.TCP_PORT
                ...
                async with await anyio.open_process(["akumulid", "--config", cfg]) as proc:
                    try:
                        with anyio.fail_after(10):
                            while True:
                                try:
                                    async with await anyio.connect_tcp("127.0.0.1", self.TCP_PORT) as s:
                                        pass
                                    break
                                except OSError:
