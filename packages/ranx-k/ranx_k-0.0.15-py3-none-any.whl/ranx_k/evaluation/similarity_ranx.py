    ...
    for i, (question, ref_docs) in iterator:
        ...
        if evaluation_mode == 'reference_based':
            ...
            for ret_idx in range(len(retrieved_texts)):
                ...
                if similarity_matrix.shape[0] > 0:
                    ...
                    if best_similarity >= similarity_threshold:
                        ...
                        # CRITICAL: Preserve retrieval order - use position-based score
                        base_score = 1000.0
                        position_decay = 10.0
                        
                        if use_graded_relevance:
                            # For graded relevance, combine scaled similarity with position
                            scaled_score = 1.0 + (best_similarity - similarity_threshold) * 9.0 / (1.0 - similarity_threshold)
                            # Strong position-based score + graded relevance bonus
                            final_score = base_score - (ret_idx * position_decay) + scaled_score
                            # Only update if this is a better score (preserves best match)
                            if ref_id not in run_dict[query_id] or final_score > run_dict[query_id][ref_id]:
                                run_dict[query_id][ref_id] = float(final_score)
                        else:
                            # For binary relevance, position dominates but similarity is considered
                            final_score = base_score - (ret_idx * position_decay) + best_similarity
                            if ref_id not in run_dict[query_id] or final_score > run_dict[query_id][ref_id]:
                                run_dict[query_id][ref_id] = float(final_score)
                    else:
                        ...
                        base_score = 1000.0
                        position_decay = 10.0
                        run_dict[query_id][ret_id] = float(base_score - (ret_idx * position_decay) + best_similarity)
                else:
            ...
            for j in range(len(retrieved_texts)):
                ...
                # Documents are already ranked by the retriever (including reranking)
                # We use decreasing scores based on position to maintain this order
                # Score = base_score - position * decay
                base_score = 1000.0  # High base to ensure all scores are positive
                position_decay = 10.0  # Significant decay to maintain order
                
                # Score that preserves retrieval order while incorporating similarity
                # Higher position (lower j) = higher score, regardless of similarity
                order_preserving_score = base_score - (j * position_decay) + max_similarity
                
                # Add to run: all retrieved documents with strict order preservation
