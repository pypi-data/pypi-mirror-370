...
def convert_images_to_gif(
    ...
    Args:
        ...
        custom_name (str, optional): Custom name for the GIF file (without extension). If None, uses automatic naming.
    ...
    # Validate and Auto-Correct Custom_name Parameter
    original_custom_name = custom_name
    if custom_name is not None and (not isinstance(custom_name, str) or custom_name.strip() == ""):
        custom_name = None  # Default Value (Automatic Naming)
        print(f"⚠️  Warning: Invalid custom_name '{original_custom_name}'. Using automatic naming instead.")
    ...
    # Generate Output Path Based on Custom Name or Automatic Naming
    if custom_name is not None:
        # Use Custom Name with .gif Extension
        base_output_path = input_path / f"{custom_name}.gif"
        ...
        base_output_path = input_path / "Animation.gif"
        output_path = generate_unique_filename(base_output_path)
