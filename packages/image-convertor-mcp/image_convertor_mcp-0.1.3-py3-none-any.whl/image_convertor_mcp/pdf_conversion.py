...
def convert_images_to_pdf(
    ...
    try:
        ...
        if file_name is None:
            ...
            try:
                ...
                if not base_name or base_name.strip() == "":
                    print(f"⚠️  Warning: Invalid file_name '{file_name}', using default")
                    base_name = "Converted PDF"
            except Exception as e:
                print(f"⚠️  Warning: Invalid file_name '{file_name}', using default")
                base_name = "Converted PDF"
        ...
        try:
            dpi = int(dpi)
            if not (72 <= dpi <= 1200):
                print(f"⚠️  Warning: Invalid DPI {dpi}, using 300")
                dpi = 300
        except (ValueError, TypeError):
            print(f"⚠️  Warning: DPI must be a number, got {type(dpi).__name__}, using 300")
            dpi = 300
        ...
        if not isinstance(background_color, str):
            print(f"⚠️  Warning: background_color must be string, got {type(background_color).__name__}, using white")
            background_color = "white"
        
        if background_color.lower() not in background_colors:
            print(f"⚠️  Warning: Invalid background_color '{background_color}', using white")
            background_color = "white"
        else:
        ...
        if not isinstance(sort_order, str):
            print(f"⚠️  Warning: sort_order must be string, got {type(sort_order).__name__}, using alphabetical")
            sort_order = "alphabetical"
        
        if sort_order not in ["alphabetical", "creation_time", "modification_time"]:
            print(f"⚠️  Warning: Invalid sort_order '{sort_order}', using alphabetical")
            sort_order = "alphabetical"
            ...
            try:
                if isinstance(param, str):
                    if param.lower() in ['true', '1', 'yes', 'on']:
                        ...
                        print(f"⚠️  Warning: Invalid {param_name} '{param}', using {default}")
                        return default
                else:
                    result = bool(param)
                    print(f"⚠️  Warning: {param_name} converted from {type(param).__name__} to {result}")
                    return result
            except Exception as e:
                print(f"⚠️  Warning: Invalid {param_name} '{param}': {e}, using {default}")
                return default
        ...
        print(f"🔄 Found {len(supported_images)} images to combine into single PDF")
        ...
        try:
            for i, image_path in enumerate(supported_images, 1):
                try:
                    print(f"🔄 Processing {i}/{len(supported_images)}: {image_path.name}")
                    
                    # Open and Convert Image
                    with Image.open(image_path) as img:
                        # Convert to RGB If Needed - minimize copies
                    ...
                    print(f"⚠️  Warning: Could not process {image_path.name}: {e}")
                    continue
            
            if not processed_images:
                raise RuntimeError("No images were successfully processed")
            
            print(f"💾 Saving {len(processed_images)} pages to PDF...")
            ...
            print(f"✅ PDF created successfully: {final_output_path}")
            print(f"📊 Pages: {len(processed_images)}, Page size: {page_size} ({page_width}x{page_height} pixels at {dpi} DPI)")
            
            return str(final_output_path)
            
        finally:
            # Clean Up Processed Images to Free Memory
            for page in processed_images:
                try:
                    page.close()
                except:
                    pass
...
def get_file_order(folder_path: str, sort_order: str) -> list:
    ...
    try:
        ...
        if not folder_path_obj.exists() or not folder_path_obj.is_dir():
            print(f"⚠️  Warning: Invalid folder path, using alphabetical order")
            return []
        ...
        try:
            ...
            print(f"⚠️  Warning: Sorting failed: {e}, using alphabetical order")
            image_files.sort(key=lambda x: x.name)
        
        return image_files
        
    except Exception as e:
        print(f"⚠️  Warning: File ordering failed: {e}, using alphabetical order")
...
def get_page_dimensions(page_size: str, dpi: int) -> tuple:
    ...
    try:
        ...
        print(f"⚠️  Warning: Page dimension calculation failed: {e}, using A4 default")
...
def fit_image_to_page(image_size: tuple, page_size: tuple) -> tuple:
    ...
    try:
        ...
        if img_width <= 0 or img_height <= 0 or page_width <= 0 or page_height <= 0:
            print(f"⚠️  Warning: Invalid dimensions, returning original size")
            return image_size
        ...
        if new_width < 1 or new_height < 1:
            print(f"⚠️  Warning: Calculated size too small, using original")
            return image_size
        
        return (new_width, new_height)
        
    except Exception as e:
        print(f"⚠️  Warning: Image fitting calculation failed: {e}, returning original size")
        return image_size
