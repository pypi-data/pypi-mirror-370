...
def MOV_SR(sr_name: str, gp_reg: str) -> bytes:
    ...
    if sr_name.lower() in seg_reg_val:
        reg_val = seg_reg_val[sr_name.lower()]
    else:
...
def MOV_imm(reg_name, value: str | int, disp: int = 0):
    ...
    if reg_name in reg8:
        ...
        return bytes([mov_r8_imm8_base + reg8[reg_name]]) + UINT8(value)
    ...
    # 16-bit registers
        ...
        if is_gp_reg:
            ...
            return bytes([mov_r16_imm16_base + reg16[reg_name]]) + UINT16(value)
    
    # Segment registers
    elif reg_name in seg:
        opcode = 0x8E  # MOV SR, r/m16
        reg_val = seg[reg_name]
        if is_gp_reg:
            rm_val = reg16[value.lower()]
        else:
            rm_val = RM["[bx+si]"]  # default memory
        mod_val = mod["normal"]
        disp_bytes = b""
        if disp != 0:
            if -128 <= disp <= 127:
        ...
        if is_gp_reg:
            ...
            return bytes([opcode, modrm_byte]) + disp_bytes + UINT16(value)
    ...
    # Memory operand
        ...
        if opcode == 0xC6:
            return bytes([opcode, modrm_byte]) + UINT8(value) + disp_bytes
        else:
            return bytes([opcode, modrm_byte]) + UINT16(value) + disp_bytes
