...
def get_db_config():
    ...
    # Encryption settings (Issue #11)
    # Handle encryption parameter - should be one of: 'default', 'off', 'require', or None
    encrypt_str = os.getenv("MSSQL_ENCRYPT", "default").lower()
    ...
    if config["server"] and ".database.windows.net" in config["server"]:
        ...
        # Azure SQL uses 'encrypt' parameter and requires encryption
        config["encrypt"] = "require"
    else:
        # For non-Azure connections, use 'encryption' parameter
        if encrypt_str in ["default", "off", "require"]:
            config["encryption"] = encrypt_str
