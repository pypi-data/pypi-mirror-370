...
class GrammarAnalyzer:
    ...
    def analyze_grammar_symbols(self, so_file_path: Path) -> dict[str, Any]:
        ...
        try:
            ...
            if result.returncode != 0:
                # Try without -D flag
                result = subprocess.run(
                    ["nm", str(so_file_path)], capture_output=True, text=True, timeout=5, check=False,
                )
    ...
    def detect_supported_features(
        self, language: str, symbols: dict[str, Any],
    ) -> list[str]:
    ...
    def determine_language_version_support(
        self, language: str, grammar_version: str,
    ) -> tuple[str, str]:
...
class GrammarMetadataExtractor:
    ...
    def extract_from_headers(so_file_path: Path) -> dict[str, Any]:
        ...
        try:
            ...
            result = subprocess.run(
                ["file", str(so_file_path)], capture_output=True, text=True, timeout=5, check=False,
            )
    ...
    def extract_from_dependencies(so_file_path: Path) -> dict[str, Any]:
        ...
        try:
            ...
            result = subprocess.run(
                ["ldd", str(so_file_path)], capture_output=True, text=True, timeout=5, check=False,
            )
