...
class CompatibilityErrorDetector:
    ...
    def _load_compatibility_patterns(self) -> dict[str, list[re.Pattern]]:
        ...
        patterns = {
            "version_mismatch": [
                ...
                re.compile(
                    r"requires\s+.*\s+version\s+(\d+\.\d+(?:\.\d+)?)", re.IGNORECASE,
                ),
            ...
            "deprecated": [
                re.compile(
                    r"deprecated\s+in\s+version\s+(\d+\.\d+(?:\.\d+)?)", re.IGNORECASE,
                ),
                re.compile(
                    r"removed\s+in\s+version\s+(\d+\.\d+(?:\.\d+)?)", re.IGNORECASE,
                ),
                ...
                for pattern in patterns:
                    if pattern.search(error_message):
                        error = self._create_compatibility_error(
                            pattern_type, error_message, file_path, language,
                        )
                        if error:
                            errors.append(error)
                        break
    ...
    def _create_compatibility_error_from_analysis(
        self, issue: dict[str, Any], file_path: Path, language: str,
    ) -> ClassifiedError | None:
    ...
    def _generate_compatibility_actions(
        self, pattern_type: str, language: str | None,
    ) -> list[str]:
        ...
        try:
            ...
            if detected_version:
                ...
                lang_version = LanguageVersion(
                    language=language, version=detected_version,
                )
                ...
                if compatible_grammar:
                    ...
                    analysis["compatibility_level"] = (
                        self.compatibility_db.get_compatibility_level(
                            lang_version, compatible_grammar,
                        ).value
                    )
    ...
    def _get_most_likely_version(
        self, version_info: dict[str, str | None],
    ) -> str | None:
    ...
    def detect_version_mismatch(
        self, detected_version: str, grammar_version: str, language: str,
    ) -> ClassifiedError | None:
        ...
        try:
            ...
            if compatibility_level in [
                ...
                return ClassifiedError(
                    ...
                    source=ErrorSource.COMPATIBILITY_CHECKER,
                    context=ErrorContext(
                        language=language, grammar_version=grammar_version,
                    ),
    ...
    def detect_feature_incompatibility(
        self, required_features: list[str], supported_features: list[str], language: str,
    ) -> list[ClassifiedError]:
    ...
    def detect_breaking_changes(
        self, from_version: str, to_version: str, language: str,
    ) -> list[ClassifiedError]:
        ...
        try:
            ...
            breaking_changes = self.compatibility_db.get_breaking_changes(
                language, from_version, to_version,
            )
        ...
        try:
            ...
            if analysis.get("detected_version"):
                compatible_versions = self.suggest_compatible_versions(
                    analysis["detected_version"], language,
                )
    ...
    def suggest_compatible_versions(
        self, current_version: str, language: str,
    ) -> list[str]:
        ...
        try:
            ...
            current_lang_version = LanguageVersion(
                language=language, version=current_version,
            )
...
class VersionCompatibilityAnalyzer:
    ...
    def analyze_version_range(
        self, language: str, min_version: str, max_version: str,
    ) -> dict[str, Any]:
            ...
            for grammar in grammar_versions:
                ...
                supports_min = (
                    self._version_compare(
                        min_version, grammar.min_language_version or "0.0",
                    )
                    >= 0
                )
                supports_max = True
    ...
    def find_optimal_grammar_version(
        self, language: str, target_version: str,
    ) -> str | None:
...
class CompatibilityErrorFormatter:
    ...
    def format_feature_incompatibility_error(
        self, missing_features: list[str], language: str, version: str,
    ) -> str:
        ...
        try:
            ...
            message = template.format(
                language=language, version=version, missing_features=features_list,
            )
    ...
    def format_breaking_change_error(
        ...
        try:
            ...
            message = template.format(
                from_version=from_version, to_version=to_version, changes=changes_list,
            )
    ...
    def generate_resolution_steps(
        self, error_type: str, language: str, version: str,
    ) -> list[str]:
