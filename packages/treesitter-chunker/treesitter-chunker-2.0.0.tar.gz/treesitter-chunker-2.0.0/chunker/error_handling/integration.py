...
class ErrorHandlingPipeline:
    ...
    def process_error(
        ...
        try:
            ...
            classified_error = self._classify_error(
                error_message, error_context, session, result,
            )
            ...
            if self.compatibility_detector and classified_error:
                compatibility_issues = self._detect_compatibility(
                    classified_error, session, result,
                )
            ...
            if self.guidance_engine and classified_error:
                guidance_sequence = self._generate_guidance(
                    classified_error, user_profile, session, result,
                )
            ...
            if self.troubleshooting_db:
                troubleshooting_entries = self._search_troubleshooting(
                    error_message, classified_error, session, result,
                )
            ...
            if not any([result.guidance_sequence, result.troubleshooting_entries]):
                fallback_response = self._generate_fallback_response(
                    error_message, classified_error, error_context,
                )
    ...
    def _get_suggested_actions(
        self, error_message: str, classified_error: ClassifiedError | None,
    ) -> list[str]:
...
class ErrorHandlingOrchestrator:
    ...
    def __init__(
        ...
        self._cleanup_timer = threading.Timer(
            300, self._cleanup_expired_sessions,
        )  # 5 minutes
    ...
    def create_session(
        self, user_id: str | None = None, context: dict[str, Any] | None = None,
    ...
    def process_error_in_session(
        ...
        result = self.pipeline.process_error(
            error_message, error_context, session, user_profile or session.user_profile,
        )
...
class CLIErrorIntegration:
    ...
    def handle_grammar_validation_error(
        self, language: str, error_message: str, grammar_path: str | None = None,
    ) -> dict[str, Any]:
        ...
        try:
            ...
            result = self.orchestrator.process_error_in_session(
                session.session_id, error_message, context,
            )
    ...
    def handle_grammar_download_error(
        self, language: str, error_message: str, url: str | None = None,
    ) -> dict[str, Any]:
    ...
    def _format_guidance_for_cli(
        self, guidance_sequence: GuidanceSequence | None,
    ) -> list[dict[str, Any]]:
    ...
    def _format_troubleshooting_for_cli(
        self, entries: list[TroubleshootingEntry],
    ) -> list[dict[str, Any]]:
...
def get_system_health_report(
    pipeline: ErrorHandlingPipeline, orchestrator: ErrorHandlingOrchestrator,
) -> dict[str, Any]:
