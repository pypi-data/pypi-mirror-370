...
class CompatibilityChecker:
        ...
        try:
            ...
            if not validation_result.is_valid:
                ...
                result = self._perform_detailed_compatibility_check(
                    result, grammar_info, code_samples or [],
                )
    ...
    def detect_breaking_changes(
        self, language: str, old_version: str, new_version: str,
    ) -> list[dict[str, Any]]:
    ...
    def suggest_grammar_version(
        ...
        try:
            ...
            # Find best version
            best_version = max(
                version_scores.keys(), key=lambda v: version_scores[v]["score"],
            )
            ...
            return {
                ...
                "reasoning": self._generate_recommendation_reasoning(
                    best_result, version_scores, requirements,
                ),
                "alternatives": sorted(
                    [
                        {
    ...
    def _compare_test_results(
        self, old_result: CompatibilityResult, new_result: CompatibilityResult,
    ) -> list[dict[str, Any]]:
...
class GrammarTester:
    ...
    def run_comprehensive_test(
        self, language: str, test_types: list[str] | None = None,
    ) -> TestResult:
    ...
    def benchmark_parsing_performance(
        self, language: str, sample_sizes: list[int] | None = None,
    ) -> dict[str, Any]:
    ...
    def analyze_error_patterns(
        self, language: str, code_samples: list[str],
    ) -> dict[str, Any]:
...
class SmartSelector:
    ...
    def select_multiple_grammars(
        self, languages: list[str], global_constraints: dict[str, Any] | None = None,
    ...
    def resolve_grammar_conflicts(
        self, conflicts: list[tuple[str, list[SelectionCandidate]]],
            ...
            for language, candidates in conflicts:
                try:
                    ...
                    resolutions[language] = min(
                        candidates, key=lambda c: c.priority.value,
                    )
    ...
    def recommend_grammar_upgrades(
        self, current_grammars: dict[str, str],
            ...
            for language, current_version in current_grammars.items():
                try:
                    ...
                    current_candidate = next(
                        (c for c in candidates if c.version == current_version), None,
                    )
                    ...
                    if better_candidates:
                        best_alternative = max(
                            better_candidates, key=lambda c: c.overall_score,
                        )

                        # Check for breaking changes
                        breaking_changes = (
                            self.compatibility_checker.detect_breaking_changes(
                                language, current_version, best_alternative.version,
                            )
                        )
                        ...
                        recommendations[language] = {
                            ...
                            "benefits": self._describe_upgrade_benefits(
                                current_candidate, best_alternative,
                            ),
                        }
        ...
        try:
            ...
            if constraints:
                overall_score = self._apply_constraint_penalties(
                    overall_score, candidate, constraints,
                )
    ...
    def _score_compatibility(self, candidate: SelectionCandidate) -> float:
        ...
        try:
            ...
            compat_result = self.compatibility_checker.check_compatibility(
                candidate.language, candidate.version,
            )
    ...
    def _resolve_single_conflict(
        self, language: str, candidates: list[SelectionCandidate],
    ) -> SelectionCandidate:
    ...
    def _describe_upgrade_benefits(
        self, current: SelectionCandidate, upgrade: SelectionCandidate,
    ) -> list[str]:
