...
class TestValidationManager:
    ...
    def test_run_full_validation(self):
        ...
        with (
            ...
            patch.object(
                self.validation_manager, "run_regression_tests",
            ) as mock_regression,
        ):
...
class TestPerformanceValidator:
    ...
    def test_validate_performance_sla(self):
        """Test SLA validation."""
        with patch.object(
            self.validator.performance_manager, "collect_system_metrics",
        ) as mock_collect:
            mock_profile = Mock()
            mock_metric = Mock()
    ...
    def test_generate_performance_report(self):
        ...
        with (
            patch.object(
                self.validator, "validate_performance_benchmarks",
            ) as mock_bench,
            patch.object(self.validator, "validate_performance_sla") as mock_sla,
            patch.object(
                self.validator, "detect_performance_regressions",
            ) as mock_regression,
        ):
...
class TestRegressionTester:
    ...
    def test_detect_regressions_with_baseline(self, mock_manager_class):
        ...
        with patch.object(
            self.regression_tester, "_run_baseline_benchmarks",
        ) as mock_benchmarks:
