...
class JavaVersionDetector:
        ...
        try:
            ...
            for pattern in patterns:
                ...
                if match:
                    ...
                    logger.debug(
                        "Found Java version via regex: %s -> %s", version, normalized,
                    )
                    return normalized
        ...
        try:
            for feature_pattern, min_version in self.feature_map.items():
                if re.search(feature_pattern, content):
                    ...
                    logger.debug(
                        "Detected Java %s feature: %s", min_version, feature_pattern,
                    )
    ...
    def parse_version_requirements(self, version_str: str) -> dict[str, str]:
        ...
        try:
            ...
            logger.debug(
                "Parsed version requirements: %s -> %s", version_str, requirements,
            )
    ...
    def detect_version(
        ...
        try:
            ...
            if features:
                ...
                if min_versions:
                    max_min_version = max(
                        min_versions, key=lambda v: int(v) if v.isdigit() else 0,
                    )
