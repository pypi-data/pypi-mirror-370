...
class GrammarValidator:
    ...
    def test_parse_samples(
        self, language: str, samples: list[str],
    ) -> tuple[bool, list[str]]:
    ...
    def _validate_extensive(
        self, grammar_path: Path, language: str,
    ) -> ValidationResult:
    ...
    def _get_large_test_samples(self, language: str) -> list[str]:
        ...
        if language == "javascript":
            return [
                "\n".join([f"function func{i}() {{ return {i}; }}" for i in range(100)]),
            ]
    ...
    def _get_cache_key(
        self, grammar_path: Path, language: str, level: ValidationLevel,
    ) -> str:
...
class GrammarInstaller:
        ...
        try:
            ...
            try:
                ...
                install_success, final_path, install_error = self._install_grammar_file(
                    language, grammar_path, priority,
                )
    ...
    def remove_grammar(
        self, language: str, clean_dependencies: bool = True,
    ) -> tuple[bool, str | None]:
    ...
    def _download_grammar(
        self, language: str, repository_url: str, version: str | None = None,
        ...
        try:
            ...
            if version:
                ...
                result = subprocess.run(
                    cmd, cwd=download_dir, capture_output=True, text=True, timeout=60, check=False,
                )
    ...
    def _build_grammar(
        self, language: str, source_path: Path,
    ) -> tuple[bool, Path | None, str | None]:
    ...
    def _build_with_tree_sitter_cli(
        self, language: str, source_path: Path, output_path: Path,
    ) -> tuple[bool, str | None]:
        ...
        try:
            ...
            result = subprocess.run(
                cmd, capture_output=True, text=True, timeout=self._build_timeout, check=False,
            )
    ...
    def _build_with_make(
        ...
        try:
            ...
            result = subprocess.run(
                ...
                capture_output=True,
                text=True,
                timeout=self._build_timeout, check=False,
            )
    ...
    def _install_grammar_file(
        self, language: str, source_path: Path, priority: GrammarPriority,
    ) -> tuple[bool, Path | None, str | None]:
...
class GrammarRegistry:
    ...
    def __init__(
        self, user_dir: Path | None = None, package_dir: Path | None = None,
    ):
    ...
    def discover_grammars(self) -> dict[str, list[tuple[Path, GrammarPriority]]]:
        ...
        if self._package_dir.exists():
            self._discover_in_directory(
                self._package_dir, GrammarPriority.PACKAGE, grammars,
            )
...
class GrammarManager:
    ...
    def install_grammar(
        ...
        try:
            ...
            success, error, install_info = self._installer.install_grammar(
                language, repository_url, version,
            )
    ...
    def remove_grammar(
        self, language: str, clean_cache: bool = True,
    ) -> tuple[bool, str | None]:
        """Remove an installed grammar.
    ...
    def validate_grammar(
        self, language: str, level: ValidationLevel = ValidationLevel.STANDARD,
    ) -> ValidationResult:
        """Validate a grammar at the specified level.
        ...
        for language in installed_grammars:
            try:
                ...
                validation_result = self.validate_grammar(
                    language, ValidationLevel.BASIC,
                )
