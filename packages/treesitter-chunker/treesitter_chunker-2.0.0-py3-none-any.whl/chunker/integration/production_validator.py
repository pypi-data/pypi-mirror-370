...
class DependencyValidator:
    ...
    def _validate_dependency(
        self, name: str, info: dict[str, Any], required: bool,
    ) -> ValidationResult:
            ...
            if check_result["available"]:
                ...
                if info.get("min_version"):
                    version_check = self._check_version(
                        check_result.get("version"), info["min_version"],
                    )
            ...
            try:
                ...
                if CORE_INTEGRATION_AVAILABLE:
                    ...
                    error_result = integrator.process_grammar_error(
                        test_error, context={"test_scenario": scenario_name},
                    )
...
class ProductionValidator:
    ...
    def __init__(
        ...
        self.report_output_path = self.config.get(
            "report_output_path", "validation_report.json",
        )
...
def main() -> int:
    ...
    parser.add_argument(
        "--config", type=str, help="Path to validation configuration file",
    )
    parser.add_argument(
        "--stage",
    ...
    parser.add_argument(
        "--timeout", type=int, default=300, help="Validation timeout in seconds",
    )
