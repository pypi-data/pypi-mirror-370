...
class SyntaxErrorAnalyzer:
    ...
    def _load_syntax_patterns(self) -> dict[str, list[re.Pattern]]:
        ...
        patterns = {
            "missing_bracket": [
                re.compile(
                    r"missing\s+closing\s+(bracket|brace|parenthesis)", re.IGNORECASE,
                ),
    ...
    def analyze_syntax_error(
        ...
        try:
            ...
            error_type = self.detect_syntax_error_type(
                error_message, language or "unknown",
            )
            ...
            suggestions = self.suggest_syntax_fix(
                error_type, language or "unknown", error_message,
            )
            ...
            severity = self.determine_syntax_error_severity(
                error_type, language or "unknown",
            )
    ...
    def suggest_syntax_fix(
        self, error_type: str, language: str, context: str,
    ) -> list[str]:
    ...
    def analyze_multiple_syntax_errors(
        self, error_messages: list[str], language: str,
        ...
        try:
            ...
            if stats["error_types"]:
                stats["most_common_type"] = max(
                    stats["error_types"], key=stats["error_types"].get,
                )
...
class SyntaxErrorFormatter:
    ...
    def format_syntax_fix_suggestions(
        self, suggestions: list[str], language: str,
    ) -> str:
    ...
    def generate_syntax_error_report(
        self, errors: list[ClassifiedError], language: str,
    ) -> str:
