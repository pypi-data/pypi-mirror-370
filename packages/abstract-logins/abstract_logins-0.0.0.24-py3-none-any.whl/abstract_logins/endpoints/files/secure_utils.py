from ..imports import *
ABS_UPLOAD_DIR = '/var/www/html/media/users'  # ← match the new base
...
def secure_remove(data, username=None, column_names="*", table_name="uploads", req=None):
    ...
    for key in ["id", "filename", "uploader_id", "filepath"]:
        ...
        if value not in (None, ""):
            search_map[key] = value
    if not search_map:
        return {"error": "No selector provided"}, 400   # ← was using undefined msg
    ...
    if not username or uploader_id.lower() != username.lower():
