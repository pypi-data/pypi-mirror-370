...
def main():
    ...
    logger.propagate = False  # stop bubbling up to root logger
    ...
    set_header_trailer_formatter(handler)
    logger.info(header + '\n')
    ...
    logger.error(f"This is a test error! Nothing to worry. Just for debug purposes. Will disappear in next release.")
    try: 
    ...
    set_header_trailer_formatter(handler)
    if response == 1: 
        print()  # separate last error from fresh prompt
        sys.exit(1)
    else: 
        # show a bye message
        logger.info('\n' + header)
