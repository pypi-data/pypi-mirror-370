...
def create_model_incore(params):
    log_queue, universe, eggpath, dbexp, args = params
    
    logger = logging.getLogger("gsrap_queued")
...
def main(args, logger):
    ...
    log_queue = multiprocessing.Manager().Queue()   # can be spwned in contrast to raw multiprocessing.Queue() (works also on Windows)
    listener = logging.handlers.QueueListener(log_queue, logger.handlers[0])
    listener.start()
    ...
    futures = [executor.submit(create_model_incore, (log_queue, universe, eggpath, dbexp, args)) for eggpath in args.eggnog]
