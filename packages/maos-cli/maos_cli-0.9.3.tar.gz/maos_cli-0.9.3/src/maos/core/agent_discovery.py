...
class AgentDiscovery:
    ...
    async def _scan_local_agents(self) -> List[AgentInfo]:
        ...
        # Look for .claude/agents in current directory and parent directories
        # This ensures we find the project root's .claude/agents
        current_path = Path.cwd()
        agents_dir = None
        
        # First try current directory
        if (current_path / ".claude/agents").exists():
            agents_dir = current_path / ".claude/agents"
        else:
        ...
        if not agents_dir or not agents_dir.exists():
            logger.debug(".claude/agents directory not found in current or parent directories")
            return agents
        
        logger.info(f"Scanning agents from: {agents_dir}")
    ...
    def _parse_agent_file(self, file_path: Path) -> Optional[AgentInfo]:
        ...
        try:
            ...
            if file_path.suffix == '.md':
                # Check if it has YAML frontmatter (Claude agent format)
                if content.startswith('---'):
                    try:
                        ...
                        if yaml_end > 0:
                            ...
                            # Parse tools as capabilities
                            tools = data.get('tools', '')
                            if isinstance(tools, str):
                                capabilities = [t.strip().lower() for t in tools.split(',')]
                            elif isinstance(tools, list):
                                capabilities = [t.lower() for t in tools]
                    except Exception as e:
                    ...
                    # Regular markdown without frontmatter
                    description = self._extract_md_description(content)
                    capabilities = self._extract_md_capabilities(content)
