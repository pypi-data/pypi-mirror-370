...
class SqlitePersistence(PersistenceInterface):
    ...
    async def _create_tables(self):
        ...
        await self._db.execute("""
            CREATE TABLE IF NOT EXISTS orchestrations (
                ...
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                completed_at TIMESTAMP,
                last_updated TIMESTAMP,  -- For auto-save tracking
                active_agents INTEGER,    -- Current active count
    ...
    async def _run_migrations(self):
        ...
        try:
            # Check if orchestrations table exists and has the new columns
            cursor = await self._db.execute("""
                SELECT sql FROM sqlite_master 
                WHERE type='table' AND name='orchestrations'
            """)
            ...
            if result and result[0]:
                ...
                if 'last_updated' not in table_sql:
                    try:
                        await self._db.execute("""
                            ALTER TABLE orchestrations 
                            ADD COLUMN last_updated TIMESTAMP
                        """)
                        self.logger.logger.info("Added last_updated column to orchestrations table")
                    except Exception:
                        pass  # Column already exists
                
                if 'active_agents' not in table_sql:
                    try:
                        await self._db.execute("""
                            ALTER TABLE orchestrations 
                            ADD COLUMN active_agents INTEGER
                        """)
                        self.logger.logger.info("Added active_agents column to orchestrations table")
                    except Exception:
                        pass  # Column already exists
                
                if 'message_count' not in table_sql:
                    try:
                        await self._db.execute("""
                            ALTER TABLE orchestrations 
                            ADD COLUMN message_count INTEGER
                        """)
                        self.logger.logger.info("Added message_count column to orchestrations table")
                    except Exception:
                        pass  # Column already exists
                        
        except Exception as e:
            self.logger.logger.warning(f"Migration check failed (non-critical): {e}")
    ...
    async def update_orchestration(self, orchestration_id: str, 
                                  ...
                                  status: str = None, summary: str = None,
                                  last_updated: str = None, active_agents: int = None,
        ...
        if active_agents is not None:
            updates.append("active_agents = ?")
            params.append(active_agents)
    ...
    # Additional helper methods for PersistentMessageBus
