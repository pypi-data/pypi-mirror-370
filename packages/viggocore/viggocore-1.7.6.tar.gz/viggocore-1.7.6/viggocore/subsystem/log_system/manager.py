...
class Manager(manager.CommonManager):
    ...
    def _get_domain_id(self, entity, domain_id=None):
        ...
        if domain_id is None:
            domain_id = (
                entity_aux.id if entity_aux.individual() == 'domain'
                else entity_aux.get_domain_id())
            if domain_id is None:
                raise exception.BadRequest(
                    f'A entidade {entity_aux.individual()} não tem domain_id.')
        return domain_id
