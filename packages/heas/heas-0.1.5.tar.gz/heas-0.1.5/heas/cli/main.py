...
def main(argv=None):
    ...
    if args.cmd == "run-graph":
        ...
        # Accept: CompositeHeasModel instance, a factory returning it, a Graph, or a Spec
        from ..hierarchy import CompositeHeasModel, Graph, make_model_from_spec, LayerSpec
        if isinstance(obj, CompositeHeasModel):
            model_factory = lambda kw: obj
        else:
            # If it's callable, call with no args to get instance/spec/graph
            if callable(obj):
                obj = obj()
            if isinstance(obj, CompositeHeasModel):
                model_factory = lambda kw: obj
            elif isinstance(obj, Graph) or (isinstance(obj, list) and all(hasattr(x, "streams") for x in obj)):
                # Build a factory from spec/graph
                def _mf(kw):
                    if isinstance(obj, Graph):
                        return CompositeHeasModel(obj, seed=kw.get("seed", args.seed))
                    else:
                        # assume spec (list[LayerSpec])
                        return make_model_from_spec(obj, seed=kw.get("seed", args.seed))({})
                model_factory = _mf
            else:
                raise TypeError("Unsupported --graph object. Provide CompositeHeasModel, Graph, or a list[LayerSpec].")
