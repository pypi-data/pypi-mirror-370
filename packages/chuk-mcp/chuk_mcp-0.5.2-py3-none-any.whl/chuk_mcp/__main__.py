...
async def test_server(config_path: str, server_name: str, verbose: bool = False) -> bool:
    ...
    try:
        ...
        logger.info(f"Connecting to server: {server_params.command} {' '.join(server_params.args)}")
...
Examples:
  ...
  %(prog)s --verbose --server mydb   # Detailed output for debugging
    ...
    if not config_path:
        ...
        if not config_path:
            print("❌ No configuration file found. Please specify --config or create server_config.json")
            sys.exit(1)
