...
if PYDANTIC_AVAILABLE:
        ...
        if hasattr(annotation, '__name__'):
            ...
            if class_module and hasattr(class_module, '__dict__'):
                ...
                if alias_name in module_dict:
                    ...
                    if hasattr(alias_value, '__origin__') or hasattr(alias_value, '__args__'):
                        return alias_value
            ...
            try:
                ...
                for module_name, module in sys.modules.items():
                    if (module and hasattr(module, '__dict__') and 
                        alias_name in module.__dict__):
    ...
    def _deep_validate(name: str, value: Any, expected: Any, path: str = "", class_module=None, original_annotation=None) -> Any:
        ...
        if resolved_type != expected:
            return _deep_validate(name, value, resolved_type, path, class_module, original_annotation=expected)
        ...
        if origin is None:
            ...
            if inspect.isclass(expected):
                ...
                if expected is str:
                    ...
                    is_permissive = (original_annotation and 
                                   _should_be_permissive_int(original_annotation, name))
        ...
        if origin in (list, List):
            ...
            for i, item in enumerate(value):
                if item_type is Any:
                    ...
                    validated_item = _deep_validate(f"[{i}]", item, item_type, current_path, class_module)
        ...
        if origin in (dict, Dict):
            ...
            for k, v in value.items():
                validated_key = k if key_type is Any else _deep_validate("key", k, key_type, current_path, class_module)
                validated_value = v if val_type is Any else _deep_validate(f"[{k}]", v, val_type, current_path, class_module)
                ...
                for name, annotation in raw_annotations.items():
                    ...
                    if name in values:
                        ...
                        if expected_type == annotation:
                            resolved_annotation = _resolve_type_alias(annotation, name, class_module)
                        ...
                        validated_value = _deep_validate(name, values[name], expected_type, class_module=class_module, original_annotation=annotation)
