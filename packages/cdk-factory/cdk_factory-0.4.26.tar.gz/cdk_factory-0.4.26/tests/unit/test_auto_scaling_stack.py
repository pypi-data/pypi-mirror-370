...
def test_auto_scaling_stack_minimal():
    ...
    # Mock the methods
    with patch.object(AutoScalingStack, "_get_vpc", return_value=mock_vpc) as mock_get_vpc:
        with patch.object(AutoScalingStack, "_create_instance_role", return_value=mock_role) as mock_create_role:
            with patch.object(AutoScalingStack, "_create_user_data") as mock_create_user_data:
                with patch.object(AutoScalingStack, "_create_launch_template", return_value=mock_launch_template) as mock_create_lt:
                    with patch.object(AutoScalingStack, "_create_auto_scaling_group", return_value=mock_asg) as mock_create_asg:
                        with patch.object(AutoScalingStack, "_configure_scaling_policies") as mock_configure_policies:
...
def test_auto_scaling_stack_full_config():
    ...
    # Mock the methods
    with patch.object(AutoScalingStack, "_get_vpc", return_value=mock_vpc) as mock_get_vpc:
        with patch.object(AutoScalingStack, "_get_security_groups", return_value=[mock_security_group]) as mock_get_sg:
            with patch.object(AutoScalingStack, "_create_instance_role", return_value=mock_role) as mock_create_role:
                with patch.object(AutoScalingStack, "_create_user_data") as mock_create_user_data:
                    with patch.object(AutoScalingStack, "_create_launch_template", return_value=mock_launch_template) as mock_create_lt:
                        with patch.object(AutoScalingStack, "_create_auto_scaling_group", return_value=mock_asg) as mock_create_asg:
                            with patch.object(AutoScalingStack, "_configure_scaling_policies") as mock_configure_policies:
                                with patch.object(AutoScalingStack, "_add_outputs") as mock_add_outputs:
                                    ...
                                    assert stack.asg_config.subnet_group_name == "private"
                                    assert stack.asg_config.security_group_ids == ["sg-12345"]
                                    assert stack.asg_config.health_check_type == "ELB"
                                    assert stack.asg_config.health_check_grace_period == 300
                                    assert stack.asg_config.cooldown == 300
                                    assert stack.asg_config.termination_policies == ["OldestInstance", "Default"]
                                    assert stack.asg_config.managed_policies == [
                                        "AmazonSSMManagedInstanceCore",
                                        "AmazonEC2ContainerRegistryReadOnly"
                                    ]
                                    assert stack.asg_config.ami_type == "amazon-linux-2023"
                                    assert stack.asg_config.detailed_monitoring is True
                                    assert len(stack.asg_config.block_devices) == 1
                                    assert stack.asg_config.block_devices[0]["device_name"] == "/dev/xvda"
                                    assert stack.asg_config.block_devices[0]["volume_size"] == 30
                                    assert stack.asg_config.container_config["ecr"]["repo"] == "app"
                                    assert stack.asg_config.container_config["port"] == 8080
                                    assert len(stack.asg_config.user_data_commands) == 3
                                    assert len(stack.asg_config.scaling_policies) == 1
                                    assert stack.asg_config.scaling_policies[0]["name"] == "cpu-scale-out"
                                    assert stack.asg_config.tags == {"Environment": "test", "Project": "cdk-factory"}
