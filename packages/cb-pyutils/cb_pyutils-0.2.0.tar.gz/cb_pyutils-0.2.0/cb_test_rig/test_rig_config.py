...
class TestRigConfig():
    ...
    def read_config(self, conf_file_name=CONFIG_FILE_NAME):
        ...
        self.mqtt_broker = config.get('MQTT-BROKER', 'host', fallback=self.mqtt_broker)
        self.mqtt_broker_port = config.getint('MQTT-BROKER', 'port', fallback=self.mqtt_broker_port)
