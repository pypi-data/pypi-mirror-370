...
class BiApiClient:
    ...
    def _add_defaults(self, indicators: list[str], query_type: str) -> list[str]:
        """添加默认指标"""
        defaults = (
            Config.AD_DEFAULTS if query_type == "ad" else Config.MATERIAL_DEFAULTS
        )

        needed = [ind for ind in defaults if ind not in indicators]
        if needed:
            logger.info(f"客户端自动添加默认指标: {needed}")
            return needed + indicators
        return indicators
    ...
    async def get_ad_count_list(
        ...
        **kwargs,
        ...
        # 字符串到列表的转换现在由Pydantic模型自动处理（mode="before"验证器）
        ...
        # 自动添加默认指标
        indicators = self._add_defaults(indicators, "ad")
        ...
        if self.indicator_manager:
            ...
            # 基于依赖规则进行派生注入（只加不减）。仅用于请求payload，不影响前端展示字段
            # 既然如此，那直接传入所有可选指标也可以（这一点可以日后出现类似问题处理）
            payload_indicators, derived = self.indicator_manager.expand_indicators(
                valid_indicators,
                app,
                "ad_query",
                hours_24=hours_24,
            ...
            if derived:
                logger.info(f"根据依赖规则自动注入指标(仅用于请求): {derived}")
        else:
        ...
        try:
            request_model = GetAdCountListRequest(
                ...
                end_time=end_date,
                zhibiao_list=payload_indicators,
                # 以下非必须参数
    ...
    async def get_material_count_list(
        ...
        **kwargs,
        ...
        # 自动添加默认指标
        indicators = self._add_defaults(indicators, "material")
