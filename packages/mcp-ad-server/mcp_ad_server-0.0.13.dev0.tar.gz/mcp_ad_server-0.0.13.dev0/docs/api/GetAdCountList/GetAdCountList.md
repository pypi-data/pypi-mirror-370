...
- 指标为“付费”口径的若干项仅做字段映射，底层 SQL 只在选择对应“基础口径”时才会生成实际列。若仅选择“付费”口径，将出现空值/0 的结果：
  - 付费分成后首日ROI → 必须同时选择“分成后首日ROI”，否则 `dividefirstDayRoi` 不会被计算
  - 付费首日ROI → 必须同时选择“首日ROI”，否则 `firstDayRoi` 不会被计算
  - 付费累计ROI、付费分成后累计ROI → 必须同时选择“累计ROI”，否则累计计算分支不会触发（`accumulativeROI`/`divideAccumulativeROI` 等不会被计算）

- 说明：
  - BI=棋牌游戏，FX=非棋牌游戏。上述依赖在两条实现中均存在（累计类通过是否包含“累计ROI”触发；首日类通过是否包含“首日/分成后首日ROI”触发）。
  - 平台ROI支持范围：
    - `付费首日平台ROI`、`付费分成后首日平台ROI` 仅在 BI 路径返回；FX 路径不返回平台口径
    - 平台累计类（`付费累计平台ROI`、`付费分成后平台累计ROI`、`累计平台roi`、`分成后累计平台ROI`）需选择`累计ROI`方会计算
    - 当 `24小时` 启用（hours_24=true）时，平台累计类由对应“首日平台ROI”派生：
      - `付费累计平台ROI` ← `付费首日平台ROI`
      - `付费分成后平台累计ROI` ← `付费分成后首日平台ROI`
      - `累计平台roi` ← `首日平台roi`
      - `分成后累计平台ROI` ← `分成后首日平台ROI`
      - 若未同时选择对应的“首日平台ROI”指标，派生值可能为空
  - 非平台累计类在 `24小时` 启用时的派生：
    - `累计ROI` ← `首日ROI`
    - `付费累计ROI` ← `付费首日ROI`
    - `分成后累计ROI` ← `分成后首日ROI`
    - `付费分成后累计ROI` ← `付费分成后首日ROI`
    - 若未同时选择对应“首日”指标，派生值可能为空
...
以下为该接口返回数据中常见字段的映射关系（节选）。完整映射见 `src/mcp_ad_server/data/propmap/GetAdCountList.json`。
...
| newPayMoney | 新增付费金额 |
| newPayScale | 新增付费率 |
| newPayPrimeCost | 新增付费成本 |
| firstDayRoi | 付费首日ROI |
| dividefirstDayRoi | 付费分成后首日ROI |
| firstDayAllRoi | 首日ROI |
| dividefirstDayAllRoi | 分成后首日ROI |
| accumulativeROI | 付费累计ROI |
| divideAccumulativeROI | 付费分成后累计ROI |
| accumulativeAllROI | 累计ROI |
| divideAccumulativeAllROI | 分成后累计ROI |
| pf_firstDayRoi | 付费首日平台ROI |
| pf_dividefirstDayRoi | 付费分成后首日平台ROI |
| pf_firstDayAllRoi | 首日平台roi |
| pf_dividefirstDayAllRoi | 分成后首日平台ROI |
| pf_accumulativeROI | 付费累计平台ROI |
| pf_divideAccumulativeROI | 付费分成后平台累计ROI |
| pf_accumulativeAllROI | 累计平台roi |
| pf_divideAccumulativeAllROI | 分成后累计平台ROI |
| click_cnt | 点击数 |
| view_count | 曝光次数 |
| clickScale | 点击率 |
| clickPrimeCost | 点击成本 |
| adBuyUsers | 广告变现人数 |
| adBuyMoney2 | 小游戏广告变现金额（平台上报） |
| income_val_24hs | 小游戏注册首日广告变现金额 |
| income_val_24h_roi | 小游戏注册首日广告变现ROI |
| vp_campaign_name | 广告计划名称 |
| vp_creative_name | 创意名称 |
