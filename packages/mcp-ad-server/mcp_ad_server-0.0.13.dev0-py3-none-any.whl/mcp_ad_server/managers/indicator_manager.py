...
class IndicatorManager(Manager):
    ...
    def __init__(self):
        ...
        # 依赖规则：query_type(ad_query/material_query) -> rules(list)
        self.dependencies_rules: dict[str, list[dict[str, Any]]] = {
            "ad_query": [],
            "material_query": [],
        }
    ...
    async def load_dependencies(self) -> None:
        ...
        try:
            indicators_dir = Config.INDICATORS_DIR
            dep_dir = indicators_dir / "dependencies"

            if not dep_dir.exists():
                self.logger.warning(f"依赖规则目录不存在: {dep_dir}")
                return

            # Reset
            self.dependencies_rules["ad_query"] = []
            self.dependencies_rules["material_query"] = []

            file_map = {
                "ad_query": dep_dir / "ad.json",
                "material_query": dep_dir / "material.json",
            }

            for qtype, path in file_map.items():
                if path.exists():
                    try:
                        async with aiofiles.open(path, encoding="utf-8") as f:
                            ...
                            self.dependencies_rules[qtype] = rules
                            self.logger.info(f"加载依赖规则 {qtype}: {len(rules)} 条 ({path})")
                    except Exception as e:
                        self.logger.error(f"加载依赖规则失败 {path}: {e}")
                else:
                    self.logger.info(f"未发现依赖规则文件: {path}")
        except Exception as e:
            self.logger.error(f"加载依赖规则目录失败: {e}")
            raise
    ...
    def get_route_for_app(self, app: str) -> str:
        """根据应用判定路由类型（'BI' 或 'FX'）。"""
        try:
            app_type = Config.get_type_by_app(app)
            return "BI" if app_type == "card_games" else "FX"
        except Exception:
            return "FX"
    ...
    def expand_indicators(
        ...
        *,
        ...
        """基于依赖规则自动注入派生指标。"""
        rules = self.dependencies_rules.get(query_type, [])
        if not rules:
            return indicators, []

        current_route = route or self.get_route_for_app(app)

        original = list(indicators)
        expanded_set = set(indicators)
        derived_added: list[str] = []

        def rule_applicable(rule: dict[str, Any]) -> bool:
            cond = rule.get("when", {}) or {}
            if "hours_24" in cond and bool(cond["hours_24"]) != bool(hours_24):
                return False
            if "route" in cond:
                routes = cond["route"]
        ...
        while changed:
            changed = False
            snapshot = list(expanded_set)
            for have in snapshot:
                for rule in rules:
                    if rule.get("indicator") != have:
                        continue
                    if not rule_applicable(rule):
                        continue
                    requires = rule.get("requires", [])
                    for req in requires:
                        if req not in expanded_set:
                            expanded_set.add(req)
                            derived_added.append(req)
                            changed = True

        expanded_ordered = original[:]
        for d in derived_added:
            if d not in expanded_ordered:
                expanded_ordered.append(d)

        if derived_added:
            self.logger.info(
                f"自动注入派生指标: added={derived_added}, route={current_route}, hours_24={hours_24}, query_type={query_type}"
            )

        return expanded_ordered, derived_added
