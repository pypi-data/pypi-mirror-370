...
# Add global variables to store the token and allowed account IDs
FB_ACCESS_TOKEN = None
FB_ALLOWED_ACCOUNT_IDS = None

# --- Helper Functions ---

def _get_fb_access_token() -> str:
    """
    Get Facebook access token from FB_TOKEN environment variable.
    ...
    Raises:
        Exception: If no token is provided in environment variable.
    """
    global FB_ACCESS_TOKEN
    if FB_ACCESS_TOKEN is None:
        # Look for FB_TOKEN environment variable
        FB_ACCESS_TOKEN = os.getenv('FB_TOKEN')
        if FB_ACCESS_TOKEN:
            print(f"The facebook token you provided is: {FB_ACCESS_TOKEN}")
        else:
            raise Exception("Facebook token must be provided via 'FB_TOKEN' environment variable")

    return FB_ACCESS_TOKEN

def _get_allowed_account_ids() -> List[str]:
    """
    Get allowed Facebook account IDs from FB_ACCOUNT_IDS environment variable.
    Caches the list in memory after first read.

    Returns:
        List[str]: List of allowed account IDs (with 'act_' prefix).

    Raises:
        Exception: If no account IDs are provided in environment variable.
    """
    global FB_ALLOWED_ACCOUNT_IDS
    if FB_ALLOWED_ACCOUNT_IDS is None:
        # Look for FB_ACCOUNT_IDS environment variable
        account_ids_str = os.getenv('FB_ACCOUNT_IDS')
        if account_ids_str:
            # Split by comma and clean up whitespace
            raw_ids = [id.strip() for id in account_ids_str.split(',') if id.strip()]
            # Ensure all IDs have 'act_' prefix
            FB_ALLOWED_ACCOUNT_IDS = []
            for account_id in raw_ids:
                if account_id.startswith('act_'):
                    FB_ALLOWED_ACCOUNT_IDS.append(account_id)
                else:
                    FB_ALLOWED_ACCOUNT_IDS.append(f'act_{account_id}')
            
            print(f"Allowed Facebook account IDs: {FB_ALLOWED_ACCOUNT_IDS}")
        else:
            raise Exception("Facebook account IDs must be provided via 'FB_ACCOUNT_IDS' environment variable (comma-separated)")

    return FB_ALLOWED_ACCOUNT_IDS

def _validate_account_access(act_id: str) -> str:
    """
    Validate that the provided account ID is in the allowed list.
    
    Args:
        act_id: The account ID to validate
        
    Returns:
        str: The validated account ID
        
    Raises:
        Exception: If the account ID is not in the allowed list
    """
    allowed_ids = _get_allowed_account_ids()
    
    # Ensure the act_id has 'act_' prefix for comparison
    if not act_id.startswith('act_'):
        act_id = f'act_{act_id}'
    
    if act_id not in allowed_ids:
        raise Exception(f"Access denied: Account ID '{act_id}' is not in the allowed list. Allowed accounts: {allowed_ids}")
    
    return act_id

def _filter_accounts_list(accounts_data: Dict) -> Dict:
    """
    Filter a list of accounts to only include those in the allowed list.
    
    Args:
        accounts_data: The response data containing accounts
        
    Returns:
        Dict: Filtered response with only allowed accounts
    """
    ...
    for account in accounts_data['data']:
        ...
        if not account_id.startswith('act_'):
            account_id = f"act_{account.get('account_id', account.get('id', ''))}"
        
        if account_id in allowed_ids:
            filtered_accounts.append(account)
    
    # Update the response with filtered accounts
    result = accounts_data.copy()
    result['data'] = filtered_accounts
    result['filtered_count'] = len(filtered_accounts)
    result['original_count'] = len(accounts_data['data'])
...
def list_ad_accounts() -> Dict:
    """List down the ad accounts from the allowed FB_ACCOUNT_IDS list.
        This function will only return accounts that are specified in the FB_ACCOUNT_IDS 
        environment variable, ensuring access is restricted to your specified accounts only.
        ...
        complete consolidated list of ALL allowed ad accounts across all pages in a single 
    ...
    params = {
        ...
        'fields': 'adaccounts{name,account_id,id}' # Get account info for filtering
    }
    
    response = _make_graph_api_call(url, params)
    
    # Filter to only include allowed accounts
    if 'adaccounts' in response and 'data' in response['adaccounts']:
        filtered_response = _filter_accounts_list(response['adaccounts'])
        response['adaccounts'] = filtered_response
...
def get_details_of_ad_account(act_id: str, fields: list[str] = None) -> Dict:
    ...
    Note: This function will only work for account IDs specified in FB_ACCOUNT_IDS.
    ...
    validated_act_id = _validate_account_access(act_id)
    effective_fields = fields if fields is not None else DEFAULT_AD_ACCOUNT_FIELDS
    return _fetch_node(node_id=validated_act_id, fields=effective_fields)
...
def get_adaccount_insights(
    ...
    access_token = _get_fb_access_token()
    url = f"{FB_GRAPH_URL}/{validated_act_id}/insights"
...
def get_campaign_insights(
    ...
    Note: This function will only work for campaigns belonging to accounts specified in FB_ACCOUNT_IDS.
    ...
    Example:
        ...
        ```
    """
    # Validate that this campaign belongs to an allowed account
    validated_campaign_id = _validate_object_access(campaign_id, 'campaign')
    
    access_token = _get_fb_access_token()
    url = f"{FB_GRAPH_URL}/{validated_campaign_id}/insights"
    params = {'access_token': access_token}
...
def get_adset_insights(
    ...
    Note: This function will only work for ad sets belonging to accounts specified in FB_ACCOUNT_IDS.
    ...
    Example:
        ...
        ```
    """
    # Validate that this ad set belongs to an allowed account
    validated_adset_id = _validate_object_access(adset_id, 'adset')
    
    access_token = _get_fb_access_token()
    url = f"{FB_GRAPH_URL}/{validated_adset_id}/insights"
    params = {'access_token': access_token}
...
def get_ad_insights(
    ...
    Note: This function will only work for ads belonging to accounts specified in FB_ACCOUNT_IDS.
    ...
    Example:
        ...
        ```
    """
    # Validate that this ad belongs to an allowed account
    validated_ad_id = _validate_object_access(ad_id, 'ad')
    
    access_token = _get_fb_access_token()
    url = f"{FB_GRAPH_URL}/{validated_ad_id}/insights"
    params = {'access_token': access_token}
...
def get_ad_creative_by_id(
    ...
    Note: This function will only work for creatives belonging to accounts specified in FB_ACCOUNT_IDS.
    ...
    Example:
        ...
        ```
    """
    # Validate that this creative belongs to an allowed account
    validated_creative_id = _validate_object_access(creative_id, 'creative')
    
    access_token = _get_fb_access_token()
    url = f"{FB_GRAPH_URL}/{validated_creative_id}"
    params = {'access_token': access_token}
...
def get_ad_creatives_by_ad_id(
    ...
    access_token = _get_fb_access_token()
    url = f"{FB_GRAPH_URL}/{validated_ad_id}/adcreatives"
    params = {
        'access_token': access_token
    }
...
def get_ad_by_id(ad_id: str, fields: Optional[List[str]] = None) -> Dict:
    ...
    access_token = _get_fb_access_token()
    url = f"{FB_GRAPH_URL}/{validated_ad_id}"
    params = {
        'access_token': access_token
    }
...
def get_ads_by_campaign(
    ...
    access_token = _get_fb_access_token()
    url = f"{FB_GRAPH_URL}/{validated_campaign_id}/ads"
    params = {
        'access_token': access_token
    }
...
def get_adsets_by_ids(
    ...
    Example:
        ...
        ```
    """
    # Validate that all ad sets belong to allowed accounts
    validated_adset_ids = []
    for adset_id in adset_ids:
        validated_id = _validate_object_access(adset_id, 'adset')
        validated_adset_ids.append(validated_id)
    
    access_token = _get_fb_access_token()
    url = f"{FB_GRAPH_URL}/"
    params = {
        'access_token': access_token,
        'ids': ','.join(validated_adset_ids)
    }
...
def get_campaigns_by_adaccount(
    ...
    access_token = _get_fb_access_token()
    url = f"{FB_GRAPH_URL}/{validated_act_id}/campaigns"
    params = {
        'access_token': access_token
    }
...
def get_activities_by_adaccount(
    ...
    access_token = _get_fb_access_token()
    url = f"{FB_GRAPH_URL}/{validated_act_id}/activities"
    params = {
        'access_token': access_token
    }
...
def _validate_object_access(object_id: str, object_type: str) -> str:
    """
    Validate that an object (campaign, adset, ad, creative) belongs to an allowed account.
    
    Args:
        object_id: The ID of the object to validate
        object_type: The type of object ('campaign', 'adset', 'ad', 'creative')
        
    Returns:
        str: The validated object ID
        
    Raises:
        Exception: If the object doesn't belong to an allowed account
    """
    try:
        access_token = _get_fb_access_token()
        
        # Get the account_id for this object
        if object_type in ['campaign', 'adset', 'ad']:
            url = f"{FB_GRAPH_URL}/{object_id}"
            params = {'access_token': access_token, 'fields': 'account_id'}
        elif object_type == 'creative':
            url = f"{FB_GRAPH_URL}/{object_id}"
            params = {'access_token': access_token, 'fields': 'account_id'}
        else:
        ...
        _validate_account_access(account_id)  # This will raise an exception if not allowed
        
        return object_id
        
    except Exception as e:
        raise Exception(f"Access denied: {object_type} '{object_id}' does not belong to an allowed account. Error: {str(e)}")

def main():
    """Main entry point for the console script."""
    _get_fb_access_token()
    _get_allowed_account_ids()  # Initialize account filtering
