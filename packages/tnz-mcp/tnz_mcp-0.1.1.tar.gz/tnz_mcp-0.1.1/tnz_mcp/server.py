...
def run_server(transport: str = "stdio", host: Optional[str] = None, port: Optional[int] = None) -> None:
    ...
    try:
        ...
        @mcp.tool()
        def find_contacts(attention: str, mobile_number:str, records_per_page: int = 10, page: int = 1) -> Dict[str, Any]:
            """Find / search contacts with pagination."""
            try:
                response = client.Addressbook.Contact.Search(Attention= attention, MobilePhone= mobile_number, RecordsPerPage=records_per_page, Page=page)
                return response.__dict__
            except Exception as e:
                return {"error": str(e)}
