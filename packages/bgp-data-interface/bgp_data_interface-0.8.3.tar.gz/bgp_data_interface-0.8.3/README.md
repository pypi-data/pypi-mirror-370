...
Calling the API with empty dict will retrieve a dummy csv file.
...
Cloud, site, forecast type and filename can be specified to retrieve a specific
file
...
A dataframe can be stored on s3 as a csv file using `store_csv`.

```py
    from bgp_data_interface.s3_energy import S3Energy

    api = S3Energy(AWS_ACCESS_KEY, AWS_SECRET_KEY, BUCKET_NAME)
    api.store_csv(df, cloud='AWS',
        site='BBO',
        forecast='solar',
