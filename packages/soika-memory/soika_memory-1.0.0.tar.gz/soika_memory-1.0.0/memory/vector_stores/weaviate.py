...
class Weaviate(VectorStoreBase):
    def __init__(
        ...
        is_local = ("localhost" in cluster_url or "127.0.0.1" in cluster_url or "weaviate" in cluster_url)
        
        if is_local:
            # For local instances, we typically don't need authentication
            if auth_client_secret:
                auth_creds = Auth.api_key(auth_client_secret)
            else:
                auth_creds = None
                
            # Extract host and port from cluster_url for local connection
            if "localhost" in cluster_url or "127.0.0.1" in cluster_url:
                host = "localhost"
                # Extract port if specified, default to 8080
                if ":8080" in cluster_url:
                    port = 8080
                else:
                    port = 8080
            else:
            ...
            # For cloud instances, API key is required
            if not auth_client_secret:
                raise ValueError("API key is required for Weaviate cloud instances")
    ...
    def create_col(self, vector_size, distance="cosine"):
        ...
        properties = [
            ...
            wvcc.Property(name="conv_id", data_type=wvcc.DataType.TEXT),
            wvcc.Property(name="space_id", data_type=wvcc.DataType.TEXT),
        ]
    ...
    def search(
        ...
        if filters:
            for key, value in filters.items():
                if value and key in ["user_id", "agent_id", "conv_id", "space_id"]:
        ...
        response = collection.query.hybrid(
            ...
            return_properties=["hash", "created_at", "updated_at", "user_id", "agent_id", "conv_id", "space_id", "data", "category"],
        ...
        for obj in response.objects:
            ...
            for id_field in ["conv_id", "agent_id", "user_id"]:
