...
class WindowSpec:
    ...
    def parse(cls, spec: str) -> "WindowSpec":
        ...
        If no prefix is provided, numeric values are treated as window IDs
        and strings are treated as window titles.

        Formats:
        - "title:Window Title" or "t:Window Title" or "Window Title" (default)
        - "id:12345" or just "12345" (window ID/HWND)
        - "process:notepad" or "p:notepad" (process name without extension)
        - "pid:1234" (process ID)
        ...
        try:
            # Default to window ID if it's an int (displayed via 'window list')
            int(spec)
            return cls(WindowSpecType.ID, spec)
        except ValueError:
            pass
