...
class AgentExecution:
    ...
    async def _execute_step(self) -> bool:
        ...
        try:
            ...
            # Prepare tool definitions based on provider
            tools: List[ProviderToolDefinition]
            if self.provider.supports_computer_use():
                # Anthropic's native computer use returns a dict
                tools = [dict(self.display_config.to_computer_tool())]
            else:
                # Non-native computer use providers return their own tool definition type
    ...
    async def _process_tool_call(self, tool_call) -> Dict[str, Any]:
        ...
        # Extract action from tool call arguments
