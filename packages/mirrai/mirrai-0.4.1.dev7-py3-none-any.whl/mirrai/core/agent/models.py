...
from mirrai.core.constants import ANTHROPIC_COMPUTER_TOOL_TYPE
from mirrai.core.utils.common import get_primary_display_size
from mirrai.core.window_manager.models import Rect, WindowInfo

# Higher screen resolutions reduce VLM accuracy. We downscale for the model
# and upscale coordinates for actual interactions.
MAX_WIDTH = 1280
MAX_HEIGHT = 800
...
class TaskLike(Protocol):
    """Protocol for objects that can be converted to task prompts."""

    def to_prompt(self) -> str:
        """Convert the task to a prompt string for the AI agent."""
        ...


@runtime_checkable
class TaskWithSystemPrompt(TaskLike, Protocol):
    """Protocol for tasks that also provide a system prompt.

    Extends TaskLike to include a system prompt that will be
    combined with any system prompt provided to AgentExecution.
    """

    def get_system_prompt(self) -> Optional[str]:
        """Get a system prompt for this task type."""
        ...


TaskInput = Union[str, TaskLike]
