...
from mirrai.core.utils.common import calc_levenshtein_dist
...
class WindowManager(ABC):
    ...
    def find_window(self, spec: str) -> Optional[WindowInfo]:
        ...
        Returns:
            Best matching WindowInfo or None if not found
        """
        parsed = WindowSpec.parse(spec)

        if parsed.type == WindowSpecType.TITLE:
            matches = self.find_windows_by_title(title=parsed.value)
            if not matches:
                return None

            # Return best match based on Levenshtein distance
            best_match = min(
                matches,
                key=lambda w: calc_levenshtein_dist(
                    w.title.lower() if w.title else "", parsed.value.lower()
                ),
            )
            return best_match
