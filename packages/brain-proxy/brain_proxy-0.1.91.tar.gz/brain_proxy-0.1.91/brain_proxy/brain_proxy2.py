...
class BrainProxy2:
    ...
    async def _handle_streaming(
        ...
        try:
            ...
            if (followup_response and followup_response.choices and 
                ...
                if hasattr(message, 'tool_calls') and message.tool_calls:
                    self._log(f"Follow-up response contains {len(message.tool_calls)} more tool calls - executing recursively")
                    # Recursively handle more tool calls
                    # This allows the agent to chain tools together
                    more_tool_calls = [
                        {
                            "id": tc.id or f"recursive_{i}",
                            "type": "function",
                            "function": {
                                "name": tc.function.name,
                                "arguments": tc.function.arguments
                    ...
                    # Make another follow-up call with all the results
                    recursive_msgs = followup_msgs + [
                        {
                            "role": "assistant",
                            "content": None,
                    ...
                    final_response = await safe_llm_call(
                        model=request.model or self.config.default_model,
                        messages=self._validate_messages(recursive_msgs),
                        stream=False,
                        # NO tools to force text response
                    )
                    ...
                    if final_response and final_response.choices and final_response.choices[0].message:
                        ...
                        response_content = "I've completed all the requested actions."
                    
                    self._log(f"Got recursive response of length {len(response_content)}")
                else:
        ...
        if response_content:
            ...
            # 1. Content chunk(s) with finish_reason: null
            # 2. Final empty chunk with finish_reason: stop
