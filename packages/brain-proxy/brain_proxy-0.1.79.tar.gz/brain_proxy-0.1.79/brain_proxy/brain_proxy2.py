...
class BrainProxy2:
    ...
    async def _handle_streaming(
        ...
        max_iterations = 5  # Prevent infinite loops (reduced from 10)
        iteration = 0
        ...
        while iteration < max_iterations:
            ...
            # Get available tools for follow-up (may have changed)
            all_tools = self.tool_service.get_tools_for_tenant(tenant, request.tools)
            
            # For follow-up calls after tools, we should reduce tool availability to encourage final response
            # Only provide tools if this is the first iteration
            tools_for_followup = None
            if iteration <= 2 and all_tools:  # Only allow tools in first 2 iterations
                # Filter tools more aggressively for follow-up
                tools_for_followup = await self.tool_service.filter_tools(followup_msgs, all_tools)
                # Limit to top 3 most relevant tools to reduce tool-calling tendency
                if len(tools_for_followup) > 3:
                    tools_for_followup = tools_for_followup[:3]
                self._log(f"Providing {len(tools_for_followup) if tools_for_followup else 0} tools for follow-up iteration {iteration}")
            ...
            try:
                ...
                # Only add tools if we have them and it's early iteration
                if tools_for_followup:
                    kwargs["tools"] = tools_for_followup
                    kwargs["tool_choice"] = "auto"
                
                followup_iter = await safe_llm_call(**kwargs)
            except Exception as e:
                ...
                async for chunk in followup_iter:
                    ...
                    if finish_reason:
                        if finish_reason == "tool_calls":
                            # Only process additional tool calls if we're in early iterations
                            if iteration < 3 and additional_tool_calls:
                                self._log(f"Additional tool calls detected in follow-up (iteration {iteration})")
                                break
                            else:
                                # Force completion if we've had enough iterations
                                self._log(f"Ignoring additional tool calls after iteration {iteration}, forcing completion")
                                if not content_streamed:
                                    # Send a summary of tool results if no content was streamed
                                    summary = "He completado las siguientes acciones:\n"
                                    for tr in tool_results[-3:]:  # Last 3 tool results
                                        summary += f"- {tr.get('name', 'Tool')}: {tr.get('content', '')[:100]}...\n"
                                    yield f"data: {json.dumps({'choices': [{'delta': {'content': summary}, 'index': 0}]})}\n\n"
                                additional_tool_calls = {}  # Clear to exit loop
                                break
                        elif finish_reason in ["stop", "length"]:
                            # Normal completion
                            if not content_streamed:
                                # If no content was streamed, ensure we send something
                                self._log(f"No content streamed, sending tool results summary")
                                summary = "Basándome en los resultados obtenidos:\n"
                                for tr in tool_results[-2:]:  # Last 2 tool results
            ...
            # If no additional tool calls or we've done enough iterations, we're done
            if not additional_tool_calls or iteration >= 3:
                if iteration >= 3 and additional_tool_calls:
                    self._log(f"Stopping tool execution after {iteration} iterations to prevent loops")
                break
            
            # Execute additional tool calls (but limit them)
            self._log(f"Executing {len(additional_tool_calls)} additional tool calls")
            new_tool_calls = list(additional_tool_calls.values())[:2]  # Limit to 2 tools per iteration
            new_tool_results = []
