...
class BrainProxy2:
    ...
    async def _handle_streaming(
        ...
        if response_content:
            ...
            # Since we have the complete response, send it all at once for reliability
            buf.append(response_content)
            tokens += len(response_content)
            
            # Send the complete response WITH finish_reason in a SINGLE chunk
            # This follows the OpenAI streaming format more closely
            response_chunk = {
                "choices": [{
                    "index": 0,
                    "delta": {"content": response_content},
                    "finish_reason": "stop"  # Include finish_reason with the content
                }],
                "object": "chat.completion.chunk",
                "created": int(time.time()),
                "model": request.model or self.config.default_model
            ...
            try:
                data = f"data: {json.dumps(response_chunk)}\n\n"
                self._log(f"[{stream_id}] Sending complete response with finish_reason=stop in single chunk")
                yield data
