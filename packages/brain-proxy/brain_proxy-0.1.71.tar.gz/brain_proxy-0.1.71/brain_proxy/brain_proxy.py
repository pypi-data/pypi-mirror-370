        ...
        async def chat(request: Request, tenant: str):
            ...
            async def _process_chunk_payload(chunk) -> dict:
                ...
                try:
                    ...
                    # If model_dump_json fails, try to extract the basic structure
                    self._log(f"Warning: chunk serialization failed: {e}")
                    
                    # Create a minimal valid streaming response
                    try:
                        ...
                        self._log(f"Warning: fallback chunk processing failed: {fallback_e}")
