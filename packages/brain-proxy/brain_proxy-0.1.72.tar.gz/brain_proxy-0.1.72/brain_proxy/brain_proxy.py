        ...
        async def chat(request: Request, tenant: str):
            ...
            async def _process_chunk_payload(chunk) -> dict:
                ...
                try:
                    ...
                    # If model_dump_json fails, try alternative serialization methods
                    self._log(f"Warning: chunk serialization failed: {e}")
                    
                    # Try using model_dump with exclude_unset and mode='json'
                    try:
                        return chunk.model_dump(mode='json', exclude_unset=True)
                    except Exception as e2:
                        self._log(f"Warning: model_dump failed: {e2}")
                        
                        # Try direct dict conversion
                        try:
                            chunk_dict = chunk.dict() if hasattr(chunk, 'dict') else chunk.__dict__
                            return self._sanitize_dict_for_json(chunk_dict)
                        except Exception as e3:
                            self._log(f"Warning: dict conversion failed: {e3}")
                            
                            # Manual extraction as last resort
                            return self._extract_chunk_manually(chunk)

            def _sanitize_dict_for_json(self, obj) -> any:
                """Recursively sanitize a dictionary/object for JSON serialization."""
                if obj is None:
                    return None
                elif isinstance(obj, (str, int, float, bool)):
                    return obj
                elif isinstance(obj, dict):
                    return {k: self._sanitize_dict_for_json(v) for k, v in obj.items() 
                            if not k.startswith('_') and v is not None}
                elif isinstance(obj, (list, tuple)):
                    return [self._sanitize_dict_for_json(item) for item in obj]
                elif hasattr(obj, '__dict__'):
                    return self._sanitize_dict_for_json(obj.__dict__)
                elif hasattr(obj, 'dict'):
                    try:
                        return self._sanitize_dict_for_json(obj.dict())
                    except:
                        pass
            ...
            def _extract_chunk_manually(self, chunk) -> dict:
                """Manually extract chunk data when all other methods fail."""
                try:
                    # Try to access basic attributes safely
                    choices = []
                        ...
                        for choice in chunk.choices:
                            ...
                            if hasattr(choice, 'delta'):
                                ...
                                if hasattr(delta, 'content') and delta.content:
                                    choice_dict["delta"]["content"] = str(delta.content)
                                if hasattr(delta, 'tool_calls') and delta.tool_calls:
                                    ...
                                    for tc in delta.tool_calls:
                                        ...
                                        choice_dict["delta"]["tool_calls"].append(tc_dict)
                            
                            choices.append(choice_dict)
                    
                    return {
                        "choices": choices or [{"index": 0, "delta": {}, "finish_reason": None}],
                        "object": "chat.completion.chunk",
                        "created": int(time.time()),
                        "model": str(getattr(chunk, 'model', 'unknown'))
                    }
                except Exception as final_e:
                    self._log(f"Warning: manual extraction failed: {final_e}")
