    ...
    async def _handle_streaming(
        ...
        async for chunk in upstream_iter:
            ...
            if choice.get("finish_reason") == "tool_calls":
                ...
                # Send a status update before breaking
                status_chunk = {
                    "choices": [{
                        "index": 0,
                        "delta": {"content": "\n"},  # Send a newline to keep stream alive
                        "finish_reason": None
                    }],
                    "object": "chat.completion.chunk",
                    "created": int(time.time()),
        ...
        self._log(f"Making non-streaming follow-up call after tool execution for tenant {tenant}")
        ...
        if response_content:
            self._log(f"Starting to stream response of {len(response_content)} characters")
            self._log(f"Full response content: {response_content[:500]}...")  # Log first 500 chars
            ...
            for i in range(0, len(response_content), chunk_size):
                ...
                try:
                    ...
                    yield data
                    # Force flush by yielding an empty string (triggers FastAPI to flush)
                    yield ""
                except Exception as e:
            ...
            self._log(f"Finished streaming {chunks_sent} chunks")
            ...
            self._log("Sending final chunk with finish_reason=stop")
            yield f"data: {json.dumps(final_chunk)}\n\n"
        else:
            self._log("WARNING: response_content is empty or None")
        
        self._log("Sending [DONE] marker")
        yield "data: [DONE]\n\n"
        
        # Store memories
        final_content = "".join(buf)
        self._log(f"Final buffered content length: {len(final_content)}")
        await self.memory_service.store(tenant, messages + [{
            "role": "assistant",
