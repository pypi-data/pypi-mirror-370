...
{{"selected_tools": ["tool_name1", "tool_name2"]}}"""
    ...
    async def execute(self, name: str, args: Dict[str, Any], local_handler: Optional[Callable] = None) -> Any:
        """Execute a tool and return its result"""
        # Try local handler first
        if local_handler:
            try:
                return await maybe_await(local_handler, name, args)
            except Exception:
                pass
...
class BrainProxy2:
    ...
    async def _execute_tool_calls(
        ...
        for tool_call in tool_calls:
            ...
            try:
                # Check if it's a local tool from request
                is_local = request_tools and any(
                    t["function"]["name"] == function_name for t in request_tools
                )
    ...
    async def _handle_streaming(
        ...
        for tool_call in tool_calls:
            ...
            try:
                is_local = request.tools and any(
                    t["function"]["name"] == name for t in request.tools
                )
