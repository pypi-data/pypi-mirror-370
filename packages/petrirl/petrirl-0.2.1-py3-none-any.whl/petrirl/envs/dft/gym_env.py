...
from petrirl.render.plot_dft import JSSPPlotter
from petrirl.utils.reward   import RewardCalculator
...
class DftEnv(Env):
    ...
    def __init__(self, 
        ...
        observation_size = 15 *len(self.sim.places.values()) + len(self.sim.action_map) 
        ...
        self.reward = RewardCalculator (self)
    ...
    def reward(self,feed_back=None,terminal=False):
       ...
       """ 
       return self.reward.calculate(self,feed_back,terminal)
    ...
    def action_masks(self):
        ...
        """ 
        mask= self.sim.guard_function()
        
        # Force bypass: override everything as True (for ablation studies)
        #mask = [True] * len(mask)
    ...
    def step(self, action):
        ...
        reward = self.reward.calculate(feedback,terminated)
        info = self._get_info()
...
if __name__ == "__main__":
    ...
    env=DftEnv(benchmark=benchmark,instance_id=instance,dynamic=dynamic,size=size,n_agv=agvs ,n_tt=tools_transport)
    
    observation,info=env.reset()
    
    env.sim.graph.plot_net()
