...
def encode_string(data_str: str, encoding: str = 'utf-8', big_endian16: bool = False):

    if len(data_str) % 2 != 0:
        data_str += "\x00"
    encode_bytes = data_str.encode(encoding)
    if big_endian16:
        words = struct.unpack('>' + 'H' * (len(encode_bytes) // 2), encode_bytes)
        encode_bytes = struct.pack('<' + 'H' * len(words), *words)

    return encode_bytes


def decode_string(encode_bytes: bytes, encoding: str = 'utf-8', big_endian16: bool = False):

    if big_endian16:
        if len(encode_bytes) % 2 != 0:
            encode_bytes += 0x00.to_bytes(1, 'little')
        
        words = struct.unpack('<' + 'H' * (len(encode_bytes) // 2), encode_bytes)
        be_bytes = struct.pack('>' + 'H' * len(words), *words)

        data_str = be_bytes.decode(encoding)
    else:
        data_str = encode_bytes.decode(encoding)

    data_str = str(data_str).replace("\x00","")
...
class ExchangeTable:
    ...
    """
    def __init__(self, descriptor: dict, msg: bytearray = None, str_big_endian16: bool = False):
        ...
        self._str_big_endian16 = str_big_endian16
    ...
    def init_from_msg(self, msg: bytearray):
        ...
        for attribute in self.attributes:
            ...
            if attribute.type == AttributeDataType.STRING:
                value = decode_string(value_array, big_endian16=self._str_big_endian16)
    ...
    def to_msg(self) -> bytearray:
        ...
        for attribute in self.attributes:
            ...
            if attribute.type == AttributeDataType.STRING:
                value_array = encode_string(value, big_endian16=self._str_big_endian16)
            elif attribute.type ==  AttributeDataType.INT:
