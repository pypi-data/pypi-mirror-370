...
class SocketClient():
    """Socket client
    """
    def __init__(self, big_endian16: bool = False) -> None:
        ...
        self._max_retries = 4
        self._retries = 0
        self._big_endian16 = big_endian16
    ...
    def read_data(self) -> Any:
        ...
        # like with s7-net, where second byte is received before the fisrt
        if self._big_endian16:
            byte_data_reorder = []
            for k in range(1, len(binary_data), 2):
                byte_data_reorder.append(binary_data[k])
                byte_data_reorder.append(binary_data[k-1])
            byte_data = bytearray(byte_data_reorder.copy())
        else:
