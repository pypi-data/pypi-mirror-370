...
async def build_smart_query_with_dspy(question: str, config: Optional[Any] = None) -> Dict[str, Any]:
    """
    Enhanced version of build_smart_query that uses DSPy for field selection.
    Falls back to static catalog if DSPy is not available or fails.
    
    Args:
        question: Natural language business question
        config: Optional configuration with Lightdash credentials
        
    Returns:
        Query configuration dict ready for lightdash_run_metric_query
    """
    # Try to use DSPy selector first
    try:
        from dbt_mcp.lightdash.dspy_field_selector import SmartFieldSelector
        
        # Identify domain first (still use static catalog for explore mapping)
        domain = identify_domain(question)
        if not domain:
            # If can't identify domain, try all explores
            logger.info("Could not identify domain, will try smart field selection")
            # Default to orders explore for now
            domain = "sales"
        
        catalog_entry = SEMANTIC_CATALOG[domain]
        explore = catalog_entry["explore"]
        
        # Use DSPy selector if config is available
        if config:
            selector = SmartFieldSelector(config)
            field_selection = await selector.select_fields(question, explore)
            
            if field_selection["metrics"] or field_selection["dimensions"]:
                logger.info(f"DSPy selected fields - Metrics: {field_selection['metrics']}, "
                           f"Dimensions: {field_selection['dimensions']}")
                
                # Build query with DSPy-selected fields
                prefixed_metrics = [f"{explore}.{m}" for m in field_selection["metrics"]]
                prefixed_dimensions = [f"{explore}.{d}" for d in field_selection["dimensions"]]
                ...
                # Still use static catalog for time filters
                time_range = parse_time_range(question)
                if time_range:
                    filter_field = f"{explore}.{catalog_entry['time_dimension']}"
                    filter_config = {
                        "field": filter_field,
                        "operator": time_range["operator"],
                        "value": time_range.get("value")
        ...
        logger.warning(f"DSPy selection failed, falling back to static catalog: {e}")
    
    # Fall back to static catalog
    return build_smart_query(question, config)