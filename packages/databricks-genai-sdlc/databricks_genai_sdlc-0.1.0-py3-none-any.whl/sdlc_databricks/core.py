...
class SDLC_DATABRICKS():
    ...
    def generate_response(self, prompt):
        self.logger.info(f"Generating response using mode: {MODE}")
        ...
        if MODE == "default":
            ...
            if not all([self.MODEL_NAME, DATABRICKS_PAT, WORKSPACE_URL]):
                self.logger.error("Databricks MODEL_NAME, PAT, and WORKSPACE_URL are required for 'mosaic' mode.")
                return None

            url = f"{WORKSPACE_URL}/serving-endpoints/{self.MODEL_NAME}/invocations"
            headers = {
