...
from pii_detection.utils.constants import (
    GLINER_MODEL,
...
def extract_pii_array(text, pii_entities) -> List[Any]:
    """
    ...
    gliner_model_results = get_gliner_mode_result(text=text, pii_entities=pii_entities)
...
def extract_pii_elements(text, pii_entities: str, preserve_keywords: str = "") -> List[Any]:
    """
    ...
    # Model 2: Get result (detected PII entities) from Gliner model
    gliner_model_results = get_gliner_mode_result(text=text, pii_entities=valid_entities)
...
def get_gliner_mode_result(text: str, pii_entities: list) -> List[Dict]:
    """
    ...
    def process_chunk(text_chunk):
        """
        Function to process a chunk of text into a list of dictionaries.
