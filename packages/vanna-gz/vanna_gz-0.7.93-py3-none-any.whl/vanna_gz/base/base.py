...
from txtsql_prompt import TxtSqlPrompt


class VannaBase(ABC):
    def __init__(self, config=None):
        """
        Initialize the VannaBase class.
        """
        if config is None:
            config = {}
    ...
    def generate_sql(
        ...
        **kwargs
        ...
        Args:
            ...
            allow_llm_to_see_data (bool): Whether to allow
            the LLM to see the data (for the purposes of introspecting the
            data to generate the final SQL).

        Returns:
            str: The SQL query that answers the question.
        """
        t = self.create_txt_sql_prompt(question, **kwargs)

        self.log(title="SQL Prompt", message=str(t.prompt))
        llm_response = self.submit_prompt(t.prompt, **kwargs)
        ...
        if 'intermediate_sql' in llm_response:
            ...
            if allow_llm_to_see_data:
                ...
                try:
                    ...
                    l: str = (
                        f"The following is a pandas DataFrame with the results "
                        f"of the intermediate SQL query {intermediate_sql}: \n"
                    )

                    prompt = self.get_sql_prompt(
                        initial_prompt=t.initial_prompt,
                        question=question,
                        question_sql_list=t.question_sql_list,
                        ddl_list=t.ddl_list,
                        doc_list=t.doc_list + [
                            l + df.to_markdown()
                        ],
                    ...
                    self.log(title="Final SQL Prompt", message=str(prompt))
    ...
    def create_txt_sql_prompt(
        ...
        **kwargs
        ...
        t = TxtSqlPrompt(initial_prompt=initial_prompt)
    ...
    def is_sql_valid(self, sql: str) -> bool:
        ...
        ```
        Checks if the SQL query is valid. This is usually
        used to check if we should run the SQL query or not.
    ...
    def should_generate_chart(self, df: pd.DataFrame) -> bool:
        ...
        Checks if a chart should be generated for the given DataFrame.
        By default, it checks if the DataFrame has more than one row and has numerical columns.
    ...
    def generate_rewritten_question(self, last_question: str, new_question: str, **kwargs) -> str:
        ...
        **Example:**
        ```python
        rewritten_question = vn.generate_rewritten_question("Who are the top 5 customers by sales?",
        "Show me their email addresses")
        ```

        Generate a rewritten question by combining the
        last question and the new question if they are related.
        If the new question is self-contained and not related to the
        last question, return the new question.
        ...
        prompt = [
            self.system_message(
                ...
                "question if they are related. "
                "If the second question does not relate to the first question "
                "and is fully self-contained, "
                "return the second question. Return just the new combined "
                "question with no additional explanations. "
                "The question should theoretically be answerable with a single SQL statement."
            ),
            self.user_message(
    ...
    def generate_followup_questions(
        ...
        **Example:**
        ...
        message_log = [
            ...
            self.user_message(
                (
                    F"Generate a list of {n_questions} followup questions that the user might ask"
                    " about this data. "
                    "Respond with a list of questions, one per line. Do not answer"
                    " with any explanations -- just the questions. "
                    "Remember that there should be an unambiguous SQL query that can be "
                    "generated from the question. "
                    "Prefer questions that are answerable outside of the context "
                    "of this conversation. "
                    "Prefer questions that are slight modifications of the SQL query "
                    "that was generated that allow digging deeper into the data. "
                    "Each question will be turned into a button that the user can click "
                    "to generate a new SQL query so don't use 'example' type questions. "
                    "Each question must have a one-to-one correspondence "
                    "with an instantiated SQL query."
                )
    ...
    def str_to_approx_token_count(self, string: str) -> float:
        return len(string) / 4
    ...
    def get_sql_prompt(
        ...
        **kwargs,
        ...
        vn.get_sql_prompt(
            ...
            question_sql_list=[{"question":
            "What are the top 10 customers by sales?", "sql": "SELECT * FROM customers ORDER BY sales DESC LIMIT 10"}],
    ...
    def generate_plotly_code(
        ...
        **kwargs
        ...
        """
        Generate the Python code to create a Plotly chart from the DataFrame.
        """
    ...
    def connect_to_snowflake(
        ...
        **kwargs
        ...
        def run_sql_snowflake(sql: str, **kwargs) -> pd.DataFrame:
            cs = conn.cursor()
    ...
    def connect_to_sqlite(
        ...
        **kwargs
        ...
        def run_sql_sqlite(sql: str, **kwargs) -> pd.DataFrame:
    ...
    def connect_to_postgres(
        ...
        **Example:**
        ...
        def run_sql_postgres(sql: str, **kwargs) -> pd.DataFrame:
            conn = None
            try:
    ...
    def connect_to_mysql(
        ...
        **kwargs
        ...
        def run_sql_mysql(sql: str, **kwargs) -> pd.DataFrame:
            if conn:
                try:
                    ...
                    if cs.description is not None:
                        df = pd.DataFrame(
                            ...
                            ] if cs.description is not None else None
                            if cs.description is not None
                            else None
                        )
                    else:
                    ...
                    conn.rollback()
                    raise e
            # Always return a DataFrame, even if connection is not available
            return pd.DataFrame()
    ...
    def connect_to_clickhouse(
        ...
        **kwargs
        ...
        def run_sql_clickhouse(sql: str, **kwargs) -> pd.DataFrame:
            if conn:
                try:
    ...
    def connect_to_oracle(
        ...
        **Example:**
        ...
        def run_sql_oracle(sql: str, **kwargs) -> pd.DataFrame:
            if conn:
                try:
                    sql = sql.rstrip()
                    ...
                    # Create a pandas dataframe from the results
                    df = pd.DataFrame(
                        results, columns=[desc[0] for desc in cs.description] if cs.description is not None else None
                    )
                    return df
    ...
    def connect_to_duckdb(
        ...
        **kwargs
        ...
        This is just a helper function to set [`vn.run_sql`][vanna.base.base.VannaBase.run_sql]

        Args:
            url (str): The URL of the database
            to connect to. Use :memory: to create an in-memory database.
            Use md: or motherduck: to use the MotherDuck database.
            init_sql (str, optional): SQL to run when connecting
        ...
        def run_sql_duckdb(sql: str, **kwargs):
            return conn.query(sql, **kwargs).to_df()
    ...
    def connect_to_mssql(self, odbc_conn_str: str, **kwargs):
        ...
        def run_sql_mssql(sql: str, **kwargs) -> pd.DataFrame:
    ...
    def connect_to_presto(
        ...
        **kwargs
        ...
        if not host:
            host_env = os.getenv("PRESTO_HOST")
        ...
        if not catalog:
            catalog = os.getenv("PRESTO_CATALOG") or "hive"

        if not catalog:
            raise ImproperlyConfigured("Please set your presto catalog")

        if not user:
            user = os.getenv("PRESTO_USER") or ""

        if not user:
            raise ImproperlyConfigured("Please set your presto user")

        if not password:
            password = os.getenv("PRESTO_PASSWORD") or ""

        if not port:
            port_env = os.getenv("PRESTO_PORT")
            if port_env is not None:
                port = int(port_env)
        ...
        def run_sql_presto(sql: str, **kwargs) -> pd.DataFrame:
            if conn:
                try:
                    sql = sql.rstrip()
    ...
    def connect_to_hive(
        ...
        **kwargs
        ...
        """
        Connect to a Hive database. This is just a helper
        function to set [`vn.run_sql`][vanna.base.base.VannaBase.run_sql].
        ...
        if not dbname:
            dbname = os.getenv("HIVE_DATABASE") or "default"
        ...
        def run_sql_hive(sql: str, **kwargs) -> pd.DataFrame:
            if conn:
                try:
                    cs = conn.cursor()
                    cs.execute(sql)
    ...
    def ask(
        ...
        **Example:**
        ...
        Args:
            ...
            auto_train (bool): Whether to automatically train
                Vanna.AI on the question and SQL query.
            visualize (bool): Whether to generate plotly code and display
                the plotly figure.
    ...
    def train(
        ...
        **Example:**
        ...
        If you call it with no arguments, it will check if
            you connected to a database and it will attempt
                to train on the metadata of that database.
        If you call it with the sql argument, it's equivalent
            to [`vn.add_question_sql()`][vanna.base.base.VannaBase.add_question_sql].
        If you call it with the ddl argument, it's equivalent
            to [`vn.add_ddl()`][vanna.base.base.VannaBase.add_ddl].
        If you call it with the documentation argument,
            it's equivalent to [`vn.add_documentation()`]
            [vanna.base.base.VannaBase.add_documentation].
        Additionally, you can pass a [`TrainingPlan`]
            [vanna.types.TrainingPlan] object. Get a training plan with
    ...
    def _get_databases(self) -> List[str]:
        """
        Get a list of all databases in the current SQL environment.
        """
        try:
    ...
    def get_training_plan_generic(self, df) -> TrainingPlan:
        ...
        into groups of table/column descriptions that can be used to pass to the LLM.
        ...
        for database in df[database_column].unique().tolist():
            for schema in (
                ...
                for table in (
                    ...
                    doc = (
                        f"The following columns are in the {table} table in the "
                        f"{database} database:\n\n"
                    )
            ...
            if use_historical_queries:
                try:
                    ...
                    df_history = self.run_sql(
                        "select * from "
                        "table(information_schema.query_history(result_limit => 5000))"
                        " order by start_time"
                    )
                except Exception as e:
                    print(e)
