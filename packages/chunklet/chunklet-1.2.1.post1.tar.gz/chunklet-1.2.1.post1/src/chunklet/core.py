...
from typing import List, Tuple, Callable, Optional, Union, Literal, Set
...
class Chunklet:
    ...
    def _get_overlap_clauses(
        ...
        # Filter out empty strings that can be produced by re.split
        detected_clauses = [c for c in detected_clauses if c]
        ...
        if overlapped_clauses:
            first_clause = overlapped_clauses[0]
            # Prepend "..." if the clause doesn't start with a capital letter
            # (checking first two characters to account for punctuation like quotes)
            is_capitalized = first_clause[0].isupper() or (
                len(first_clause) > 1 and first_clause[1].isupper()
            )
            if not is_capitalized:
                overlapped_clauses[0] = "... " + first_clause

        return overlapped_clauses

    def _count_tokens(self, text: str, token_counter: Callable[[str], int]) -> Tuple[int, Optional[str]]:
        """Safely count tokens, handling potential errors from the token_counter."""
        try:
            return token_counter(text), None
        except Exception as e:
            return math.inf, f"Token counter failed for text: '{text}'. Error: {e}"
    ...
    def _find_clauses_that_fit(
        ...
        for i in range(len(clauses)):
            clause_tokens, warning_msg = self._count_tokens(clauses[i], token_counter)
            if warning_msg:
        ...
        return fitted, unfitted, warnings
    ...
    def _group_by_chunk(
        ...
        Returns:
            Tuple[List[List[str]], Set[str]]: A list of chunks, where each chunk is a list of sentences, and a set of warning messages.
        """
        chunks = []
        curr_chunk = []
        token_count = 0
        ...
        while index < len(sentences):
            ...
            if config.mode in {"token", "hybrid"}:
                sentence_tokens, warning_msg = self._count_tokens(
            ...
            if (
                ...
                if token_count + sentence_tokens > config.max_tokens:
                    ...
                    fitted, unfitted, find_clauses_warnings = self._find_clauses_that_fit(
                        sentences[index], remaining_tokens, config.token_counter
                    )
                    warnings.update(find_clauses_warnings)
                ...
                if config.mode in {"token", "hybrid"}:
                    ...
                    for s in curr_chunk:
                        tokens, warning_msg = self._count_tokens(s, config.token_counter)
                        if warning_msg:
                            warnings.add(warning_msg)
                        current_chunk_token_count += tokens
        ...
        final_chunks = tuple("".join(chunk) for chunk in chunks)
        return final_chunks, tuple(sorted(list(warnings)))
    ...
    def _chunk(
        ...
        sentences, split_warnings = self._split_by_sentence(text, config.lang)
        all_warnings = set(split_warnings)

        if self.verbose:
            logger.debug(
                "Text splitted into sentences. Total sentences detected: {}", len(sentences),
            )
        if not sentences:
            return [], all_warnings
        ...
        chunks, group_warnings = self._group_by_chunk(sentences, config)
        all_warnings.update(group_warnings)

        if self.verbose:
            logger.info("Finished chunking text. Generated {} chunks.\n", len(chunks))
        return chunks, all_warnings
    ...
    def _chunk_cached(
        ...
        Returns:
            Tuple[List[str], Set[str]]: A list of text chunks and a set of warning messages.
        """
        params = locals()
        params.pop("self")
    ...
    ) -> Union[List[str], Tuple[Tuple[str, ...], Tuple[str, ...]]]:
        """
    ...
    def batch_chunk(
        ...
        *,
        ...
        for warning_set in collected_warnings:
            warnings_counter.update(warning_set)
