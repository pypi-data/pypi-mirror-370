...
- 🛡️ **Enhanced Token Counter Reliability:** Introduced robust error handling for custom token counters, preventing crashes and improving overall stability.
- 📊 **Aggregated Warning System:** Reworked the warning mechanism to collect and display all unique warnings in a single, clear message.
- 🐛 **Critical Bug Fixes:** Addressed an `IndexError` in overlap calculation
