...
class ParallelGymAgent(GymAgent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """Create an agent from a function to create a Gymnasium environment
    ...
    def _reset(self, k: int) -> Dict[str, Tensor]:
        ...
        self._last_frame = [None for _ in range(self.num_envs)]
    ...
    def _step(self, k: int, action: Tensor):
        """Step for the k^th environment

        :param k: The environment number
        ...
        # Just reset the state if not including last state (but keep all the
        # other informations)
        if done:
