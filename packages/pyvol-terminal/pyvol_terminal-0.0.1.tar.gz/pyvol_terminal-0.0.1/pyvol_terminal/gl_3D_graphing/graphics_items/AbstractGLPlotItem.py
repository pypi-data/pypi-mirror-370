...
class BaseAbstractPlotDataItem(AbstractGLGraphicsItem, metaclass=ABCGraphicsItemMeta):
    ...
    def __init__(self, topology_type="", dataKwargs=[], view_box=None, parentItem=None, **kwargs):
    ...
    def _getDisplayDataset(self) -> AbstractPlotDataset | None:
        ...
        if not view_range is None and all(allFinite):
            data = self.clipDataFromVRange(view_range, data)        

        self._datasetDisplay = self.plotdataset_cls(data, allFinite)
        
        self.property('xViewRangeWasChanged', False)
        self.property('yViewRangeWasChanged', False)
        self.property('zViewRangeWasChanged', False)
        return self._datasetDisplay
    ...
    def _paintHelper(self):
        data = self._compareDataUpdate()
        dataKwargs = {key : value for key, value in zip(self._dataKwargs, data)}
        
        self.blockUpdates(True)
        self._setDataChild(**dataKwargs)
        self.blockUpdates(False)

    @abstractmethod
    def _compareDataUpdate(self):...


class _Temp(BaseAbstractPlotDataItem):
    pass

class _ABCPlotDataItemMeta(ABCGraphicsItemMeta):
    def __call__(cls, *args, **kwargs):
        ...
        topology_type = cls.topology_type
        dataKwargs = cls.dataKwargs
        view_box = kwargs.pop("view_box", None)
        parentItem = kwargs.pop("parentItem", None)

        BaseAbstractPlotDataItem.__init__(self, topology_type=topology_type, dataKwargs=dataKwargs, parentItem=parentItem, view_box=view_box, )
...
class BaseMeshPlotDataItem(BaseAbstractPlotDataItem, metaclass=_ABCPlotDataItemMeta):
    topology_type="Mesh"
    dataKwargs=["x", "y", "z"]
    ...
    def __init__(self, view_box=None, parentItem=None, **kwargs):
        ...
        self._y=None
        self._z=None
        BaseAbstractPlotDataItem.__init__(self, view_box=view_box, parentItem=parentItem)
    ...
    @classmethod
    def clipDataFromVRange(cls, dataset, view_range):
        x, y, z = dataset
...
class BaseMeshFlatPlotDataItem(BaseAbstractPlotDataItem, metaclass=_ABCPlotDataItemMeta):
...
class CustomGLSurfacePlotDataItem(BaseMeshPlotDataItem, opengl.GLSurfacePlotItem):
    def __init__(self, x=None, y=None, z=None, colors=None, parentItem=None, **kwds):
        BaseMeshPlotDataItem.__init__(self, parentItem=parentItem, x=x, y=y, z=z, **kwds)
    
    
    def _setDataChild(self, *args, **kwargs):
        return opengl.GLSurfacePlotItem.setData(self, *args, **kwargs)
    
    def _childPaint(self):
        return opengl.GLSurfacePlotItem.paint(self)

    
class CustomGLScatterPlotDataItem(BaseMeshFlatPlotDataItem, opengl.GLScatterPlotItem):
    def __init__(self, parentItem=None, **kwds):
        BaseMeshFlatPlotDataItem.__init__(self, parentItem=parentItem, **kwds)
    
    def _setDataChild(self, *args, **kwargs):
        return opengl.GLScatterPlotItem.setData(self, *args, **kwargs)
    
    def _childPaint(self):
        return opengl.GLScatterPlotItem.paint(self)


class CustomGraphPlotDataItem(BaseMeshFlatPlotDataItem, opengl.GLGraphItem):
    def __init__(self, parentItem=None, **kwds):
        BaseMeshFlatPlotDataItem.__init__(self, parentItem=parentItem, **kwds)
    
    def _setDataChild(self, *args, **kwargs):
        return opengl.GLGraphItem.setData(self, *args, **kwargs)
    
    def _childPaint(self):
        return opengl.GLGraphItem.paint(self)

class AbstractGLPlotItem:
    pass