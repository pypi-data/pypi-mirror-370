...
class FileUtility:
    ...
    def _has_pathological_lines(sample: bytes, is_known_type: bool) -> bool:
        ...
        """
        # Stricter thresholds - even for known types, extremely long lines are problematic
        max_line_length = 100000 if is_known_type else 50000  # 100KB vs 50KB
        max_avg_line_length = 20000 if is_known_type else 5000  # 20KB vs 5KB
        
        # Check if there are any newlines at all in a reasonable sample
        if b'\n' not in sample and b'\r' not in sample and len(sample) >= 8192:
            # No line breaks in 8KB+ suggests a single massive line
            # Be stricter even for known types
            if not is_known_type or len(sample) >= 32 * 1024:
                return True
    ...
    def _process_and_store_file(self, file_path: Union[str, Path], *, allow_problematic: bool = False, max_bytes_on_problem: int = 2 * 1024 * 1024) -> Optional[Dict[str, Any]]:
        """Process a file, create an MCard, and store it in the collection.
        
        If allow_problematic is True, files flagged as problematic are loaded as binary blobs
        with optional size cap and without decoding to avoid pathological processing issues.
        """
        ...
        try:
            ...
            if FileUtility._is_problematic_file(file_path):
                if not allow_problematic:
                    ...
                    return None
                logger.warning(f"Problematic file detected, loading as binary blob (capped to {max_bytes_on_problem} bytes): {file_path}")
...
def load_file_to_collection(path: Union[str, Path], 
                         ...
                         max_bytes_on_problem: int = 2 * 1024 * 1024) -> List[Dict[str, Any]]:
    """
    ...
    Args:
        ...
        include_problematic: If True, files flagged as problematic are loaded as binary blobs
    ...
    if path.is_file():
        ...
        result = utility._process_and_store_file(path, allow_problematic=include_problematic, max_bytes_on_problem=max_bytes_on_problem)
        if result:
        ...
        for i, file_path in enumerate(file_paths):
            ...
            result = utility._process_and_store_file(file_path, allow_problematic=include_problematic, max_bytes_on_problem=max_bytes_on_problem)
            if result:
                results.append(result)
