...
"""
        ...
        for app_config, models in models_by_app.items():
            ...
            for model in models:
                ...
                tag_model_names.append(f"{model_name}Tag")
            ...
            if dry_run:
                ...
                for model_data in models_data:
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"Created tag model for {model_data['app_label']}.{model_data['model_name']} at {file_path}"
                        )
                    )
    ...
    def _update_imports(self, models_file_path, tag_model_names):
        ...
        if existing_import_line:
            ...
            import_line = f"{import_pattern} {', '.join(tag_model_names)}  # noqa"
            lines[last_import_index] = import_line
        else:
            # Add new import after last import
            if last_import_index >= 0:
                import_line = f"\n\nfrom .flexi_generated_model import {', '.join(tag_model_names)}  # noqa"
