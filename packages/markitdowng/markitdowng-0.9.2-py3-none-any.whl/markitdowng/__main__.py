...
def main():
    parser = argparse.ArgumentParser(
        ...
        usage=dedent(
            ...
            SYNTAX:
                ...
                markitdowng <OPTIONAL: FILENAME>
                If FILENAME is empty, markitdowng reads from stdin.

            EXAMPLE:

                markitdowng example.pdf

                OR

                cat example.pdf | markitdowng
                ...
                markitdowng example.pdf -o example.md

                OR

                markitdowng example.pdf > example.md
            """
        ).strip(),
    )
    ...
    if args.list_plugins:
        ...
        plugin_entry_points = list(entry_points(group="markitdowng.plugin"))
        if len(plugin_entry_points) == 0:
            print("  * No 3rd-party plugins installed.")
            print(
                "\nFind plugins by searching for the hashtag #markitdowng-plugin on GitHub.\n"
            )
        else:
