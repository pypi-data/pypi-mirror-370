...
class TestImportCommand:
    ...
    def test_import_no_args(self, db, capsys):
        ...
        # Verify
        output = capsys.readouterr().out
        assert "Please provide a BibTeX file path" in output or "Usage:" in output
    ...
    def test_import_invalid_extension(self, db, capsys, tmp_path):
        ...
        # Verify
        output = capsys.readouterr().out
        assert "File must have .bib or .bibtex extension" in output
...
class TestPromptCommand:
    ...
    def test_prompt_view(self, config, db, capsys):
        ...
        # Verify
        output = capsys.readouterr().out
        assert "No user prompt set" in output or "Research Context" in output
...
class TestContextCommands:
    ...
    def test_cadd_empty_adds_all_papers(self, config, db, sample_papers, capsys):
        """Test: /cadd (empty) - should add all papers from collection as abstracts"""
        ...
        # Verify all papers were added to session context with abstract type
        papers_in_db = db.list_papers()
    ...
    def test_cadd_empty_replaces_different_context_type(self, config, db, sample_papers, capsys):
        """Test: /cadd (empty) replaces papers with different context types"""
        from litai.context_manager import SessionContext
        ...
        # Setup: Create session context and add first paper with full_text
        session_context = SessionContext()
        ...
        # Verify output mentions adding all papers
        output = capsys.readouterr().out
        assert "Added 3 papers" in output  # All papers added/updated
        
        # Verify first paper now has abstract type (replaced from full_text)
        context_entry = session_context.papers[papers_in_db[0].paper_id]
        assert context_entry.context_type == "abstract"
    ...
    def test_cadd_with_tag_filter(self, config, db, sample_papers, capsys):
        ...
        context_entry = session_context.papers[first_paper.paper_id]
        assert context_entry.paper_title == first_paper.title
        assert context_entry.context_type == "abstract"  # Default for tag operations
