...
async def handle_context_add(
    ...
    if not args:
        return "[red]Usage: /cadd <paper reference> [full-text|abstract|notes] OR /cadd --tag <tag_name> [full-text|abstract|notes][/red]"
...
async def handle_context_modify(
    ...
    if not args:
        return "[red]Usage: /cmodify <paper reference> <new_context_type> OR /cmodify --tag <tag_name> <new_context_type>[/red]"
