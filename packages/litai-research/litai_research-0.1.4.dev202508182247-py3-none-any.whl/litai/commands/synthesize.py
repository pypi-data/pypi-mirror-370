...
def handle_synthesize_command(
    ...
    # Get tool approval before proceeding
    from litai.tool_approval import ToolApprovalManager, ToolCall
    
    approval_manager = ToolApprovalManager(config, status_manager=status)
    
    # Create a tool call for the synthesis operation
    tool_call = ToolCall(
        id="synthesize_1",
        name="synthesize",
        description=f"Synthesize insights from {session_context.get_paper_count()} papers to answer your question",
        arguments={"query": args, "sharded": sharded},
    )
    
    # Get approval (asyncio.run is needed since this is a sync function)
    approved_calls = asyncio.run(approval_manager.get_approval([tool_call]))
    
    if not approved_calls:
        logger.info("synthesis_command_cancelled_by_user", query=args[:100])
        console.print("[yellow]Synthesis cancelled by user[/yellow]")
        return
    ...
    try:
        if sharded:
            ...
            asyncio.run(
                _run_sharded_synthesis(args, session_context, db, config, token_tracker, status),
            )
        else:
            logger.info(
