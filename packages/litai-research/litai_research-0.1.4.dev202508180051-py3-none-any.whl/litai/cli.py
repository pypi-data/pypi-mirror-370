...
def handle_command(
    ...
    if cmd == "/help":
        ...
        if session_context:
            handle_context_show(session_context)
        else:
...
def show_help() -> None:
    ...
    console.print("[dim]* = AI assistant can use this command[/dim]\n")

    # Core commands
    console.print("[bold cyan]Find Papers:[/bold cyan]")
    console.print("* [dim]/find[/dim]        Search for papers to add to your collection")
    console.print()

    console.print("[bold cyan]Collection Management:[/bold cyan]")
    console.print("* [dim]/add[/dim]         Add papers from `/find` results to your collection")
    console.print("  [dim]/import[/dim]      Import from BibTeX to your collection")
    console.print("* [dim]/remove[/dim]      Remove papers from your collection")
    console.print("* [dim]/collection[/dim]  List papers in your collection")
    console.print("* [dim]/note[/dim]        Add notes to papers in your collection")
    console.print("* [dim]/tag[/dim]         Tag papers in your collection")
    console.print()

    console.print("[bold cyan]Context Management:[/bold cyan]")
    console.print("* [dim]/cadd[/dim]        Add papers to context from your collection")
    console.print("* [dim]/cremove[/dim]     Remove papers from context")
    console.print("* [dim]/cshow[/dim]       Show current context")
    console.print("* [dim]/cclear[/dim]      Clear all context")
    console.print("* [dim]/cmodify[/dim]     Modify paper context type")
    console.print()

    console.print("[bold cyan]Analysis:[/bold cyan]")
    console.print("* [dim]/synthesize[/dim]  Synthesize insights from papers in your context")
    console.print()
...
def show_find_help() -> None:
    ...
    console.print(
        "Search for papers on Semantic Scholar with flexible result management.\n",
    )

    console.print("[bold]Usage:[/bold]")
    console.print(
        "[blue]/find <query>[/blue] — Search and replace current results (default)",
    )
    console.print(
        "[blue]/find <query> --append[/blue] — Add to existing search results",
    )
    console.print("[blue]/find --clear[/blue] — Clear all accumulated search results")
    console.print(
        "[blue]/find --recent[/blue] — Show cached results from previous searches",
    )
    ...
    console.print(
        "  • After searching, use /add <number> to add papers to your collection\n",
    )
...
def show_synthesis_help() -> None:
    ...
    console.print("  [blue]/collection[/blue]      List all papers in your collection")
    console.print("  [blue]/selected[/blue]    Show papers selected in current session")
    console.print(
        "  [blue]/context[/blue]     Change context depth (abstracts, notes, sections, full_text)",
    )
...
async def handle_tag_command(args: str, db: Database) -> None:
    ...
    try:
        ...
        if paper_num < 1 or paper_num > len(papers):
            output.error(
                f"Invalid paper number. Please choose between 1 and {len(papers)}",
            )
            return
        paper = papers[paper_num - 1]
        await logger.ainfo(
