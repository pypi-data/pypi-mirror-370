    ...
    async def handle_query(self, query: str) -> None:
        ...
        try:
            ...
            if len(self.conversation.messages) == 1:  # Only system message exists
                ...
                if papers:
                    paper_context = "\n[System: Current paper collection contains "
                    paper_context += f"{len(papers)} papers:\n"
                    # Show first 10 papers with titles and years
                    for i, p in enumerate(papers[:10], 1):
                        paper_context += f"  {i}. {p.title} ({p.year})\n"
                    if len(papers) > 10:
                        paper_context += f"  ... and {len(papers) - 10} more papers\n"
                    paper_context += "]\n\n"
        ...
        context_tools = [
            {
                ...
                "function": {
                    ...
                    "description": "Create a context management operation for a single paper or papers with a tag",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "paper_reference": {
                                "type": "string",
                                "description": "Paper reference (number or natural language)",
                            },
                            "tag": {
                                "type": "string",
                                "description": "Tag name to operate on all papers with that tag",
                            },
...
Use create_context_operation for each paper you want to add/remove, or use the 'tag' parameter to operate on all papers with a specific tag.
Use list_papers if the user wants to add "all papers".
Use list_papers_by_tag if you need to check what papers have a specific tag before operating on them.
            ...
            try:
                ...
                for tool_call_idx, tool_call in enumerate(response["tool_calls"]):
                    ...
                    if tool_name == "create_context_operation":
                        # Check if this is a tag-based operation or paper reference
                        if "tag" in tool_args and tool_args["tag"]:
    ...
    async def _update_operations_with_feedback(
        ...
        for op in operations:
            operations_summary.append(
                f"- {op['paper_title']}: {op['action']} as {op['context_type']}",
            )
...
When the user requests to use "full text", "full-text", "fulltext", or "the full text", they mean context_type should be "full_text".
