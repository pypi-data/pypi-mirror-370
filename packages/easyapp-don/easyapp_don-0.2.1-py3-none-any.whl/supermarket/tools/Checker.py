        ...
        for b in bi:
            if not isinstance(b,list):
                ...
                if not isinstance(bi_size,tuple) or len(bi_size) != 2:
                    warnings.warn(f"\n[EasyappError] Argument <bi>'s {b}'s size (b[1]) must be tuple,"
                                  f"and its len must be 2.",UserWarning)
                    bi_size=(0.2,0.1)
                if not isinstance(bi_pos,(list,tuple)) or len(bi_pos) < 2:
                    warnings.warn(f"\n[EasyappError] Argument <bi>'s {b}'s position (b[2]) must be tuple or list,"
                                  f"and its len must be 2.",UserWarning)
                    bi_pos=(0.5,0.5)
                if not isinstance(bi_o,(int,float)) or not 0<=bi_o<=1:
                    warnings.warn(f"\n[EasyappError] Argument <bi>'s {b}'s opacity (b[3]) must be int or float,"
                                  f"and its value must between 0 and 1.",UserWarning)
                    bi_o=0.9
                if not isinstance(bi_is_close,bool):
                    warnings.warn(f"\n[EasyappError] Argument <bi>'s {b}'s is_close (b[4]) must be bool.",UserWarning)
                    bi_is_close=True
        ...
        for b in bs:
            if not isinstance(b,list):
                ...
                if not isinstance(b_size,tuple) or len(b_size) != 2:
                    warnings.warn(f"\n[EasyappError] Argument <bs>'s {b}'s size (b[2]) must be tuple,"
                                  f"and its len must be 2.",UserWarning)
                    b_size=(0.1,0.05)
                if not isinstance(b_pos,(list,tuple)) or len(b_pos)<2 :
                    warnings.warn(f"\n[EasyappError] Argument <bs>'s {b}'s position (b[3]) must be list or tuple,"
                                  f"and its len must be 2.",UserWarning)
                    b_pos=(0.5,0.1)
...
p_em={"Page0":{"tie":"",
               ...
               "tip":{},
               "back":{}}}
p_mode={"Page1":{"tie":"SuperEasyapp-1",
                 "ts":[["Hello,easyapp_don!",(0.5,0.9)],
                       ["Text1",(0.1,0.65)],
                       ["Text2",(0.5,0.65)],
                       ["Text3",(0.9,0.65)],
...
i_em={"Page0":{"tie":"",
               ...
               "bs":[],
               "es":[]}}
i_mode={"Page1":{"tie":"SuperEasyapp-1",
                 "ts":[["Hello,easyapp_don!",(0.5,0.95)],
                       ["Text1",(0.1,0.6)],
                       ["Text2",(0.5,0.6)],
                       ["Text3",(0.9,0.6)],
