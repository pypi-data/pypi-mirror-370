...
qu=queue.Queue()
class MsgApp(MDApp):
    def __init__(self, t=None, tie="MsgEasyapp", color=WHITE12, theme=LIGHT,
        ...
        if tip is None:
            tip = {"tie": "Tip",
                   ...
                   "cal": ["No", BLUE13]}
        if t is None:
            t = ["Hello,easyapp-lazydonkey!", (0.5, 0.5)]
        if ok is None:
            ok = ["Ok", BLUE13, (0.15, 0.05), (0.15, 0.1), True]
        if cal is None:
            cal = ["Cancel", GREY4, (0.15, 0.05), (0.85, 0.1), True]
        tie=str(tie)
        t=check_type(t,list,["Hello,easyapp-lazydonkey!", (0.5, 0.5)])
    ...
    def build(self):
        ...
        if self.t:
            t_txt=str(self.t[0]) if len(self.t)>0 else "Hello,easyapp-lazydonkey!"
    ...
    def on_space(self,instance):
        pass
def msg(t=None, tie="MsgEasyapp", color=WHITE12, theme=LIGHT,
    ...
    """
        ...
        KeyNotes:
        - The `theme` parameter values (`LIGHT`/`DARK`) can be imported from `easyapp-lazydonkey.tools.colors`.
        - All color parameters should be imported from `easyapp-lazydonkey.tools.colors` (e.g., `WHITE12`, `BLUE13`).
        ...
        :param t: Configuration for the main message text. Format:
            ...
            ```
            Defaults to `["Hello,easyapp-lazydonkey!", (0.5, 0.5)]` if None.

        :param tie: Title of the message window (string)
            Example: `tie = "Confirmation Required"`
            Defaults to "MsgEasyapp".

        :param color: Background color of the message window.
            Should be a color from `easyapp-lazydonkey.tools.colors`
            Example: `color = GREY2` (light grey background)
            Defaults to WHITE12.

        :param theme: Overall theme style for the window.
            Import `LIGHT` or `DARK` from `easyapp-lazydonkey.tools.colors`
            Example:
            ```python
            from easyapp-lazydonkey.tools.colors import DARK
        ...
        :param ok: Configuration for the OK/Confirm button. Format:
            ...
            - `button_color`: Color from `easyapp-lazydonkey.tools.colors`
        ...
        Example Usage:
        ```python
        from easyapp-lazydonkey.tools.colors import LIGHT, BLUE13, RED6, GREY2
