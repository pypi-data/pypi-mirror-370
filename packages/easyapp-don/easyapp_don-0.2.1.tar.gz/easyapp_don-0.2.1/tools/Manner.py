...
def check_type(obj:object=(1,2,3,4),t:object=tuple,mr:object=LIGHT) -> object:
    ...
    if t == "t":
        if obj != LIGHT and obj != DARK:
            warnings.warn(f"\n[EasyappError] Argument <theme> must be variable <LIGHT> or <DARK> from easyapp-lazydonkey.manner.colors.",UserWarning)
            return mr
        else:
            return obj
    elif t == "c":
        if not isinstance(obj,tuple):
            ...
            warnings.warn(f"\n[EasyappError] Argument <{obj}> must be tuple, not {o_e}",UserWarning)
            return mr
        else:
            if len(obj) != 4:
                warnings.warn(f"\n[EasyappError] Argument <{obj}>'s len must be 4.",UserWarning)
                return mr
            else:
                return obj
    else:
        if not isinstance(obj,t):
            ...
            warnings.warn(f"\n[EasyappError] Argument <{obj}> must be {t}, not {o_error}.",UserWarning)
            return mr
        else:
            return obj
def fill_bs(bs=None) -> list:
    ...
    if bs:
        if not isinstance(bs,list):
            warnings.warn("\n\n[EasyappTypeError] Argument <bs> must be list,below will set it to [].\n\n",UserWarning)
            return []
        filled_bs=[]
        for b in bs:
            if not isinstance(b,list):
                warnings.warn(f"\n[EasyappError] Argument <bs>'s {b} must be list.",UserWarning)
                b=["ERROR",(0.15,0.05),(0.5,0.1),RED1,False]
                filled_bs.append(b)
            else:
                if len(b) != 5:
                    warnings.warn(f"\n[EasyappError] Argument <bs>'s {b}'s len must be 5.",UserWarning)
                ...
                if not isinstance(b_size,(list,tuple)) or len(b_size) != 2:
                    warnings.warn(f"\n[EasyappError] Argument <bs>'s {b}'s size must be list or tuple,"
                                  f"and its len must be 2.",UserWarning)
                    b_size=(0.15,0.05)
                if not isinstance(b_pos,(list,tuple)) or len(b_pos) != 2:
                    warnings.warn(f"\n[EasyappError] Argument <bs>'s {b}'s pos must be list or tuple,"
                                  f"and its len must be 2.",UserWarning)
                    b_pos=(0.5,0.1)
                if not isinstance(b_color,tuple) or len(b_color) != 4:
                    warnings.warn(f"\n[EasyappError] Argument <bs>'s {b}'s color must be tuple,"
                                  f"and its len must be 4.",UserWarning)
                    b_color=RED1
                if not isinstance(is_close,bool):
                    warnings.warn(f"\n[EasyappError] Argument <bs>'s {b}'s is_close must be bool.",UserWarning)
                    is_close=True
    ...
    if es:
        ...
        for e1 in es:
            if not isinstance(e1,list):
                warnings.warn(f"\n[EasyappError] Argument <es>'s {e1} must be list.",UserWarning)
                e1=["ERROR",(0.9,0.1),(0.5,0.6)]
                filled_es.append(e1)
            else:
                if len(e1) != 3:
                    warnings.warn(f"\n[EasyappError] Argument <es>'s {e1}'s len must be 3.",UserWarning)
                ...
                if not isinstance(e_size,(list,tuple)) or len(e_size)!=2:
                    warnings.warn(f"\n[EasyappError] Argument <es>'s {e1}'s size must be list or tuple "
                                  f"and its len must be 2.",UserWarning)
                    e_size=(0.9,0.1)
                if not isinstance(e_pos,(list,tuple)) or len(e_pos) != 2:
                    warnings.warn(f"\n[EasyappError] Argument <es>'s {e1}'s pos must be list or "
    ...
    if ts:
        ...
        for t in ts:
            if not isinstance(t,list):
                warnings.warn(f"\n[EasyappError] Argument <ts>'s {t} must be list.",UserWarning)
                t=["ERROR",(0.5,0.5)]
                filled_ts.append(t)
            else:
                if not len(t)==2:
                    warnings.warn(f"\n[EasyappError] Argument <ts>'s {t}'s len must be 2.",UserWarning)
                t_t=str(t[0]) if len(t)>0 else "ERROR"
                t_pos=t[1] if len(t)>1 else (0.5,0.5)
                if not isinstance(t_pos,(list,tuple)) or len(t_pos)!=2:
                    warnings.warn(f"\n[EasyappError] Argument <ts>'s {t}'s position's len must be 2 "
    ...
    if fs:
        ...
        for f in fs:
            if not isinstance(f,list):
                warnings.warn(f"\n[EasyappError] Argument <fs>'s {f} must be list.",UserWarning)
                f=["ERROR",GREY13,RED1,(0.15,0.05),(0.5,0.65)]
                filled_fs.append(f)
            else:
                if len(f) != 5:
                    warnings.warn(f"\n[EasyappError] Argument <fs>'s {f}'s len must be 5.",UserWarning)
                ...
                if not isinstance(f_size,tuple) or len(f_size) != 2:
                    warnings.warn(f"\n[EasyappError] Argument <fs>'s {f}'s size's len must be 2 and it's must be tuple.",UserWarning)
                    f_size=(0.15,0.05)
                if not isinstance(f_pos,(list,tuple)) or len(f_pos) != 2:
                    warnings.warn(f"\n[EasyappError] Argument <fs>'s {f}'s position must be list or tuple and its len must be 2.",UserWarning)
                    f_pos=(0.5,0.65)
                if not isinstance(f_color,tuple) or len(f_color) != 4:
                    warnings.warn(f"\n[EasyappError] Argument <fs>'s {f}'s color must be tuple and its len must be 4.",UserWarning)
                    f_color=GREY3
                if not isinstance(f_c_color,tuple) or len(f_c_color) != 4:
                    warnings.warn(f"\n[EasyappError] Argument <fs>'s {f}'s clicked_color must be tuple and its len must be 4.",UserWarning)
                    f_c_color=RED8
        ...
        return []
m_dg={"tie":"Easyapp-example",
    "color":WHITE12,
    "theme":LIGHT,
    "ts":[["Hello,easyapp-lazydonkey!",(0.5,0.5)],
          ["1/1",(0.5,0.1)]]}
m_mode={"Page1":{"tie":"Easyapp-example-1",
                "color":WHITE12,
                "theme":LIGHT,
                "ts":[["Hello,easyapp-lazydonkey!",(0.5,0.5)],
                      ["1/2",(0.5,0.1)]]},
        "Page2":{"tie":"Easyapp-example-2",
                "color":WHITE12,
                "theme":LIGHT,
...
def fill_ps(ps:dict=None, mode:dict=None, empty:dict=None) -> dict:
    ...
    if not isinstance(ps,dict):
        ...
        warnings.warn(f"\n[EasyappError] Argument <ps> must be dict, not {p_e}.",UserWarning)
        ps={}
    if ps:
    ...
    if i:
        for ii in i:
            if not isinstance(i,list):
                warnings.warn(f"\n[EasyappError] Argument <i>'s {ii} must be list.",UserWarning)
                ii=[["images/error.png",(0.6,0.3),(0.5,0.75),0]]
                filled_i.append(ii)
            else:
                if len(ii) != 4:
                    warnings.warn(f"\n[EasyappError] Argument <i>'s {ii}'s len must be 4.",UserWarning)
...
def fill_bi(bi=None):
    ...
    if bi:
        for b in bi:
            if not isinstance(b,list):
                warnings.warn(f"\n[EasyappError] Argument <bi>'s {b} must be list.",UserWarning)
                b=["images/none.png",(0.15,0.05),(0.5,0.05),1]
                filled_bi.append(b)
            else:
                if not len(b) == 4:
                    warnings.warn(f"\n[EasyappError] Argument <bi>'s {b}'s len must be 5.",UserWarning)
                ...
                if not isinstance(size,tuple) or len(size) != 2:
                    warnings.warn(f"\n[EasyappError] Argument <bi>'s {b} [2] (size_hint) must be tuple "
                                  f"and its len must be 2.",UserWarning)
                    size=(0.15,0.05)
                if not isinstance(pos,(list,tuple)) or len(pos) != 2:
                    warnings.warn(f"\n[EasyappError] Argument <bi>'s {b}'s pos [3] must be tuple or list "
                                  f"and its len must be 2.",UserWarning)
                    pos=(0.5,0.05)
                if not isinstance(o,(float,int)) or not 0<=o<=1:
                    warnings.warn(f"\n[EasyappError] Argument <bi>'s {b}'s opacity must be float or int "
...
def fill_fi(fi=None) -> list :
    ...
    if fi:
        for f in fi:
            if not isinstance(f,list):
                warnings.warn(f"\n[EasyappError] Argument <fi>'s {f} must be list.",UserWarning)
                f=["images/easyapp_don.png","images/four_d.png",(0.05,0.05),(0.5,0.5)]
                filled_fi.append(f)
            else:
                if len(f) != 4:
                    warnings.warn(f"\n[EasyappError] Argument <fi>'s {f}'s len must be 4.",UserWarning)
                ...
                if not isinstance(f_size,tuple) or len(f_size) != 2:
                    warnings.warn(f"\n[EasyappError] Argument <fi>'s {f}'s size's len must be 2 "
                                  f"and it must be tuple.",UserWarning)
                    f_size=(0.05,0.05)
                if not isinstance(f_pos,(tuple,list)) or len(f_pos) != 2:
                    warnings.warn(f"\n[EasyappError] Argument <fi>'s {f}'s position's len must be 2 "
                                  f"and it must be tuple or list.",UserWarning)
                    f_pos=(0.5,0.5)
