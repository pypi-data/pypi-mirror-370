...
def sample_expectation_ps(
    ...
    **kws: Any,
    ...
    """
    Deprecated, please use :py:meth:`tyxonq.cloud.wrapper.batch_expectation_ps`.
    """
    logger.warning(
        "This method is deprecated and not maintained, \
        please use `tyxonq.cloud.wrapper.batch_expectation_ps` instead"
    )
...
def batch_expectation_ps(
    ...
    .. code-block:: python
        ...
        tc.cloud.wrapper.batch_expectation_ps(c, [[1, 0], [0, 3]], device="tyxonq::9gmon")
    ...
    if with_rem:
        if getattr(device, "readout_mit", None) is None:
            mit = ReadoutMit(run)
            # TODO(@refraction-ray) only work for tyxonq provider
