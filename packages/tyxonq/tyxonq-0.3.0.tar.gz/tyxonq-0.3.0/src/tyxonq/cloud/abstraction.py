...
class Provider:
    """
    Provider abstraction for cloud connection, eg. "tyxonq", "local"
    """
    ...
    @classmethod
    def from_name(cls, provider: Union[str, "Provider"] = "tyxonq") -> "Provider":
        if provider is None:
            provider = "tyxonq"
...
class Task:
    ...
    def results(
        ...
        if device.provider.name != "tyxonq":
            raise ValueError("Only tyxonq provider supports auto readout mitigation")
