...
def set_tensornetwork_backend(
    ...
    r"""To set the runtime backend of tyxonq.

    Note: ``tq.set_backend`` and ``tq.cons.set_tensornetwork_backend`` are the same.

    :Example:

    >>> tq.set_backend("numpy")
    numpy_backend
    >>> tq.gates.num_to_tensor(0.1)
    ...
    :return: The `tq.backend` object that with all registered universal functions.
...
tq.set_contractor("custom", optimizer=opt, preprocessing=True)
tq.set_contractor("custom_stateful", optimizer=oem.RandomGreedy, max_time=60, max_repeats=128, minimize="size")
tq.set_contractor("plain-experimental", local_steps=3)
...
tq.set_contractor("custom", optimizer=opt_reconf)
"""
