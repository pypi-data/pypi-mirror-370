...
try:
    from . import tyxonq_provider  # type: ignore
except (ImportError, ModuleNotFoundError):
    print("fail to load cloud provider module: tyxonq_provider")
    logger.info("fail to load cloud provider module: tyxonq_provider")
...
default_provider = Provider.from_name("tyxonq")
avail_providers = ["tyxonq", "local"]


def list_providers() -> List[Provider]:
    """
    list all cloud providers that tyxonq supports
...
default_device = Device.from_name("tyxonq::simulator:tc")
...
def set_token(
    ...
    authpath = os.path.join(homedir, ".tyxonq.auth.json")
...
# {"tyxonq::": token1, "tyxonq::20xmon":  token2}
...
def list_devices(
    ...
    **kws: Any,
    ...
    if provider.name == "tyxonq":
        return tyxonq_provider.list_devices(token, **kws)  # type: ignore
...
def list_properties(
    ...
    if provider.name == "tyxonq":  # type: ignore
        return tyxonq_provider.list_properties(device, token)  # type: ignore
    elif provider.name == "local":
...
def get_task_details(
    ...
    if provider.name == "tyxonq":
        return tyxonq_provider.get_task_details(task, device, token, prettify)  # type: ignore
    elif provider.name == "local":
...
def submit_task(
    ...
    **task_kws: Any,
    ...
    if provider.name == "tyxonq":  # type: ignore
        return tyxonq_provider.submit_task(device, token, **task_kws)  # type: ignore
...
def resubmit_task(
    ...
    if provider.name == "tyxonq":  # type: ignore
        return tyxonq_provider.resubmit_task(task, token)  # type: ignore
    elif provider.name == "local":
        raise ValueError("Unsupported method for local backend")
    else:
        raise ValueError("Unsupported provider: %s" % provider.name)  # type: ignore
...
def remove_task(
    ...
    if provider.name == "tyxonq":  # type: ignore
        return tyxonq_provider.remove_task(task, token)  # type: ignore
    elif provider.name == "local":
        raise ValueError("Unsupported method for local backend")
    else:
        raise ValueError("Unsupported provider: %s" % provider.name)  # type: ignore
...
def list_tasks(
    ...
    **filter_kws: Any,
    ...
    if device is not None:
        device = Device.from_name(device)
    if provider.name == "tyxonq":  # type: ignore
        return tyxonq_provider.list_tasks(device, token, **filter_kws)  # type: ignore
    elif provider.name == "local":  # type: ignore
