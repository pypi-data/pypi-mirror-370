...
class Table:
    ...
    def render_to_tensor(self, buffer: MultiprocessSafeTensorBuffer):
        ...
        # 3. Create and position all Text objects
        text_objects = []
        for r, row in enumerate(self.data):
            ...
            for c, cell_text in enumerate(row):
                ...
                # Get font and text width for alignment
                text_obj = Text(str(cell_text), 0,0, font_path=self.font_path, font_size=self.font_size)
                font = text_obj.font
                ...
                # Check for numerical values and right-align them
                if r > 0 and c > 0: # This targets all cells except the first row and column
                    cell_x += self.column_widths[c] - text_width - 4 # 4 pixels for padding
