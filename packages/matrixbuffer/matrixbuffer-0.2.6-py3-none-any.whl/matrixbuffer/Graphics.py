...
class Table:
    ...
    def render_to_tensor(self, buffer: MultiprocessSafeTensorBuffer):
            ...
            for c in range(cols):
                ...
                for r in range(rows):
                    if c < len(self.data[r]):
                        ...
                        if w > max_text_width:
                            max_text_width = w
                expanded_cw.append(max(cw, max_text_width + 8))  # small padding
            cw = max(expanded_cw)
        ...
        for t in text_objects:
            # Compute position relative to table bounds
            rel_x = t.x - x_start
            rel_y = t.y - y_start
            # If wrapping is disabled, render text in one line
            if self.expand_cells:
                draw.text((rel_x, rel_y), t.text, font=t.font, fill=(*t.color,255))
            else:
                # Simple wrapping: split by characters that fit cell width
                words = str(t.text)
                x_cursor = rel_x
                y_cursor = rel_y
                for char in words:
                    char_bbox = t.font.getbbox(char)
                    char_width = char_bbox[2] - char_bbox[0]
                    if x_cursor + char_width > t.x - x_start + cw:
                        x_cursor = rel_x
                        y_cursor += t.font_size
                    draw.text((x_cursor, y_cursor), char, font=t.font, fill=(*t.color,255))
                    x_cursor += char_width
