...
class Table:
    ...
    def render_to_tensor(self, buffer: MultiprocessSafeTensorBuffer):
        ...
        for t in text_objects:
            font = t.font
            cw_inner = self.cell_width - 4  # small padding
            ch_inner = self.cell_height - 4
            lines = []
            current_line = ""
            for char in t.text:
                test_line = current_line + char
                bbox = font.getbbox(test_line)  # Pillow 10+ compatible
                w = bbox[2] - bbox[0]
                if w <= cw_inner:
            ...
            # Draw lines within the cell
            for i, line in enumerate(lines):
                bbox = font.getbbox(line)
                th = bbox[3] - bbox[1]
                y_offset = t.y - y_start + i*th
                if y_offset + th > table_height:
                    break  # avoid overflow
                draw.text((t.x - x_start, y_offset), line, font=font, fill=(*t.color, 255))
