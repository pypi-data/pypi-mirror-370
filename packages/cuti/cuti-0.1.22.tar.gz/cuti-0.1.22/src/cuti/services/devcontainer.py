...
RUN which claude || (echo "ERROR: Claude CLI not installed!" && exit 1) && \
    which claude-code || (echo "ERROR: claude-code symlink not created!" && exit 1)
...
'''
    ...
    def rebuild_container(self) -> bool:
        ...
        # Determine which container images to remove based on current directory
        current_dir = Path.cwd()
        is_cuti_source = (current_dir / "src" / "cuti").exists() and (current_dir / "pyproject.toml").exists()
        
        container_images = []
        if is_cuti_source:
            container_images.append("cuti-dev-cuti")
        else:
            container_images.append("cuti-dev-universal")
        
        # Also remove the other image to ensure clean rebuild
        container_images.extend(["cuti-dev-cuti", "cuti-dev-universal"])
        # Remove duplicates
        container_images = list(set(container_images))
        ...
        for image in container_images:
            ...
            if check_result.stdout.strip():
                print(f"🗑️  Removing existing container image: {image}")
                ...
                if remove_result.returncode != 0:
                    print(f"⚠️  Failed to remove {image}: {remove_result.stderr}")
                else:
                    print(f"✅ Removed {image}")
        
        # Clean up dangling images
        print("🧹 Cleaning up dangling Docker images...")
        subprocess.run(
        ...
        print("✅ Container images cleared. They will be rebuilt on next run.")
        return True
