...
def add_control_columns(
    ...
    try:
        ...
        result_df.select("_metadata.file_path").limit(1).collect()
        logger.info(f"Using _metadata.file_path for column '{column_name_file}'")

    except AnalysisException:
        result_df = result_df.withColumn(column_name_file, F.input_file_name())
        logger.info(f"Using input_file_name() for column '{column_name_file}'")
