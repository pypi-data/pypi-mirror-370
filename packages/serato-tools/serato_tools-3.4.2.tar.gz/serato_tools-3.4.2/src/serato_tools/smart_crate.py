...
def main():
    ...
    def set_rule(rule: SmartCrate.Rule):
        for key, value in set_rules.items():
            key = key.upper()
            rule_key = get_enum_key(SmartCrate.RuleField, key)
            if rule_key is None:
                raise KeyError(f"Unknown RuleField: {key} (must be one of {[rf.name for rf in SmartCrate.RuleField]})")
            if rule.field == rule_key:
