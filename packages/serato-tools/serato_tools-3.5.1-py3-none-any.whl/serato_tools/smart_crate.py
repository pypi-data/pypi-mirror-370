...
class SmartCrate(CrateBase):
    ...
    def set_rule(self, field: "SmartCrate.RuleField", comparison: "SmartCrate.RuleComparison", value: str | int):
        ...
        for i, (f, v) in enumerate(self.data):
        ...
        if not rule_exists:
            ...
            self.data.append((CrateBase.Fields.SMARTCRATE_RULE.value, new_rule.to_struct()))
    ...
    for p in paths:
        ...
        if args.set_rules:
            for key, value in set_rules.items():
                ...
                field = SmartCrate.RuleField(key.upper())
                comparison_key = str(value[0]).upper()
                try:
                    comparison = SmartCrate.RuleComparison[comparison_key]
                except KeyError as exc:
                    raise KeyError(
                        f"bad comparison {comparison_key} , must be one of {[c.name for c in SmartCrate.RuleComparison]}"
                    ) from exc
