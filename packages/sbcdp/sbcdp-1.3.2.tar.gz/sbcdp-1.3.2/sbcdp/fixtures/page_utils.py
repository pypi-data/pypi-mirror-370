...
def is_partial_link_text_selector(selector):
    ...
    return selector.startswith(
        ("partial_link=", "partial_link_text=", "partial_text=", "p_link=", "p_link_text=", "p_text="))
...
def recalculate_selector(selector, by, xp_ok=True):
    ...
    if not is_valid_by(by):
        valid_by_options = ["css selector", "link text", "partial link text", "name", "xpath", "id", "tag name",
            "class name", ]
...
def looks_like_a_page_url(url):
    ...
    return url.startswith((
        "http:", "https:", "://", "about:", "blob:", "chrome:", "data:", "edge:", "file:", "view-source:",
        "chrome-search:", "chrome-extension:", "chrome-untrusted:", "isolated-app:", "chrome-devtools:", "devtools:"))
...
def is_valid_url(url):
    regex = re.compile(r"^(?:http)s?://"  # http:// or https://
                       ...
                       r"(?::\d+)?"  # optional port
                       r"(?:/?|[/?]\S+)$", re.IGNORECASE, )
    if regex.match(url) or url.startswith(("about:", "blob:", "chrome:", "data:", "edge:", "file:")):
        return True
    else:
        return False
...
def _get_link_status_code(link, allow_redirects=False, timeout=5, verify=False, ):
    ...
    status_code = None
    try:
        response = requests.head(link, allow_redirects=allow_redirects, timeout=timeout, verify=verify, )
