...
def _cpu() -> dict[str, Callable[..., CvImage]]:
    ...
    return {
        # color
        "convert_bgr_to_rgb": lambda img: cv2.cvtColor(img, cv2.COLOR_BGR2RGB),
        "convert_rgb_to_bgr": lambda img: cv2.cvtColor(img, cv2.COLOR_RGB2BGR),
        "convert_bgr_to_hsv": lambda img: cv2.cvtColor(img, cv2.COLOR_BGR2HSV),
...
def _gpu() -> dict[str, Callable[..., CvImage]]:
    ...
    return {
        "convert_bgr_to_rgb":  lambda img: (lambda g=cv2.cuda.cvtColor(_upload(img), cv2.COLOR_BGR2RGB): g.download())(),
        "convert_rgb_to_bgr":  lambda img: (lambda g=cv2.cuda.cvtColor(_upload(img), cv2.COLOR_RGB2BGR): g.download())(),
        "convert_bgr_to_hsv":  lambda img: (lambda g=cv2.cuda.cvtColor(_upload(img), cv2.COLOR_BGR2HSV): g.download())(),
