...
__all__ = ["convert_bgr_to_rgb", "convert_rgb_to_bgr", "convert_bgr_to_hsv", "in_range"]
...
def convert_bgr_to_rgb(image: CvImage, *,
    ...
    return dispatch("convert_bgr_to_rgb", dev, image)

def convert_bgr_to_hsv(image: CvImage, *,
                       device: str | ComputeDevice | None = None,
                       default: ComputeDevice = ComputeDevice.CPU) -> CvImage:
    """ Конвертирует изображение BGR -> HSV. """
    dev = _normalize(device, default)
    return dispatch("convert_bgr_to_hsv", dev, image)

def convert_rgb_to_bgr(image: CvImage, *,
                       device: str | ComputeDevice | None = None,
                       default: ComputeDevice = ComputeDevice.CPU) -> CvImage:
    """ Конвертирует изображение BGR -> HSV. """
    dev = _normalize(device, default)
    return dispatch("convert_rgb_to_bgr", dev, image)
