...
class MatriceDeployServer:
    ...
    def __init__(
        ...
        try:
            ...
            self._stream_manager_task = None
    ...
    def _start_stream_manager_if_enabled(self):
        ...
        try:
            self._stream_manager_task = asyncio.create_task(
                run_stream_manager_until_shutdown(), name="StreamManager"
            )
            logging.info("Stream manager started successfully")
        except Exception as exc:
    ...
    def stop_server(self):
        ...
        try:
            ...
            if self._stream_manager_task:
                logging.info("Cancelling stream manager task...")
                try:
                    self._stream_manager_task.cancel()
                except asyncio.CancelledError:
                    logging.info("Stream manager task cancelled")
                except Exception as exc:
                    logging.error("Error cancelling stream manager task: %s", str(exc))
