...
class LicensePlateMonitorUseCase(BaseProcessor):
    ...
    def _update_plate_texts(self, detections: List[Dict]):
        """Update set of seen plate texts and track the longest plate_text per unique plate."""
        for det in detections:
            text = det.get('plate_text')
            track_id = det.get('track_id')
            if text and track_id is not None and text.strip():  # Ensure text is not empty
                # Update _tracked_plate_texts with the longest text for this track_id
                current_track_text = self._tracked_plate_texts.get(track_id, '')
                if len(text) > len(current_track_text):
                    self._tracked_plate_texts[track_id] = text
                    self.logger.debug(f"Updated track_id={track_id} with new longest plate_text={text}")
                # Normalize the plate text for comparison
                normalized_text = self._normalize_plate_text(text)
                if not normalized_text:  # Skip if normalized text is empty/invalid
                    self.logger.debug(f"Skipping invalid normalized plate_text={text} for track_id={track_id}")
                    continue
                # Update _unique_plate_texts with the longest original text for this normalized plate
                current_unique_text = self._unique_plate_texts.get(normalized_text, '')
                if len(text) > len(current_unique_text):
                    self._unique_plate_texts[normalized_text] = text
                    self.logger.debug(f"Updated normalized_plate={normalized_text} with longest text={text}")
