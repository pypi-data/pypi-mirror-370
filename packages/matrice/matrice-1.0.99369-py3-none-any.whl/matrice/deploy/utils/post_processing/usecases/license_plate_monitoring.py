...
class LicensePlateMonitorUseCase(BaseProcessor):
    ...
    def _generate_tracking_stats(self, counting_summary: Dict, alerts: Any, config: LicensePlateMonitorConfig,
        ...
        # Use _unique_plate_texts to list only the longest text per unique plate
        unique_plate_texts = self._unique_plate_texts.values()
        if unique_plate_texts:
            human_text_lines.append("\t- Unique License Plates:")
            for text in sorted(unique_plate_texts):
    ...
    def _update_plate_texts(self, detections: List[Dict]):
        ...
        self._unique_plate_texts: Dict[str, str] = getattr(self, '_unique_plate_texts', {})
        for det in detections:
            ...
            if text and track_id is not None:
                ...
                current_text = self._unique_plate_texts.get(normalized_text, '')
                if len(text) > len(current_text):
                    self._tracked_plate_texts[normalized_text] = text
                    self.logger.debug(f"Updated track_id={normalized_text} with new longest plate_text={text}")
    ...
    def _normalize_plate_text(self, text: str) -> str:
        ...
        if not text:
            return ""
        # Add more normalization rules as needed (e.g., handle common OCR errors)
