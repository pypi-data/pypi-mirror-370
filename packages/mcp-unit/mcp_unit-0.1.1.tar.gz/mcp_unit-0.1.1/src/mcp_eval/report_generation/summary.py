...
def generate_combined_summary(
    ...
    for result in test_results:
        ...
        if verbose:
            ...
            used_servers_str = ", ".join(sorted(used_servers)) if used_servers else "none"
    ...
    for report in dataset_reports:
        ...
        status = f"[green]{passed}/{total}[/]" if passed == total else f"[yellow]{passed}/{total}[/]"
        ...
        if verbose:
            ...
            agent_name = report.agent_name if hasattr(report, 'agent_name') and report.agent_name else "unknown"
            ...
            configured_str = ", ".join(sorted(configured_servers)) if configured_servers else "none"
...
def _print_configuration_details(test_results: List[TestResult], dataset_reports: List[EvaluationReport], console: Console) -> None:
    ...
    try:
        ...
        if hasattr(settings, 'judge') and settings.judge:
            ...
            if judge_provider:
                ...
                console.print(f"  Provider: [dim]Auto-detected from model[/dim]")
                
            if judge_model:
                console.print(f"  Model: {judge_model}")
            else:
                console.print(f"  Model: [dim]Using model selector[/dim]")
        else:
...
def _print_test_breakdown(test_results: List[TestResult], dataset_reports: List[EvaluationReport], console: Console) -> None:
    ...
    if agents_used:
        ...
        for agent_name, tests in agents_used.items():
            ...
            for test in tests:
                ...
                console.print(f"    [{status_color}]{status}[/{status_color}] {test.test_name}")
                
                # Show servers used
                if test.servers:
                    console.print(f"      Servers: {', '.join(test.servers)}", style="dim")
                ...
                if test.metrics and isinstance(test.metrics, dict):
                    if 'tool_calls' in test.metrics:
                        ...
                        if tool_names:
                            console.print(f"      Tools: {', '.join(sorted(tool_names))}", style="dim")
                
                # Show evaluators used (especially for failures)
                if not test.passed and test.evaluation_results:
                    failed_evals = [e.name for e in test.evaluation_results if not e.passed]
                    if failed_evals:
                        console.print(f"      Failed: {', '.join(failed_evals)}", style="red dim")
            ...
            if failed_cases:
                ...
                for case in failed_cases[:5]:  # Show first 5 failures
                    ...
                    if case.evaluation_results:
                        failed_evals = [e.name for e in case.evaluation_results if not e.passed]
                        if failed_evals:
                            console.print(f"        Failed: {', '.join(failed_evals)}", style="red dim")
                if len(failed_cases) > 5:
                    console.print(f"      ... and {len(failed_cases) - 5} more", style="dim")


def _print_aggregate_info(test_results: List[TestResult], dataset_reports: List[EvaluationReport], console: Console) -> None:
    ...
    console.print(f"  Total Duration: {total_duration:.1f}ms ({total_duration/1000:.2f}s)")
    console.print(f"  Total Tokens: {total_tokens:,}")
    # Only show cost if there is actual cost data
    if total_cost > 0:
        console.print(f"  Estimated Cost: ${total_cost:.4f}")
    else:
        console.print(f"  Estimated Cost: $0.0000")


def _print_tool_coverage(test_results: List[TestResult], dataset_reports: List[EvaluationReport], console: Console) -> None:
    ...
    server_coverage: Dict[str, Dict[str, set]] = {}  # server -> {available: set, used: set}
        ...
        if result.metrics and isinstance(result.metrics, dict):
            ...
            for server_name, coverage_data in tool_coverage.items():
                ...
                if isinstance(coverage_data, dict):
                    server_coverage[server_name]['available'].update(coverage_data.get('available_tools', []))
                    server_coverage[server_name]['used'].update(coverage_data.get('used_tools', []))
        ...
        for case in report.results:
            if case.metrics and hasattr(case.metrics, 'tool_coverage'):
                for server_name, coverage in case.metrics.tool_coverage.items():
                    if server_name not in server_coverage:
                        server_coverage[server_name] = {'available': set(), 'used': set()}
                    server_coverage[server_name]['available'].update(coverage.available_tools)
