...
def VacancyAnalysis():
    ...
    if cfg.pipeline.use_external_reference == False:
        config_dict = {
            "paths": {
        ...
        out_directory = Path(cfg.paths.outputs_root)
        
        out_directory = Path(cfg.paths.outputs_root)
        
        generator = CrystalStructureGenerator(config_dict, out_directory)
        dump_path = generator.generate()
