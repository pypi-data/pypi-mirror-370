...
class CrystalStructureGenerator:
    ...
    def __init__(self, config: Union[dict, "V1Config"], out_dir: Path, auto_fit_to_box: bool = False):
        ...
        # Leer encabezado original y box numérica
    ...
    def _read_header_and_box(self) -> None:
        ...
        self.header_boxbounds_line: str = text[b_idx]                # copiar tal cual
        self.header_boxbounds_vals: List[str] = text[b_idx + 1 : b_idx + 4]  # copiar tal cual (3 líneas)

        # Parseo numérico para centro/longitudes (soporta ortogonal y triclinic con tilts)
    ...
    def _build_replica(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # Periódico perfecto dentro de la celda réplica
    ...
    def _write_dump(self, coords: np.ndarray, out_file: Path) -> None:
        ...
        with Path(out_file).open("w", encoding="utf-8") as f:
            ...
            # 2) NUMBER OF ATOMS (recalculado)
            f.write("ITEM: NUMBER OF ATOMS\n")
            f.write(f"{len(coords)}\n")

            # 3) BOX BOUNDS (literal: línea + 3 renglones)
            f.write(self.header_boxbounds_line + "\n")
            for line in self.header_boxbounds_vals:
                f.write(line + "\n")

            # 4) ATOMS (definimos columnas explícitas)
    ...
    def generate(self) -> Path:
        ...
        # centrar réplica en el centro de la caja original
        coords_centered = coords - dims / 2.0
        coords_aligned = coords_centered + self.box_center

        # asegurar que todo cae en la caja copiada
