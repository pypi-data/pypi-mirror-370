...
def _get_local_tzinfo(explicit: str | None) -> timezone:
    """
    tz 선택 우선순위:
      1) explicit == 'utc' → UTC
    ...
    return _DEF_LOCAL_TZ  # type: ignore[return-value]
...
def _to_aware_utc(dt: datetime, local_tz: timezone) -> datetime:
    if dt.tzinfo is None:
        dt = dt.replace(tzinfo=local_tz)
    return dt.astimezone(UTC)


def _parse_event_ts_to_utc(ts: Any, local_tz: timezone) -> datetime | None:
    """
...
def _expand_day_to_utc_window(day: str, local_tz: timezone) -> tuple[datetime, datetime]:
...
class Class1:
    ...
    def filter_events(
        ...
        *,
        ...
        tz: str | None = None,  # 'local' | 'utc' | '+09:00' 등 (Optional)
        **_ignore_kwargs,  # 미지의 인자 무시(호환성)
    ) -> list[Event]:
        """
        반환: 선택/정렬/페이징 적용된 '새로운' 이벤트 사본들(원본 변형 없음)
        """
        # timezone 선택
