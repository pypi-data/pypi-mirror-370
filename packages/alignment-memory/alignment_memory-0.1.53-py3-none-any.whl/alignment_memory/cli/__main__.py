...
from alignment_memory.license import check_license  # 라이선스 체크
...
def cmd_stats(args) -> int:
    root = Path(args.data_dir)
    sid = _norm_sid(args.sid) if "sid" in args and args.sid else None
    if not sid:
        print("ERROR: --sid/--id is required", file=sys.stderr)
        return 2
