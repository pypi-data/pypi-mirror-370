...
def run_pos_tagger(column_name, train=True):
    ...
    # File paths (assuming data files are in the same directory)
    pos_data_path = "PritomBanglaversion 2.2 (1).xlsx"
    training_data_path = "Regional_sentence.csv"
    
    # Check if data files exist
    if not os.path.exists(pos_data_path) or not os.path.exists(training_data_path):
        print("❌ Error: Data files not found. Please ensure 'PritomBanglaversion 2.2 (1).xlsx' and 'Mymensingh Train Translation.csv' are in the current working directory.")
        return None, None, None

    # File names based on column
    ...
    # Load POS dictionary
    pos_df = pd.read_excel(pos_data_path)
    ...
    if train:
        ...
        sent_df = pd.read_csv(training_data_path)
