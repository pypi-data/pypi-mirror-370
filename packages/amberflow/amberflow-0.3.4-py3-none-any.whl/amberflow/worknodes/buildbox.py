...
class BuildAFEBoxes(BaseBatchWorkNode, TleapMixin):
    ...
    Known issues:
        - IT ONLY WORKS WITH IDENTICAL TARGETS SET TU TRUE, IF MORE THAN 1 TARGET, THEN MDANALYSIS ALIG
          BuildAFEBoxes
        -
...
"""
        ...
        **kwargs,
        ...
        for sysname in self.input_artifacts.keys():
            ...
            self.rejoin_with_box(
                self.complexes[sysname], boxed_complexes, self.solvent_selection, biomolecule, selection=biomolecule, out_filepath=complex_out_filepath
            )

            binder_out_filepath = self.out_dirs[sysname] / f"binder_{sysname}.pdb"
            self.rejoin_with_box(
                self.binders[sysname], boxed_binders, self.solvent_selection, biomolecule, selection=f"resnum {self.binders_resnum[sysname]}", out_filepath=binder_out_filepath
            )
    ...
    def rejoin_with_box(
        self, sys_atoms: AtomGroup, box_filepath: Path, box_selection: str, biomolecule: str, *, selection: str, out_filepath: Path
    ):
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            ...
            # TODO: fix for different targets and radically different ligands
