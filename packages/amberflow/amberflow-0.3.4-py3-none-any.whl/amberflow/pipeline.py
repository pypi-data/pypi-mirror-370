...
class Pipeline:
    ...
    def _check_force_start(self, _, value):
        """
        Validate that `force_restart` and `force_start` are not both set to True.
        ...
        ------
        ValueError
            If both `force_restart` and `force_start` are True.
        """
    ...
    def append_flow(
        ...
        """
        common_nodes_ids: set[str] = set(n.id for n in flow.dag.nodes) & set(n.id for n in self.flow.nodes)
        if len(common_nodes_ids) != 0:
            err_msg = (
                f"Some WorkNodes from flow '{flow.name}' already exist in the pipeline: {common_nodes_ids}. "
    ...
    @staticmethod
    def _write_checkpoint(dag: nx.DiGraph, checkpoint_path: filepath_t) -> None:
        """
        Write the current pipeline state to a checkpoint file.

        Parameters
        ----------
        dag : nx.DiGraph
            The graph representing the current state of the pipeline.
        checkpoint_path : filepath_t
