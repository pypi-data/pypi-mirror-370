...
class FlowBuild(BaseFlow):
    ...
    def __init__(
        ...
        wnf3_complex = WorkNodeFilter(
            wnid="wnf3_complex", artifact_types=(BaseComplexStructureFile,), skippable=skippable
        )
        gtc = GenerateTopology(wnid="gtc", solvent=solvent, ions=ions, box_or_oct=box_or_oct, skippable=skippable)
        wnf3_binder = WorkNodeFilter(wnid="wnf3_binder", artifact_types=(BaseBinderStructureFile,), skippable=skippable)
        ...
        ffopt_binder: Optional[FFOPT] = None
        if ffopt:
            ffopt_binder = FFOPT(wnid="ffopt", skippable=skippable)
        ...
        # `hmr_complex` and `ffopt_binder` (or `hmr_binder`) have the complex and binder topologies, respectively.
