...
class JBSMutationClient(JBSBaseClient):
    ...
    async def getCommonResultJsonData_POST(self, path, data,header:Optional[Literal['token','login']]='login'):
        if header == 'token':
            header = self.getTokenHeader()
        else:
            header = await self.getGoodHeader()
        async with httpx.AsyncClient(verify=False, timeout=timeout, limits=limits) as client:
            res = await client.post(path, json=data, headers=header)
            try:
                if res.status_code == 200:
    ...
    async def getCommonResultJsonData_GET(self, path,queryParams=None,header:Optional[Literal['token','login']]='token'):
        async with httpx.AsyncClient(verify=False, timeout=timeout, limits=limits) as client:
            if header == 'token':
                header = self.getTokenHeader()
            else:
            ...
            if queryParams is None:
                res = await client.get(path, headers=header)
            else:
                res = await client.get(path, params=queryParams, headers=header)
            try:
                if res.status_code == 200:
                    return res.json()['data']
            except Exception as e:
    ...
    async def getHRBaseContract(self, id):
        res = await self.getCommonResultJsonData_GET(f'{self.BASEURL}/contract/hrBaseContract/{id}')
        return res

    async def getHRBaseContractList(self,hrId):
        queryparams = {
            "hrId":hrId,
            "pageNum":1,
            "pageSize":100
        ...
        async with httpx.AsyncClient(verify=False, timeout=timeout, limits=limits) as client:
            header =  self.getTokenHeader()
            res = await client.get(f'{self.BASEURL}/contract/hrBaseContract/list', params=queryparams,headers=header)
            try:
                if res.status_code == 200:
    ...
    async def updateHrBaseContract(self, jsonData):
        res = await self.getCommonResultJsonData_POST(f'{self.BASEURL}/contract/hrBaseContract/save', jsonData,header='token')
        return res
