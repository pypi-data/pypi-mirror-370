    ...
    if device == 'cuda' and torch.cuda.device_count() > 1:
        ...
        class CustomDataParallel(nn.DataParallel):
            ...
            def scatter(self, inputs, kwargs, device_ids):
                ...
                weight, pt, p0 = inputs
                scattered_weight = torch.nn.parallel.scatter(weight, device_ids, dim=0)
                # 对 pt 和 p0 不进行分割，而是复制到所有设备
                scattered_pt = [pt.to(device) for device in device_ids]
                scattered_p0 = [p0.to(device) for device in device_ids]
                # 重新组合输入
                scattered_inputs = [
                    (sw, sp, sc) for sw, sp, sc in zip(scattered_weight, scattered_pt, scattered_p0)
                ]
                return scattered_inputs, [kwargs] * len(device_ids)
