...
class RandomWalkModel(nn.Module):
    ...
    def forward(self, weight, pt, p0):
        ...
        while delta > self.epsilon:
            p1 = factor * torch.matmul(weight, pt.T) + self.gamma * p0.T
            delta = torch.linalg.norm(pt.T - p1, ord=self.p).item()
            pt = p1.T
            k += 1
...
def random_walk_parallel(seed_cell_vector, weight, gamma: float = 0.05, epsilon: float = 1e-5, p: int = 2, device='auto'):
    ...
    p0 = torch.as_tensor(seed_cell_vector, device=device, dtype=torch.float32)
    pt = p0.clone()
