...
class RandomWalkModel(nn.Module):
    ...
    def __init__(self, weight: Union[matrix_data, Tensor], gamma: float = 0.05, epsilon: float = 1e-5, p: int = 2, device: str = 'auto', pbar=None):
        ...
        self.weight = torch.as_tensor(to_dense(weight, is_array=True), device=self.device, dtype=torch.float32) if isinstance(weight, matrix_data) else weight.to(self.device)
