...
class RandomWalk:
    ...
    def _random_walk_(self, seed_cell_vector: collection, weight: matrix_data = None, gamma: float = 0.05, device: str = 'auto') -> matrix_data:
        ...
        if device == 'auto':
            if self.is_gpu_available:
                try:
                    ...
                    ul.log(__name__).error("No module named 'cupy'. For example: pip install cupy-cuda11x.")
                    raise ImportError("No module named 'cupy'. For example: pip install cupy-cuda11x.")
        ...
        return pt.get().flatten() if self.is_gpu_available else pt.flatten()
