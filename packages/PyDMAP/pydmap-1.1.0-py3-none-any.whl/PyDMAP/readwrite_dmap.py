...
dsizes = {1: 1, 2: 2, 3: 4, 4: 4, 8: 8, 9: -1, 10: 8, 16: 1, 17: 2, 18: 4, 19: 8, 255:-1}
dformats = {
    1: "c",
    2: "h",
    3: "i",
...
class DataMapReader:
    ...
    def read_file(self, byblock=False):
        """
        ...
        while totptr <= file_size:
            ...
            self._decode(block, my_dmap, retypes)
            if byblock:
                blockList.append(copy.deepcopy(my_dmap))
                my_dmap = {}
        ...
        for name in my_dmap:
            if isinstance(my_dmap[name], list):
                ...
                if name in retypes:
                    for key in dtypes:
                        if dtypes[key] == retypes[name]:
                            type = key
                if type is None:
                    my_dmap[name] = np.asarray(my_dmap[name])
                else:
                    if re.search(r'int\d*', type):
                        type = eval('np.'+type)
                    elif type == 'float':
                        type = np.float32
                    elif type == 'double':
                    ...
                    if type is None:
                        ...
                        my_dmap[name] = np.asarray(my_dmap[name], dtype=type)
    ...
    def read_block_fromhandle(self):
        ...
        for name in my_dmap:
            if isinstance(my_dmap[name], list):
                if not isinstance(my_dmap[name][0], dict):
                    type = None
                    if name in retypes:
                        for key in dtypes:
                            if dtypes[key] == retypes[name]:
    ...
    def _decode(self, block, my_dmap, retypes):
        """
        ...
        for n in range(num_scalars):
            ...
            if name not in retypes:
                retypes[name] = blocktype
            if blocktype == 255:
                tDict = {}
                tmpsz = int.from_bytes(block[ptr:ptr + 4], self.endian)
                ptr += 4

                byteinfo = {
                    "endian": self.endian,
                    "endiansymbol": self.endiansymbol,
                    "compressed": self.compressed,
                }

                if tmpsz != 0:
                    nitems = int.from_bytes(block[ptr:ptr + 4], self.endian)
                    ptr += 4
                    for ii in range(nitems):
                        szblk = int.from_bytes(block[ptr:ptr + 4], self.endian)
                        ptr += 4
                        if ptr > len(block):
                            print(f"WARNING - Reached end of block with lower limit")
                            break
                        if (ptr + szblk) > len(block):
                            print(f"WARNING - Reached end of block with upper limit")
                            break
                        newblock = block[ptr:ptr + szblk]
                        ptr += szblk
                        procStruct(newblock, tDict, byteinfo, 0, 0)

                    items = list(tDict.keys())
                    for it in items:
                        if isinstance(tDict[it], list):
                            tDict[it] = np.asarray(tDict[it])

                    if name in my_dmap:
                        my_dmap[name].append(tDict)
                    else:
                        my_dmap[name] = [tDict]
                    continue
...
class DataMapWriter:
    ...
    def set_filehand(self):
        ...
        if self.outfile is not None:
            ...
            if self.verbose:
                print(f"WARNING - Output file for writing is not yet specified")

    def write2file(self, indict, returnCode=True):
        """
        ...
        if self.compress:
            ...
            block = (
                self.datacode + struct.pack(self.endiansymb + "i", sizeblock + 8) + block
            )
...
def decode(block, my_dmap, byteinfo, iblk, verb):
    ...
    for n in range(num_scalars):
        ...
        if blocktype == 255:
            tDict = {}
            tmpsz = int.from_bytes(block[ptr:ptr+4], endian)
            ptr += 4

            if tmpsz != 0:
                nitems = int.from_bytes(block[ptr:ptr+4], endian)
                ptr += 4
                for ii in range(nitems):
                    szblk = int.from_bytes(block[ptr:ptr+4], endian)
                    ptr += 4
                    if ptr > len(block):
                        print(f"WARNING - Reached end of block with lower limit")
                        break
                    ...
                    newblock = block[ptr:ptr+szblk]
                    ptr += szblk
                    procStruct(newblock, tDict, byteinfo, iblk, 0)
...
def procStruct(block, my_dmap, byteinfo, iblk, verb):
    """
    This function does the actual parsing of the scalars and arrays contained within the passed block.
    The function is either called when run in single mode, in which case my_dmap accumulates all parsed
    data, or in multi-processing mode, in which case each thread will initialize the dictionary that will
    contain the parsed data for the single block, populate it with data, and return it.
    ...
    for n in range(num_arrays):
        ...
        if blocktype == 255:
            tDict = {}
            newblock = block[ptr:]
            tDict = decode(newblock, tDict, byteinfo, iblk, verb)
...
def read_datamap(fname, num_threads=0, verb=0):  # , debug=True):
    ...
    for name in my_dmap:
        if isinstance(my_dmap[name], list):
            if not isinstance(my_dmap[name][0], (np.ndarray, dict)):
                if verb:
