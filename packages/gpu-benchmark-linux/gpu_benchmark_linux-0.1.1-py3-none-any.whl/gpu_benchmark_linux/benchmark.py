        ...
        if not self.gpu_burn_dir.joinpath("gpu_burn").exists():
            ...
            if clone_repository(GPU_BURN_REPO, self.gpu_burn_dir):
                ...
                try:
                    ...
                    env = os.environ.copy()
                    env["NVCC_EXTRA_FLAGS"] = "-allow-unsupported-compiler"
                    run_command("make", env=env)
                except Exception as e:
                    ...
                    try:
                        ...
                        run_command("/usr/local/cuda/bin/nvcc -I/usr/local/cuda/include -arch=compute_50 -allow-unsupported-compiler -ptx compare.cu -o compare.ptx", check=False)
