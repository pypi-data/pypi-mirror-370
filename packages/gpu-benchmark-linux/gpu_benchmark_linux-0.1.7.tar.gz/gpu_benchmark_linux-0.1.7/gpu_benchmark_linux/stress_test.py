...
from .cuda_ops import cuda_ops, CUDAOperations
from .monitor import gpu_monitor, GPUMonitor, GPUMetrics
...
class StressTestConfig:
    ...
    device_ids: List[int] = None  # 要测试的设备ID列表，None表示所有设备
    matrix_size: int = 4096  # 矩阵大小
    memory_usage_ratio: float = 0.8  # 内存使用比例
    test_types: List[str] = None  # 测试类型列表
...
class GPUStressTester:
    ...
    def __init__(self, cuda_ops: CUDAOperations = None, monitor: GPUMonitor = None):
        """初始化压力测试器"""
        self.cuda_ops = cuda_ops or cuda_ops
        self.monitor = monitor or gpu_monitor
        self.running = False
        self.test_threads = {}
        self.results = {}
    ...
    def run_stress_test(self, config: StressTestConfig) -> StressTestResult:
        """运行压力测试"""
        if not self.cuda_ops.is_available():
