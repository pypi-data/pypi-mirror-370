...
class StressTestResult:
    ...
    success: bool
    error_message: str = ""
    device_results: Optional[Dict[int, Dict[str, Any]]] = None
    monitoring_data: Optional[List[GPUMetrics]] = None
    ...
    def run_stress_test(self, config: StressTestConfig) -> StressTestResult:
        ...
        try:
            ...
            try:
                ...
                if charts:
                    logger.info("已生成可视化图表:")
                    for chart_type, chart_path in charts.items():
                        logger.info(f"  - {chart_type}: {chart_path}")
                else:
                    logger.warning("未能生成任何图表")
