    ...
    def install_gpu_burn(self):
        ...
        if not self.gpu_burn_dir.joinpath("gpu_burn").exists():
            ...
            if clone_repository(GPU_BURN_REPO, self.gpu_burn_dir):
                ...
                # 尝试编译 - 根据官方文档使用正确的编译参数
                try:
                    # 方法1：使用官方推荐的NVCCFLAGS参数
                    logger.info("尝试使用NVCCFLAGS参数编译...")
                    run_command("make NVCCFLAGS=-allow-unsupported-compiler", shell=True, check=False)
                    
                    if not os.path.exists("gpu_burn"):
                        # 方法2：使用COMPUTE参数指定计算能力
                        logger.info("尝试使用COMPUTE参数编译...")
                        run_command("make clean", shell=True, check=False)
                        run_command("make COMPUTE=7.5 NVCCFLAGS=-allow-unsupported-compiler", shell=True, check=False)
                    
                    if not os.path.exists("gpu_burn"):
                        # 方法3：使用CUDAPATH参数
                        logger.info("尝试使用CUDAPATH参数编译...")
                        run_command("make clean", shell=True, check=False)
                        run_command("make CUDAPATH=/usr/local/cuda NVCCFLAGS=-allow-unsupported-compiler", shell=True, check=False)
                    
                    if os.path.exists("gpu_burn"):
                        logger.info("gpu_burn编译成功")
                    else:
                        raise Exception("所有编译方法都失败")
                    ...
                    try:
                        ...
                        # 第二步：使用nvcc编译CUDA文件，使用Tesla T4的计算能力7.5
                        run_command("/usr/local/cuda/bin/nvcc -I/usr/local/cuda/include -arch=compute_75 -code=sm_75 -allow-unsupported-compiler -ptx compare.cu -o compare.ptx", check=False)
                        
                        # 验证compare.ptx是否生成成功
                        if not os.path.exists("compare.ptx"):
                            logger.warning("compare.ptx文件未生成，尝试使用compute_50")
                            run_command("/usr/local/cuda/bin/nvcc -I/usr/local/cuda/include -arch=compute_50 -code=sm_50 -allow-unsupported-compiler -ptx compare.cu -o compare.ptx", check=False)
                        
                        # 第三步：链接生成最终可执行文件
                        run_command("g++ -O3 -Wno-unused-result -o gpu_burn gpu_burn-drv.o -L/usr/local/cuda/lib64 -lcuda -lcudart -lcublas", check=False)
                        
                        if os.path.exists("gpu_burn") and os.path.exists("compare.ptx"):
                            logger.info("gpu_burn编译成功")
                        else:
