...
def _page(html: str, q: str = "", summary: str = "") -> HTMLResponse:
    ...
    <div x-data="{ open: false, features: [] }" 
        ...
        <div class="bg-slate-900 rounded-lg border border-slate-700 p-6 w-full max-w-lg mx-4">
            ...
            <form x-data="{ id: '', feature: '', kind: 'REQUIREMENT', statement: '', parent: '', loading: false }" @submit.prevent="
                loading = true;
                fetch('/api/specs', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    ...
                    if (data.error) {
                        ...
                        open = false;
                        id = ''; feature = ''; kind = 'REQUIREMENT'; statement = ''; parent = '';
                        window.location.reload();
                    }
                }).catch(e => {
                    loading = false;
            ...
            ">
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea x-model="statement" placeholder="When the user submits valid credentials, the system shall establish a session" class="w-full px-3 py-2 bg-slate-800 border border-slate-600 rounded text-sm" rows="3" required></textarea>
                    </div>
