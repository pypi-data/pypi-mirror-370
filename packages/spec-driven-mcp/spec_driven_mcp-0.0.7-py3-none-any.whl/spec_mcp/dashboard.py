...
<script>
    function SpecOverview(){
        ...
        function FeatureDetail(featureKey){
            return {
                featureKey, requirements: [], editing:false, current:null, form:{ description:'', override_status:'' },
                init(){ this.reload(); },
                ...
                statusClass(st){ return {FAILING:'bg-rose-700/40 text-rose-300', VERIFIED:'bg-emerald-700/40 text-emerald-300', PARTIAL:'bg-amber-700/40 text-amber-300', UNTESTED:'bg-slate-700/40 text-slate-300'}[st] || 'bg-slate-700/40 text-slate-300'; },
                openEdit(item){ this.current=item; this.form.description=item.description||''; this.form.override_status=item.override_status||''; this.editing=true; },
                ...
                submitEdit(){ if(!this.current) return; const payload={ description:this.form.description, override_status: this.form.override_status || null, clear_override: this.form.override_status==='' };
    ...
    <div x-data='SpecOverview()' x-init='init()' class='space-y-10'>
        ...
        <section>
            <div class='rounded-lg border border-slate-800 bg-slate-900/60 p-4'>
                <h3 class='font-medium mb-3 text-slate-200'>System Information</h3>
                ...
                <template x-for='req in requirements' :key='req.id'>
                    <div class='mb-6 last:mb-0'>
                        <div class='flex items-center justify-between gap-3 mb-3'>
                            <div class='flex items-center gap-2 flex-1 cursor-pointer' @click='openEdit(req)'>
                                <code class='text-sky-400 text-sm' x-text='req.id'></code>
                                <h3 class='font-semibold leading-tight hover:text-slate-300' x-text='req.description'></h3>
                            </div>
                            ...
                            <template x-for='c in req.children' :key='c.id'>
                                <li class='rounded border border-slate-800 bg-slate-900/50 px-3 py-2'>
                                    <div class='flex items-center justify-between gap-2'>
                                        <div class='flex items-center gap-2 flex-1 cursor-pointer' @click='openEdit(c)'>
                                            <code class='text-amber-300 text-xs' x-text='c.id'></code>
                                            <div class='text-xs text-slate-300 hover:text-slate-200' x-text='c.description'></div>
                                        </div>
            ...
            <div x-show='editing' class='fixed inset-0 bg-black/60 flex items-center justify-center p-4'>
                <form @submit.prevent='submitEdit' class='w-full max-w-lg rounded-lg border border-slate-700 bg-slate-900 p-6 space-y-4'>
                    ...
                    <div class='space-y-1'>
                        ...
                        <input x-model='form.description' class='w-full bg-slate-800 rounded border border-slate-600 px-2 py-1 text-sm'/>
                    </div>
                    <div class='space-y-1'>
                        <label class='text-xs text-slate-400'>Description</label>
