...
from .db import ensure_schema, DB_PATH, reset_all_caches
import threading
import socket
import time
import sys
...
def start_dashboard_background():
    ...
    try:
        ...
        # Note: Import dashboard AFTER database path setup to ensure it uses correct path
...
Example VS Code MCP configuration (.vscode/mcp.json):
{
  "servers": {
    ...
    if args.db_path:
        ...
        # Reset both database path and connection caches to pick up the new path
        reset_all_caches()
    ...
    # Reset caches one more time before starting dashboard to ensure consistency
    reset_all_caches()
