...
</head><body class='bg-gray-950 text-slate-100 min-h-screen'>
      ...
      <div class='relative flex flex-col md:flex-row md:items-start md:justify-between gap-8'>
          ...
          <div class='flex flex-col gap-3 w-full md:w-auto'>
              <div class='flex gap-2 justify-start md:justify-end'>
                  <button x-data @click="$dispatch('open-create-feature')" class='inline-flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-md bg-emerald-600 hover:bg-emerald-500 text-white shadow-sm shadow-emerald-500/20'>+ Feature</button>
                  <button x-data @click="$dispatch('open-create-spec')" class='inline-flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-md bg-blue-600 hover:bg-blue-500 text-white shadow-sm shadow-blue-500/20'>+ Spec</button>
              ...
              <div class='flex flex-col gap-1 text-[10px] text-slate-500 md:text-right'>
...
def _page(html: str, q: str = "", summary: str = "") -> HTMLResponse:
    ...
    <div x-data="{ open: false }" @open-create-feature.window="open = true" x-show="open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" @click.self="open = false">
        <div class="bg-slate-900 rounded-lg border border-slate-700 p-6 w-full max-w-md mx-4">
            <h3 class="text-lg font-semibold mb-4">Create Feature</h3>
            <form x-data="{ key: '', name: '', loading: false }" @submit.prevent="
                loading = true;
                fetch('/api/features', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ feature_key: key, name: name })
            ...
            ">
                <div class="space-y-4">
                    <div>
                        ...
                        <input x-model="key" type="text" placeholder="AUTH" class="w-full px-3 py-2 bg-slate-800 border border-slate-600 rounded text-sm" required>
                        <p class="text-xs text-slate-400 mt-1">Short uppercase identifier (e.g., AUTH, EDITOR)</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium mb-1">Name</label>
                        <input x-model="name" type="text" placeholder="Authentication & Login" class="w-full px-3 py-2 bg-slate-800 border border-slate-600 rounded text-sm" required>
                    </div>
                </div>
                <div class="flex gap-2 mt-6">
                    <button type="submit" :disabled="loading" class="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 disabled:bg-emerald-800 text-white rounded text-sm">
                        <span x-show="!loading">Create</span>
                        <span x-show="loading">Creating...</span>
                    </button>
                    <button type="button" @click="open = false" class="px-4 py-2 bg-slate-600 hover:bg-slate-500 text-white rounded text-sm">Cancel</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <!-- Create Spec Modal -->
    <div x-data="{ open: false, features: [] }" 
         @open-create-spec.window="open = true; fetch('/api/features').then(r=>r.json()).then(data=>features=data)" 
         x-show="open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" @click.self="open = false">
        <div class="bg-slate-900 rounded-lg border border-slate-700 p-6 w-full max-w-lg mx-4">
            <h3 class="text-lg font-semibold mb-4">Create Spec Item</h3>
            <form x-data="{ id: '', feature: '', kind: 'REQUIREMENT', description: '', parent: '', loading: false }" @submit.prevent="
                loading = true;
                fetch('/api/specs', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    ...
                    if (data.error) {
                        ...
                        open = false;
                        id = ''; feature = ''; kind = 'REQUIREMENT'; description = ''; parent = '';
                        window.location.reload();
                    }
                }).catch(e => {
                    loading = false;
            ...
            ">
                <div class="space-y-4">
                    <div>
                        ...
                        <input x-model="id" type="text" placeholder="REQ-AUTH-001" class="w-full px-3 py-2 bg-slate-800 border border-slate-600 rounded text-sm" required>
                        <p class="text-xs text-slate-400 mt-1">Format: REQ-KEY-001, AC-KEY-001, NFR-KEY-001</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium mb-1">Feature</label>
                        <select x-model="feature" class="w-full px-3 py-2 bg-slate-800 border border-slate-600 rounded text-sm" required>
                            <option value="">Select feature...</option>
                            <template x-for="f in features" :key="f.feature_key">
                                <option :value="f.feature_key" x-text="f.feature_key + ' - ' + f.name"></option>
                            </template>
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-medium mb-1">Kind</label>
                        <select x-model="kind" class="w-full px-3 py-2 bg-slate-800 border border-slate-600 rounded text-sm">
                            <option value="REQUIREMENT">Requirement</option>
                            <option value="AC">Acceptance Criterion</option>
                            <option value="NFR">Non-Functional Requirement</option>
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-medium mb-1">Description</label>
                        <textarea x-model="description" placeholder="When the user submits valid credentials, the system shall establish a session" class="w-full px-3 py-2 bg-slate-800 border border-slate-600 rounded text-sm" rows="3" required></textarea>
                    </div>
                    <div x-show="kind === 'AC' || kind === 'NFR'">
                        <label class="block text-sm font-medium mb-1">Parent Requirement (optional)</label>
                        <input x-model="parent" type="text" placeholder="REQ-AUTH-001" class="w-full px-3 py-2 bg-slate-800 border border-slate-600 rounded text-sm">
                        <p class="text-xs text-slate-400 mt-1">Leave empty for top-level NFR</p>
                    </div>
                </div>
                <div class="flex gap-2 mt-6">
                    <button type="submit" :disabled="loading" class="px-4 py-2 bg-blue-600 hover:bg-blue-500 disabled:bg-blue-800 text-white rounded text-sm">
                        <span x-show="!loading">Create</span>
                        <span x-show="loading">Creating...</span>
                    </button>
                    <button type="button" @click="open = false" class="px-4 py-2 bg-slate-600 hover:bg-slate-500 text-white rounded text-sm">Cancel</button>
...
<script>
    function SpecOverview(){
        return {
            ...
            loadSystemInfo(){ this.fetchJson('/api/metrics/system-info').then(data=>this.systemInfo=data); },
        ...
        function FeaturesList(){
            return {
                ...
                init(){ this.reload(); },
                statusClass(st){ return {FAILING:'bg-rose-700/40 text-rose-300', VERIFIED:'bg-emerald-700/40 text-emerald-300', PARTIAL:'bg-amber-700/40 text-amber-300', UNTESTED:'bg-slate-700/40 text-slate-300'}[st] || 'bg-slate-700/40 text-slate-300'; },
                reload(){ fetch('/api/features').then(r=>r.json()).then(list=>{ const body=this.$refs.featuresBody; if(!body) return; body.innerHTML=''; list.forEach(f=>{ const counts=f.counts||{}; const tr=document.createElement('tr'); tr.className='border-b border-slate-800 last:border-0 hover:bg-slate-800/40 cursor-pointer'; tr.onclick=()=>window.location.href=`/feature/${f.feature_key}`; tr.innerHTML=`<td class='py-1 pr-4'><span class='text-sky-400 font-mono text-xs'>${f.feature_key}</span></td><td class='py-1 pr-4 text-xs text-slate-300'>${f.name||''}</td><td class='py-1 pr-4 text-emerald-400 font-mono text-xs'>${counts.VERIFIED||0}</td><td class='py-1 pr-4 text-rose-400 font-mono text-xs'>${counts.FAILING||0}</td><td class='py-1 pr-4 text-slate-400 font-mono text-xs'>${counts.UNTESTED||0}</td><td class='py-1 pr-4 text-amber-400 font-mono text-xs'>${counts.PARTIAL||0}</td><td class='py-1'><span class='px-2 py-0.5 rounded text-[10px] ${this.statusClass(f.status)}'>${f.status}</span></td>`; body.appendChild(tr); }); }); }
            }
        }
    function FeatureDetail(featureKey, initialReqs=[], initialOrphans=[], initialNfrs=[]){
            return {
                featureKey, requirements: [], orphanACs: [], topLevelNfrs: [], editing:false, current:null, form:{ new_id:'', description:'', status:'' },
                init(){
                    if(initialReqs.length){
                        this.requirements = initialReqs.map(r=>this._augment(r));
                        this.orphanACs = initialOrphans.map(c=>this._augmentChild(c));
                        this.topLevelNfrs = initialNfrs.map(c=>this._augmentChild(c));
                    } else {
                        this.reload();
                    }
                },
                fetch(u){ return fetch(u).then(r=>r.json()); },
                reload(){ this.fetch(`/api/features/${this.featureKey}/tree`).then(d=>{ this.requirements = (d.requirements||[]).map(r=>this._augment(r)); this.orphanACs=(d.orphan_acs||[]).map(c=>this._augmentChild(c)); this.topLevelNfrs=(d.top_level_nfrs||[]).map(c=>this._augmentChild(c)); }); },
                ...
                _augmentChild(c){ c.effective_status=this.effective(c.status,c.override_status); return c; },
                statusClass(st){ return {FAILING:'bg-rose-700/40 text-rose-300', VERIFIED:'bg-emerald-700/40 text-emerald-300', PARTIAL:'bg-amber-700/40 text-amber-300', UNTESTED:'bg-slate-700/40 text-slate-300'}[st] || 'bg-slate-700/40 text-slate-300'; },
                openEdit(item){ this.current=item; this.form.new_id=item.id; this.form.description=item.description||''; this.form.status=item.status||''; this.editing=true; },
                submitEdit(){ if(!this.current) return; const payload={ new_id:this.form.new_id, description:this.form.description, status:this.form.status||null };
                    fetch(`/api/specs/${this.current.id}`, { method:'PATCH', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload)}).then(r=>r.json()).then(updated=>{ if(updated && !updated.error){ window.location.reload(); }});
                }
            }
        }
</script>
"""
    return HTMLResponse(filled + banner + modals + html + "</main></div>" + script + "</body></html>")
    ...
    <div x-data='SpecOverview()' x-init='init()' class='space-y-10'>
        <section>
            <div class='grid gap-4 sm:grid-cols-2 lg:grid-cols-4'>
                <a href='/features' class='kpi-card relative rounded-lg border border-slate-800 bg-slate-900/60 p-4 hover:bg-slate-900/80 transition-colors'><p class='text-xs uppercase tracking-wide text-slate-400 mb-1'>Features</p><h2 class='text-3xl font-semibold' x-text='summary.features ?? "–"'>–</h2></a>
                        ...
                        <tbody x-ref='topFailingBody'>
                            <template x-for='r in topFailing' :key='r.id'>
                                <tr class='border-b border-slate-800 last:border-0 hover:bg-slate-800/40'>
                                    <td class='py-1 pr-4'><code class='text-rose-300' x-text='r.id'></code><div class='text-xs text-slate-400 truncate max-w-xs' x-text='r.description || ""'></div></td>
        ...
        <!-- Collapsible System Information at bottom -->
        <section x-data="{ expanded: false }">
            <div class='rounded-lg border border-slate-800 bg-slate-900/60'>
                <button @click="expanded = !expanded" class='w-full flex items-center justify-between p-4 text-left hover:bg-slate-800/40 transition-colors'>
                    <h3 class='font-medium text-slate-200'>System Information</h3>
                ...
                <div x-show="expanded" x-collapse class='border-t border-slate-800 p-4'>
                    <div class='grid gap-3 text-sm'>
                        <div class='grid grid-cols-1 sm:grid-cols-3 gap-2'>
                            <div class='text-slate-400'>Database Path:</div>
                            <div class='sm:col-span-2 font-mono text-xs bg-slate-800 p-2 rounded border break-all' x-text='systemInfo.database_path || "Loading..."'>Loading...</div>
                        </div>
                        <div class='grid grid-cols-1 sm:grid-cols-3 gap-2'>
                            <div class='text-slate-400'>Database Size:</div>
                            <div class='sm:col-span-2' x-text='(systemInfo.database_size_mb || 0) + " MB"'>-</div>
                        </div>
                        <div class='grid grid-cols-1 sm:grid-cols-3 gap-2'>
                            <div class='text-slate-400'>Working Directory:</div>
                            <div class='sm:col-span-2 font-mono text-xs bg-slate-800 p-2 rounded border break-all' x-text='systemInfo.working_directory || "Loading..."'>Loading...</div>
                        </div>
                        <div class='grid grid-cols-1 sm:grid-cols-3 gap-2' x-show='systemInfo.env_override'>
                            <div class='text-slate-400'>Environment Override:</div>
                            <div class='sm:col-span-2 font-mono text-xs bg-slate-800 p-2 rounded border break-all' x-text='systemInfo.env_override'>-</div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div x-data='FeaturesList()' x-init='init()' class='space-y-6'>
        <div class='flex items-center justify-between'>
            ...
            <div class='flex gap-2'>
                <button x-data @click="$dispatch('open-create-feature')" class='text-xs px-3 py-1.5 rounded bg-emerald-600 hover:bg-emerald-500 text-white'>+ New Feature</button>
...
def _dashboard_feature(request: Request):
    ...
    reqs = db.query(
        "SELECT id,description,status FROM spec_item WHERE feature_key=? AND kind='REQUIREMENT' ORDER BY id",
        (feature_key,),
    )
    req_html = []
    for r in reqs:
        leaves = db.query(
            "SELECT id,kind,description,status FROM spec_item WHERE parent_id=? ORDER BY id", (r['id'],)
        )
        crit_items = (
            "".join(
                f"<li class='py-1 border-b border-slate-800 last:border-0'><div class='flex items-start gap-2'><code class='text-sky-400'>{c['id']}</code><span class='flex-1 text-sm' hx-get='/fragments/spec/{c['id']}' hx-target='closest li' hx-swap='innerHTML' data-spec='{c['id']}'>{c['description']}</span>{_status_chip(c['status'])}</div></li>"
                for c in leaves
                if c['kind'] == 'AC'
            )
        ...
        nfr_items = "".join(
            f"<li class='py-1 border-b border-slate-800 last:border-0'><div class='flex items-start gap-2'><code class='text-amber-400'>{c['id']}</code><span class='flex-1 text-sm' hx-get='/fragments/spec/{c['id']}' hx-target='closest li' hx-swap='innerHTML' data-spec='{c['id']}'>{c['description']}</span>{_status_chip(c['status'])}</div></li>"
            for c in leaves
            if c['kind'] == 'NFR'
        )
        nfr_block = (
        ...
        req_html.append(
            ...
            f"<div class='req-header flex items-start justify-between gap-4'><div><h3 class='font-semibold text-slate-200'>{r['description']}</h3><p class='text-xs font-mono text-slate-500'>{r['id']}</p></div>{_status_chip(r['status'])}</div>"
            f"<div class='criteria-list'><h4>Acceptance Criteria</h4><ul class='divide-y divide-slate-800'>{crit_items}</ul></div>"
            f"{nfr_block}" "</section>"
        )
    top_nfrs = db.query(
        "SELECT id,description,status FROM spec_item WHERE feature_key=? AND kind='NFR' AND parent_id IS NULL",
        (feature_key,),
    )
    top_nfr_block = ''
    if top_nfrs:
        t_items = "".join(
            f"<li class='py-1 border-b border-slate-800 last:border-0'><div class='flex items-start gap-2'><code class='text-amber-400'>{n['id']}</code><span class='flex-1 text-sm' hx-get='/fragments/spec/{n['id']}' hx-target='closest li' hx-swap='innerHTML' data-spec='{n['id']}'>{n['description']}</span>{_status_chip(n['status'])}</div></li>"
            for n in top_nfrs
        )
        top_nfr_block = (
...
async def admin_seed_sample(request: Request):
    # Force seeding only if database currently has no features OR user passes force=1
    force = request.query_params.get('force') == '1'
    has_any = db.query_one("SELECT feature_key FROM feature LIMIT 1") is not None
    if has_any and not force:
        return JSONResponse({"message":"Sample data already present (use ?force=1 to reseed)"})
    if force:
        # Drop and recreate schema to get a clean slate
        db.ensure_schema(reset=True)
    created = db.ensure_sample_data()
    return JSONResponse({"seeded": bool(created), "message": "Sample data seeded" if created else "Already populated"})
...
def _feature_detail(request: Request):
    ...
    # Build table rows
    rows_html = []
    for r in db.query("SELECT id,description,status,override_status FROM spec_item WHERE feature_key=? AND kind='REQUIREMENT' ORDER BY id", (feature_key,)):
            # Build clickable requirement row
            r_obj = {
                'id': r['id'], 'kind': 'REQUIREMENT', 'description': r['description'] or '',
                'override_status': r['override_status'], 'status': r['status']
            }
            r_js = json.dumps(r_obj)
            rows_html.append(
                f"<tr class='border-b border-slate-800 hover:bg-slate-800/40 cursor-pointer' @click='openEdit({r_js})'>"
                f"<td class='py-2 pr-4 whitespace-nowrap'><code class='text-sky-400 text-sm'>{r['id']}</code></td>"
                f"<td class='py-2 pr-4'><span class='text-xs bg-blue-600/20 text-blue-300 px-2 py-0.5 rounded'>REQ</span></td>"
                f"<td class='py-2 pr-4'>{r['description']}</td>"
                f"<td class='py-2'>{_status_chip(r['status'])}</td>"
                f"</tr>"
            )
            for c in db.query("SELECT id,kind,description,status,override_status FROM spec_item WHERE parent_id=? ORDER BY id", (r['id'],)):
                badge_class = 'bg-green-600/20 text-green-300' if c['kind']=='AC' else 'bg-orange-600/20 text-orange-300'
                c_obj = {
                    'id': c['id'], 'kind': c['kind'], 'description': c['description'] or '',
                    'override_status': c['override_status'], 'status': c['status']
                }
                c_js = json.dumps(c_obj)
                rows_html.append(
                    f"<tr class='border-b border-slate-800 hover:bg-slate-800/40 cursor-pointer' @click='openEdit({c_js})'>"
                    f"<td class='py-1 pr-4 pl-6 whitespace-nowrap'><code class='text-amber-300 text-xs'>{c['id']}</code></td>"
                    f"<td class='py-1 pr-4'><span class='text-xs px-2 py-0.5 rounded {badge_class}'>{c['kind']}</span></td>"
                    f"<td class='py-1 pr-4 text-xs'>{c['description']}</td>"
                    f"<td class='py-1'>{_status_chip(c['status'])}</td>"
                    f"</tr>"
                )
    orphan_html = ''
    orphan_acs = db.query("SELECT id,description,status,override_status FROM spec_item WHERE feature_key=? AND kind='AC' AND parent_id IS NULL ORDER BY id", (feature_key,))
    if orphan_acs:
            orphan_html += "<tr class='border-b border-slate-800 bg-slate-800/30'><td colspan='4' class='py-2 px-2 text-[10px] uppercase tracking-wide text-slate-400'>Orphan Acceptance Criteria</td></tr>"
            for a in orphan_acs:
                a_obj = {
                    'id': a['id'], 'kind': 'AC', 'description': a['description'] or '',
                    'override_status': a['override_status'], 'status': a['status']
                }
                a_js = json.dumps(a_obj)
                orphan_html += (
                    f"<tr class='border-b border-slate-800 hover:bg-slate-800/40 cursor-pointer' @click='openEdit({a_js})'>"
                    f"<td class='py-1 pr-4 whitespace-nowrap'><code class='text-amber-300 text-xs'>{a['id']}</code></td>"
                    f"<td class='py-1 pr-4'><span class='text-xs px-2 py-0.5 rounded bg-green-600/20 text-green-300'>AC</span></td>"
                    f"<td class='py-1 pr-4 text-xs'>{a['description']}</td>"
                    f"<td class='py-1'>{_status_chip(a['status'])}</td>"
                    f"</tr>"
                )
    top_nfrs = db.query("SELECT id,description,status,override_status FROM spec_item WHERE feature_key=? AND kind='NFR' AND parent_id IS NULL ORDER BY id", (feature_key,))
    top_html = ''
    if top_nfrs:
        top_html += "<tr class='border-b border-slate-800 bg-slate-800/30'><td colspan='4' class='py-2 px-2 text-[10px] uppercase tracking-wide text-slate-400'>Top-level NFRs</td></tr>"
        for n in top_nfrs:
                n_obj = {
                    'id': n['id'], 'kind': 'NFR', 'description': n['description'] or '',
                    'override_status': n['override_status'], 'status': n['status']
                }
                n_js = json.dumps(n_obj)
                top_html += (
                    f"<tr class='border-b border-slate-800 hover:bg-slate-800/40 cursor-pointer' @click='openEdit({n_js})'>"
                    f"<td class='py-1 pr-4 whitespace-nowrap'><code class='text-amber-300 text-xs'>{n['id']}</code></td>"
                    f"<td class='py-1 pr-4'><span class='text-xs px-2 py-0.5 rounded bg-orange-600/20 text-orange-300'>NFR</span></td>"
                    f"<td class='py-1 pr-4 text-xs'>{n['description']}</td>"
                    f"<td class='py-1'>{_status_chip(n['status'])}</td>"
                    f"</tr>"
                )
    table_body = ''.join(rows_html) + orphan_html + top_html or "<tr><td colspan='4' class='py-6 text-center text-slate-500 text-xs'>No spec items</td></tr>"
    html = f"""
    <div x-data='FeatureDetail("{feature_key}")' class='space-y-8'>
            ...
            <section class='rounded-lg border border-slate-800 bg-slate-900/60 p-4'>
                <div class='flex items-center justify-between mb-4'>
                    ...
                    <a href='/feature/{feature_key}' class='text-xs text-slate-400 hover:text-slate-200'>Reload</a>
                </div>
                <div class='overflow-x-auto'>
                    <table class='w-full text-sm'>
                        <thead>
                            <tr class='text-xs text-slate-400 text-left border-b border-slate-800'>
                                <th class='py-1 pr-4 whitespace-nowrap'>KEY</th>
            ...
            <div x-show='editing' class='fixed inset-0 bg-black/60 flex items-center justify-center p-4'>
                <form @submit.prevent='submitEdit' class='w-full max-w-lg rounded-lg border border-slate-700 bg-slate-900 p-6 space-y-4'>
                    ...
                    <div class='space-y-1'>
                        <label class='text-xs text-slate-400'>Key (ID)</label>
                        <input x-model='form.new_id' type='text' class='w-full bg-slate-800 rounded border border-slate-600 px-2 py-1 text-xs font-mono'/>
                        <p class='text-[10px] text-slate-500'>Changing this renames the spec ID and moves verification history.</p>
                    </div>
                    <div class='space-y-1'>
                        <label class='text-xs text-slate-400'>Description</label>
                        <textarea x-model='form.description' rows='4' class='w-full bg-slate-800 rounded border border-slate-600 px-2 py-1 text-xs'></textarea>
                    </div>
                    <div class='space-y-1'>
                        <label class='text-xs text-slate-400'>Status</label>
                        <select x-model='form.status' class='w-full bg-slate-800 rounded border border-slate-600 px-2 py-1 text-xs'>
                        ...
                        <p class='text-[10px] text-slate-500'>For requirements the status is rolled-up; changes here affect AC/NFR only.</p>
                    </div>
...
dashboard_app = Starlette(
    ...
    Route('/admin/seed-sample', admin_seed_sample),
        Mount('/assets', StaticFiles(directory=str((__import__('pathlib').Path(__file__).parent / 'assets'))), name='assets'),
    ]
    + api_routes,
)
