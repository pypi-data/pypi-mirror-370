...
def db_path_callback(ctx: typer.Context, param: typer.CallbackParam, value: str | None):
    """Set the database path from --db-path option."""
    if value is not None:
        os.environ["SPEC_MCP_DB_PATH"] = value
    return value
...
@cli.command()
def seed_samples(force: bool = typer.Option(False, '--force', help='Force reseed by resetting schema first')):
    """Seed illustrative sample data (features, requirements, ACs, NFRs)."""
    if force:
        db.ensure_schema(reset=True)
    ...
    if created:
        ...
        print("Sample data already present. Use --force to reseed.")
