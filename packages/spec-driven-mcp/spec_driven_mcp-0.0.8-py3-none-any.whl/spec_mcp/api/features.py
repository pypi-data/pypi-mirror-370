...
async def feature_tree(request: Request):
    ...
    # Orphan acceptance criteria (AC with no parent) so they are not hidden from the UI
    orphan_acs = db.query("SELECT id,description,status,override_status FROM spec_item WHERE feature_key=? AND kind='AC' AND parent_id IS NULL ORDER BY id", (key,))
    return JSONResponse({'feature': dict(feat), 'requirements': req_list, 'top_level_nfrs': [dict(n) for n in top_nfrs], 'orphan_acs': [dict(a) for a in orphan_acs]})
