...
async def create_spec(request: Request):
    try:
        ...
        db.execute(
            ...
            (spec_id, feature_key, kind, description, parent_id, body.get('status') or 'UNTESTED', db.now_iso())
        )
...
async def list_specs(request: Request):
    ...
    sql = "SELECT id,feature_key,kind,description,status,parent_id FROM spec_item WHERE 1=1"
    args = []
    if feature:
...
async def get_spec(request: Request):
    ...
    if row['kind'] == 'REQUIREMENT':
        kids = db.query(
            "SELECT id,kind,description,status FROM spec_item WHERE parent_id=? ORDER BY id",
            (sid,)
        )
...
async def patch_spec(request: Request):
    ...
    body = await request.json()
    try:
        # Handle rename (key change)
        new_id = body.get('new_id')
        if new_id and new_id != sid:
            try:
                renamed = db.rename_spec(sid, new_id)
            except ValueError as e:
                return JSONResponse({'error': str(e)}, status_code=422)
            if not renamed:
                return JSONResponse({'error':'not found'}, status_code=404)
            sid = new_id
        updated = db.patch_spec(
            sid,
