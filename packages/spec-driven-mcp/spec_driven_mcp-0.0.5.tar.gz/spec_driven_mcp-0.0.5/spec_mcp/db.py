...
def default_db_path(workspace_roots: Optional[list[str]] = None) -> str:
    ...
    2. Project-local database in workspace folder: <workspace>/.spec-mcp/specs.db
    3. User data directory as fallback
    
    Args:
        workspace_roots: List of workspace root paths (from MCP client context)
    ...
    # 2. Use project-local database in workspace folder
    if workspace_roots and len(workspace_roots) > 0:
        root = Path(workspace_roots[0])
        if root.exists() and root.is_dir():
            db_path = root / ".spec-mcp" / "specs.db"
            db_path.parent.mkdir(parents=True, exist_ok=True)
            return str(db_path)
    
    # 3. Fallback to user data directory
    # This ensures consistent behavior when no workspace context is available
    user_data_dir = platformdirs.user_data_dir("spec-mcp", "SpecMCP")
    db_path = Path(user_data_dir) / "specs.db"
    db_path.parent.mkdir(parents=True, exist_ok=True)
    return str(db_path)

# Get workspace roots from MCP client context
workspace_roots = None

# VS Code and other MCP clients may provide workspace folders
    ...
    try:
        ...
        for folder in folders:
            ...
            if uri.startswith("file://"):
                # Handle file URIs properly across platforms
                from urllib.parse import urlparse, unquote
                parsed = urlparse(uri)
                path = unquote(parsed.path)
                # On Windows, urlparse might give us /C:/path, so remove leading slash
...
# Allow explicit workspace root (for other MCP clients)
if not workspace_roots and os.getenv("SPEC_MCP_WORKSPACE_ROOT"):
    workspace_roots = [os.getenv("SPEC_MCP_WORKSPACE_ROOT")]
