...
def default_db_path(workspace_roots: Optional[list[str]] = None) -> str:
    """Return the project-local database path (no user-level fallback).

    The path is always: <workspace-root>/.spec-mcp/specs.db
    If multiple workspace roots are provided we pick the first.
    If none are provided we fall back to the current working directory.
    """
    root = Path(workspace_roots[0]) if workspace_roots else Path.cwd()
