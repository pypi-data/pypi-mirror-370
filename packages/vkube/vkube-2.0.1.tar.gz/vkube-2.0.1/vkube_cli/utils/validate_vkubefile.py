    ...
    def get_real_used_resource(self)-> Tuple[int,int]:
        ...
        for container_doc in self.container_docs:
            ...
            if deploy_doc:
                ...
                ports_configs = deploy_doc.get("ports",[])
                for port in ports_configs:
                    if port.get("hostPort",None) is not None:
                        total_host_port_num = total_host_port_num + 1
        return total_resourceUnit, total_host_port_num
    ...
    def validate_resources(self,resource_unit_str:str) -> Tuple[bool,str]:
        try:
            ...
            if "-" in resource_unit_str:
                ...
                real_used_unit, host_port_num = self.get_real_used_resource()
                ...
                if host_port_num > buy_resource_unit:
                    return False,"Error: the number of configured host ports is greater than the resource unit of service"
                else:
                    return True, ""
