...
def start_proxy(run_proxy_script=None, script_args=None, debug=False, proxy_logger=None):
    ...
    # BNY wants a flag to mute proxy stdout because it is noisy and consumes too many of their splunk tokens.
    proxy_log_stdout = subprocess.DEVNULL if config.LOG_MUTE_PROXY_STDOUT else None

    proxy_logger.info("Starting proxy: %s", ' '.join(command_args))
    proc = subprocess.Popen([run_proxy_script] + command_args[1:], stdout=proxy_log_stdout)
    proxy_pid = proc.pid
