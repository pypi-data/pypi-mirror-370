"""Python dataclass code generator for Typelang."""
...
class PythonDataclassGenerator:
    """Generate Python dataclass code from IR."""
    ...
    def generate_type_def(self, def_: IRTypeDef) -> str:
        ...
        # Generate type alias or dataclass
        if isinstance(def_.body, CTStruct):
            lines.append(self.generate_dataclass(def_))
        else:
            # Generate type alias
    ...
    def generate_dataclass(self, def_: IRTypeDef) -> str:
    ...
    def generate_field(self, field: IRField) -> List[str]:
        ...
        if field.optional or field.default_value is not None:
            lines.append(self.indent_str(f"{field.name}: {field_type} = None"))
        else:
            lines.append(self.indent_str(f"{field.name}: {field_type}"))
