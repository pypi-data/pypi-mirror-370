...
class Partition:
    ...
    def initialize_partition(self):
        ...
        # leiden on subgraph
        subsample_subpartition = subgraph.community_leiden(objective_function='modularity', resolution_parameter=RESOLUTION, weights='weight')
    ...
    def optimize(self):
        ...
        partition = G_ig.community_leiden(objective_function='modularity', initial_membership=self.membership,
                                          n_iterations=3, resolution_parameter=RESOLUTION, weights='weight')
...
def load_graph(source):
    ...
    if isinstance(source, str):
        ...
        # Relabel to contiguous integers (igraph expects 0..n-1 IDs)
        G = nx.convert_node_labels_to_integers(source)

        # Use the same edge order for igraph and for collecting weights
        edges = list(G.edges())
        ig_graph = ig.Graph(edges=edges, directed=False)

        # Assign weights unconditionally in the SAME order
...
if __name__ == "__main__":
    ...
    # For weights: intra-community edges get weights from a higher range, inter-community from a lower range
    intra_weight_low, intra_weight_high = 3.0, 7.0
    inter_weight_low, inter_weight_high = 0.5, 2.0
    ...
    # Assign weights: intra-community edges get heavier (but random), inter-community lighter (but random)
    # Get block label for each node
    block_labels = []
    for comm_id, size in enumerate(sizes):
        block_labels.extend([comm_id] * size)
    for u, v in G.edges():
        if block_labels[u] == block_labels[v]:
            G[u][v]['weight'] = float(np.random.uniform(intra_weight_low, intra_weight_high))
        else:
            G[u][v]['weight'] = float(np.random.uniform(inter_weight_low, inter_weight_high))
