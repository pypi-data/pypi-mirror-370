...
async def run(cfg: Config):
    ...
    try:
        ...
        def render_panel():
            ...
            chart = BarChart(latencies, width - 2, top_height)  # -2 for panel padding
            ...
            for i in range(10):
                if i < len(events):
                    ...
                    host_str = ""
                    if row.host_address:
                        # Check if IP changed from previous measurement
                        prev_ip = None
                        if i > 0 and i - 1 < len(events) and events[i - 1].host_address:
                            prev_ip = events[i - 1].host_address
                        elif len(events) > 1:
                            # Look for the most recent measurement with an IP
                            for j in range(i - 1, -1, -1):
                                if j < len(events) and events[j].host_address:
                                    prev_ip = events[j].host_address
                                    break
                ...
                # Add empty row to maintain fixed height
                log_panel.add_row("", "", "", "", "")
