...
class AgentSwitcher:
    ...
    def __init__(self, providers: list[str], using_remote_credential_pools: bool = False):
        ...
        self._available_providers: list[str] = providers
        self._credential_pools: defaultdict[str, dict[str, CredentialPoolProtocol]] = defaultdict(
            dict
        )
        self.provider_mappings = self.register_providers()

    def register_providers(self) -> dict[str, ProviderMapping]:
        """
        获取所有已注册的 provider 的映射。
        可以复写这个方法，实现自定义的 Agent 类，并添加注册逻辑。
        确保所有 providers 都已注册 (参考 provider_registry.py 中注册)。
        """
        register_providers(self._available_providers)
...
if __name__ == "__main__":
    ...
    async def test_model_switcher():
        ...
        switcher = AgentSwitcher(providers=['laozhang', 'laozhang_singapore'])
        await switcher.start_all_pools()
        credential_pool = switcher.get_credential_pool('laozhang_singapore', 'gpt-4o')
