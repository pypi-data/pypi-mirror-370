...
class DelayedVariantModel(Protocol):
    """just to keep the type checkers happy"""

    def _build_variants(self) -> None: ...


def variants(*pipelines: VariantPipe, delayed_build: bool = False) -> Callable[[type[BaseModel]], type[BaseModel] | DelayedVariantModel]:
    """
    ...
    def delayed_decorator(model_cls: type[BaseModel]) -> type[BaseModel] | DelayedVariantModel:
        def _build_variants():
            for pipeline in pipelines:
                pipeline(model_cls)
