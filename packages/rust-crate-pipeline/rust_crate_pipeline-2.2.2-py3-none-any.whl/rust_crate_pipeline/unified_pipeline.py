...
class UnifiedSigilPipeline:
    ...
    async def _gather_documentation(self, crate_name: str) -> Dict[str, ScrapingResult]:
        ...
        try:
            ...
            successful_sources = [source for source, result in results.items() if result.error is None]
            failed_sources = [source for source, result in results.items() if result.error is not None]
    ...
    async def _perform_sacred_chain_analysis(
        ...
        try:
            ...
            try:
                validated_docs = DocumentationResults.model_validate(documentation_results)
                ...
                for error in e.errors():
                    ...
                    error_msg = error["msg"]
                    error_details.append(f"Field '{field_path}': {error_type} - {error_msg}")

                error_summary = "; ".join(error_details)
                self.logger.error(f"Documentation results validation failed: {error_summary}")
                raise PipelineError(f"Documentation results validation failed: {error_summary}")
        ...
        for toolchain_file in toolchain_files:
            if toolchain_file.exists():
                ...
                try:
                    ...
                    self.logger.info(
                        f"Temporarily disabled toolchain override: " f"{toolchain_file} -> {backup_path}"
                    )
                    return backup_path
                except Exception as e:
    ...
    async def _download_and_extract_crate(
        ...
        crate_url = f"https://static.crates.io/crates/{crate_name}/" f"{crate_name}-{crate_version}.crate"
        try:
                            ...
                            for member in tar_file.getmembers():
                                ...
                                if not os.path.abspath(member_path).startswith(os.path.abspath(target_dir)):
                                    raise SecurityException(
                                        f"Attempted path traversal in tar file: " f"{member.name}"
                                    )
    ...
    async def _add_unified_llm_enrichment(
        ...
        try:
            ...
            trace.audit_info["enriched_crate"] = self.sanitizer.sanitize_data(enriched_crate.to_dict())
...
def add_llm_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    llm_group.add_argument(
        "--llm-model",
        default="gpt-4o",
        help="Model name/identifier (e.g., gpt-4, llama2, claude-3, " "qwen25-coder-32b-instruct)",
    )

    llm_group.add_argument("--llm-api-base", help="API base URL (for local providers or custom endpoints)")
