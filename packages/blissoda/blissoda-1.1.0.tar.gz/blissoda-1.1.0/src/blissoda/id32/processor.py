...
from bliss.shell.getval import getval_float, getval_int, getval_yes_no
...
from ..persistent.parameters import ParameterInfo, autocomplete_property
...
class Id32SpecGenProcessor(
    ...
    def _get_workflow_inputs(self, scan: Scan, detector) -> list:
        return [
            {
                "name": "scan_number",
    ...
    def _get_scan_filename(self, scan: Scan) -> str:
        filename = scan.scan_info.get("filename")
        if filename:
            return filename
        return current_session.scan_saving.filename

    def _get_scan_processed_directory(self, scan: Scan) -> str:
        return get_dataset_processed_dir(self._get_scan_filename(scan))

    def _get_workflow_destination(self, scan: Scan) -> str:
