...
class SingleSliceImshow(BasePlotter):
    ...
    def handle_workflow_result(self, future: Any) -> None:
        ...
        self.set_image(np.squeeze(img))

    def _extract_result(self, future: Any) -> dict | None:
        """
    ...
    def _compute_axes(self, image: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        """
        ...
        try:
            ...
            rows, cols = image.shape[-2:]
            return np.arange(cols, dtype=float), np.arange(rows, dtype=float)
