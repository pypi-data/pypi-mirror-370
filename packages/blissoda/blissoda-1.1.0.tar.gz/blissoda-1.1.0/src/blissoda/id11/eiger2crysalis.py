...
from ewoksjob.client import submit, convert_graph
...
class Id11Eiger2Crysalis(
    ...
    def get_lima_name(self, scan: Scan) -> List[str]:
    ...
    def get_output_path(self, scan: Scan) -> str:
    ...
    def get_processed_output_path(self, scan: Scan) -> str:
        scan_nb = scan.scan_info["scan_nb"]
        dataset_processed_dir = get_dataset_processed_dir(self.get_filename(scan))
        scan_processed_dir = os.path.join(dataset_processed_dir, f"scan{scan_nb:04d}")
        dataset_name = os.path.basename(dataset_processed_dir)
    ...
    def get_lima_filenames(self, scan: Scan) -> List[str]:
    ...
    def get_omega(self, scan: Scan) -> str:
        """
        ...
        return f"-{omega_start_symetric}-index*{step_size}"

    def get_scan_parameters(self, scan: Scan) -> Dict[str, Any]:
        scan_parameters = dict()
    ...
    def get_run_parameters(self, scan: Scan) -> Dict[str, Any]:
        return {
            "count": scan.scan_info["npoints"],
            "omega": 0,
            "omega_start": self._omega_start_symetric,
            "omega_end": self._omega_end_symetric,
            "pixel_size": 0.075,
            "omega_runs": None,
            "theta": 0,
            "kappa": 0,
    ...
    def get_inputs(self, scan: Scan) -> List[Dict[str, Any]]:
    ...
    def get_submit_arguments(self, scan: Scan) -> Dict[str, Any]:
        return {
            "inputs": self.get_inputs(scan),
            "outputs": [{"all": "False"}],
        }

    def run_conversion(self, scan: Scan) -> None:
