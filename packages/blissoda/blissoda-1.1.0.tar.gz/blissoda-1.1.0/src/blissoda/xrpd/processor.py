...
class XrpdProcessor(
    ...
    def on_new_scan_metadata(self, scan: Scan) -> Optional[dict]:
        metadata, _ = self._on_new_scan(scan)
        return metadata

    def on_new_scan(self, scan: Scan) -> Optional[Any]:
        _, future = self._on_new_scan(scan)
        return future

    def _on_new_scan(self, scan: Scan) -> Tuple[Optional[dict], Optional[Any]]:
    ...
    def _data_to_plot_url(self, scan: Scan, lima_name: str):
    ...
    def _trigger_plotting(self, scan: Scan, lima_name: str, future) -> None:
    ...
    def get_submit_arguments(self, scan: Scan, lima_name) -> dict:
        return {
            "inputs": self.get_inputs(scan, lima_name),
            "outputs": [{"all": False}],
        }

    def _get_workflow(self, scan: Scan) -> Optional[str]:
    ...
    def _get_default_workflow(self, scan: Scan) -> Optional[str]:
        """Get the workflow filename for the scan"""
        if scan.scan_info.get("save"):
    ...
    def _set_workflow(self, scan: Scan, filename) -> None:
    ...
    def get_workflow(self, scan: Scan) -> Optional[str]:
    ...
    def get_lima_names(self, scan: Scan) -> List[str]:
    ...
    def check_monitor(self, scan: Scan) -> bool:
    ...
    def workflow_destination(self, scan: Scan, lima_name: str) -> str:
        filename = self.get_filename(scan)
        scan_nb = scan.scan_info.get("scan_nb")
    ...
    def external_output_filename(self, scan: Scan, lima_name: str) -> Optional[str]:
    ...
    def online_output_filename(self, scan: Scan, lima_name: str) -> Optional[str]:
    ...
    def master_output_url(self, scan: Scan) -> str:
    ...
    def external_output_url(self, scan: Scan, lima_name: str) -> Optional[str]:
    ...
    @h5py_utils.retry()
    def get_data_keys(self, scan: Scan, lima_name: str):
    ...
    @h5py_utils.retry()
    def get_data(self, scan: Scan, channel: str, idx=tuple()):
        """
    ...
    def online_output_url(self, scan: Scan, lima_name: str) -> str:
        """URL which can be used to inspect the results during and after the processing."""
        scan_nb = scan.scan_info.get("scan_nb")
        filename = self.online_output_filename(scan, lima_name)
        return f"{filename}::/{scan_nb}.1"

    def get_inputs(self, scan: Scan, lima_name: str) -> List[dict]:
    ...
    def get_integrate_inputs(
        self, scan: Scan, lima_name: str, task_identifier: str
    ) -> List[dict]:
        filename = self.get_filename(scan)
        scan_nb = scan.scan_info.get("scan_nb")
        inputs = [
    ...
    def get_integrate_list_inputs(self, scan: Scan, lima_name: str) -> List[dict]:
    ...
    def get_integrate_1d_inputs(self, scan: Scan, lima_name: str) -> List[dict]:
    ...
    def _get_workflow_upload_parameters(self, scan: Scan) -> dict:
