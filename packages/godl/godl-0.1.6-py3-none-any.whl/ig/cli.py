...
""",
    ...
    return min(min1, min2), max(min1, max1)
...
def copy_to_clipboard(text: str):
    """Copies the given text to the clipboard using platform-specific commands."""
    try:
        if sys.platform == "win32":
            subprocess.run(["clip.exe"], input=text.encode('utf-16le'), check=True, text=True)
        elif sys.platform == "darwin":
            subprocess.run(["pbcopy"], input=text, check=True, text=True)
        else:
            print("Warning: Clipboard functionality not supported on this OS.")
    except FileNotFoundError:
        print("Error: Clipboard command not found. 'clip.exe' on Windows or 'pbcopy' on macOS is required.")
...
def main():
    ...
    if cmd in python_programs:
        ...
        program_code = r_programs[cmd]
        print(program_code)
        copy_to_clipboard(program_code)
        print("\n\n-- R program code copied to clipboard! --")
    else:
