...
volBar = np.interp(vol_Per,(0,1),(135,35))
...
kt_udxc_sau = 0.5
music_active = False  # Trạng thái của biểu tượng Music (màu xám hoặc đỏ)
volume_active = False
myassistant_active = False  # Trạng thái của biểu tượng My Assistant (màu xám hoặc xanh lá)
...
while cap.isOpened():
    ...
    cv2.putText(image, "My Computer", (260, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (52, 97, 235), 2)
    ...
    cv2.putText(image, "Music", (125, 300), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (52, 97, 235), 2)
    ...
    cv2.putText(image, "My Assistant", (280, 300), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (52, 97, 235), 2)
    ...
    cv2.putText(image, "Exit", (495, 300), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (52, 97, 235), 2)
    ...
    cv2.circle(image, circle1_center, 40, volume_color, -1)  # Hình tròn màu vàng
    cv2.putText(image, "Volume", (circle1_center[0] - 30, circle1_center[1] + 60), 
            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (52, 97, 235), 2)

    # Vẽ hình tròn giữa Exit và My Assistant
    circle2_center = (int((370 + 470) / 2), int((280 + 200) / 2))
    cv2.circle(image, circle2_center, 40, (255, 255, 0), -1)  # Hình tròn màu xanh dương nhạt
...
# *****************************************************************************************************************************
            ...
            if isInside(points2, centroid) and fingers[1] == 1 and lClick_distance < normal_distance and not music_active:
                ...
                sleep(0.3)
                phanhoi()
            elif isInside(points2, centroid) and fingers[1] == 1 and lClick_distance < normal_distance and music_active:
                music_active = False
                tatMusic()
                sleep(0.3)
                phanhoi()
...
# *****************************************************************************************************************************
            ...
            if distance(centroid, circle1_center) <= 40 and fingers[1] == 1 and lClick_distance < normal_distance and not volume_active:
                ...
                volBar = np.interp(length,(35,170),(135,35))
       
                volume.SetMasterVolumeLevel(vol, None)
                volBar = np.interp(length, (35, 170), (minVol, maxVol))
                vol_Per = volume.GetMasterVolumeLevelScalar()
                if length <= 35:
                    cv2.circle(image, (cvx, cvy), 7, (0, 255, 0), -1) # vẽ 1 đường tròn khác màu để báo giá trị min
                cv2.rectangle(image, (545, int(volBar)), (575, 135), (0,255,0), -1)
            elif distance(centroid, circle1_center) <= 40 and fingers[1] == 1 and lClick_distance < normal_distance and volume_active:
                volume_active = False
