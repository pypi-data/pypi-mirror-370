...
class SpurGear(object):
    ...
    def mesh(self, at):
        return Cylinder(
            radius=self.d_p / 2 - 0.005,
            height=self.b + self.stretch,
            transform=at.copy(),
        )
...
class GearPair(Model):
    ...
    def set_working_conditions(self):
        ...
        g = self.gears.g
        try:
            self.alpha_p = optimize.brentq(
                alpha_p_with_shifts, 0.1, pi / 2, args=(self.alpha, p.Z, p.x, g.Z, g.x)
            )
        except ValueError:
            raise
    ...
    def sigma_h_0(self, Ft):
        ...
        Z_E = sqrt(
            1
            / (
                (1 - p.material.nu**2) / p.material.E
                + (1 - g.material.nu**2) / g.material.E
            )
            * 1
            / pi
        )
        ...
        return (
            ...
            * Z_E
            * Z_eps
            * sqrt(Ft / (p.d * 1e-3) / (p.b * 1e-3) * (self.u + 1) / self.u)
        )
    ...
    def sigma_h(self, speed, torque):
        ...
        p = self.gears.p
        g = self.gears.g
        M1 = p.tan_alpha_t_p / sqrt(
            (sqrt(p.da**2 / p.db**2 - 1) - 2 * pi / p.Z)
            * (sqrt(g.da**2 / g.db**2 - 1) - (self.contact_ratio - 1) * 2 * pi / g.Z)
        )
        ZB = M1 if M1 >= 1.0 else 1.0
        try:
            M2 = p.tan_alpha_t_p / sqrt(
                (sqrt(g.da**2 / g.db**2 - 1) - 2 * pi / g.Z)
                * (
                    sqrt(p.da**2 / p.db**2 - 1)
                    - (self.contact_ratio - 1) * 2 * pi / p.Z
                )
            )
        except ValueError:
            M2 = 1.0
        ZD = M2 if M2 >= 1.0 else 1.0
    ...
    def sigma_f_0(self, Ft, gear):
        ...
        E = (
            pi / 4 * gear.m
            - gear.hf * tan(gear.alpha)
            - (1 - sin(gear.alpha)) * rho_fp / cos(gear.alpha)
        )
        ...
        den = 2 * sqrt(
            (
                sqrt(dan**2 / 4 - dbn**2 / 4)
                - pi * gear.d * cos(gear.alpha) / gear.Z * (eps_an - 1)
            )
            ** 2
            + dbn**2 / 4
        )
        ...
        ge = (
            (0.5 * pi + 2 * tan(gear.alpha) * gear.x) / zn
            + inv(gear.alpha)
            - inv(alpha_en)
        )
        alpha_Fen = alpha_en - ge
        T = pi / 3  # exterior gear
        H = 2.0 / zn * (pi / 2 - E / gear.m) - T

        def f_theta(x):
            return x - 2 * G / zn * np.tan(x) + H

        theta = optimize.fsolve(f_theta, pi / 6)[0]
        sFnom = zn * sin(T - theta) + sqrt(3) * (G / cos(theta) - rho_fp / gear.m)
        hFeom = 0.5 * (
            (cos(ge) - sin(ge) * tan(alpha_Fen)) * den / gear.m
            - zn * cos(T - theta)
            - (G / cos(theta) - rho_fp / gear.m)
        )
        rhofom = rho_fp / gear.m + 2 * G**2 / (
            cos(theta) * (zn * cos(theta) ** 2 - 2 * G)
        )
        Y_F = 6 * hFeom * cos(alpha_Fen) / (sFnom**2 * cos(gear.alpha))
        L = sFnom / hFeom
        qs = sFnom / rhofom / 2
        Y_S = (1.2 + 0.13 * L) * qs ** (1 / (1.21 + 2.3 / L))
        Y_DT = 1.0  # For classes > 4
    ...
    def mesh(self, at, groups=None):
        ...
        if abs(self.disp) < self.stretch_margin:
            ...
            stretch = (
                sign
                * (self.height + self.gears.p.stretch + 2 * self.stretch_margin)
                / 2
            )
        ...
        at.dot(
            translation_matrix([self.ap, 0, sign * self.gears.p.stretch / 2]), out=at
        )
