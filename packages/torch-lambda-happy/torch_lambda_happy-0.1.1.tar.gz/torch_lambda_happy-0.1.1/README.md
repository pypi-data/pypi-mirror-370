...
A high-performance CPU/GPU solver for estimating the lambda_happy factor in sparse linear models (≈99% sparsity) using C++/CUDA and PyTorch, compatible with Windows, Linux, and macOS.
...
# Single estimate
lambda_ = solver.compute(m=5000)
print(f"λ ≈ {lambda_:.4f}")

# Multiple runs
lambdas_ = solver.compute_many(m=5000, nb_run=50)

# Aggregated (mean)
lambda_mean = solver.compute_agg(m=5000, nb_run=50, func=torch.mean)
```

## Recommended use case (single estimation)
...
## Recommended use case (many estimations)
...
## Recommended use case (aggregated estimation)
...
```py
    ...
    matX = torch.randn(1_000, 1_000, device="cuda")  # Already in cuda
    model = LambdaHappy(X=matX, force_fastest=True, use_multigpu=True)
    lambda_ = model.compute_agg(m=10_000, nb_run=100)
    print(f"Estimated λ: {lambda_:.4f}")
```
