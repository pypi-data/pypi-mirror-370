...
class Factorial:
    ...
    def summary(self, confidence_level: float = 0.95) -> None:
        sst_dict = self.sst()
        ...
        for factor, values in self.ssf().items():
            ...
            for key, value in values.items():
                sst_val = sst_dict.get(key, 1.0)  # Evitar divisão por zero
                percent = (value / sst_val) * 100 if sst_val != 0 else 0
                print(f"  {key}: {value:.2f} ({percent:.2f}%)")

        for interaction, values in self.ssi().items():
            print(f"{interaction}:")
            for key, value in values.items():
                sst_val = sst_dict.get(key, 1.0)
                percent = (value / sst_val) * 100 if sst_val != 0 else 0
                print(f"  {key}: {value:.2f} ({percent:.2f}%)")
        ...
        for key, value in sst_dict.items():
            print(f"{key}: {value:.2f} (100.00%)")
        ...
        for effect_name, resp_dict in CI.items():
            ...
            for resp_name, (lower, estimate, upper) in resp_dict.items():
                print(f"  '{resp_name}': [{lower}, {estimate}, {upper}]")
