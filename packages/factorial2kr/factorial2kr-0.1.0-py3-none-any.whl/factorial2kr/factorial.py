...
class Factorial:
    ...
    def _calculate_ss(self, means: np.ndarray, grand_mean: float, n_obs: int) -> float:
    ...
    def ssy(self) -> dict[str, float]:
        ...
        for i, resp_name in enumerate(self.data.response_names):
            ...
            result[resp_name] = float(np.sum(y**2))
        return result

    def ss0(self) -> dict[str, float]:
        """
        ...
        for i, resp_name in enumerate(self.data.response_names):
            ...
            mean_y = np.mean(y)
            n_obs = y.size
            result[resp_name] = float(n_obs * mean_y**2)
        return result

    def sst(self) -> dict[str, float]:
        """
    ...
    def sse(self) -> dict[str, float]:
        ...
        for i, resp_name in enumerate(self.data.response_names):
            ...
            result[resp_name] = float(np.sum((y - mean_per_comb) ** 2))

        return result

    def ssf(self) -> dict[str, dict[str, float]]:
        ...
        for factor_idx, factor in enumerate(self.data.factor_names):
            ...
            for resp_idx, resp_name in enumerate(self.data.response_names):
                ...
                result[factor][resp_name] = float(ss)

        return result

    def ssi(self) -> dict[str, dict[str, float]]:
        ...
        for order in range(2, n_factors + 1):
            for combo in combinations(range(n_factors), order):
                ...
                for resp_idx, resp_name in enumerate(self.data.response_names):
                    ...
                    result[interaction_name][resp_name] = float(ss)

        return result

    def variance_explained(self) -> dict[str, dict[str, float]]:
    ...
    def se(self) -> dict[str, float]:
        ...
        for response, sse_value in sse_dict.items():
            se_dict[response] = np.sqrt(sse_value / denominator)

        return se_dict

    def sq_i(self) -> dict[str, float]:
        ...
        s_qi_dict = {}
        for response, se_value in se_dict.items():
            s_qi_dict[response] = se_value / np.sqrt(denominator)
    ...
    def ci(self, confidence_level: float = 0.95) -> dict[str, tuple[float | np.ndarray, float | np.ndarray, float | np.ndarray]]:
        """
        Calcula intervalos de confiança para o efeito médio e todos os efeitos fatoriais.
        
        Returns:
            Dicionário onde:
            - Chave: nome do efeito ('mean', 'A', 'B', 'AB', etc.)
            - Valor: tupla (limite_inferior, ponto_estimado, limite_superior)
        """
        # Efeito médio
        q_0_list = self.mean_responses()   # list[np.ndarray]
        q_0 = np.mean(q_0_list)            # valor médio
        alpha, _ = self.distribution(confidence_level)     # valor crítico (z ou t) + tipo
        s_q0_dict = self.sq_i()            # dict[str, float]
        s_q0 = s_q0_dict.get("mean", 1.0)  # erro padrão do efeito médio
        
        ci_dict: dict[str, tuple[float | np.ndarray, float | np.ndarray, float | np.ndarray]] = {
            "mean": (
                np.round(q_0 - alpha * s_q0, 2),
                q_0,
                np.round(q_0 + alpha * s_q0, 2),
            )
        }
        ...
        # Efeitos fatoriais
        effects = self.effects()           # dict[str, np.ndarray]
        for effect_name, effect_value in effects.items():
            s_qi = s_q0_dict.get(effect_name, 1.0)  # erro padrão específico
            ci_dict[effect_name] = (
                np.round(effect_value - alpha * s_qi, 2),
                effect_value,
                np.round(effect_value + alpha * s_qi, 2),
            )
    ...
    def effects(self) -> dict[str, np.ndarray]:
        ...
        """
        # Obtém as médias das respostas (shape: [n_responses, n_runs])
        mean_resps = np.array(self.mean_responses())  # Converte para array numpy

        # Obtém os fatores e interações
        factors = self.data.factors
        interactions = self.get_interactions()

        n_runs = factors.shape[0]
        scaling_factor = n_runs / 2  # Fator de escala correto

        effects = {}

        # 1. Calcula efeitos principais (para cada fator)
        for i, factor_name in enumerate(self.data.factor_names):
            contrast = factors[:, i]
            # Calcula para todas as respostas simultaneamente
            effect = np.sum(contrast * mean_resps) / n_runs
            effects[factor_name] = effect

        # 2. Calcula efeitos de interação
        for interaction_name, contrast in interactions.items():
            # Calcula para todas as respostas simultaneamente
            effect = np.sum(contrast * mean_resps) / n_runs
            effects[interaction_name] = effect
    ...
    def sumarry(self, confidense_level: float = 0.95) -> None:
        ...
        for term, (mean, lower, upper) in CI.items():
            print(f"{term}: Mean = {mean:.2f}, CI = [{lower:.2f}, {upper:.2f}]")
