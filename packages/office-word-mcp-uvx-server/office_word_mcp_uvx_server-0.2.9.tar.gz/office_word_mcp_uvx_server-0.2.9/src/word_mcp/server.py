...
一个基于 FastMCP 框架的 Word 文档操作 MCP 服务器，允许 AI 代理创建、读取和修改 Word 文档。
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] in ['--version', '-v', 'version']:
        print(f"Office Word MCP Server v{get_version()}")
        return
...
def print_custom_help():
    ...
    print(f"""
╭─ Office Word MCP Server v{version} ─────────────────────────────────────────╮
│                                                                            │
│  🏢 Word 文档操作 MCP 服务器                                                 │
│  一个基于 FastMCP 框架的 Word 文档操作服务器，允许 AI 代理创建、读取和修改     │
│  Word 文档，无需安装 Microsoft Word。                                        │
│                                                                            │
│  🚀 技术栈:                                                                 │
│     • FastMCP: 2.10.6 - MCP 服务器框架                                     │
│     • python-docx: 0.8.11+ - Word 文档操作库                               │
...
📚 使用方法:
  启动 MCP 服务器:
    uvx --from office-word-mcp-uvx-server word-mcp-server

  查看版本:
    uvx --from office-word-mcp-uvx-server word-mcp-server --version
  ...
  MCP 配置示例:
    {{
      "mcpServers": {{
        "word-mcp": {{
          "command": "uvx",
          "args": ["--from", "office-word-mcp-uvx-server", "word-mcp-server"]
        }}
      }}
    }}
