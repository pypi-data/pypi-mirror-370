...
def _test_multiprocessing_worker(x: int) -> str:
    """Simple test function for multiprocessing capability."""
    return f"Worker {os.getpid()} processed {x}"
...
class SQLAlchemyDatabase:
    ...
    def handle_insertion_multi_line_copy_parallel(
        ...
        *,
        ...
        logger.info(f"About to start {num_workers} workers to process {len(chunks)} chunks")
        logger.info(f"Chunk sizes: {[chunk[1].height for chunk in chunks[:5]]}{'...' if len(chunks) > 5 else ''}")
        
        # Test if multiprocessing works at all
        try:
            logger.info("Testing multiprocessing with simple function...")
            with mp.Pool(processes=2) as test_pool:
                test_results = list(test_pool.map(_test_multiprocessing_worker, [1, 2]))
                logger.info(f"Multiprocessing test successful: {test_results}")
        except Exception as e:
            logger.error(f"Multiprocessing test failed: {e}")
            logger.info("Falling back to single-threaded COPY method")
            return self.handle_insertion_multi_line_copy(
                model_class, data, mode, chunk_size=chunk_size, returning_id=returning_id
            )

        # Use multiprocessing Pool with imap_unordered for real-time updates
        try:
            with mp.Pool(processes=num_workers) as pool:
                logger.info(f"Pool created successfully with {num_workers} processes")
                ...
                # Add timeout for first result to detect worker startup issues
                import signal
                
                def timeout_handler(signum, frame):
                    raise TimeoutError("Timeout waiting for first worker result")
                
                # Set up a timeout for the first result (60 seconds)
                ...
                try:
                    for result in pool.imap_unordered(worker_func, chunks):
                        if not first_result_received:
                            logger.info("First worker result received - workers are functioning")
                            first_result_received = True
                            if hasattr(signal, 'SIGALRM'):
                                signal.alarm(0)  # Cancel timeout
                        
                        batch_size, id_df, worker_times = result
                        all_worker_times.append(worker_times)
                        completed_count += 1
                        ...
                        if completed_count % 10 == 0:
                            logger.info(f"Completed {completed_count}/{len(chunks)} chunks")
                            
                except TimeoutError:
                    logger.error("Timeout waiting for worker results - falling back to single-threaded processing")
                    pool.terminate()
                    pool.join()
                    
                    # Fallback to single-threaded COPY
                    logger.info("Falling back to single-threaded COPY method")
                    return self.handle_insertion_multi_line_copy(
                        model_class, data, mode, chunk_size=chunk_size, returning_id=returning_id
                    )
                ...
                logger.info(f"All {completed_count} chunks completed successfully")
        ...
        # Aggregate and log performance statistics
        if all_worker_times:
            # Calculate averages across all workers
            avg_times = {}
            for key in all_worker_times[0].keys():
                avg_times[key] = sum(worker[key] for worker in all_worker_times) / len(all_worker_times)
            
            total_avg_time = sum(avg_times.values())
            total_rows = data.height
            
            logger.info(
                f"Parallel COPY Performance Breakdown (average per worker) for {total_rows:,} rows with {num_workers} workers:\n"
                f"  CSV generation:   {avg_times['csv_gen']:.3f}s ({avg_times['csv_gen']/total_avg_time*100:.1f}%)\n"
                f"  COPY to temp:     {avg_times['copy_data']:.3f}s ({avg_times['copy_data']/total_avg_time*100:.1f}%)\n"
                f"  INSERT upsert:    {avg_times['insert']:.3f}s ({avg_times['insert']/total_avg_time*100:.1f}%)\n"
                f"  Fetch IDs:        {avg_times['fetch_ids']:.3f}s ({avg_times['fetch_ids']/total_avg_time*100:.1f}%)\n"
                f"  Temp table ops:   {avg_times['temp_table_ops']:.3f}s ({avg_times['temp_table_ops']/total_avg_time*100:.1f}%)\n"
                f"  Avg worker time:  {total_avg_time:.3f}s\n"
                f"  Workers used:     {len(all_worker_times)}\n"
                f"  Parallel efficiency: {len(all_worker_times) * total_avg_time:.1f}s total worker time"
            )
...
def _copy_worker_static(
    ...
    *,
    ...
    cols: list[str],
    returning_id: bool,
) -> tuple[int, pl.DataFrame | None, dict[str, float]]:
    """
    ...
    Returns:
        tuple: (batch_size, id_dataframe_or_none, timing_stats)
    ...
    print(f"Worker {worker_pid} starting: processing offset {offset}, batch size {batch_size}")
    
    # Performance tracking
    worker_times = {"csv_gen": 0, "copy_data": 0, "insert": 0, "fetch_ids": 0, "temp_table_ops": 0}

    try:
        print(f"Worker {worker_pid}: Creating database connection...")
        ...
        cursor = raw_conn.cursor()
        print(f"Worker {worker_pid}: Database connection established successfully")
        ...
        # Time temp table creation
        start = time.time()
        cursor.execute(
            f'CREATE TEMP TABLE "{temp_table}" AS SELECT {non_id_col_list} FROM "{table}" WHERE FALSE'  # noqa: S608
        )
        worker_times["temp_table_ops"] += time.time() - start

        # 1) Time CSV generation and COPY data into staging
        start = time.time()
        buf = io.StringIO()
        batch.write_csv(buf)
        buf.seek(0)
        worker_times["csv_gen"] += time.time() - start
        
        start = time.time()
        cursor.copy_expert(f'COPY "{temp_table}" ({non_id_col_list}) FROM STDIN WITH CSV HEADER', buf)
        worker_times["copy_data"] += time.time() - start

        # 2) Time INSERT FROM staging into real table
        start = time.time()
        insert_sql = (
        ...
        cursor.execute(insert_sql)
        worker_times["insert"] += time.time() - start
        ...
        if returning_id:
            ...
            worker_times["fetch_ids"] += time.time() - start
        ...
        return (batch_size, id_df, worker_times)
    finally:
        cursor.close()
        raw_conn.close()
