...
class WSI:
    ...
    def extract_patches(
        ...
        Args:
            ...
            percentages: The tuple of percentages of pixels below/above the thresholds to consider the patch as
                black/white.
            num_workers: The number of workers to use for the extraction.
        """
        assert all(
    ...
    def extract_patches_roi(
        ...
        if num_workers > 1:
            ...
            log.info(f"Using {num_workers} workers for patch extraction.")
            pool = mp.Pool(
                num_workers,
            ...
            log.info("Using single process for patch extraction.")
            valid_coords = []
            for coord in coord_candidates:
                result = WSI.process_coord_candidate(coord, cont_check_fn, patch_level, patch_size, rgb_threshs, percentages)
                valid_coords.append(result)
