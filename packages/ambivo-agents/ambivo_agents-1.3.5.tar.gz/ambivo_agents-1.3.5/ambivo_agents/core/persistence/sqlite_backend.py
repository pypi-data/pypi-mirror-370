...
class SQLiteWorkflowPersistence:
    ...
    async def create_checkpoint(self, conversation_id: str, step_id: str, state_snapshot: Dict[str, Any]) -> Optional[str]:
    ...
    async def save_session_data(self, session_id: str, user_id: str, session_data: Dict[str, Any]) -> bool:
        """Save session data"""
        await self.initialize()
    ...
    async def get_database_stats(self) -> Dict[str, Any]:
        ...
        try:
            async with aiosqlite.connect(self.db_path, timeout=self.timeout) as db:
                ...
                # Get table counts
                for table_name in [self.conversations_table, self.steps_table, 
                                 self.checkpoints_table, self.sessions_table]:
