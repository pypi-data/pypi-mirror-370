...
from alea_llm_client.llms.utils.api_keys import get_openai_compatible_api_key
...
class OpenAICompatibleModel(BaseAIModel):
    ...
    def get_api_key(self) -> str:
        ...
        Returns:
            str: The API key.
        """
        return get_openai_compatible_api_key(self.init_kwargs, self.endpoint)
    ...
    def _make_request(self, *args: Any, **kwargs: Any) -> httpx.Response:
        ...
        if "reasoning_effort" in kwargs:
            ...
            if effort in ["minimal", "low", "medium", "high"]:
                ...
                raise ValueError(f"Invalid reasoning_effort '{effort}'. Must be one of: minimal, low, medium, high")
        ...
        if "verbosity" in kwargs:
            ...
            if verbosity in ["low", "medium", "high"]:
                ...
                raise ValueError(f"Invalid verbosity '{verbosity}'. Must be one of: low, medium, high")
    ...
    def _get_chat_choices(response_data: dict) -> list[str | dict]:
        ...
        Returns:
            The response choices.
        """
        return [choice.get("message", {}).get("content", "") for choice in response_data.get("choices", [])]
    ...
    def _json(self, *args: Any, **kwargs: Any) -> JSONModelResponse:
        ...
        if completion:
            prompt = kwargs.pop("prompt", None) or (args[0] if len(args) > 0 else None)
            if not prompt:
                raise ValueError("No prompt provided for JSON completion in completion mode.")
    ...
    def _handle_responses_response(self, response: httpx.Response) -> ModelResponse:
        ...
        if response_data.get("output"):
            for output_item in response_data["output"]:
                ...
                if output_type == "message" and "content" in output_item and output_item["content"]:
    ...
    def _make_responses_request(self, *args: Any, **kwargs: Any) -> httpx.Response:
        ...
        try:
            ...
            self.logger.exception(f"Error in responses request: {e!s}")
            raise ALEAModelError(f"Error in responses request: {e!s}") from e

    async def _make_responses_request_async(self, *args: Any, **kwargs: Any) -> httpx.Response:
        """
        ...
        try:
            ...
            self.logger.exception(f"Error in async responses request: {e!s}")
            raise ALEAModelError(f"Error in async responses request: {e!s}") from e
    ...
    def _responses(self, *args: Any, **kwargs: Any) -> ModelResponse:
        ...
        return self._handle_responses_response(self._make_responses_request(*args, **kwargs))
    ...
    async def _responses_async(self, *args: Any, **kwargs: Any) -> ModelResponse:
        ...
        return self._handle_responses_response(await self._make_responses_request_async(*args, **kwargs))
