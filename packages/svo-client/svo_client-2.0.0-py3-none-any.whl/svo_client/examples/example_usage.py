...
async def main():
    ...
    async with ChunkerClient() as client:
        ...
        for i, test_case in enumerate(test_cases, 1):
            ...
            try:
                chunks = await client.chunk_text(test_case['text'], **test_case['params'])
                print(f"Generated {len(chunks)} chunks:")
                for j, chunk in enumerate(chunks):
                    print(f"  Chunk {j+1}: {chunk.text[:50]}... (ordinal: {chunk.ordinal})")
                ...
                print(f"Texts match: {reconstructed.strip() == test_case['text'].strip()}")
    ...
    try:
        ...
        for i, chunk in enumerate(chunks_with_embeddings):
            ...
            print(f"  Chunk {i+1}: {chunk.body[:50]}... (embedding: {embedding_length} dimensions)")
    ...
    try:
        ...
        for window in [1, 3, 5]:
            chunks = await client.chunk_text(
                "This text will be chunked with different window sizes to demonstrate the effect on chunking granularity.",
                window=window,
                type="Message",
                language="en"
            )
        ...
        for chunk_type in chunk_types:
            ...
            print(f"  Type {chunk_type}: {len(chunks)} chunks, first chunk type: {chunks[0].type}")
    ...
    try:
        ...
        chunks = await client.chunk_text(long_text[:10000], type="Draft")  # Limit to 10k chars
    ...
    try:
        chunks = await client.chunk_text(
            "Testing metadata validation and completeness.",
            type="Task",
            language="en"
        )
        ...
        for i, chunk in enumerate(chunks):
            ...
            if chunk.metrics:
                print(f"    Tokens: {len(chunk.metrics.tokens) if chunk.metrics.tokens else 0}")
            print()
