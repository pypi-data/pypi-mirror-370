        ...
        try:
            ...
            while retry_count <= max_retries:
                try:
                    ...
                    # Push to remote with force push as fallback
                    origin = self.repo.remote('origin')
                    ...
                    try:
                        ...
                        if "rejected" in str(push_error).lower() or "non-fast-forward" in str(push_error).lower():
                            print(f"⚠ Normal push failed, trying force push...")
                            origin.push(refspec=f'{branch}:{branch}', force=True)
                            print(f"✓ Force pushed {item_type}: {item_path} to remote/{branch}")
                            return True
                        else:
                            raise push_error
