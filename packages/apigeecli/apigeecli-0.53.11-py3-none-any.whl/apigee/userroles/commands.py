...
def _add_a_user_to_a_role(username, password, mfa_secret, token, zonename, org,
                          profile, name, user_email, **kwargs):
    return (Userroles(
        generate_authentication(username, password, mfa_secret, token,
                                zonename), org,
        name).add_a_user_to_a_role(user_email).text)
...
def add_permissions(*args, **kwargs):
    console.echo(
        _add_permissions_for_a_resource_to_a_user_role(*args, **kwargs))
...
def add_permissions_multiple(*args, **kwargs):
    console.echo(
        _add_permissions_for_multiple_resources_to_a_user_role(
            *args, **kwargs))


def _create_a_user_role_in_an_organization(username, password, mfa_secret,
                                           token, zonename, org, profile,
                                           names, **kwargs):
    return (Userroles(
        generate_authentication(username, password, mfa_secret, token,
                                zonename), org,
        None).create_a_user_role_in_an_organization(names).text)
...
@userroles.command(
    help=
    "Removes a permission from a resource for the role specified. Permissions are case sensitive. Specify the permission as get, put, or delete."
)
@common_auth_options
@common_verbose_options
@common_silent_options
...
def _delete_resource_from_permissions(username, password, mfa_secret, token,
                                      zonename, org, profile, name,
                                      resource_path, **kwargs):
    return (Userroles(
        generate_authentication(username, password, mfa_secret, token,
                                zonename), org,
        name).delete_resource_from_permissions(resource_path).text)


@userroles.command(
    help=
    "Removes all permissions for a resource for the role specified. Permissions are case sensitive."
)
@common_auth_options
@common_verbose_options
@common_silent_options
...
def _delete_a_user_role(username, password, mfa_secret, token, zonename, org,
                        profile, name, **kwargs):
    return (Userroles(
        generate_authentication(username, password, mfa_secret, token,
                                zonename), org,
        name).delete_a_user_role().text)
...
def _get_a_role(username, password, mfa_secret, token, zonename, org, profile,
                name, **kwargs):
    return (Userroles(
        generate_authentication(username, password, mfa_secret, token,
                                zonename), org, name).get_a_role().text)
...
def _get_resource_permissions_for_a_specific_role(username,
                                                  password,
                                                  mfa_secret,
                                                  token,
                                                  zonename,
    ...
    return (Userroles(
        generate_authentication(username, password, mfa_secret, token,
                                zonename), org,
        name).get_resource_permissions_for_a_specific_role(
...
def get_permissions(*args, **kwargs):
    console.echo(_get_resource_permissions_for_a_specific_role(
        *args, **kwargs))


def _get_users_for_a_role(username, password, mfa_secret, token, zonename, org,
                          profile, name, **kwargs):
    return (Userroles(
        generate_authentication(username, password, mfa_secret, token,
                                zonename), org,
        name).get_users_for_a_role().text)
...
def _list_permissions_for_a_resource(username, password, mfa_secret, token,
                                     zonename, org, profile, name, **kwargs):
    return (Userroles(
        generate_authentication(username, password, mfa_secret, token,
                                zonename), org,
        name).list_permissions_for_a_resource().text)
...
def _list_user_roles(username, password, mfa_secret, token, zonename, org,
                     profile, **kwargs):
    return (Userroles(
        generate_authentication(username, password, mfa_secret, token,
                                zonename), org, None).list_user_roles().text)
...
def _remove_user_membership_in_role(username, password, mfa_secret, token,
                                    zonename, org, profile, name, user_email,
                                    **kwargs):
    return (Userroles(
        generate_authentication(username, password, mfa_secret, token,
                                zonename), org,
        name).remove_user_membership_in_role(user_email).text)
...
def verify_permission(*args, **kwargs):
    console.echo(
        _verify_a_user_roles_permission_on_a_specific_RBAC_resource(
            *args, **kwargs))


def _verify_user_role_membership(username, password, mfa_secret, token,
                                 zonename, org, profile, name, user_email,
                                 **kwargs):
    return (Userroles(
        generate_authentication(username, password, mfa_secret, token,
                                zonename), org,
        name).verify_user_role_membership(user_email).text)
