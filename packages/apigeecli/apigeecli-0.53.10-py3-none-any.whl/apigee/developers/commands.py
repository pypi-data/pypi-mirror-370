...
def _create_developer(username,
                      password,
                      mfa_secret,
                      token,
                      zonename,
    ...
    return (Developers(
        generate_authentication(username, password, mfa_secret, token,
                                zonename), org,
        name).create_developer(first_name,
                               last_name,
                               user_name,
...
@click.option(
    ...
    "--name",
    help=
    "The developer's email. This value is used to uniquely identify the developer in Apigee Edge.",
    required=True,
)
@click.option("--first-name",
              help="The first name of the developer.",
              required=True)
@click.option("--last-name",
              help="The last name of the developer.",
              required=True)
@click.option(
    "--user-name",
...
def _delete_developer(username, password, mfa_secret, token, zonename, org,
                      profile, name, **kwargs):
    return (Developers(
        generate_authentication(username, password, mfa_secret, token,
                                zonename), org, name).delete_developer().text)
...
def _get_developer(username, password, mfa_secret, token, zonename, org,
                   profile, name, **kwargs):
    return (Developers(
        generate_authentication(username, password, mfa_secret, token,
                                zonename), org, name).get_developer().text)
...
def _get_developer_by_app(username, password, mfa_secret, token, zonename, org,
                          profile, app_name, **kwargs):
    return (Developers(
        generate_authentication(username, password, mfa_secret, token,
                                zonename), org,
        None).get_developer_by_app(app_name).text)
...
def _list_developers(username,
    ...
    return Developers(
        generate_authentication(username, password, mfa_secret, token,
                                zonename), org,
        None).list_developers(prefix=prefix,
                              expand=expand,
                              count=count,
                              startkey=startkey)
...
@click.option(
    ...
    show_default=True,
    help=
    "Limits the list to the number you specify. Use with the startKey parameter to provide more targeted filtering. The limit is 1000.",
)
@click.option(
    "--startkey",
    default="",
...
def _set_developer_status(username, password, mfa_secret, token, zonename, org,
                          profile, name, action, **kwargs):
    return (Developers(
        generate_authentication(username, password, mfa_secret, token,
                                zonename), org,
        name).set_developer_status(action).text)
...
def _update_developer(username, password, mfa_secret, token, zonename, org,
                      profile, name, body, **kwargs):
    return (Developers(
        generate_authentication(username, password, mfa_secret, token,
                                zonename), org,
        name).update_developer(body).text)
...
def _update_a_developer_attribute(username, password, mfa_secret, token,
                                  zonename, org, profile, name, attribute_name,
...
def _delete_developer_attribute(username, password, mfa_secret, token,
                                zonename, org, profile, name, attribute_name,
                                **kwargs):
    return (Developers(
        generate_authentication(username, password, mfa_secret, token,
                                zonename), org,
        name).delete_developer_attribute(attribute_name).text)
...
def _get_all_developer_attributes(username, password, mfa_secret, token,
                                  zonename, org, profile, name, **kwargs):
    return (Developers(
        generate_authentication(username, password, mfa_secret, token,
                                zonename), org,
        name).get_all_developer_attributes().text)
...
def _update_all_developer_attributes(username, password, mfa_secret, token,
                                     zonename, org, profile, name, body,
                                     **kwargs):
    return (Developers(
        generate_authentication(username, password, mfa_secret, token,
                                zonename), org,
        name).update_all_developer_attributes(body).text)
