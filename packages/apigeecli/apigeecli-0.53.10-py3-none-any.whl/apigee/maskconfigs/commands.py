...
def _create_data_masks_for_an_api_proxy(username, password, mfa_secret, token,
                                        zonename, org, profile, name, body,
                                        **kwargs):
    return (Maskconfigs(
        generate_authentication(username, password, mfa_secret, token,
                                zonename), org,
        name).create_data_masks_for_an_api_proxy(body).text)
...
def _delete_data_masks_for_an_api_proxy(username, password, mfa_secret, token,
                                        zonename, org, profile, name,
                                        maskconfig_name, **kwargs):
    return (Maskconfigs(
        generate_authentication(username, password, mfa_secret, token,
                                zonename), org,
        name).delete_data_masks_for_an_api_proxy(maskconfig_name).text)
...
def _get_data_mask_details_for_an_api_proxy(username, password, mfa_secret,
                                            token, zonename, org, profile,
                                            name, maskconfig_name, **kwargs):
    return (Maskconfigs(
        generate_authentication(username, password, mfa_secret, token,
                                zonename), org,
...
def _list_data_masks_for_an_api_proxy(username, password, mfa_secret, token,
                                      zonename, org, profile, name, **kwargs):
    return (Maskconfigs(
        generate_authentication(username, password, mfa_secret, token,
                                zonename), org,
        name).list_data_masks_for_an_api_proxy().text)
...
def _list_data_masks_for_an_organization(username, password, mfa_secret, token,
                                         zonename, org, profile, **kwargs):
    return (Maskconfigs(
        generate_authentication(username, password, mfa_secret, token,
                                zonename), org,
        None).list_data_masks_for_an_organization().text)
...
def _push_data_masks_for_an_api_proxy(username, password, mfa_secret, token,
                                      zonename, org, profile, name, file,
                                      **kwargs):
    return Maskconfigs(
        generate_authentication(username, password, mfa_secret, token,
                                zonename), org,
        name).push_data_masks_for_an_api_proxy(file)


@maskconfigs.command(
    help=
    "Push data mask for an API proxy to Apigee. This will create/update a data mask."
)
@common_auth_options
@common_silent_options
@common_verbose_options
