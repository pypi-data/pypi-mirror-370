        ...
        for i, (response, calling_function, timing) in enumerate(zip(self.responses, self.call_stack, self.call_timestamps)):
            try:
                ...
                if usage:
                    ...
                    log_token_usage_to_db(self.request_id, self.customer_id, function_name, usage)
...
def log_token_usage_to_db(request_id, customer_id, function_name, usage):
    """
    Persist token usage with dynabits to Postgres. Ensures customer_id is stored.
    """
    ...
    with conn:
        with conn.cursor() as cur:
            cur.execute("""
                INSERT INTO dynameter_token_usage (
                    request_id,
                    customer_id,
                    function_name,
                    model,
...
def log_to_db(request_id, customer_id, function_name, duration, cpu_seconds, gb_seconds, detailed_cpu_metrics, resource_dynabits):
    db_config = get_db_config()  # Get config when actually needed
    conn = psycopg2.connect(**db_config)
    with conn:
        with conn.cursor() as cur:
            cur.execute("""
                INSERT INTO dynameter_request_metrics (
                    request_id,
                    customer_id,
                    function_name,
                    duration_sec,
...
def track_resources_db(request_id, customer_id):
    ...
    and logs everything into a PostgreSQL database.
    """
    def decorator(func):
        @functools.wraps(func)
        def wrapper(*args, **kwargs):
...
def get_token_usage_summary(request_id):
    """Get total token usage for a request from the database."""
    try:
        ...
        with conn:
            with conn.cursor() as cur:
                for domain, count in self.api_counter.items():
                    ...
                    cur.execute("""
                        INSERT INTO dynameter_api_usage (
                            request_id,
                            customer_id,
                            function_name,
                            api_domain,
...
def track_api_usage_db(request_id: str, customer_id: str):
    """Decorator to track third-party API usage and log to Postgres."""
    def decorator(func):
        @functools.wraps(func)
        def wrapper(*args, **kwargs):
...
def track_all_metrics_db(request_id: str, customer_id: str):
    """
    Combined decorator that tracks all metrics (tokens, resources, APIs) and logs to DB with dynabits.
    """
    def decorator(func):
        @functools.wraps(func)
...
def get_request_dynabits_summary(request_id: str) -> Dict[str, float]:
    """
    Get a comprehensive summary of dynabits for a specific request from the database.
    ...
    try:
        ...
        with conn:
            with conn.cursor() as cur:
                # Get comprehensive dynabits summary using the view
                cur.execute("""
                    SELECT 
                        COALESCE(function_total_dynabits, 0),
                        COALESCE(openai_api_dynabits, 0),
                        COALESCE(function_resource_dynabits, 0),
                        COALESCE(third_party_api_dynabits, 0),
                        COALESCE(total_billable_dynabits, 0)
                    FROM dynabits_summary 
                    WHERE request_id = %s
                """, (request_id,))
                result = cur.fetchone()
                
                if result:
                    function_total, openai_api, function_resource, third_party_api, total_billable = result
                else:
                    function_total = openai_api = function_resource = third_party_api = total_billable = 0
                
        conn.close()
        
        return {
            'total_dynabits': total_billable,
            'function_dynabits': function_total,
            'token_dynabits': openai_api,
            'resource_dynabits': function_resource,
            'third_party_api_dynabits': third_party_api,
            'individual_api_calls_dynabits': openai_api  # Individual OpenAI API call dynabits sum
        }
    except Exception as e:
