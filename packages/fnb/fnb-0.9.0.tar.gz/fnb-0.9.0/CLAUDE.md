...
fnb (Fetch'n'Backup) is a Python-based CLI tool (version 0.8.0) that provides a two-step backup workflow using `rsync`. The tool allows users to:
...
- `task pytest` - Run tests with coverage
- `task format` - Format code with ruff
- `task pre-commit` - Run all pre-commit hooks
- `task docs` - Serve documentation locally
- `task build-docs` - Build documentation as static HTML

### CI/CD Testing Tasks
- `task test-ci` - Run unit tests only (as they run in CI environment)
- `task test-integration` - Run integration tests only
- `task test-all` - Run all tests including integration tests
- `task ci-local` - Simulate complete CI pipeline locally (test → format → pre-commit)

### Version Management Tasks
- `task bump-dry` - Preview version bump and changelog changes
- `task bump` - Execute version bump with changelog update
...
- Version management is automated with commitizen (currently v0.4.1)
- Assets directory contains template files for configuration and environment setup

## Test Coverage Status
- **Overall Coverage**: 83% (Target: 80%+) ✅ **ACHIEVED**
