...
def _reset_policy_violations_directory() -> Path:
    """
    Reset the policy-violations directory for a fresh run.
    
    This function:
    1. Deletes the entire policy-violations/ directory if it exists
    2. Creates a fresh empty policy-violations/ directory
    3. Returns the path to the directory
    
    This ensures deterministic, clean output for every run.
    """
    import shutil
    import time
    
    policy_violations_dir = Path("policy-violations")
    
    # Delete existing directory completely
    if policy_violations_dir.exists():
        try:
            shutil.rmtree(policy_violations_dir)
            ...
            for attempt in range(3):
                ...
                try:
                    ...
                    if attempt == 2:
                        # Fall back to clearing contents only
                        for item in policy_violations_dir.iterdir():
                            if item.is_dir():
                                shutil.rmtree(item)
                            else:
                                item.unlink()
    ...
    policy_violations_dir.mkdir(parents=True, exist_ok=True)
    
    return policy_violations_dir


def _get_output_paths(detailed: bool) -> Tuple[Path, Path]:
    """
    Get deterministic output paths for policy check results.
    
    Args:
        detailed: Whether detailed report is requested
        
    Returns:
        Tuple of (report_path, traces_path)
    """
    policy_violations_dir = Path("policy-violations")
    
    # Always create report.md
    report_path = policy_violations_dir / "report.md"
    
    # Traces go in traces/ subdirectory if detailed
    traces_path = policy_violations_dir / "traces" / "violations-detailed.json"
    
    return report_path, traces_path
...
@click.option('--detailed', is_flag=True, help='Generate detailed JSON traces in addition to markdown report')
...
def policy_check(logfile: Path, policy_template: Optional[str] = None, policy_file: Optional[Path] = None, 
                 ...
                 detailed: bool = False, quiet: bool = False, no_content: bool = False, strip_pii: bool = False):
    """🔍 Check logs against policy rules and generate clean reports
    ...
    📁 Output Structure (always fresh):
    - policy-violations/report.md (always created)
    - policy-violations/traces/ (only with --detailed)
    
    🧹 Reset Behavior:
    The policy-violations/ directory is completely deleted and recreated on every run
    for deterministic, CI/CD-friendly output.
    """
    ...
    try:
        ...
        # Reset and prepare output directories
        policy_violations_dir = _reset_policy_violations_directory()
        report_path, traces_path = _get_output_paths(detailed)
        ...
        markdown_writer.generate_report(report_path)
        
        # Generate detailed JSON traces if requested
        if detailed:
            ...
            json_writer.generate_report(traces_path)
        ...
        if quiet:
            click.echo(f"Saved summary to {report_path}")
            if detailed:
                click.echo(f"Saved detailed traces to {traces_path.parent}/")
        else:
            click.echo(f"📄 Saved summary to {report_path}")
            if detailed:
                click.echo(f"📊 Saved detailed traces to {traces_path.parent}/")
