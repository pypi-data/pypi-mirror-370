...
class JWTConfig:
    ...
    def __init__(self, public_key_url: str=settings.public_key_url):
...
class AuthConfig:
    ...
    def __init__(self):
        ...
        self.unauthorized_message = settings.unauthorized_message
        self.cache_enabled = settings.cache_enabled
        self.cache_ttl_seconds = settings.cache_ttl_seconds
        self.token_header = settings.token_header
        self.token_prefix = settings.token_prefix
        self.algorithm = settings.algorithm


class AuthMiddleware(BaseHTTPMiddleware):
    """Auth middleware"""
    def __init__(self, app, access_controller: AccessController = None):
        super().__init__(app)
...
class ValidateToken:
    ...
    def get_claims_from_token(self, token: str):
        ...
        try:
            ...
            logger.debug("Successfully get public key", public_key)
            if not public_key:
                raise HTTPException(status_code=401, detail="Invalid public key")
            logger.info("Successfully get public key")
            decode_token = jwt.decode(token, public_key, algorithms=[self.config.algorithm], options={"verify_signature": True})
            logger.debug("Successfully decode token", decode_token)
            return decode_token
        except jwt.ExpiredSignatureError:
...
def protected(request: Request):
    ...
    # # Create custom access controller
    # custom_access_controller = AccessController()

    # # Example 1: Add custom public routes
    # custom_access_controller.add_public_route("/custom-public")
    # custom_access_controller.add_public_route("/api/status")

    # # Example 2: Set route-specific permissions
    # custom_access_controller.set_route_permission(
    #     "/admin/users", 
    ...
    # custom_access_controller.set_route_permission(
    #     "/api/data", 
    #     required_permissions=[RoleEnum.READ]
    # )

    # custom_access_controller.set_route_permission(
    #     "/api/modify", 
    #     required_permissions=[RoleEnum.WRITE, RoleEnum.UPDATE]
    # )

    # # Example 3: Custom role permissions
    # custom_access_controller.set_role_permissions("moderator", [RoleEnum.READ, RoleEnum.WRITE])
    # custom_access_controller.set_role_permissions("viewer", [RoleEnum.READ])

    # # Example 4: Custom access function
    # def my_custom_access_logic(url: str, claims: dict = None) -> bool:
    #     """
    #     Custom access control logic - users can implement their own rules
    #     """
    #     # Allow access to documentation
    #     if url.startswith("/docs") or url.startswith("/redoc"):
    #         return True
        
    #     # Special logic for API versioning
    #     if url.startswith("/api/v1/"):
    #         # v1 API requires at least user role
    #         if claims and "user" in claims.get("roles", []):
    #             return True
    #         return False
        
    #     # Special business logic
    #     if url == "/special-endpoint":
    #         # Only allow access during business hours or for admin users
    ...
    #     return None  # Let the default AccessController logic handle it

    # # Set the custom function
    # custom_access_controller.set_custom_access_function(my_custom_access_logic)

    # # Add the middleware with custom access controller
    # app.add_middleware(AuthMiddleware, access_controller=custom_access_controller)

    # # Routes for testing
    # @app.get("/")
    # def root():
    #     return {"message": "Custom Access Control API"}

    # @app.get("/custom-public")
    # def custom_public():
    #     return {"message": "This is a custom public route"}
    ...
    #     user_claims = get_current_user(request)
    #     return {"message": "Admin users endpoint", "user": user_claims}

    # @app.get("/api/data")
    # def get_data(request: Request):
    #     user_claims = get_current_user(request)
    #     return {"data": "Some data", "user": user_claims}

    # @app.get("/api/modify")
    # def modify_data(request: Request):
    #     user_claims = get_current_user(request)
    #     return {"message": "Data modified", "user": user_claims}

    # @app.get("/api/v1/test")
    # def v1_test(request: Request):
    #     user_claims = get_current_user(request)
    #     return {"message": "V1 API endpoint", "user": user_claims}

    # @app.get("/special-endpoint")
    # def special_endpoint(request: Request):
    #     user_claims = get_current_user(request)
    #     return {"message": "Special endpoint with time-based access", "user": user_claims}
