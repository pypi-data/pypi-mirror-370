...
def validate_account_id(customer_id: str) -> str:
    """Validate that the customer_id is in the configured account list, or return it if no restrictions."""
    formatted_id = format_customer_id(customer_id)
    configured_accounts = get_configured_account_ids()
    
    # If no accounts are configured, allow any account
    if not configured_accounts:
        logger.info(f"No account restrictions configured, allowing access to account {formatted_id}")
        return formatted_id
    ...
    if formatted_id in configured_accounts:
        ...
        raise ValueError(f"Account {formatted_id} is not in the configured account list: {configured_accounts}")
...
async def list_configured_accounts() -> str:
    ...
    Shows which account IDs are configured in the GOOGLE_ADS_ACCOUNT_IDS environment variable.
    If no accounts are configured, all accessible accounts can be used.
    ...
    if not configured_accounts:
        return """No specific accounts configured in GOOGLE_ADS_ACCOUNT_IDS.
        
This means you can access any Google Ads account your credentials have access to.
Use the list_accounts() tool to see all accessible accounts.

To restrict access to specific accounts, set the GOOGLE_ADS_ACCOUNT_IDS environment variable
with a comma-separated list of account IDs (e.g., "1234567890,0987654321")."""
    ...
    result_lines.append("")
    result_lines.append("Note: You can only use these account IDs with other tools.")
    result_lines.append("To access different accounts, update the GOOGLE_ADS_ACCOUNT_IDS environment variable.")
