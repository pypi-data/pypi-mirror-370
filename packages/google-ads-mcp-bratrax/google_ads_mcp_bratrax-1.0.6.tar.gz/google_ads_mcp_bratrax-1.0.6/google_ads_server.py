...
Please set the GOOGLE_ADS_ACCOUNT_IDS environment variable with a comma-separated 
list of account IDs you want to access."""
    
    try:
        creds = get_credentials()
        headers_base = get_headers(creds)
        
        # First, try to get accessible customers to potentially get descriptive names
        ...
        for i, account_id in enumerate(configured_accounts, 1):
            ...
            # Method 1: Try direct customer query
            try:
                customer_query = """
                    SELECT
                ...
                customer_response = requests.post(customer_url, headers=headers_base, json=customer_payload)
            ...
            try:
                ...
                campaign_response = requests.post(campaign_url, headers=headers_base, json=campaign_payload)
                
                if campaign_response.status_code == 200:
                    campaign_data = campaign_response.json()
                    has_campaigns = len(campaign_data.get('results', [])) > 0
            ...
            if account_id in accessible_customers:
                ...
                result_lines.append(f"   Status: ⚠️  May need manager account access")
            
            result_lines.append("-" * 60)
        
        result_lines.append(f"\nTotal configured accounts: {len(configured_accounts)}")
        result_lines.append("\nNote: If account names show 'Name not available', these may be")
        result_lines.append("client accounts that require manager account credentials.")
