...
def parse_account_ids(account_ids_str: str) -> List[str]:
    """Parse comma-separated account IDs and format them properly."""
    ...
    for account_id in account_ids_str.split(','):
        ...
        if account_id:
            formatted_id = format_customer_id(account_id)
            account_ids.append(formatted_id)
...
def get_headers(creds):
    ...
    # Note: We no longer use login-customer-id header since we work with accounts directly
    # Each API call will specify the customer_id in the URL path instead
...
async def list_accounts() -> str:
    ...
    This shows only the account IDs. For detailed information including names,
    use the get_account_details() tool instead.
    
    Returns:
        A formatted list of configured Google Ads account IDs
    """
    configured_accounts = get_configured_account_ids()
...
list of account IDs you want to access (e.g., "1234567890,0987654321").
...
Without configured accounts, this server cannot access any Google Ads data."""
    ...
    for i, account_id in enumerate(configured_accounts, 1):
        result_lines.append(f"{i}. Account ID: {account_id}")
...
list of account IDs you want to access."""
    ...
    try:
        ...
        accessible_response = requests.get(accessible_url, headers=headers)
...
async def test_account_access(
    ...
    try:
        ...
        try:
            accessible_url = f"https://googleads.googleapis.com/{API_VERSION}/customers:listAccessibleCustomers"
            response = requests.get(accessible_url, headers=headers)
