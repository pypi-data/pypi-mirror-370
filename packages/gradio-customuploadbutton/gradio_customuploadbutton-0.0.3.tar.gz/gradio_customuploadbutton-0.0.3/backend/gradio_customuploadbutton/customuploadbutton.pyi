...
class CustomUploadButton(Component):
    """
    ...
    def __init__(
        ...
        *,
        ...
        Parameters:
            ...
            interactive: If False, the CustomUploadButton will be in a disabled state.
    ...
    def _process_single_file(self, f: FileData) -> bytes | NamedString:
        src = f.path  # 现在是 URL
        # 用原始文件名优先做类型校验，退化到 URL
        name_for_validation = getattr(f, "orig_name", None) or src

        # 如是 URL，则先下载到缓存目录
        if client_utils.is_http_url_like(src):
            local_path = processing_utils.save_url_to_cache(
                src, self.GRADIO_CACHE)
        else:
            local_path = src

        if self.type == "filepath":
            if self.file_types and not client_utils.is_valid_file(
                    name_for_validation, self.file_types):
                raise Error(
                    f"Invalid file type. Please upload a file that is one of these formats: {self.file_types}"
                )
            # 直接返回缓存后的本地路径
            return NamedString(local_path)
    ...
    def preprocess(
        ...
        if isinstance(payload, ListFiles):
            return [self._process_single_file(f)
                    for f in payload]  # type: ignore
    ...
    def postprocess(
            self,
            value: str | list[str] | None) -> ListFiles | FileData | None:
        """
        Parameters:
