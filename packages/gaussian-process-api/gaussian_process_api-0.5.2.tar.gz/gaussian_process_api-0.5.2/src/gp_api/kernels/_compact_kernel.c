...
static PyObject *_compact_kernel_eval(PyObject *self, PyObject *args) {
    ...
    for (i = 0; i < npts_x; i++) {
        for (j = 0; j < npts_x; j++) {
            ...
            double r = 0;
            for (k = 0; k < ndim; k++) {
                double *unprimed_ptr = PyArray_GETPTR2(x_scaled_array, i, k);
                double *primed_ptr = PyArray_GETPTR2(x_scaled_array, j, k);
                r += pow(*unprimed_ptr - *primed_ptr, 2);
            }
            r = sqrt(r);
            // K value
            if (r < 1) {
                double *kij_ptr = PyArray_GETPTR2(K_array, i, j);
                switch (order) {
                    case 0:
                        *kij_ptr = K_q_0(r, j_Dq);
                        break;
                    case 1:
                        *kij_ptr = K_q_1(r, j_exp);
                        break;
                    case 2:
                        *kij_ptr = K_q_2(r, j_exp, j_poly_r1, j_poly_r2);
                        break;
                    case 3:
                        *kij_ptr = K_q_3(r, j_exp, j_poly_r1, j_poly_r2, j_poly_r3);
                        break;
                    default:
                        sentinel("Invalid order: %d", order);
                }
...
static PyObject *_compact_kernel_train_err_eval(PyObject *self, PyObject *args) {
    ...
    for (i = 0; i < npts_x; i++) {
        double *kij_ptr = PyArray_GETPTR2(K_array, i, i);
        double *train_err_ptr = PyArray_GETPTR1(train_err_array, i);
        *kij_ptr += *train_err_ptr;
    }
    Py_END_ALLOW_THREADS
    return K_obj;
...
static PyObject *_compact_kernel_sample_eval(PyObject *self, PyObject *args) {
    ...
    for (i = 0; i < npts_x; i++) {
        for (j = 0; j < npts_xp; j++) {
            ...
            for (k = 0; k < ndim; k++) {
                double *unprimed_ptr = PyArray_GETPTR2(x_scaled_array, i, k);
                double *primed_ptr = PyArray_GETPTR2(xp_scaled_array, j, k);
                r += pow(*unprimed_ptr - *primed_ptr, 2);
            }
            r = sqrt(r);
            // K value
