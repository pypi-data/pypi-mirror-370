...
def histogram_1d(
    ...
    # Show data
    ax.plot(np.linspace(xmin, xmax, nbins), rho, **kwargs)
    ax.set_xlim([xmin, xmax])
    return ax

def gaussian_pdf_1d_from_2d(
                            ax,
                            approx,
                            tag1, tag2,
                            nbins = 100,
    ...
    '''\
    make a 1d pdf for a 2d gaussian
    
    ax is a plot axis
    approx is a MultivariateNormal object from basil
    dim is the index of the dimension we are using
    limits is like [[xmin, xmax],[ymin, ymax]]
    '''
    import numpy as np
    
    dim = int(tag1.lstrip("mean_"))
    xlims = approx._parameters[tag1].limits
    ylims = approx._parameters[tag2].limits
    xmin = xlims[0]
    xmax = xlims[1]
    ymin = ylims[0]
    ymax = ylims[1]

    xspace = np.linspace(xmin, xmax, nbins)
    yspace = np.linspace(ymin, ymax, nbins)

    xgrid, ygrid = np.meshgrid(xspace, yspace)
    samples = np.asarray([xgrid,ygrid]).T
    pdf =  approx.pdf(samples).reshape((nbins, nbins))
    pdf = np.sum(pdf, axis = 1 - dim)

    if dim == 0:
        dx = (xmax - xmin)/nbins
        pdf = pdf / np.sum(dx*pdf)
        ax.plot(xspace, pdf, **kwargs)
        ax.set_xlim([xmin, xmax])
    else:
        dy = (ymax - ymin)/nbins
        pdf = pdf / np.sum(dy*pdf)
        ax.plot(yspace, pdf, **kwargs)
...
def gaussian_contour_2d(
    ...
    levels = percentile_levels(xgrid, ygrid, pdf, levels)
...
def density_contour_2d(
    ...
    rho = K.pdf(rv_input).reshape((nbins, nbins)).T
