...
def update_migration_status_from_db(migrations: list[Migration], config, console):
    """Update migration status from Neo4j database."""
    try:
        with MigrationExecutor(config, console) as executor:
            for migration in migrations:
                status_info = executor.get_migration_status(migration.id)
                if status_info:
                    migration.status = status_info.get("status", "pending")
                    if status_info.get("applied_at"):
                        # Parse datetime if needed
                        pass
    except Exception:
        # If we can't connect, assume all loaded migrations are applied
        for migration in migrations:
