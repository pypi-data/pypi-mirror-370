...
tool_manager.add_tool(app_instance.update_paragraph_style)
...
async def main():
    ...
    tool3=tool_manager.get_tool("delete_content")
    tool4=tool_manager.get_tool("update_paragraph_style")
    print(tool)
    if tool:
    ...
    result=await tool_manager.call_tool(name="update_paragraph_style", arguments={"document_id": "1zPdJM91Y4RaD5nMyKQq1-NbJJ8EVn6MabFE-9jUW0T4", "start_index": 1, "end_index": 10, "space_above": {"magnitude": 12.0, "unit": "PT"}})
    print(result)
    print(type(result))
