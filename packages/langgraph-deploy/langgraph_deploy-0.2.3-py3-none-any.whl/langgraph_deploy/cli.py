...
def _load_official_entrypoint():
    try:
        ...
        # このラッパ自身の entry point は除外して、公式の 'langgraph' を探す
        eps = md.entry_points(group="console_scripts")
        candidates = []
        for ep in eps:
            if ep.name != "langgraph":
                continue
            dist_name = getattr(getattr(ep, "dist", None), "name", None)
            if dist_name and dist_name.lower() == "langgraph-deploy":
                continue
    ...
    # フォールバック：よくあるモジュール名候補
    import importlib
    for mod_attr in [
        ("langgraph_cli.__main__", "main"),
        ("langgraph_cli.cli", "app"),
        ("langgraph_cli.main", "app"),
    ]:
        try:
            mod = importlib.import_module(mod_attr[0])
            target = getattr(mod, mod_attr[1])
            return target
        except Exception:
            continue
    return None
...
def _delegate_to_official(argv: List[str]) -> int:
    """
    非 deploy サブコマンドは公式 CLI に委譲する。
    これまでの `python -m langgraph` ではなく、console-script entry point を直接起動。
    """
    app = _load_official_entrypoint()
    if app is None:
        print(
            "Could not locate the official 'langgraph' CLI entry point. "
            "Is 'langgraph-cli' installed in this environment?",
            file=sys.stderr,
        )
        return 1

    # Typer/Click アプリ想定：argv を差し替えて直接実行
    sys.argv = ["langgraph", *argv]
    try:
        rv = app()
