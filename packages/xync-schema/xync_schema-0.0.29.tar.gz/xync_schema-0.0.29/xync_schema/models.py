...
class Cur(Model):
    ...
    symbol: str | None = CharField(2, null=True)
...
class User(TgUser, TsTrait):
    ...
    async def req(self, amount: int, cur_id: int, sender_id: int = None) -> "Transaction":
...
class Transaction(Model):
    ...
    async def vld_sign(self):
        ...
        if (
            not self.validator.pub or not self.validator.prv or not len(self.validator.pub + self.validator.prv) == 64
        ):  # todo: remove check
            await self.validator.keygen()
        pk = Ed25519PrivateKey.from_private_bytes(self.validator.prv)
        self.proof += pk.sign(self.pack() + self.proof)
