...
try:
    ...
    # If local_tool_responses is not available, create empty registry
    TOOL_REGISTRY = {}
...
class LightberryToolServer:
    ...
    def set_tool_channel_name(self, name: str):
        """Set or update the tool channel name."""
    ...
    def set_response_callback(self, callback: Callable[[Dict[str, Any]], None]):
        """
        Set the callback function to send responses back (deprecated for RPC).
        
        Args:
            callback: Function that takes a response dict
        """
        self._response_callback = callback
        
    async def process_tool_call(self, message: str) -> Dict[str, Any]:
        """
        Process a tool call message received via RPC.
    ...
    async def _execute_tool(self, tool_name: str, args: Dict[str, Any]) -> Any:
        ...
        if tool_name not in TOOL_REGISTRY:
            print(f"Unknown tool call - Tool call received: name: {tool_name} args: {args}")
