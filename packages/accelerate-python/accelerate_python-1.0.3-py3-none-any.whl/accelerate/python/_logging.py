...
class AccelerateLogger(verboselogs.VerboseLogger):
    ...
    def audit_method(
        ...
        return lambda func: self.__inspect_method(func, exclude, include, level)

    def audit_class(self, exclude: list[str] = [], include: list[str] = []):
        def class_decorator(target_class):
            ...
            for attr_name, attr_value in _all_methods:
                # Check if the attribute is a method
                if (
                    attr_value.__name__ not in exclude
                    and attr_value.__qualname__.startswith(target_class.__name__ + ".")
                ):
                    # Replace the method with the decorated version
                    setattr(target_class, attr_name, self.__inspect_method(attr_value))
            return target_class
