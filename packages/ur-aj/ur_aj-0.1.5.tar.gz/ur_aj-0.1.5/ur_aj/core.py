...
def jointupdate(joint_number: int, angle_desired: float, speed: float = 0.5):
    """
    Move only one joint to a new absolute angle (in degrees).
    Keeps other joints unchanged.
    """
    current_joints = list(getjoints())  # in degrees
    if 1 <= joint_number <= 6:
        current_joints[joint_number - 1] = angle_desired
    else:
        raise ValueError("Joint number must be between 1 and 6")

    # Convert all joints to radians for URScript
    joints_rad = [math.radians(j) for j in current_joints]

    urscript = f"""
    movej([{", ".join(str(j) for j in joints_rad)}], a={speed}, v={speed})
    """
    send_urscript(urscript)
    time.sleep(0.3)
    wait_until_stopped()
...
def jointmove_relative(joint_number: int, delta_angle: float, speed: float = 0.5):
    """
    Move only one joint by a relative delta (in degrees).
    Keeps other joints unchanged.
    """
    current_joints = list(getjoints())  # in degrees
    if 1 <= joint_number <= 6:
