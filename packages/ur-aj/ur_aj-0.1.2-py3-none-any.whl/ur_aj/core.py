...
def jointupdate(joint_number: int, angle_desired: float, speed: float = 0.5):
    """
    Move a specific joint to an absolute angle (degrees).
    Example: jointupdate(6, 40) → moves joint 6 to exactly 40°.
    """
    current_joints = list(getjoints())  # degrees
    ...
    send_urscript(urscript, wait=0.1)
    wait_until_stopped()


def jointmove_relative(joint_number: int, delta_angle: float, speed: float = 0.5):
    """
    Move a specific joint by a relative delta (degrees).
    Example: jointmove_relative(6, 15) → moves joint 6 by +15°.
    """
    current_joints = list(getjoints())  # degrees
    if not 1 <= joint_number <= 6:
        raise ValueError("Joint number must be 1–6")

    current_joints[joint_number - 1] += delta_angle
    joints_rad = [math.radians(j) for j in current_joints]
