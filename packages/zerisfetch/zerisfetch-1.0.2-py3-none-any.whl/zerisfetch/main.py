...
def get_color(distro):
    """Возвращает цвет для дистрибутива"""
    colors = {
        'debian': '\033[31m',  # Красный
        'ubuntu': '\033[33m',  # Оранжевый
        'arch': '\033[36m',    # Голубой
        'linux': '\033[37m'    # Белый
    }
    return colors.get(distro, '\033[37m')
...
def main():
    ...
    # Выводим арт и информацию рядом - ИСПРАВЛЕНО!
    for i, art_line in enumerate(art):
        info_line = info_lines[i] if i < len(info_lines) else ""
        # Применяем цвет только к ASCII арту, информация остается белой
        print(f"{color}{art_line}{reset} {info_line}")
