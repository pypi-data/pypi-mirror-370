...
class AzureSecretsManager:
    ...
    def __init__(self, key_vault_name: str = None):
        ...
        self.key_vault_name = key_vault_name or os.getenv("AZURE_KEY_VAULT_NAME", "cso-kv")
        ...
        try:
            ...
            try:
                ...
                console.print(f"[green]✓ Connected to Azure Key Vault: {self.key_vault_name}[/green]")
            except StopIteration:
                # No secrets exist yet, but connection is valid
                console.print(f"[green]✓ Connected to Azure Key Vault: {self.key_vault_name}[/green]")
        except Exception as e:
            console.print(f"[red]Failed to connect to Azure Key Vault: {e}[/red]")
            console.print("[yellow]Ensure you're authenticated with Azure CLI: az login[/yellow]")
            raise
    ...
    def create_secret(self, name: str, value: str, **kwargs) -> Dict[str, Any]:
        ...
        full_name = f"{self.prefix}{name}".replace("_", "-")  # Azure Key Vault requires hyphens
    ...
    def delete_secret(self, name: str) -> bool:
        ...
        try:
            ...
            console.print(f"[green]✓ Deleted secret '{full_name}' (soft delete)[/green]")
            console.print(f"[yellow]Note: Secret will be permanently deleted after retention period[/yellow]")
            return True
        except ResourceNotFoundError:
            console.print(f"[red]Secret '{full_name}' not found[/red]")
            return False
...
def init(
    ...
    key_vault_name: Optional[str] = Option(None, "--key-vault", help="Azure Key Vault name"),
    credentials: Optional[str] = Option(
    ...
    try:
        ...
        if provider == Provider.AZURE:
            azure_mgr = AzureSecretsManager(key_vault_name=key_vault_name)
        ...
        console.print("\nYou can now use:")
        console.print("  cso secrets create <name> --provider <aws|gcp|azure>")
        console.print("  cso secrets list --provider <aws|gcp|azure>")
        console.print("  cso secrets get <name> --provider <aws|gcp|azure>")
