...
class AzureKubernetesBootstrapper(KubernetesBootstrapper):
    ...
    def __init__(self, console: Console):
        super().__init__(console)
        self.terraform_dir = Path.home() / ".cso" / "terraform" / "azure"
...
resource "azurerm_key_vault" "main" {
  name                = "${substr(var.cluster_name, 0, 17)}-cso-kv"
  resource_group_name = azurerm_resource_group.main.name
  location            = azurerm_resource_group.main.location
  sku_name           = "standard"
...
resource "azurerm_container_registry" "main" {
  name                = replace("${var.cluster_name}cso", "-", "")
  resource_group_name = azurerm_resource_group.main.name
  location            = azurerm_resource_group.main.location
  sku                 = "Standard"
  admin_enabled       = false
...
"""
    ...
    async def bootstrap(self, **kwargs) -> Dict[str, Any]:
        ...
        resource_group = kwargs.get("resource_group", "cso-resources")
        location = kwargs.get("location", "eastus2")
        cluster_name = kwargs.get("cluster_name", "cso-cluster")
...
def create_cluster(
    ...
    resource_group: str = typer.Option(
        "cso-resources", "--resource-group", "-g", help="Azure Resource Group name"
    ),
    ...
    if provider == BootstrapProvider.LOCAL:
        ...
        console.print("\n[cyan]Next steps:[/cyan]")
        console.print(
            "1. Run 'cso setup --provider azure --domain your-domain.com' to install CSO"
        )
        console.print("2. Configure DNS for your domain")
        console.print("3. Access ArgoCD at https://argocd.your-domain.com")
