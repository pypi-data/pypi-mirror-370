"""CSO Branch deployment management commands."""
...
def create_branch_application(
    ...
    return {
        ...
        "metadata": {
            ...
            "annotations": {
                "cso.cloudstack.io/branch": branch,
                "cso.cloudstack.io/environment": environment
            }
        },
        "spec": {
            "project": f"{environment}-project" if environment != "prod" else "production-project",
            "source": {
...
def deploy_branch(
    ...
    environment: str = typer.Option("dev", "--env", "-e", help="Environment (dev, staging, prod)"),
    namespace_suffix: str = typer.Option(None, "--namespace", "-n", help="Custom namespace suffix"),
    auto_sync: bool = typer.Option(True, "--auto-sync/--no-auto-sync", help="Enable automatic sync"),
    apply: bool = typer.Option(False, "--apply", "-a", help="Apply to cluster immediately"),
    force: bool = typer.Option(False, "--force", "-f", help="Force overwrite existing application")
):
...
def list_deployments(
    ...
    try:
        ...
        # Filter CSO-managed apps
        cso_apps = []
        for app in apps:
            labels = app.get("metadata", {}).get("labels", {})
            if labels.get("managed-by") == "cso":
                # Apply filters
                ...
                cso_apps.append({
                    ...
                    "branch": app["metadata"].get("annotations", {}).get("cso.cloudstack.io/branch", ""),
        ...
        # Display table
        table = Table(title="CSO Branch Deployments")
...
def promote_deployment(
    ...
    branch: Optional[str] = typer.Option(None, "--branch", "-b", help="Override target branch"),
    ...
    if apply:
        try:
            ...
            console.print(f"[green]✓[/green] Promotion completed successfully!")
...
def cleanup_deployment(
    ...
    namespace_suffix: Optional[str] = typer.Option(None, "--namespace", "-n", help="Custom namespace suffix"),
    ...
    if not confirm:
        console.print(f"[yellow]Warning:[/yellow] This will delete the application [red]{app_name}[/red]")
