...
from cloudstack_orchestrator.providers.local import LocalSecretsProvider
from cloudstack_orchestrator.providers.aws import AWSSecretsProvider
from cloudstack_orchestrator.providers.github import GitHubProvider
...
class TestLocalSecretsProvider:
    ...
    def test_init_creates_directory(self, temp_storage):
        ...
        provider = LocalSecretsProvider(storage_path)
    ...
    def test_init_uses_default_path(self):
        ...
        with patch('pathlib.Path.home') as mock_home:
            ...
            assert provider.storage_path == Path("/home/user/.cso/secrets")
...
class TestAWSSecretsProvider:
    ...
    def mock_boto_client(self):
        """Mock boto3 client."""
        with patch('cloudstack_orchestrator.providers.aws.boto3.client') as mock_client:
            yield mock_client.return_value
    
    def test_init_with_profile(self, mock_boto_client):
        """Test initialization with AWS profile."""
        with patch('cloudstack_orchestrator.providers.aws.boto3.Session') as mock_session:
    ...
    async def test_get_secret_success(self, mock_boto_client):
        ...
        mock_boto_client.get_secret_value.assert_called_once_with(
            SecretId="cso/test/test-key"
        )
    
    @pytest.mark.asyncio
    async def test_get_secret_not_found(self, mock_boto_client):
        """Test getting a non-existent secret."""
        mock_boto_client.get_secret_value.side_effect = Exception("ResourceNotFoundException")
    ...
    async def test_set_secret_new(self, mock_boto_client):
        ...
        mock_boto_client.create_secret.assert_called_once_with(
            Name="cso/test/test-key",
            SecretString="test-value",
            Description="Test description",
            Tags=[
                {'Key': 'ManagedBy', 'Value': 'CloudStackOrchestrator'},
    ...
    async def test_set_secret_update_existing(self, mock_boto_client):
        ...
        mock_boto_client.create_secret.side_effect = Exception("ResourceExistsException")
...
class TestGitHubProvider:
    ...
    def mock_github(self):
        """Mock GitHub API."""
        with patch('cloudstack_orchestrator.providers.github.Github') as mock_github:
    ...
    async def test_create_oauth_app(self, mock_github):
        ...
        mock_user.create_oauth_app.assert_called_once_with(
            ...
            callback_url="https://example.com/callback",
            description="CloudStack Orchestrator OAuth Application"
        )
