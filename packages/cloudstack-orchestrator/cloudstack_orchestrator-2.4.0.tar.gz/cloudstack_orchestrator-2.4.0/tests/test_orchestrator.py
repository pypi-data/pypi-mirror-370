...
from cloudstack_orchestrator.core import Orchestrator, Config, CloudProvider
from cloudstack_orchestrator.core.config import GitHubConfig, KeycloakConfig
...
async def test_validate_prerequisites_success(test_config, mock_k8s_api):
    ...
    with patch('cloudstack_orchestrator.core.orchestrator.shutil.which') as mock_which, \
...
async def test_validate_prerequisites_no_k8s_connection(test_config, mock_k8s_api):
    ...
    with patch.object(Orchestrator, '_command_exists', return_value=True):
        ...
        assert result["valid"] is False
        assert any("Cannot connect to Kubernetes cluster" in error for error in result["errors"])
...
async def test_create_kubernetes_secrets(test_config, mock_k8s_api):
    ...
    with patch('cloudstack_orchestrator.core.orchestrator.SecretsManager') as mock_secrets:
        orchestrator = Orchestrator(test_config)
        orchestrator.k8s_api = mock_core_api
        
        # Mock secrets manager
        mock_secrets_instance = mock_secrets.return_value
        mock_secrets_instance.export_kubernetes_secrets = AsyncMock(return_value={
            "test-secret": {
                "namespace": "default",
                "data": {
                    "key1": "value1",
...
async def test_install_argocd(test_config):
    ...
    with patch('subprocess.run') as mock_run, \
        ...
        assert ["helm", "repo", "add", "argo", "https://argoproj.github.io/argo-helm"] in calls
...
async def test_deploy_platform(test_config):
    ...
    with patch('subprocess.run') as mock_run, \
        ...
        orchestrator.secrets_manager.provider.get_secret = AsyncMock(return_value="test-token")
        
        result = await orchestrator.deploy_platform()
        
        assert result["deployed"] is True
        assert result["application"] == "cloudstack-platform"
        
        # Verify kubectl apply was called
        kubectl_calls = [call[0][0] for call in mock_run.call_args_list if call[0][0][0] == "kubectl"]
...
async def test_setup_complete_flow(test_config, mock_k8s_api):
    ...
    with patch.object(Orchestrator, '_command_exists', return_value=True), \
        ...
        # Mock secrets manager
        with patch.object(orchestrator.secrets_manager, 'ensure_all_secrets') as mock_ensure, \
             patch.object(orchestrator.secrets_manager, 'export_kubernetes_secrets') as mock_export:
