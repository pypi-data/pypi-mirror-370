...
from cloudstack_orchestrator.core.secrets import SecretsManager
from cloudstack_orchestrator.core import Config, CloudProvider
...
def test_secrets_manager_creates_aws_provider(aws_config):
    ...
    with patch('cloudstack_orchestrator.providers.aws.boto3.client'):
...
async def test_ensure_secret_provided_value(local_config):
    ...
    with patch.object(manager.provider, 'get_secret') as mock_get, \
        ...
        result = await manager._ensure_secret("test-key", "provided-value", "Test secret")
...
async def test_ensure_secret_generated(local_config):
    ...
    with patch.object(manager.provider, 'get_secret') as mock_get, \
        ...
        result = await manager._ensure_secret("test-key", None, "Test secret", generator=test_generator)
...
async def test_export_kubernetes_secrets(local_config):
    ...
    with patch.object(manager, 'ensure_all_secrets') as mock_ensure:
        ...
        assert k8s_secrets["keycloak-admin-creds"]["data"]["password"] == "keycloak-pass"
        ...
        assert k8s_secrets["oauth2-proxy-secrets"]["data"]["client-secret"] == "app-secret"
        assert k8s_secrets["oauth2-proxy-secrets"]["data"]["cookie-secret"] == "cookie-secret"