...
def setup(
    ...
    cluster_name: str = typer.Option(
        "cso-cluster", "--cluster", "-c", help="Kubernetes cluster name"
    ),
    ...
    if not non_interactive:
        ...
        if not github_token:
            ...
            console.print(
                "[yellow]ArgoCD needs access to the cloudstack-orchestrator repository[/yellow]"
            )
    ...
    try:
        ...
        if config.provider == CloudProvider.LOCAL:
            ...
            if results.get("argocd_api", {}).get("ready"):
                ...
                console.print("   cso apps                    (List all applications)")
                console.print("   cso sync <app>              (Sync an application)")
                console.print("   cso repos                   (Configure repositories)")
...
def status(
    ...
    from cloudstack_orchestrator.core.terraform import TerraformManager
    ...
    try:
        ...
        if infra_status and infra_status.get("provisioned"):
            ...
            console.print("[bold]☁️  Infrastructure (GCP)[/bold]")
            console.print(
                "   Status: [yellow]Cluster exists but not managed by CSO[/yellow]"
            )
            console.print(
                "   [dim]Run 'cso setup --provision-infrastructure' to manage with Terraform[/dim]\n"
            )
        ...
        if repo_type == "Local":
            ...
            if not repo_connected:
                console.print(
                    "   [dim]Run 'cso auth' to connect to private repositories[/dim]"
                )
...
def teardown(
    ...
    keep_infrastructure: bool = typer.Option(
        False,
        "--keep-infrastructure",
        help="Keep the underlying cloud infrastructure (GKE cluster, etc.) - only remove CSO platform",
    ),
):
    """Teardown CloudStack Orchestrator platform and optionally infrastructure."""
    ...
    if infrastructure_exists and not keep_infrastructure:
        ...
        console.print("[dim]You can redeploy CSO later with 'cso setup'[/dim]")
    ...
    try:
        ...
        console.print("\n[cyan]Removing CSO platform components...[/cyan]")
        ...
        if result["success"]:
            ...
            if infrastructure_exists and not keep_infrastructure:
                ...
                if infra_result["success"]:
                    ...
                    console.print(
                        f"[yellow]Try manually: cd ~/.cso/terraform/modules/bootstrap && terraform destroy -var-file=~/.cso/terraform/{cluster_name}.tfvars[/yellow]"
                    )
                    raise typer.Exit(1)
            elif keep_infrastructure:
                console.print("\n[green]✅ Infrastructure preserved[/green]")
                console.print(
                    f"[dim]You can redeploy CSO with: cso setup --cluster {cluster_name}[/dim]"
                )
        else:
            console.print(
...
def auth(
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            console.print(
                "[yellow]Please run 'cso setup' first to install the platform.[/yellow]"
            )
            raise typer.Exit(1)
    except Exception as e:
        ...
        try:
            with httpx.Client(timeout=10) as client:
                ...
                if login_response.status_code == 200:
                    ...
                    console.print(
                        "[yellow]You may need to reset the password: cso argocd reset-password[/yellow]"
                    )
                    return ""
    ...
    try:
        ...
        result = subprocess.run(
            [
                ...
                "merge",
                "-p",
                f'{{"operation":{{"initiatedBy":{{"username":"cso"}},"sync":{{"revision":"{revision}"}}}}}}',
            ],
            capture_output=True,
            text=True,
        )
