"""Module validation utilities for CSO."""
...
class ModuleValidator:
    """Validates CSO module specifications."""
    ...
    def _validate_structure(self, spec: Dict) -> List[ValidationResult]:
        ...
        # Check API version
        if spec.get("apiVersion") != "cso.platform/v1alpha1":
            results.append(ValidationResult(
                passed=False,
                message=f"Invalid apiVersion: {spec.get('apiVersion')}",
                severity="error",
                details={"expected": "cso.platform/v1alpha1"}
            ))
    ...
    def _validate_deployment(self, spec: Dict) -> List[ValidationResult]:
        ...
        if not deployment.get("path"):
            results.append(ValidationResult(
                passed=False,
                message="Missing deployment path",
                severity="error"
            ))
    ...
    def _validate_security(self, spec: Dict) -> List[ValidationResult]:
        ...
        if not security.get("imageRegistry"):
            results.append(ValidationResult(
                ...
                severity="info",
                details={"recommendation": "Specify approved registry for production"}
            ))
    ...
    def _validate_compliance(self, spec: Dict) -> List[ValidationResult]:
        ...
        if not compliance:
            results.append(ValidationResult(
                ...
                severity="info",
                details={"recommendation": "Define compliance requirements for production"}
            ))
            return results
    ...
    def validate_deployment_files(self, module_path: Path, spec: Dict) -> List[ValidationResult]:
    ...
    def validate_test_files(self, module_path: Path, spec: Dict) -> List[ValidationResult]:
