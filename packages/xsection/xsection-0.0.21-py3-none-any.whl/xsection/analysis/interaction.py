...
def _linearity_break(r0, r1, r, k, tol):
    ...
    # if r1 is tiny, avoid division blowup and declare linear (keep going)
...
def limit_surface(section_tuple,
    ...
    section_tuple: ("Fiber", shape, materials) like your code
    control_dof: 1..6 (node DOF at node=2) whose value we hold fixed via a Constant load vector
    control_points: iterable of scalar control values to sweep
    dof_i, dof_j: the two response DOFs (node=2) that define the plane
    nr: number of direction angles in [0, 2π)
    max_scale: total proportional-load scale in the (dof_i, dof_j) plane
    numIncr: steps along each ray
    linear_tol: relative deviation from linearity at which we declare the “limit”
    resp_node: node ID for responses (default 2)
    """
...
def triangulate(samples, elev=25, azim=-60, alpha=0.9, figure=None):
    ...
    if len(rings) < 2:
        # Nothing to triangulate; just scatter
        x = np.array([s["ri"] for s in samples], float)
        y = np.array([s["rj"] for s in samples], float)
        z = np.array([s["control"] for s in samples], float)
        if figure is None:
            figure = plt.figure(figsize=(7,6))
        ax = figure.add_subplot(111, projection="3d")
        ax.scatter(x, y, z, s=6)
        ax.set_xlabel("Response DOF i")
        ax.set_ylabel("Response DOF j")
        ax.set_zlabel("Control DOF")
...
def plot_limit_surface(samples, elev=25, azim=-60, alpha=0.9, figure=None):
    ...
    # If everything degenerates, fall back to scatter
    if tris.size == 0 or len(x) < 3:
        if figure is None:
            figure = plt.figure(figsize=(7,6))
        ax = figure.add_subplot(111, projection="3d")
        ax.scatter(x, y, z, s=6)
...
# def plot_limit_surface(samples, elev=25, azim=-60, alpha=0.9, figure=None):
#     """
#     Plot a triangulated surface of the limit points.
#     samples: list of {"control": c, "angle": th, "ri": ri, "rj": rj}
#     """
#     if not samples:
#         raise ValueError("samples is empty")

#     x = np.array([s["ri"] for s in samples], dtype=float)
#     y = np.array([s["rj"] for s in samples], dtype=float)
#     z = np.array([s["control"] for s in samples], dtype=float)
...
#     ax = figure.add_subplot(111, projection="3d")
#     surf = ax.plot_trisurf(tri, z, linewidth=0.2, antialiased=True, alpha=alpha)
#     ax.scatter(x, y, z, s=5)  # points for reference
