...
def _plot_bucket_thumbnails(bucket_stats: List[Dict[str, Any]], viz_data: Dict) -> None:
    ...
    for bucket_idx, bucket_data in enumerate(sorted_buckets):
        ...
        def create_bucket_grid(images, max_images=100):
            ...
            # Take first 100 images (not random) for consistent display
...
def sample_diverse(
    ...
    if show_progress:
        print("Creating diversity grid with enhanced visual features...")
    ...
    for features in hashes:
        ...
        # Use distributed bits from dhash for structure
        total_dhash_bits = len(dhash_bits)
        if total_dhash_bits >= 4:
            dhash_indices = [0, total_dhash_bits//4, total_dhash_bits//2, 3*total_dhash_bits//4]
            structure_bits = tuple(dhash_bits[i] > 0 for i in dhash_indices)
        else:
            structure_bits = tuple(dhash_bits > 0)
        
        # Use first 2 bits from ahash for brightness/contrast patterns
        brightness_bits = tuple(ahash_bits[:2] > 0) if len(ahash_bits) >= 2 else ()
        
        # Quantize color variance (0=grayscale, 1=low color, 2=medium color, 3=high color)
        if color_var < 50:
            color_bucket = 0  # Grayscale/monochrome
        elif color_var < 200:
            color_bucket = 1  # Low color variation
        elif color_var < 800:
            color_bucket = 2  # Medium color variation
        else:
            color_bucket = 3  # High color variation
        
        # Quantize brightness (0=dark, 1=medium, 2=bright, 3=very bright)
        if brightness < 64:
            bright_bucket = 0
        elif brightness < 128:
            bright_bucket = 1
        ...
        bucket_key = structure_bits + brightness_bits + (color_bucket, bright_bucket)
    ...
    if show_progress:
        print(f"Grouped into {len(buckets)} diversity buckets with enhanced visual features")
        print(f"Features: structure (4 bits) + brightness (2 bits) + color variance (4 levels) + brightness (4 levels)")
