...
def sample_diverse(
    ...
    for features in hashes:
        ...
        # Use fewer bits from dhash for structure (reduce from 4 to 3)
        total_dhash_bits = len(dhash_bits)
        if total_dhash_bits >= 3:
            dhash_indices = [0, total_dhash_bits//3, 2*total_dhash_bits//3]
        ...
        # 3 structure + 1 brightness + 1 color + 1 overall_brightness = 6 dimensions
        # Max buckets: 2^3 × 2^1 × 2 × 2 = 8 × 2 × 2 × 2 = 64 buckets
        bucket_key = structure_bits + brightness_bit + (color_bucket, bright_bucket)
    ...
    if show_progress:
        ...
        print(f"Features: structure (3 bits) + brightness pattern (1 bit) + color type (2 levels) + brightness level (2 levels)")
