...
def sample_diverse_with_stats(
    ...
    # Add remaining buckets as excluded
    for bucket_idx in range(len(bucket_stats), len(bucket_list)):
        _, indices = bucket_list[bucket_idx]
        bucket_stats.append({
            'original_size': len(indices),
            'kept': 0,
            'excluded': len(indices),
            'stride': 0
        })
