...
def sample_diverse(
    ...
    # Use more bits spread across hash for finer visual similarity detection
    n_bucket_bits = min(6, hash_size)  # Use up to 6 bits = 64 buckets max
    bucket_keys = []
    
    total_bits = hash_size * hash_size
    if total_bits >= n_bucket_bits:
        # Spread bucket bits across the entire hash for better coverage
        if n_bucket_bits == 6:
            bit_indices = [
                0,                              # Top-left
                total_bits // 6,               # First sixth
                total_bits // 3,               # First third
                total_bits // 2,               # Middle
                2 * total_bits // 3,           # Two thirds
                5 * total_bits // 6            # Five sixths
            ]
        elif n_bucket_bits == 5:
            bit_indices = [
                0,                              # Top-left
                total_bits // 5,               # First fifth
                2 * total_bits // 5,           # Two fifths
                3 * total_bits // 5,           # Three fifths  
                4 * total_bits // 5            # Four fifths
            ]
        else:  # 4 bits (fallback)
            bit_indices = [
                0,                              # Top-left
                total_bits // 4,               # First quarter  
                total_bits // 2,               # Middle
                3 * total_bits // 4            # Third quarter
            ]
    else:
        # If hash is very small, use all available bits
        bit_indices = list(range(total_bits))
        n_bucket_bits = len(bit_indices)
    
    for h in hash_arrays:
        h_flat = h.flatten()
        # Create bucket key from distributed bits across the hash
        key = tuple(h_flat[i] > 0 for i in bit_indices if i < len(h_flat))
    ...
    if show_progress:
        ...
        print(f"Using {n_bucket_bits} bits (max {2**n_bucket_bits} buckets) from indices: {bit_indices}")
        print(f"Total hash bits available: {total_bits}")
