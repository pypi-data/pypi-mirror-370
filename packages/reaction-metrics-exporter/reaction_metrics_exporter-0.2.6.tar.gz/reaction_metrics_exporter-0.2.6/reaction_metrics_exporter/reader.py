...
class FileReader(LogReader):
    ...
    def _to_log(self, entry: str) -> Log:
        ...
        try:
            ...
            return Log(datetime.now(timezone.utc), loglevel, entry)
...
class JournalReader(LogReader):
    def __init__(self, path: str):
        ...
        if not last_entry:
            ...
            if State.last_log < tail_date and config.restore:
    ...
    def _to_log(self, entry: dict[str, Any]) -> Log:
        # timestamp is already a datetime object in UTC timezone, but not actually localized
        return Log(entry["__REALTIME_TIMESTAMP"].replace(tzinfo=timezone.utc), entry["PRIORITY"], entry["MESSAGE"].strip())
