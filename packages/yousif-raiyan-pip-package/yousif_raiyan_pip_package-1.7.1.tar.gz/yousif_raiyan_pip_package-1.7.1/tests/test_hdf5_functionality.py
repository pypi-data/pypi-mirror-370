#!/usr/bin/env python3
"""
Test script for new HDF5 functionality in EEGGraphProcessor.
...
def test_hdf5_imports():
    ...
    try:
        from src.yousif_raiyan_pip_package import EEGGraphProcessor, EDFLoader
        print("✔ EEGGraphProcessor imported successfully")
    except ImportError as e:
        print(f"❌ Failed to import EEGGraphProcessor: {e}")
        return False
...
def test_hdf5_method_exists():
    ...
    try:
        from src.yousif_raiyan_pip_package import EEGGraphProcessor
        
        # Check if method exists
        if not hasattr(EEGGraphProcessor, 'generate_graphs_from_edf'):
            print("❌ generate_graphs_from_edf method not found")
            return False
        
        # Check method signature
        import inspect
        sig = inspect.signature(EEGGraphProcessor.generate_graphs_from_edf)
