...
processor = EEGGraphProcessor(edf_loader=loader, window_step_ratio=1.0)

# Level 1: Quick exploration
processor.compute_correlation(start_time=0, stop_time=300, interval_seconds=30)
processor.compute_coherence_average(start_time=0, stop_time=300, interval_seconds=30)
...
hdf5_path = processor.generate_graphs_from_edf(segment_duration_minutes=1.0)
```
...
The EEGGraphProcessor provides **three levels of analysis complexity** to match different research needs:
...
The EEGGraphProcessor supports a **progressive complexity approach** - start simple and add detail as needed:
...
processor_detailed = EEGGraphProcessor(
    edf_loader=loader,
...
processor_advanced = EEGGraphProcessor(
    edf_loader=loader,
...
explorer = EEGGraphProcessor(edf_loader=loader, window_step_ratio=1.0)
...
detailed = EEGGraphProcessor(edf_loader=loader, window_step_ratio=0.5)
...
advanced = EEGGraphProcessor(edf_loader=loader, window_step_ratio=0.125)
...
processor = EEGGraphProcessor(window_step_ratio=0.125)  # 87.5% overlap
# Result: 8x more windows, 8x longer processing, 8x more storage

# Moderate resolution, balanced performance  
processor = EEGGraphProcessor(window_step_ratio=0.5)   # 50% overlap
# Result: 2x more windows, 2x longer processing

# Low resolution, fast processing
processor = EEGGraphProcessor(window_step_ratio=1.0)   # No overlap
