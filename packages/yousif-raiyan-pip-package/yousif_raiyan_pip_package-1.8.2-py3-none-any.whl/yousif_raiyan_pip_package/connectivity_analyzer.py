...
class ConnectivityAnalyzer:
    ...
    def __init__(
        ...
        *,
        ...
        output_dir : str, optional
            ...
            'graph_representation' subdirectory in the same directory as the EDF file.
        ...
        window_step_ratio : float, default=0.125
            Step ratio for sliding windows (0.125 = 87.5% overlap). Controls temporal
            sampling density and computational efficiency trade-off.
    ...
    def generate_graphs_from_edf(self, segment_duration_minutes: float = 3.0, 
        ...
        **Edge Feature Analysis**: Quantifies pairwise connectivity properties with
        ...
        Parameters
        ----------
        segment_duration_minutes : float, default=3.0
            Duration of each processing segment in minutes. Smaller values reduce memory
            usage but may decrease computational efficiency. Valid range: 0.1 to 60 minutes.
        ...
        ------
        ValueError
            If segment_duration_minutes is outside valid range or temporal parameters
            are inconsistent with file duration.
            
        Examples
        --------
        >>> analyzer = ConnectivityAnalyzer(edf_loader=loader)
        >>> # Process entire recording
        >>> hdf5_path = analyzer.generate_graphs_from_edf(segment_duration_minutes=5.0)
        >>> 
        >>> # Analyze specific temporal window (5-15 minutes)
        >>> hdf5_path = analyzer.generate_graphs_from_edf(start_time=300, end_time=900)
        >>> 
        >>> # Process initial 10 minutes only
        >>> hdf5_path = analyzer.generate_graphs_from_edf(end_time=600)
        ...
        actual_end_time = end_time if end_time is not None else file_duration_seconds
        ...
        if actual_end_time <= actual_start_time or actual_end_time > file_duration_seconds:
            raise ValueError(f"end_time must be between {actual_start_time:.1f} and {file_duration_seconds:.1f} seconds")
        
        # Use the subset duration for processing
        total_duration_seconds = actual_end_time - actual_start_time
        ...
        if start_time is not None or end_time is not None:
            print(f"→ Processing time range: {actual_start_time:.1f}s - {actual_end_time:.1f}s ({total_duration_seconds:.1f}s total)")
        else:
        ...
        if start_time is not None or end_time is not None:
            time_suffix = f"_{actual_start_time:.0f}s-{actual_end_time:.0f}s"
            hdf5_path = hdf5_dir / f"{self.filename}_graphs{time_suffix}.h5"
        else:
        ...
        with h5py.File(hdf5_path, 'w') as h5_file:
            ...
            h5_file.attrs['processing_end_time'] = actual_end_time
        ...
        if start_time is not None or end_time is not None:
            print(f"✔ Time range processed: {actual_start_time:.1f}s - {actual_end_time:.1f}s ({total_duration_seconds:.1f}s)")
