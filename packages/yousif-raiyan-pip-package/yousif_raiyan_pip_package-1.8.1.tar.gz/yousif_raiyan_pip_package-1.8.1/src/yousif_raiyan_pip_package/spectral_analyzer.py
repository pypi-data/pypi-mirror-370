...
class SpectralAnalyzer:
    ...
    def analyze_spectral_parameterization(self, channels_to_analyze=None):
        ...
        **Periodic Components**: Identifies and quantifies discrete oscillatory peaks
        ...
        output_dir = os.path.join(self.loader.folder_path, self.loader.name, 'spectral_parameterization')
    ...
    def _save_multiband_results(self, channel, band_results, ma_windows_samples, 
        ...
        for band_name in self.frequency_bands:
            ...
            for window_samples in ma_windows_samples:
                ...
                # Save to CSV
                csv_filename = f"{channel}_{band_name}_ma{window_ms}ms_median.csv"
            ...
            plot_filename = f"{channel}_{band_name}_multiband_analysis.png"
    ...
    def _save_fooof_channel_results(self, result, output_dir):
        ...
        # Save FOOOF model fit plot
        fig_path = os.path.join(channel_dir, f'{channel}_fooof_model_fit.png')
        ...
        psd_path = os.path.join(channel_dir, f'{channel}_power_spectral_density.png')
        plt.savefig(psd_path, dpi=300, bbox_inches='tight')
        plt.close()
        
        # Save FOOOF parameters to CSV
        params_data = {
            'channel': [channel],
        ...
        params_path = os.path.join(channel_dir, f'{channel}_fooof_parameters.csv')
        ...
        band_path = os.path.join(channel_dir, f'{channel}_frequency_band_powers.csv')
    ...
    def _save_fooof_summary_results(self, output_dir):
        ...
        summary_path = os.path.join(output_dir, 'fooof_parameters_summary.csv')
        summary_df.to_csv(summary_path, index=False)
        
        band_summary_df = pd.DataFrame(band_powers_data)
        band_summary_path = os.path.join(output_dir, 'frequency_band_powers_summary.csv')
        band_summary_df.to_csv(band_summary_path, index=False)
        
        # Create summary visualization plots
        self._create_fooof_summary_plots(summary_df, band_summary_df, output_dir)
    ...
    def _create_fooof_summary_plots(self, summary_df, band_df, output_dir):
        ...
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, 'aperiodic_exponent_comparison.png'), 
        ...
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, 'spectral_peaks_comparison.png'), 
                   dpi=300, bbox_inches='tight')
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, 'frequency_band_powers_heatmap.png'), 
                   dpi=300, bbox_inches='tight')
        plt.close()
        ...
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, 'model_fit_quality_comparison.png'), 
                   dpi=300, bbox_inches='tight')
        plt.close()