...
PADDLE_API Tensor ap_trivial_fusion_begin(const paddle::optional<std::vector<Tensor>>& xs);

PADDLE_API Tensor ap_trivial_fusion_end(const paddle::optional<std::vector<Tensor>>& xs);
...
PADDLE_API Tensor expand_as(const Tensor& x, const paddle::optional<Tensor>& y, const std::vector<int64_t>& target_shape = {});

PADDLE_API Tensor expand_modality_expert_id(const Tensor& expert_id, int64_t num_expert_per_modality, int64_t group_size, int64_t modality_offset, bool is_group_expert);
...
PADDLE_API Tensor index_elementwise_get(const Tensor& x, const std::vector<Tensor>& index, const std::vector<int64_t>& input_dims, const std::vector<int64_t>& input_strides, const std::vector<int64_t>& index_dims, const std::vector<int64_t>& index_stride, int64_t slice_offset = 0, bool accumulate = true);

PADDLE_API Tensor index_elementwise_put(const Tensor& x, const std::vector<Tensor>& index, const Scalar& value, const std::vector<int64_t>& input_dims, const std::vector<int64_t>& input_strides, const std::vector<int64_t>& index_dims, const std::vector<int64_t>& index_strides, int64_t slice_offset);
...
PADDLE_API Tensor index_elementwise_put_with_tensor(const Tensor& x, const std::vector<Tensor>& index, const Tensor& value, const std::vector<int64_t>& input_dims, const std::vector<int64_t>& input_strides, const std::vector<int64_t>& index_dims, const std::vector<int64_t>& index_strides, int64_t slice_offset);
...
PADDLE_API Tensor masked_fill(const Tensor& x, const Tensor& mask, const Tensor& value);
...
PADDLE_API Tensor moe_combine(const Tensor& x, const Tensor& combine_weights, const Tensor& scatter_index);

PADDLE_API Tensor moe_combine_no_weight(const Tensor& x, const Tensor& combine_weight, const Tensor& scatter_index, float epsilon = 1.0e-15);
...
PADDLE_API std::tuple<Tensor, Tensor, Tensor, Tensor> moe_permute(const Tensor& hidden_states, const paddle::optional<Tensor>& scale, const Tensor& expert_routemap_topk, const Tensor& expert_prob_topk, int num_experts, const std::vector<int>& tokens_per_expert, int padding_alignment);
...
PADDLE_API Tensor pool2d(const Tensor& x, const IntArray& kernel_size, const std::vector<int64_t>& strides, const std::vector<int64_t>& paddings, bool ceil_mode, bool exclusive, const std::string& data_format, const std::string& pooling_type, bool global_pooling, bool adaptive, const std::string& padding_algorithm);

PADDLE_API Tensor pool3d(const Tensor& x, const std::vector<int64_t>& kernel_size, const std::vector<int64_t>& strides, const std::vector<int64_t>& paddings, bool ceil_mode, bool exclusive, const std::string& data_format, const std::string& pooling_type, bool global_pooling, bool adaptive, const std::string& padding_algorithm);
...
PADDLE_API Tensor rint(const Tensor& x);
...
PADDLE_API Tensor weight_dequantize(const Tensor& x, const Tensor& scale, const std::string& algo = "weight_only_int8", int group_size = -1);
...
PADDLE_API Tensor int_bincount(const Tensor& x, int64_t low, int64_t high, int64_t dtype);
