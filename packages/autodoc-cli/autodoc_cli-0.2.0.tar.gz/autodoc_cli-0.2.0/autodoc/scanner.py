...
def process_file(
            ...
            if ("```" in raw) or ("/*" in raw[2:] or "*/" in raw[:-2]):
                ...
                if sanitized:
                    ...
                    indentation = indentation[
                        : len(indentation) - len(indentation.lstrip(" \t"))
                    ]
    ...
    for func in functions:
        ...
        if has_doc and func.doc_range:
            ...
            sig_text = (
                source[func.signature_range.start : func.signature_range.end]
        ...
        if has_doc:
            if dry_run:
                ...
                comment_block = build_comment_block(
                    adapter.language_name, doc, indentation
                )
                if func.doc_range:
                    staged_edits.append(
                        ReplaceEdit(
