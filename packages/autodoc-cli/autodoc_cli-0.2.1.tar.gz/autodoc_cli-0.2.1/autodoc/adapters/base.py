...
class LanguageAdapter:
    ...
    @property
    def parser(self) -> Any:
        """Get the tree-sitter parser for this language adapter."""
        raise NotImplementedError

    def iter_functions(
        self, source_code: bytes
    ) -> Iterable[FunctionInfo]:  # pragma: no cover - interface
        raise NotImplementedError

    def get_function_source(self, source_code: bytes, function: FunctionInfo) -> str:
        return source_code[function.full_range.start : function.full_range.end].decode(
            "utf-8", errors="replace"
        )

    def get_function_body(self, source_code: bytes, function: FunctionInfo) -> str:
        return source_code[function.body_range.start : function.body_range.end].decode(
            "utf-8", errors="replace"
        )
