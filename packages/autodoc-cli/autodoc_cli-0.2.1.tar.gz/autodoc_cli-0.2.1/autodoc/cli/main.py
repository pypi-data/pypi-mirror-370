...
app = typer.Typer(
    ...
    help="Generate documentation comments using Tree-sitter and a local Ollama model.",
)
...
def run(
    ...
    model: str = typer.Option(
        "qwen2.5-coder:7b", "--model", help="Ollama model name/tag"
    ),
    db_path: Optional[str] = typer.Option(
        None,
        "--db",
        help="Path to SQLite database (defaults to .autodoc.sqlite in project root)",
    ),
    dry_run: bool = typer.Option(
        False, "--dry-run", help="Do not modify files; print planned changes"
    ),
):
    ...
    database_path = (
        Path(db_path).resolve() if db_path else (target_path / ".autodoc.sqlite")
    )
    ...
    if not dry_run and not llm.is_available():
        typer.echo(
            "Ollama is not reachable at http://localhost:11434. Start it with 'ollama serve' and ensure the model is pulled (e.g., 'ollama pull ' + model).",
            err=True,
        )
        raise typer.Exit(code=2)
