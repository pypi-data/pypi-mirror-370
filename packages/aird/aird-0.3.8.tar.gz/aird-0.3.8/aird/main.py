...
class UploadHandler(BaseHandler):
    ...
    async def prepare(self):
        ...
        try:
            dir_header = self.request.headers.get("X-Upload-Dir", "")
            filename_header = self.request.headers.get("X-Upload-Filename", "")

            self.directory = unquote(dir_header or "")
            self.filename = unquote(filename_header or "")
            ...
            if not self.final_path_abs.startswith(os.path.abspath(ROOT_DIR)):
                self.set_status(403)
            ...
            self.temp_file = await loop.run_in_executor(
            ...
            logging.info(f"Upload prepared for {self.filename} in directory {self.directory}")
        except Exception as e:
            logging.error(f"Error preparing upload: {e}")
            self.set_status(500)
    ...
    def data_received(self, chunk):
        try:
            # Synchronous write is generally fast for local disks and avoids coroutine handling pitfalls
            self.temp_file.write(chunk)
            self.bytes_received += len(chunk)
        except Exception as e:
            logging.error(f"Error receiving upload data: {e}")
            try:
                self.set_status(500)
    ...
    async def post(self):
        loop = tornado.ioloop.IOLoop.current()
        try:
            await loop.run_in_executor(self.settings['executor'], self.temp_file.close)
            ...
            await loop.run_in_executor(
                self.settings['executor'],
                functools.partial(os.makedirs, os.path.dirname(self.final_path_abs), exist_ok=True)
            )
            await loop.run_in_executor(self.settings['executor'], shutil.move, self.temp_file.name, self.final_path_abs)
            ...
            logging.error(f"Error completing upload for {self.filename}: {e}")
            self.set_status(500)
            self.write(f"Error completing upload: {e}")
        finally:
            # Ensure the temporary file is cleaned up in case of an error
            try:
                exists = await loop.run_in_executor(self.settings['executor'], os.path.exists, self.temp_file.name)
                if exists:
                    await loop.run_in_executor(self.settings['executor'], os.remove, self.temp_file.name)
            except Exception as e:
                logging.error(f"Error cleaning up temp file {self.temp_file.name}: {e}")
