...
def test_main_fast_click_option(mocker: MockerFixture) -> None:  # noqa
    # Arrange
    basic_click, mock_factory, mock_clicker = _make_and_mock_basic_click(
        mocker, fast=True
    )
    ...
    assert basic_click.debug is False
...
def test_list_output_with_debug_and_fast_and_click_type_flags(
...
def test_fast_flag_gets_passed_in_correctly_with_click_type_flag(
    mocker: MockerFixture,
):  # noqa
    basic_click, mock_factory, mock_click = _make_and_mock_basic_click(
        mocker, fast=True, debug=True
...
def test_click_factory_throws_ClickStrategyNotFound_and_stdout_correctly(
    ...
    mocker.patch("clickpy.click_strategy_factory", side_effect=ClickStrategyNotFound)
    mocker.patch("clickpy.auto_click", side_effect=KeyboardInterrupt)
...
def test_click_factory_throw_ClickStrategyNotFound_and_stdout_correctly_with_debug_flag(
    mocker: MockerFixture,
):  # noqa
    mocker.patch("clickpy.click_strategy_factory", side_effect=ClickStrategyNotFound)
    mocker.patch("clickpy.auto_click", side_effect=KeyboardInterrupt)
