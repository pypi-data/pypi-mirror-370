...
class FileSelectionGUI(QtWidgets.QMainWindow):
    ...
    def __init__(self, files):
        ...
        self.vibe_label.setStyleSheet(
            "background: qlineargradient(x1:0, y1:0, x2:1, y2:0, stop:0 #40c03f, stop:1 #1f503f); "
        ...
        # Add "Buy Pro" link for non-Pro users
        if not pro.enabled:
            buy_pro_layout = QtWidgets.QHBoxLayout()
            buy_pro_layout.addStretch()
            buy_pro_label = QtWidgets.QLabel(
                '<a href="https://tombrothers.gumroad.com/l/zthvs" style="color:#28a745;">Buy lifetime Pro license here</a>')
            buy_pro_label.setOpenExternalLinks(True)
            buy_pro_layout.addWidget(buy_pro_label)
            premium_content_layout.addLayout(buy_pro_layout)
