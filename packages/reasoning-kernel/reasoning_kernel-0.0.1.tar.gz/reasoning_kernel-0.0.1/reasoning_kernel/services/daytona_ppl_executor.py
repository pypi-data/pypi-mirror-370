...
class DaytonaPPLExecutor:
    ...
    async def validate_ppl_program(self, program: PPLProgram) -> List[str]:
        ...
        try:
            ...
            if program.entry_point and f"def {program.entry_point}" not in program.code:
                validation_errors.append(f"Entry point function '{program.entry_point}' not found")
    ...
    async def execute_ppl_program(self, program: PPLProgram, use_existing_sandbox: bool = False) -> PPLExecutionResult:
        """
        Execute probabilistic program in secure Daytona sandbox.

        Args:
            program: PPL program to execute
            use_existing_sandbox: Use current sandbox if available
