    ...
    async def _execute_with_retry_async(self, func: Callable, *args, **kwargs) -> Any:
        ...
        for attempt in range(self.config.max_retries + 1):
            ...
            try:
                ...
                self._record_failure()
                simple_log_error(logger, "execute_with_retry_sync_non_retriable", e, service_type=self.config.service_type.value)
                raise
    ...
    def _execute_with_retry_sync(self, func: Callable, *args, **kwargs) -> Any:
        ...
        for attempt in range(self.config.max_retries + 1):
            ...
            try:
                ...
                # Non-retriable exception
                self._record_failure()
                simple_log_error(logger, "execute_with_retry_async_non_retriable", e, service_type=self.config.service_type.value)
                raise
