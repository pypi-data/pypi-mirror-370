...
"""
    ...
    def run_lm_eval(self, tasks="hellaswag", num_fewshot=0):
        ...
        # Use 'hf' instead of 'hf-causal-experimental'
        if config.model_type == "gguf":
            lm_eval_model = "gguf"
            model_args = f"model_file={model_name}"
        else:
            lm_eval_model = "hf"
        ...
        try:
            results = evaluator.simple_evaluate(
                model=lm_eval_model,  # Changed from "hf-causal-experimental"
                model_args=model_args,  # String format, not dict
                tasks=tasks,
