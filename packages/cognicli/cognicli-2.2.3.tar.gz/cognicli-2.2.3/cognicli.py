...
class HuggingFaceAuth:
    """Enhanced HuggingFace authentication manager"""
    ...
    def _initialize_auth(self):
        """Initialize authentication by checking various sources"""
        # Try to get token from environment variables
        self.token = os.environ.get('HUGGINGFACE_HUB_TOKEN') or os.environ.get('HF_TOKEN')
        ...
        if self.token:
            ...
            if self._validate_token():
                self.console.print(f"[green]✅ Authenticated with HuggingFace as: {self.user_info.get('name', 'Unknown')}[/green]")
            else:
                self.console.print(f"[yellow]⚠️  HuggingFace token found but invalid[/yellow]")
                self.token = None
        else:
            self.console.print(f"[dim]💡 No HuggingFace token found. Some gated models may be inaccessible.[/dim]")

    def _validate_token(self):
        """Validate HuggingFace token with user info retrieval"""
        try:
            ...
            self.console.print(f"[red]Failed to validate HuggingFace token: {e}[/red]")
            return False
...
class ModelManager:
    ...
    def is_model_loaded(self) -> bool:
        ...
        # For GGUF models, tokenizer is None, so we only check current_model
        # For Transformers models, we check both model and tokenizer
        if self.current_model is None or self.current_model_id is None:
            return False
        
        # If it's a GGUF model (no tokenizer), just check the model
        if hasattr(self.current_model, 'model_path'):  # GGUF model
            return True
        
        # For Transformers models, check both model and tokenizer
        return self.current_tokenizer is not None
...
class CogniCLI:
    ...
    def show_logo(self):
        ...
        version_text = Text("v2.1.1 - Premium Edition", style="dim")
...
"""
    ...
    def display_search_results(self, models: List[dict], query: str):
        ...
        for model in models:
            ...
            # Store gguf_available in the model dict for later use
            model['gguf_available'] = gguf_available
        ...
        gguf_models = [m for m in models if "✅" in str(m.get('gguf_available', '❌'))]
        if gguf_models:
    ...
    def show_quantization_status(self):
        """Show current quantization support status"""
        self.console.print(Panel(
            f"[bold {COGNICLI_ACCENT}]Quantization Support Status[/bold {COGNICLI_ACCENT}]\n\n"
            f"bitsandbytes: {'✅ Available' if BNB_AVAILABLE else '❌ Not installed'}\n"
            f"4-bit quantization: {'✅ Supported' if BNB_AVAILABLE else '❌ Requires bitsandbytes'}\n"
            f"8-bit quantization: {'✅ Supported' if BNB_AVAILABLE else '❌ Requires bitsandbytes'}\n\n"
            f"[bold]Usage Examples:[/bold]\n"
            f"• Load model with 4-bit quantization: [cyan]--model HuggingFaceH4/zephyr-7b-beta --type q4[/cyan]\n"
            f"• Load model with 8-bit quantization: [cyan]--model HuggingFaceH4/zephyr-7b-beta --type q8[/cyan]\n"
            f"• Load model with fp16 precision: [cyan]--model HuggingFaceH4/zephyr-7b-beta --type fp16[/cyan]\n\n"
            f"[dim]Install quantization support: pip install bitsandbytes[/dim]",
            title="🔧 Quantization Status",
            border_style=COGNICLI_ACCENT
        ))

    def show_welcome_message(self, model_loaded: bool = False):
        """Show welcome message with enhanced help and tips"""
        welcome_content = f"""
[bold {COGNICLI_ACCENT}]Welcome to CogniCLI v2.1.1![/bold {COGNICLI_ACCENT}] 🚀

[green]✨ Features:[/green]
• [bold]Local AI Models:[/bold] Hugging Face Transformers and GGUF support
• [bold]Ollama Integration:[/bold] List, search, and pull models from Ollama
• [bold]Advanced Quantization:[/bold] Support for q4, q5, q8, f16, f32 precisions
• [bold]Tool Use:[/bold] AI agents with file operations, web search, and more
• [bold]Benchmarking:[/bold] Performance testing and lm-eval-harness integration
• [bold]Synapse Models:[/bold] Special handling for <think>/<answer> models

[green]🔧 Quick Start:[/green]
• [bold]--model <id>[/bold]     Load a Hugging Face model
• [bold]--search <query>[/bold] Search for models on Hugging Face
• [bold]--ollama list[/bold]    List available Ollama models
• [bold]--ollama search <query>[/bold] Search Ollama models
• [bold]--ollama pull <model>[/bold] Pull model from Ollama
• [bold]--chat[/bold]          Start interactive chat mode after loading a model

[green]💡 Tips:[/green]
• Use [bold]--help[/bold] for detailed command information
• Models are cached locally for faster loading
• GGUF models support various quantization levels
• Ollama models can be pulled with specific quantizations

[dim]Type [green]help[/green] for commands or [red]exit[/red] to quit.[/dim]
"""
        self.console.print(Panel(welcome_content, title="🎯 Getting Started", border_style=COGNICLI_ACCENT))
...
[green]quantization[/green]  - Show quantization support status
...
[green]--quantization-status[/green]   Show quantization support status
[green]--help[/green]                  Show this help message
...
[green]Name:[/green] {config.name}
[green]Type:[/green] {config.model_type.title()}
[green]Precision:[/green] {config.precision}
[green]Context Length:[/green] {config.context_length:,}
[green]Temperature:[/green] {config.temperature}
[green]Top P:[/green] {config.top_p}
[green]Max Tokens:[/green] {config.max_tokens}
"""
...
[green]Model Loaded:[/green] {'Yes' if self.model_manager.is_model_loaded() else 'No'}
[green]GPU Available:[/green] {'Yes' if torch.cuda.is_available() else 'No'}
[green]Memory Usage:[/green] {psutil.virtual_memory().percent}%
[green]Python Version:[/green] {sys.version.split()[0]}
"""
...
"""
    ...
    def _handle_cli_argument(self, prompt: str, model_loaded: bool):
        ...
        try:
            ...
            if args[0] == '--model' and len(args) > 1:
                ...
                self.show_interactive_help()
            elif args[0] == '--quantization-status':
                self.show_quantization_status()
                
            elif args[0] == '--search' and len(args) > 1:
                query = args[1]
                self.console.print(f"[{COGNICLI_ACCENT}]Searching for models: {query}[/{COGNICLI_ACCENT}]")
                models = self.search_models(query, limit=20)
                if models:
                    self.display_search_results(models, query)
                else:
                    self.console.print("[yellow]No models found.[/yellow]")
                ...
                if args[1] == 'list':
                    ...
                    quantization = args[3] if len(args) > 3 else None
                    self._pull_ollama_model(args[2], quantization)
                elif args[1] == 'quantizations':
                    self.show_quantization_options()
                else:
    ...
    def start_interactive_chat(self):
        ...
        self.console.print(f"\n[bold {COGNICLI_ACCENT}]🤖 Interactive Chat Mode[/bold {COGNICLI_ACCENT}]")
        self.console.print(f"[dim]Type your message to chat with the AI. Use Ctrl+C to exit.[/dim]\n")
        ...
        while True:
            try:
                ...
                # Handle special commands
                if prompt.lower() in ["exit", "quit", "q"]:
                    self.console.print(f"\n[bold yellow]Exiting chat mode. Goodbye![/bold yellow] 🎉")
                    break
                ...
                elif prompt.lower() == "quantization":
                    self.show_quantization_status()
                    continue
...
Examples:
  ...
  cognicli --ollama quantizations         # Show quantization options
    ...
    parser.add_argument('--version', action='version', version='CogniCLI v2.1.1 - Premium Edition')
    ...
    if args.ollama:
        ...
        if len(ollama_args) == 1:
            ...
            cli._pull_ollama_model(ollama_args[1], ollama_args[2])
        else:
    ...
    # If model was loaded via --model and no specific mode requested, auto-enter chat mode
    if loaded_model and not any([args.chat, args.generate, args.benchmark, args.lm_eval]):
        cli.console.print(f"\n[bold {COGNICLI_ACCENT}]🚀 Model loaded successfully! Entering interactive chat mode...[/bold {COGNICLI_ACCENT}]")
        cli.start_interactive_chat()
    else:
