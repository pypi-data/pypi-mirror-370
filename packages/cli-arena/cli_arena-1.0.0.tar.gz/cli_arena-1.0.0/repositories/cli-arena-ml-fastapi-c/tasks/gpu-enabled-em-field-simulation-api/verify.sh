...
echo -e "${BLUE}🔍 Verifying GPU-Enabled EM Field Simulation API...${NC}"

if [ -d "/repo" ]; then REPO_DIR="/repo"; else REPO_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"; fi
SRC_DIR="$REPO_DIR/src"; TASK_DIR="$REPO_DIR/tasks/gpu-enabled-em-field-simulation-api"
MOD="gpu_enabled_em_field_simulation_api"
...
check "task test present" "test -f '$TASK_DIR/test_gpu_enabled_em_field_simulation_api.py'"
