...
echo -e "${BLUE}🔍 Verifying Chemistry-Aware REST API in Kubernetes...${NC}"

if [ -d "/repo" ]; then REPO_DIR="/repo"; else REPO_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"; fi
SRC_DIR="$REPO_DIR/src"; TASK_DIR="$REPO_DIR/tasks/chem-aware-rest-api-in-kubernetes"
MOD="chem_aware_rest_api_in_kubernetes"
...
check "task test present" "test -f '$TASK_DIR/test_chem_aware_rest_api_in_kubernetes.py'"
...
check_out "deploy to chem-ns1" "cd '$SRC_DIR' && python3 -m $MOD.cli deploy --namespace chem-ns1" "API deployed to namespace"
check_out "status deployed" "cd '$SRC_DIR' && python3 -m $MOD.cli status --namespace chem-ns1" "API status for\|deployed"
check_out "query ethanol" "cd '$SRC_DIR' && python3 -m $MOD.cli query --smiles CCO" "Query result\|Ethanol"
