...
echo -e "${BLUE}🔍 Verifying Hybrid-Cloud Tensor Calculus Solver...${NC}"

if [ -d "/repo" ]; then REPO_DIR="/repo"; else REPO_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"; fi
SRC_DIR="$REPO_DIR/src"; TASK_DIR="$REPO_DIR/tasks/hybrid-cloud-tensor-calculus-solver"
MOD="hybrid_cloud_tensor_calculus_solver"
...
check "task test present" "test -f '$TASK_DIR/test_hybrid_cloud_tensor_calculus_solver.py'"
...
check_out "deploy to aws" "cd '$SRC_DIR' && python3 -m $MOD.cli deploy --provider aws" "Solver deployed to"
check_out "status shows deployed" "cd '$SRC_DIR' && python3 -m $MOD.cli status --provider aws" "Deployment status for\|deployed"
check_out "solve expression" "cd '$SRC_DIR' && python3 -m $MOD.cli solve --expr 'diff(x**3, x)'" "Solution"
