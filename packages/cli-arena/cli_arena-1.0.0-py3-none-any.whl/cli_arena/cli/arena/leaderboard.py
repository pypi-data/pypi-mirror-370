...
def web(
    ...
    if enhanced:
        ...
        console.print("💡 To start the real-time server, run: [bold]python -m cli_arena.leaderboard.server[/bold]")
    else:
...
def serve(
    ...
    try:
        ...
        console.print(f"🚀 Starting CLI Arena Leaderboard Server at http://{host}:{port}")
...
def ci_setup(
    ...
    console.print(f"✅ Created GitHub Actions workflow: {gh_workflow_path}")
    
    # Create GitLab CI config  
    gitlab_ci_path = output_dir / ".gitlab-ci.yml"
    pipeline.create_gitlab_ci_config(gitlab_ci_path)
    console.print(f"✅ Created GitLab CI config: {gitlab_ci_path}")
    
    console.print("\n[bold green]🚀 CI/CD Setup Complete![/bold green]")
    console.print("\nNext steps:")
...
def ci_update(
    ...
    console.print(f"✅ Processed {len(processed_files)} files, added {total_added} rows")
    
    # Generate all artifacts
    console.print("🎨 Generating web artifacts...")
    artifacts = pipeline.generate_all_artifacts()
    
    console.print(f"✅ Generated {len(artifacts)} artifact types:")
