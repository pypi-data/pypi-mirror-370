...
def cmd_scan_tasks(args) -> int:
    ...
    if not tasks:
        print("❌ No tasks found in repository")
        return 1
    
    print(f"📋 Found {len(tasks)} tasks in {repo_path.name}:")
    print("-" * 60)
...
def cmd_create_dataset(args) -> int:
    ...
    try:
        ...
        print(f"✅ Created dataset metadata: {output_file}")
...
def cmd_package_dataset(args) -> int:
    ...
    try:
        ...
        print(f"✅ Packaged dataset: {package_path}")
...
def cmd_publish_remote(args) -> int:
    ...
    if not args.api_key:
        print("❌ API key required for remote publishing")
...
def cmd_list_registry(args) -> int:
    ...
    try:
        ...
        print(f"📋 Local Registry ({len(datasets)} datasets):")
        print("-" * 60)
