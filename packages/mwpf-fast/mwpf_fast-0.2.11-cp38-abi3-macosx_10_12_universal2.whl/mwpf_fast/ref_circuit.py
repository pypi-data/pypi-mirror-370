...
class RefDetectorErrorModel:
    ...
    @functools.cached_property
    def initializer(self) -> mwpf_fast.SolverInitializer:
        ...
        return mwpf_fast.SolverInitializer(vertex_num, weighted_edges)
...
class Predictor(Protocol):
    ...
    def syndrome_of(self, dets_bit_packed: np.ndarray) -> mwpf_fast.SyndromePattern: ...
    def prediction_of(
        self, syndrome: mwpf_fast.SyndromePattern, subgraph: Sequence[int]
    ) -> int: ...
...
class StaticPredictor(Predictor):
    ...
    def syndrome_of(self, dets_bit_packed: np.ndarray) -> mwpf_fast.SyndromePattern:
        ...
        return mwpf_fast.SyndromePattern(defect_vertices=defect_vertices)

    def prediction_of(
        self, _syndrome: mwpf_fast.SyndromePattern, subgraph: Sequence[int]
    ) -> int:
