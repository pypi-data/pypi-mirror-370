...
class EIPClient:
    ...
    def track_propagations(self,df_list: List[pd.DataFrame], propagation_level: int) -> pd.DataFrame:
        """
        Tracks how directional information propagates through a system at different time resolutions.

        Parameters:
        -----------
        df_list : List[pd.DataFrame]
            A list of DataFrames, where each DataFrame should have 'datetime', 'trend_identified',
            and 'level' columns.
        propagation_level : int
            The highest level that must be present in a group for it to be considered valid.

        Returns:
        --------
        pd.DataFrame
            A DataFrame containing only the groups that meet the propagation level criteria.
        
        Raises:
        -------
        ValueError
            If a DataFrame in df_list does not contain the required columns.
        """
        
        # Helper function to read a file or validate a DataFrame
        def read_dataframe(data: Union[str, pd.DataFrame], **kwargs) -> pd.DataFrame:
            """
            Reads a file into a pandas DataFrame or validates an existing DataFrame.
            ...
            **kwargs : dict, optional
            ...
            ohlc_cols = {'datetime', 'trend_identified'}
            current_cols = set(df.columns.str.lower())
            # Check if the required columns are present
            has_ohlc = ohlc_cols.issubset(current_cols)

            if not has_ohlc:
                raise ValueError("DataFrame must contain the columns: 'datetime', 'trend_identified")
            ...
            # Loop through a list of valid date formats to try and parse the 'datetime' column
            for fmt in valid_date_formats:
                try:
                    # Try to convert using a specific format
                    datetime_series = pd.to_datetime(df['datetime'], format=fmt)
        ...
        def initial_directional_changes(df: pd.DataFrame) -> pd.DataFrame:
            """Filters a DataFrame to find initial directional changes."""
            
            # Keep only rows where trend is either 1 or -1
            df_filtered = df[df['trend_identified'].isin([-1, 1])].copy()
            
            # Check for a change in 'trend_identified' from the previous row
            has_changed = df_filtered['trend_identified'] != df_filtered['trend_identified'].shift(1)
            
            # The first row of each group is considered a change, so we fill the first value with True
            has_changed = has_changed.fillna(True)
            
            # The mask identifies rows with a trend change
            mask = has_changed
            
            return df_filtered[mask].reset_index(drop=True)

        # Validate propagation_level
        if propagation_level < 1:
            raise ValueError("Propagation level must be at least 1.")

        formatted_dfs = []

        # Iterate through the list of dataframes with their position (level)
        for position, df in enumerate(df_list):
            required_columns = {'datetime', 'trend_identified'}
            
            # Check if the required columns are in the current dataframe
            if not required_columns.issubset(df.columns):
                raise ValueError("Each DataFrame must contain 'datetime' and 'trend_identified' columns.")

            # Read or validate the current dataframe
            formatted_df = read_dataframe(df)

            # Apply the initial directional changes filter
            filtered_df = initial_directional_changes(formatted_df)

            # Set the level for the filtered DataFrame
            filtered_df['propagation_level'] = position

            formatted_dfs.append(filtered_df)

        # Return an empty dataframe if the list of formatted dataframes is empty
        ...
        combined_df = combined_df.sort_values(['datetime', 'propagation_level']).reset_index(drop=True)
        
        # Assign a unique group ID to each sequence of levels starting from level 0
        combined_df['group_id'] = (combined_df['propagation_level'] == 0).cumsum()

        # Define the set of required levels that a valid group must contain
        # e.g., if propagation_level is 2, required_levels = {0, 1, 2}
        required_levels = set(range(propagation_level + 1))

        # Filter out groups that do not contain all the required levels
        df_filtered = combined_df.groupby(['group_id', 'trend_identified']).filter(
        lambda group: required_levels.issubset(set(group['propagation_level'].unique()))
        )
        
        # Check if the filtered dataframe has any data before returning
        if df_filtered.empty:
            print("Directional changes did not propagate to required level.")
            return(None)
        else:
            # Re-sort to maintain order after filtering
            df_filtered = df_filtered.sort_values(['group_id', 'datetime']).drop('group_id',axis=1).reset_index(drop=True)
            conditions = [
                (df_filtered['propagation_level'] == 0) & (df_filtered['trend_identified'] == 1),
                (df_filtered['propagation_level'] == 0) & (df_filtered['trend_identified'] == -1)
            ]
            choices = ['initial positive indicator', 'initial negative indicator']

            # Create a new 'status' column using np.select
            # For rows that don't match the conditions, the original propagation_level value is kept
            df_filtered['propagation_level'] = np.select(conditions, choices, default=df_filtered['propagation_level'])
            return df_filtered


    def calculate_directional_change(self,price_data,propagation_data):

            
        # Helper function to read a file or validate a DataFrame
        def read_price_data(data: Union[str, pd.DataFrame], **kwargs) -> pd.DataFrame:
            """
            Reads a file into a pandas DataFrame or validates an existing DataFrame.
            ...
            **kwargs : dict, optional
            ...
            ohlc_cols = {'datetime', 'open'}
            current_cols = set(df.columns.str.lower())
            # Check if the required columns are present
            has_ohlc = ohlc_cols.issubset(current_cols)

            if not has_ohlc:
                raise ValueError("DataFrame must contain the columns: 'datetime' and 'open'")
        ...
        # Helper function to read a file or validate a DataFrame
        def read_propagation_data(data: Union[str, pd.DataFrame], **kwargs) -> pd.DataFrame:
            """
            Reads a file into a pandas DataFrame or validates an existing DataFrame.
            ...
            **kwargs : dict, optional
            ...
            ohlc_cols = {'datetime', 'trend_identified','propagation_level'}
            current_cols = set(df.columns.str.lower())
            # Check if the required columns are present
            has_ohlc = ohlc_cols.issubset(current_cols)

            if not has_ohlc:
                raise ValueError("DataFrame must contain the columns: 'datetime', ,'trend_identified' and 'propagation_level'.")
        ...
        price = read_price_data(price_data)
        propagation = read_propagation_data(propagation_data)
        

        df = pd.merge(price,propagation,on='datetime',how='left')[['datetime','open','trend_identified','propagation_level']].dropna()

        df = df.sort_values(by='datetime')

        # Create a new column 'group_id' to identify each trend cycle.
        # We use `cumsum()` on a boolean Series that is True for each "initial" row.
        # This assigns a unique group ID to each trend sequence.
        df['group_id'] = (df['propagation_level'].str.contains('initial')).cumsum()

        # Use the `groupby()` and `transform('first')` methods to get the 'open' value
        # of the first row in each group (i.e., the initial price).
        # The `transform` method ensures the resulting Series has the same size as the original DataFrame.
        df['initial_open'] = df.groupby('group_id')['open'].transform('first')

        # Calculate the 'directional_change' by subtracting the initial price from the current price.
        df['directional_change'] = df['open'] - df['initial_open']

        # Drop the auxiliary columns created for the calculation
        df = df.drop(columns=['group_id', 'initial_open']).reset_index(drop=True)
