...
def get_user_by_token(request: Request,name:str="token"):
...
UPLOAD_DIR_ = "static/img/uploads"
    ...
    try:
        ...
        try:
            ...
            for f in files_list:
                ...
                # English: Optimize image only if it's a supported image
                # Español: Optimizar imagen solo si es una imagen soportada
                if extension.lower() in ["jpg", "jpeg", "png"]:
                    file_path = optimize_image(content, file_path, extension)

                files_to_return.append(str(file_path))

            # English: Return response with uploaded files
            # Español: Devolver respuesta con archivos subidos
...
def optimize_image(file_bytes: bytes, original_path: Path, extension: str, max_width=1920, quality=75) -> Path:
    """
    # English: Optimize an image and save as WebP using the bytes already in memory
    # Español: Optimiza una imagen y la guarda como WebP usando los bytes ya cargados en memoria
    """
    try:
        ...
        optimized_filename = f"{original_path.stem}.webp"
        optimized_path = original_path.parent / optimized_filename

        img.save(optimized_path, format="WEBP", optimize=True, quality=quality)

        # English: Remove original file after optimization
        # Español: Eliminar archivo original después de optimizar
