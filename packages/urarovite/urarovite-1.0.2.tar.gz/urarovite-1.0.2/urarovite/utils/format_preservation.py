...
class FormatExtractor:
    ...
    def extract_excel_formatting(
        ...
        try:
            ...
            logger.info(f"Extracted formatting for {len(cell_formats)} cells from {sheet_name}")
            return cell_formats
    ...
    def _has_formatting(self, cell: Any) -> bool:
        ...
        try:
            ...
            if (hasattr(cell.fill, 'fgColor') and 
                ...
                cell.fill.fgColor.rgb != '00000000' and  # Not black/default
                cell.fill.fgColor.rgb != 'FFFFFFFF' and  # Not white/default
                cell.fill.fgColor.rgb != 'FF000000' and  # Not default black
                len(cell.fill.fgColor.rgb) == 8 and      # Valid RGB format
                cell.fill.fill_type and                  # Has actual fill type
                cell.fill.fill_type != 'none'):          # Not "no fill"
                return True
            
            # Check for borders
            if (cell.border.top.style or 
                cell.border.bottom.style or 
                cell.border.left.style or 
                cell.border.right.style):
                return True
            
            # Check for alignment
            if (cell.alignment.horizontal != 'general' or 
    ...
    def _extract_excel_cell_format(self, cell: Any) -> CellFormat:
        ...
        try:
            ...
            if cell.fill and hasattr(cell.fill, 'fgColor'):
                ...
                if (bg_color and 
                    bg_color.lower() not in ['#000000', '#ffffff', '#ff000000', '#ffffffff'] and
                    cell.fill.fill_type and 
    ...
    def extract_google_sheets_formatting(
        ...
        try:
            ...
            logger.info(f"Extracted formatting for {len(cell_formats)} cells from Google Sheets")
            return cell_formats
        ...
        try:
            ...
            if 'borders' in effective_format:
                ...
                if 'bottom' in borders:
                    format_obj.border_bottom = self._google_border_style(borders['bottom'])
                if 'left' in borders:
                    format_obj.border_left = self._google_border_style(borders['left'])
                if 'right' in borders:
                    format_obj.border_right = self._google_border_style(borders['right'])
            ...
            if 'numberFormat' in effective_format:
                if 'pattern' in effective_format['numberFormat']:
                    format_obj.number_format = effective_format['numberFormat']['pattern']
...
class FormatApplier:
    ...
    def apply_excel_formatting(
        ...
        try:
            ...
            logger.info(f"Applied formatting to {len(cell_formats)} cells in {sheet_name}")
    ...
    def _apply_excel_cell_format(self, cell: Any, format_obj: CellFormat) -> None:
        ...
        try:
            # Font formatting
            if any([format_obj.font_name, format_obj.font_size, format_obj.font_bold, 
                   format_obj.font_italic, format_obj.font_underline, format_obj.font_color]):
                
                font = Font(
                    name=format_obj.font_name or cell.font.name,
                    size=format_obj.font_size or cell.font.size,
                    bold=format_obj.font_bold if format_obj.font_bold is not None else cell.font.bold,
                    italic=format_obj.font_italic if format_obj.font_italic is not None else cell.font.italic,
                    underline='single' if format_obj.font_underline else None,
                    color=format_obj.font_color[1:] if format_obj.font_color else None  # Remove #
                )
                cell.font = font
            ...
            # Alignment
            if any([format_obj.horizontal_align, format_obj.vertical_align, format_obj.text_wrap]):
                alignment = Alignment(
                    ...
                    wrap_text=format_obj.text_wrap if format_obj.text_wrap is not None else cell.alignment.wrap_text
                )
    ...
    def apply_google_sheets_formatting(
        ...
        try:
            ...
            if max_row > 0 or max_col > 0:
                ...
                logger.info(f"Expanded Google Sheets to {target_rows} rows x {target_cols} columns")
            ...
            if requests:
                ...
                logger.info(f"Applied formatting to {len(cell_formats)} cells in Google Sheets")
    ...
    def _create_google_format_request(
        ...
        try:
            ...
            if format_obj.font_color:
                text_format['foregroundColor'] = self._hex_to_google_color(format_obj.font_color)
            ...
            # Background color
            if format_obj.background_color:
                cell_format['backgroundColor'] = self._hex_to_google_color(format_obj.background_color)
            ...
            if format_obj.text_wrap is not None:
                cell_format['wrapStrategy'] = 'WRAP' if format_obj.text_wrap else 'OVERFLOW_CELL'
            ...
            if format_obj.number_format:
                ...
                if any(date_indicator in pattern.lower() for date_indicator in ['mm', 'dd', 'yyyy', 'yy', 'm/', 'd/']):
                    format_type = "DATE"
                elif any(time_indicator in pattern.lower() for time_indicator in ['hh', 'mm', 'ss', 'am', 'pm']):
                    format_type = "TIME"
                elif '%' in pattern:
                    format_type = "PERCENT"
...
def _translate_google_to_excel_format(format_obj: CellFormat) -> CellFormat:
    ...
    if format_obj.font_name:
        ...
        format_obj.font_name = font_mapping.get(format_obj.font_name, format_obj.font_name)
