...
class PlatformNeutralizerValidator(BaseValidator):
        ...
        def validation_logic(
            ...
            **kwargs
            ...
            try:
                ...
                result.set_automated_log(f"Column '{target_column}' not found in sheet - skipping platform neutralization")
                return

            prompts_with_platforms = []
            cleaned_prompts = []
            fixes_to_apply = []

            # Check each row for platform mentions
            for row_index, row in enumerate(data_rows, start=2):  # Start at 2 for sheet row numbering
                ...
                if mentions:
                    cell_ref = self._generate_cell_reference(row_index - 1, target_col_index)
                    
                    if mode == "flag":
                        prompts_with_platforms.append({
                            "row": row_index,
                            "cell_ref": cell_ref,
                            "original_text": prompt_text,
                            "mentions": [m["original"] for m in mentions]
                        })
                        result.add_issue()
                    else:  # fix mode
                        cleaned_text, editor_note = self._neutralize_platform_language(prompt_text, mentions)
                        
                        # Prepare fix
                        fixes_to_apply.append({
                            "row": row_index - 2,  # Convert to 0-based for data_rows
                        ...
                        cleaned_prompts.append({
                            ...
                            "mentions_neutralized": [m["original"] for m in mentions]
                        })
                        result.add_fix()
            ...
            if mode == "fix" and fixes_to_apply:
                ...
                for fix in fixes_to_apply:
                    ...
                    if len(data_rows[row_idx]) <= col_idx:
                        data_rows[row_idx].extend([''] * (col_idx + 1 - len(data_rows[row_idx])))
            ...
            # Set results
            if mode == "flag":
                result.details["prompts_with_platforms"] = prompts_with_platforms
                if result.issues_found == 0:
                    result.set_automated_log("No platform-specific language found")
                else:
                    result.set_automated_log(f"Found {result.issues_found} prompts with platform-specific language")
            else:
                result.details["cleaned_prompts"] = cleaned_prompts
                if result.fixes_applied == 0:
                    result.set_automated_log("No platform-specific language found")
                else:
                    result.set_automated_log(f"Neutralized platform language in {result.fixes_applied} prompts")

        return self._execute_validation(
            validation_logic, spreadsheet_source, auth_credentials, target_column=target_column, **kwargs
        )
    ...
    def _find_platform_mentions(self, text: str) -> List[Dict[str, str]]:
        ...
        for mention in mentions:
            ...
            for existing in filtered_mentions:
                if (mention["start_pos"] < existing["end_pos"] and 
                    mention["end_pos"] > existing["start_pos"]):
                    overlaps = True
                    break
    ...
    def _neutralize_platform_language(self, text: str, mentions: List[Dict[str, str]]) -> Tuple[str, str]:
        ...
        for mention in reversed(mentions):
            ...
            # Replace the mention
            cleaned_text = cleaned_text[:start_pos] + replacement + cleaned_text[end_pos:]
            replaced_terms.append(f"'{original}' → '{replacement}'")

        # Generate editor note
        if len(replaced_terms) == 1:
            editor_note = f"[Editor: Neutralized platform language: {replaced_terms[0]}]"
        else:
