...
def test_detect_hidden_unicode_cells():
    ...
    assert "affected_cells" in result["details"]
    assert len(result["details"]["affected_cells"]) == 2
    counts = result["details"]["counts_by_code"]
    assert counts.get("\\u00A0") == 1
    assert counts.get("\\u2009") == 1
    assert counts.get("\\u200B") == 1
    cell_map = {(c["row"], c["col"]): c for c in result["details"]["affected_cells"]}
    assert cell_map[(2, 1)]["normalized_preview"] == "A B"
    assert cell_map[(3, 2)]["normalized_preview"] == "Y Z"
    assert "Hidden unicode detected in 2 cells" in result["automated_log"]
    assert "3 characters flagged" in result["automated_log"]


def test_no_issues_found():
    """
    Test that the validator does not find any issues in a sheet with no hidden unicode.
    """
    validator = HiddenUnicodeValidator()
    data = [["A", "B"], ["C", "D"]]
    sheet_url = "https://docs.google.com/spreadsheets/d/sheet123/edit"
    ...
    assert result["details"]["affected_cells"] == []
    assert result["details"]["counts_by_code"] == {}
    rules = result["details"]["normalization_rules"]
    rule_map = {r["code"]: r for r in rules}
    assert rule_map.get("\\u00A0")["rule"] in {"space", "remove"}


def test_empty_sheet():
    """
    Test that the validator does not find any issues in an empty sheet.
    """
    validator = HiddenUnicodeValidator()
    sheet_url = "https://docs.google.com/spreadsheets/d/sheet123/edit"
...
def test_invalid_url():
    ...
    with patch.object(validator, "_get_spreadsheet", side_effect=Exception("Invalid URL")):
        result = validator.validate(
            spreadsheet_source=sheet_url, 
            mode="flag",
            auth_credentials={"encoded_credentials": "fake_creds"}
...
def test_run_function_delegates():
    ...
    """
    with patch("urarovite.validators.hidden_unicode.HiddenUnicodeValidator.validate", return_value={"issues_found": 0, "details": {}}) as mock_validate:
