...
class TestPlatformNeutralizerValidator:
    ...
    def test_find_platform_mentions_excel_patterns(self):
        ...
        test_cases = [
            ...
            ("Check the Excel sheet for errors.", 1, "the Excel"),  # More general pattern matches
        ...
        for text, expected_count, expected_mention in test_cases:
            ...
            if expected_count > 0:
                assert expected_mention.lower() in mentions[0]["original"].lower(), f"Expected mention '{expected_mention}' not found in: {text}"
    ...
    def test_neutralize_platform_language_simple(self):
        ...
        for original, expected_cleaned in test_cases:
            ...
            assert first_line == expected_cleaned, f"Expected: '{expected_cleaned}', Got: '{first_line}'"
    ...
    def test_validate_flag_mode_with_issues(self):
        ...
        with patch.object(self.validator, '_get_all_sheet_data') as mock_get_data:
            with patch.object(self.validator, '_get_spreadsheet') as mock_get_spreadsheet:
                ...
                assert "prompts_with_platforms" in result["details"]
                assert len(result["details"]["prompts_with_platforms"]) == 2
    ...
    def test_automated_log_messages(self):
        ...
        with patch.object(self.validator, '_get_all_sheet_data') as mock_get_data:
            with patch.object(self.validator, '_get_spreadsheet') as mock_get_spreadsheet:
                ...
                assert "Found 1 prompts with platform-specific language" in result["automated_log"]
        ...
        with patch.object(self.validator, '_get_all_sheet_data') as mock_get_data:
            with patch.object(self.validator, '_update_sheet_data'):
                with patch.object(self.validator, '_get_spreadsheet') as mock_get_spreadsheet:
                    ...
                    assert "Neutralized platform language in 1 prompts" in result["automated_log"]
...
class TestPlatformNeutralizerIntegration:
    ...
    def test_real_world_examples(self):
        ...
        real_examples = [
            ("Clean the data in the Excel file for analysis.", "Clean the data in the spreadsheet for analysis."),
            ("Update the Google Sheet with quarterly results.", "Update the spreadsheet with quarterly results."),
            ("Export the Excel workbook as CSV format.", "Export the spreadsheet as CSV format."),
            ("Create charts in the Google Sheets document.", "Create charts in the spreadsheet document."),
            ("Review Excel spreadsheet for data accuracy.", "Review spreadsheet for data accuracy."),
        ]
    ...
    def test_comprehensive_workflow(self):
        ...
        test_prompt = "Please clean the Excel file and update the Google Sheet with results."
        ...
        cleaned, note = self.validator._neutralize_platform_language(test_prompt, mentions)
    ...
    def test_edge_cases(self):
        ...
        for text, expected_originals in edge_cases:
            ...
            if mentions:
                ...
                cleaned, note = self.validator._neutralize_platform_language(text, mentions)
                first_line = cleaned.split('\n')[0]
