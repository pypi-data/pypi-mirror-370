...
class TestAttachedFilesCleanerValidator:
    ...
    def test_find_attachment_mentions_basic_patterns(self):
        ...
        test_cases = [
            ...
            ("The attached HR Food Desert Analysis spreadsheet combines multiple sources.", 1),
    ...
    def test_skip_non_file_attachments(self):
        ...
        for text in non_file_cases:
            mentions = self.validator._find_attachment_mentions(text)
            assert len(mentions) == 0, f"Should have skipped non-file attachment: {text}"
    ...
    def test_clean_attachment_mentions_simple_removal(self):
        ...
        test_cases = [
            ...
            ("Review the attached spreadsheet carefully.", "Review the spreadsheet carefully."),
            ("Update the attached document with changes.", "Update the document with changes."),
            
            # Preposition patterns
            ("In the attached sheet, highlight the rows.", "In the sheet, highlight the rows."),
            ("From the attached report, extract data.", "From the report, extract data."),
            
            # "Please find attached" → "Please review"
            ("Please find attached the 2025 Performance Files.", "Please review the 2025 Performance Files."),
            
            # "that I have attached" → "provided"
            ("Clean the Google Sheet that I have attached.", "Clean the Google Sheet provided."),
            
            # "data attached in" → "data in"
            ("Clean the data attached in the Google Sheet.", "Clean the data in the Google Sheet."),
            ...
            ("The attached HR Food Desert Analysis spreadsheet", "the HR Food Desert Analysis spreadsheet"),
        ]
        ...
        for original, expected in test_cases:
            ...
            if mentions:
                ...
                assert first_line == expected, f"Expected: '{expected}', Got: '{first_line}'"
    ...
    def test_clean_attachment_mentions_multiple(self):
        ...
        original = "Review the attached report and check the data in the attached spreadsheet."
        mentions = self.validator._find_attachment_mentions(original)
    ...
    def test_validate_flag_mode_with_issues(self):
        ...
        with patch.object(self.validator, '_get_all_sheet_data') as mock_get_data:
            with patch.object(self.validator, '_get_spreadsheet') as mock_get_spreadsheet:
                ...
                # Should find 2 issues (rows 1 and 2)
                assert result["fixes_applied"] == 0
                assert result["issues_found"] == 2
                assert len(result["errors"]) == 0
                
                # Should have details about the issues
                assert "prompts_with_attachments" in result["details"]
                assert len(result["details"]["prompts_with_attachments"]) == 2
        ...
        with patch.object(self.validator, '_get_all_sheet_data') as mock_get_data:
            with patch.object(self.validator, '_update_sheet_data') as mock_update_data:
                with patch.object(self.validator, '_get_spreadsheet') as mock_get_spreadsheet:
                    ...
                    assert "cleaned_prompts" in result["details"]
                    assert len(result["details"]["cleaned_prompts"]) == 2
    ...
    def test_should_skip_mention_functionality(self):
        ...
        skip_cases = [
            ("The parts are attached to each other in the assembly.", 15, 26),  # "attached to"
        ]
        ...
        dont_skip_cases = [
            ("These wires are attached to the circuit board.", 16, 27),  # Different usage, not skipped currently
    ...
    def test_automated_log_messages(self):
        ...
        # Test with issues found in flag mode
        test_data = [
            ["prompt"],
        ...
        with patch.object(self.validator, '_get_all_sheet_data') as mock_get_data:
            with patch.object(self.validator, '_get_spreadsheet') as mock_get_spreadsheet:
                ...
                assert "Found 1 prompts with attached file mentions" in result["automated_log"]
        ...
        with patch.object(self.validator, '_get_all_sheet_data') as mock_get_data:
            with patch.object(self.validator, '_update_sheet_data'):
                with patch.object(self.validator, '_get_spreadsheet') as mock_get_spreadsheet:
                    ...
                    assert "Cleaned 1 prompts with attached file mentions" in result["automated_log"]
...
class TestAttachedFilesCleanerIntegration:
    ...
    def test_edge_cases_and_boundary_conditions(self):
        ...
        edge_cases = [
            ...
            ("The attached file and the attached document", 1),  # Compound pattern may match as one
        ...
        for text, expected_count in edge_cases:
            mentions = self.validator._find_attachment_mentions(text)
            assert len(mentions) == expected_count, f"Expected {expected_count} mentions in: '{text}', got {len(mentions)}"
