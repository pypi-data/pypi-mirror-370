...
class TestCSVToJSONTransformValidator:
    ...
    def test_transform_row_basic(self):
        ...
        assert result["input_file"] == "https://docs.google.com/spreadsheets/d/1wVWKw_l5eIWiLsiQPDrzjmodg2hMK9-RYxgDpT1YWaI/edit?usp=sharing"
        assert result["output_file"] == "https://docs.google.com/spreadsheets/d/1ODsg5EHf7992lXcCZkilnsBeGEjJPqhafdrZj-4IhlE/edit?usp=sharing"
    ...
    def test_a1_range_detection(self):
        ...
        for range_value in malformed_ranges:
            assert validator._detect_malformed_a1_range(range_value), f"Should detect '{range_value}' as malformed A1 range"
        ...
        for range_value in well_formed_ranges:
            assert not validator._detect_malformed_a1_range(range_value), f"Should NOT detect '{range_value}' as malformed A1 range"
    ...
    def test_a1_range_fixing(self):
        ...
        test_cases = [
            ...
            ("Data SheetA1:B10", "'Data Sheet'!A1:B10"),        # Missing quotes and exclamation
            ("Q1 Results'!B1:C100", "'Q1 Results'!B1:C100"),    # Missing opening quote with spaces
    ...
    def test_validate_flag_mode(self):
        ...
        mock_result = {
            ...
            "details": {
                ...
                "json_schema": ["worker_id", "task_id", "prompt", "input_file", "output_file"],
                "sample_output": [
                    {
    ...
    def test_validate_fix_mode(self, mock_update):
        ...
        with patch.object(validator, '_execute_validation', side_effect=mock_execute_validation):
            result = validator.validate(
                spreadsheet_source="https://docs.google.com/spreadsheets/d/test_sheet_id/edit",
                mode="fix",
                auth_credentials={"auth_secret": "eyJ0eXBlIjogInNlcnZpY2VfYWNjb3VudCIsICJwcm9qZWN0X2lkIjogInRlc3QifQ=="}
    ...
    def test_realistic_a1_range_scenarios(self):
        ...
        for case in test_cases:
            ...
            assert result["verification_field_ranges"] == case["expected"], f"Failed for {case['description']}: {case['original']}"
            assert "_a1_range_fixes" in result
            assert len(result["_a1_range_fixes"]) == 1
    ...
    def test_default_fields_completeness(self):
        ...
        expected_defaults = {
            "input_excel_file", "output_excel_file", "field_mask",
            "case_sensitivity", "numeric_rounding", "color_matching",
            "editor_fixes", "editor_comments", "estimated_task_length"
        }
    ...
    def test_malformed_csv_non_string_headers(self):
        ...
        assert all(isinstance(v, str) for k, v in result.items() if not k.startswith('_'))
    ...
    def test_json_output_when_many_columns_exist(self, mock_update):
        ...
        def mock_execute_validation(*args, **kwargs):
            # Simulate that the validator calls update_sheet_values during execution
            mock_update("test_sheet_id", "Sheet1", [["JSON_Output"], ['{"worker_id": "TEST123"}']], "Z1:Z2")
            ...
            result.details["sample_output"] = [{"worker_id": "TEST123", "task_id": "TASK456", "prompt": "Test prompt"}]
            result.set_automated_log("Transformed 1 rows to JSON format and wrote to sheet")
        ...
        assert "Z1" in range_name, f"Expected JSON output in column Z, but got range: {range_name}"
...
class TestTransformCSVToJSONFunction:
    ...
    def test_transform_csv_to_json_function(self):
        ...
        assert result[0]["verification_field_ranges"] == "'March 2025'!A2:A91"  # A1 range fixed
        assert result[0]["input_file"] == "https://docs.google.com/spreadsheets/d/1wVWKw_l5eIWiLsiQPDrzjmodg2hMK9-RYxgDpT1YWaI/edit?usp=sharing"
        ...
        assert result[1]["verification_field_ranges"] == "'Sheet1'!A11:D72"  # A1 range fixed
        assert result[1]["input_file"] == "https://docs.google.com/spreadsheets/d/1E7RF7lBnVAEIVB7NoF24l4vBoV-t7VMRdrsAWHHU4YQ/edit?usp=sharing"
...
class TestValidatorIntegration:
    ...
    def test_validator_in_registry(self):
        ...
        csv_transform_criteria = next(
            (c for c in criteria if c['id'] == 'csv_to_json_transform'), 
            None
        )
