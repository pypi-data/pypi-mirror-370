...
from urarovite.utils.sheets import fetch_sheet_tabs, get_sheet_values, update_sheet_values
...
class CSVToJSONTransformValidator(BaseValidator):
            ...
            if snake_case_field != json_field:
                if csv_field in row_data:
                    ...
                    if value is None or (isinstance(value, str) and not value.strip()):
                        ...
                        if self._detect_malformed_a1_range(str_value):
                            fixed_a1_value = self._fix_a1_range(str_value)
                            snake_case_original = csv_field.lower().replace(' ', '_').replace('-', '_')
                            if not any(fix["field"] == snake_case_original for fix in a1_range_fixes):
                                a1_range_fixes.append({
                                    "field": json_field,
                                    "original": str_value,
                                    "fixed": fixed_a1_value
    ...
    def _read_csv_data(self, sheets_service: Any, sheet_id: str) -> List[Dict[str, Any]]:
    ...
    def _write_json_output(self, sheets_service: Any, sheet_id: str, json_data: List[Dict[str, Any]]) -> None:
        if not json_data:
            return
        ...
        if not update_result["success"]:
            raise ValidationError(f"Failed to write JSON output: {update_result['error']}")
        ...
        **kwargs: Any
        ...
        def validation_logic(
            ...
            **kwargs
            ...
            try:
                from urarovite.utils.generic_spreadsheet import read_csv_data_from_spreadsheet
            ...
            for i, row in enumerate(csv_data, 1):
                ...
                if "_a1_range_fixes" in transformed_row:
                    total_a1_range_fixes += len(transformed_row["_a1_range_fixes"])
                
                json_data.append(transformed_row)
            
            result.details["total_rows"] = len(csv_data)
            result.details["transformed_rows"] = len(json_data)
            result.details["a1_range_fixes_applied"] = total_a1_range_fixes
            result.details["json_schema"] = list(json_data[0].keys()) if json_data else list(self.FIELD_MAPPING.values()) + list(self.TEMPLATE_FIELDS.keys())
            result.details["sample_output"] = json_data[:3] if json_data else []
            
            if mode == "fix":
                if json_data:
                    # Use generic utility that works with both Google Sheets and Excel
                    from urarovite.utils.generic_spreadsheet import write_json_to_spreadsheet
                    write_json_to_spreadsheet(
                        spreadsheet_source, json_data, auth_credentials=auth_credentials
                    )
                    result.add_fix(len(json_data))
                    
                    log_msg = f"Transformed {len(json_data)} rows to JSON format and wrote to sheet"
                    if total_a1_range_fixes > 0:
                        log_msg += f". Applied {total_a1_range_fixes} A1 range notation fixes"
                ...
                if total_a1_range_fixes > 0:
                    log_msg += f". Would apply {total_a1_range_fixes} A1 range notation fixes"
                result.set_automated_log(log_msg)
