...
class AttachedFilesCleanerValidator(BaseValidator):
    ...
    def _should_skip_mention(self, text: str, start: int, end: int) -> bool:
        ...
        if 'attached to each other' in text[max(0, start-10):min(len(text), end+10)].lower():
            return True
        ...
        **kwargs
        ...
        try:
            ...
            try:
                ...
                result.set_automated_log(f"Column '{target_column}' not found in sheet - skipping attached files cleaning")
                return result.to_dict()

            prompts_with_attachments = []
            cleaned_prompts = []
            ...
            for row_index, row in enumerate(data_rows, start=2):  # Start at 2 for sheet row numbering
                ...
                if mentions:
                    if mode == "flag":
                        prompts_with_attachments.append({
                            "row": row_index,
                            "original_text": prompt_text,
                            "mentions": [m["matched_text"] for m in mentions]
                        })
                        result.add_issue()
                    else:  # fix mode
                        cleaned_text, editor_note = self._clean_attachment_mentions(prompt_text, mentions)
                        
                        # Update the row data
                        updated_row = row.copy()
                        if len(updated_row) <= target_col_index:
                            # Extend row if needed
                            updated_row.extend([''] * (target_col_index + 1 - len(updated_row)))
                        ...
                        cleaned_prompts.append({
                            ...
                            "mentions_removed": [m["matched_text"] for m in mentions]
                        })
            ...
            # Set results
            if mode == "flag":
                result.details["prompts_with_attachments"] = prompts_with_attachments
                if result.issues_found == 0:
                    result.set_automated_log("No attached file mentions found")
                else:
                    result.set_automated_log(f"Found {result.issues_found} prompts with attached file mentions")
            else:
                result.details["cleaned_prompts"] = cleaned_prompts
                if result.fixes_applied == 0:
                    result.set_automated_log("No attached file mentions found")
                else:
                    result.set_automated_log(f"Cleaned {result.fixes_applied} prompts with attached file mentions")
    ...
    def _clean_attachment_mentions(self, text: str, mentions: List[Dict[str, Any]]) -> Tuple[str, str]:
        ...
        for pattern_info in self.ATTACHMENT_PATTERNS:
            ...
            if callable(replacement):
                ...
                cleaned_text = re.sub(pattern, replace_func, cleaned_text, flags=re.IGNORECASE)
            else:
    ...
    def _apply_grammar_cleanup(self, text: str) -> str:
        ...
        text = re.sub(r'\b(refer to|check|review|see|update|clean)\s+the\s*\.', r'\1 it.', text)
