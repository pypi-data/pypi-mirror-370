...
class EmptyInvalidRangesValidator(BaseValidator):
        ...
        def validation_logic(
            ...
            **kwargs
            ...
            if invalid_ranges:
                ...
                result.details["valid_ranges"] = valid_count
                result.set_automated_log(f"Invalid ranges: {len(invalid_ranges)} found")
            else:
                result.details["valid_ranges"] = valid_count
                result.details["message"] = "All ranges are valid and non-empty"
    ...
    def _zero_sized(self, a1: str) -> bool:
        ...
        if m:
            ...
            return c1n <= 0 or c2n <= 0 or r1n <= 0 or r2n <= 0 or (c2n - c1n + 1) <= 0 or (r2n - r1n + 1) <= 0
        ...
        try:
            ...
            if not result["success"]:
                ...
                if error == "forbidden_or_not_found":
                    return (False, "No access to the spreadsheet or sheet not found — manual fix required")
                elif "request_exception" in error:
                    return (False, f"Google Sheets API error: {error} — manual fix required")
                else:
                    return (False, f"Failed to access range: {error} — manual fix required")
            ...
            return (False, f"Unexpected error: {e.__class__.__name__} — manual fix required")
            ...
            for row in sample_rows:
                if col_idx < len(row) and row[col_idx]:
                    ...
                    if ("!" in cell_value and 
                        (self._A1_RECT_RE.search(cell_value.split("!")[-1]) or
                         self._A1_CELL_RE.search(cell_value.split("!")[-1]) or
                         self._A1_COL_RE.search(cell_value.split("!")[-1]) or
                         self._A1_ROW_RE.search(cell_value.split("!")[-1]))):
