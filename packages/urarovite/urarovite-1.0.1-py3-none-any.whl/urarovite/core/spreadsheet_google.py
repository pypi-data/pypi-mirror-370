...
from urarovite.core.spreadsheet import SpreadsheetInterface, SpreadsheetMetadata, SheetData
...
class GoogleSheetsSpreadsheet(SpreadsheetInterface):
    ...
    def __init__(
        ...
        if isinstance(auth_credentials, str):
            ...
            encoded_creds = auth_credentials.get("encoded_credentials") or auth_credentials.get("auth_secret")
            if not encoded_creds:
                raise ValidationError("No encoded credentials found in auth_credentials")
        else:
    ...
    def get_metadata(self) -> SpreadsheetMetadata:
        ...
        if self._metadata is None:
            try:
                ...
                spreadsheet_data = execute_with_backoff(_get_metadata, config=READ_CONFIG)
    ...
    def get_sheet_data(
        ...
        try:
            ...
            result = get_sheet_values(self.sheets_service, self.spreadsheet_id, full_range)
    ...
    def get_sheet_data_with_hyperlinks(
        ...
        """Get data from Google Sheets with hyperlink URLs instead of display text."""
        try:
            # Get sheet name if not provided
            if not sheet_name:
                metadata = self.get_metadata()
            ...
            # Extract hyperlinks and replace display text with URLs
            values_with_hyperlinks = self._extract_hyperlinks_from_values(
                result["values"], sheet_name, full_range
            )
            
            return SheetData(
                values=values_with_hyperlinks,
                sheet_name=sheet_name,
                rows=result["rows"],
                cols=result["cols"]
            )
            ...
            if isinstance(e, ValidationError):
                raise
            raise ValidationError(f"Failed to get Google Sheets data with hyperlinks: {str(e)}")
    ...
    def update_sheet_data(
        ...
        try:
            if range_name:
                ...
                end_col_letter = col_index_to_letter(
                    start_col + len(values[0]) - 2
                ) if values and values[0] else start_col_letter
    ...
    def update_sheet_properties(
        ...
        **properties: Any
        ...
        try:
            ...
            spreadsheet_data = execute_with_backoff(_get_sheet_metadata, config=READ_CONFIG)
            ...
            if isinstance(e, ValidationError):
                raise
            raise ValidationError(f"Failed to update Google Sheets properties: {str(e)}")
    ...
    def get_formulas(self, sheet_name: Optional[str] = None) -> Dict[str, Dict[str, str]]:
        ...
        try:
            ...
            return fetch_workbook_with_formulas(self.sheets_service, self.spreadsheet_id)
        except Exception as e:
            raise ValidationError(f"Failed to read formulas from Google Sheets: {str(e)}")
