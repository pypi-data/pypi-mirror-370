...
class TestVolatileFormulasValidator:
    ...
    def test_detects_all_issue_types(self, mock_get_formulas):
        ...
        assert result["issues_found"] == 9
        issues = result["details"].get("issues", [])
        types = [i["type"] for i in issues]
        functions = [
            i.get("function") for i in issues if i["type"] == "volatile_function"
        ]
    ...
    @patch("urarovite.utils.generic_spreadsheet.get_spreadsheet_formulas")
    def test_no_issues(self, mock_get_formulas):
        """
        Test that the validator returns no issues when there are no volatile functions.
        """
    ...
    def test_missing_sheet_url(self, mock_get_formulas):
        """
        Test that the validator returns no issues when the sheet URL is missing.
        """
    ...
    def test_unexpected_error_handling(self):
        """
        Test that the validator returns no issues when there is an unexpected error.
        """
        service = MagicMock()
