...
This script demonstrates the new validation API, showing how to validate Google Sheets
data using the modernized library structure.
"""
...
def demonstrate_new_api() -> None:
    ...
    for check in example_checks:
        mode_desc = (
            "🔧 Fix automatically" if check["mode"] == "fix" else "🔍 Flag issues only"
        )
        validator_name = next(
