...
def execute_all_validations(
    ...
    try:
        ...
        if isinstance(sheet_url, str):
            ...
            is_google_drive_url = (
                "drive.google.com" in sheet_url or 
                "docs.google.com" in sheet_url or 
                "sheets.google.com" in sheet_url
            )
            
            if is_google_drive_url:
                if not auth_secret:
                    result["errors"].append("Authentication required for Google Drive files")
                    result["success"] = False
                    return result
                ...
                try:
                    ...
                    if metadata["success"]:
                        ...
                        logger.info(f"Detected file type: {file_type} (Google Sheets: {source_is_google})")
                    else:
                        result["errors"].append(f"Failed to detect file type: {metadata.get('error_msg', 'Unknown error')}")
                        result["success"] = False
                        return result
                except Exception as e:
            ...
            try:
                ...
                if individual_details:
                    if "details" not in result:
                        result["details"] = {"issues": [], "fixes": []}
                    
                    # Add issues with validator context
                    if "issues" in individual_details:
                        for issue in individual_details["issues"]:
                            issue_with_validator = issue.copy()
                            issue_with_validator["validator"] = check_id
                            result["details"]["issues"].append(issue_with_validator)
    ...
    try:
        ...
        if isinstance(source, str):
            ...
            is_google_drive_url = (
                ...
                "docs.google.com" in source or 
                "sheets.google.com" in source
            )
