...
class SpreadsheetDifferencesValidator(BaseValidator):
    ...
    def _compare_workbooks(
        ...
        self._compare_sheets(
            input_sheets[0], output_sheets[0], differences
        )

        return differences

    def _compare_sheets(
        self, input_sheet: Dict[str, Any], output_sheet: Dict[str, Any], differences: Dict[str, Any]
    ) -> None:
        ...
        for row_idx in range(max_rows):
            ...
            input_values = input_row.get("values", []) if isinstance(input_row, dict) else []
            output_values = (
            ...
            for col_idx in range(max_cols):
                input_cell = input_values[col_idx] if col_idx < len(input_values) else {}
                output_cell = (
        ...
        def validation_logic(
            ...
            for row_idx, row in enumerate(sheet_values):
                ...
                try:
                    ...
                    # Log detailed issues for each difference
            ...
            if mode == "fix":
                ...
                if total_differences_found > 0:
                    log_msg += (
                        f", found {total_differences_found} total differences."
                    )
                else:
