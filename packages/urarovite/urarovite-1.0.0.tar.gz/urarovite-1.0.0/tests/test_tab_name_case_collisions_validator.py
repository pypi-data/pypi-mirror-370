...
class TestTabNameCaseCollisionsValidator:
    ...
    def test_validate_no_collisions_flag_mode(self, mock_get_tabs):
        ...
        with patch.object(validator, '_get_spreadsheet') as mock_get_spreadsheet:
            with patch.object(validator, '_create_sheets_service') as mock_create_service:
        ...
        assert result["details"]["tabs_affected"] == 0
    ...
    def test_validate_with_collisions_flag_mode(self, mock_get_tabs):
        ...
        assert result["issues_found"] == 2  # Two collision groups
        assert result["fixes_applied"] == 0
        assert len(result["details"]["collisions"]) == 2
        assert result["details"]["tabs_affected"] == 4
        assert "2 case collision groups affecting 4 tabs" in result["automated_log"]
    ...
    def test_validate_with_collisions_fix_mode(self, mock_get_tabs, mock_rename):
        ...
        assert result["issues_found"] == 1  # One collision group
        assert result["fixes_applied"] == 1  # One rename performed
        assert len(result["details"]["collisions"]) == 1
        assert result["details"]["tabs_affected"] == 2
        
        # Check that rename mapping was created
        mapping = result["details"]["rename_mapping"]
        assert mapping["Sheet1"] == "Sheet1"  # First stays same
        assert mapping["sheet1"] == "sheet1 (2)"  # Second gets suffix
        
        # Verify API calls were made
        assert mock_service.spreadsheets().get.call_count >= 1
        assert mock_service.spreadsheets().batchUpdate.call_count >= 1
    ...
    def test_validate_api_error_during_fix(self, mock_get_tabs, mock_rename):
        ...
        assert result["fixes_applied"] == 0  # No fixes due to error
        assert len(result["errors"]) > 0
