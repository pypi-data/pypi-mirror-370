...
def determine_source_location(source: str) -> Dict[str, Any]:
    ...
    if isinstance(source, str) and ("docs.google.com" in source or "sheets.google.com" in source):
...
def validate_target_compatibility(
    ...
    if target_format == "sheets" and target == "local":
        errors.append("Format 'sheets' is only allowed for remote targets (Google Drive folder ID)")
    
    # Check authentication requirements
    if source_info["is_google_sheets"] and target != "local":
        warnings.append("Google Sheets source with remote target requires authentication")
    
    # Check conversion complexity
    if source_info["is_google_sheets"] and target_format == "excel":
        warnings.append("Converting from Google Sheets to Excel may lose some functionality (formulas, etc.)")
    elif not source_info["is_google_sheets"] and target_format == "sheets":
        warnings.append("Converting from Excel to Google Sheets may have compatibility issues")
