...
class TestVerificationRangesValidator(unittest.TestCase):
    ...
    def test_curly_quotes_conversion(self):
        ...
        # Use Unicode escape sequences to avoid syntax issues
        left_double = "\u201C"  # "
        right_double = "\u201D"  # "
        left_single = "\u2018"  # '
        right_single = "\u2019"  # '
        ...
        curly_quote_cases = [
            ...
            # Mixed curly quotes
            (f"{left_double}Sheet1{left_single}!A1@@{left_single}Sheet2{right_double}!B2", "'Sheet1'!A1@@'Sheet2'!B2"),
            (f"{left_single}Left{right_single}!A1@@{left_double}Right{right_double}!B2", "'Left'!A1@@'Right'!B2"),
            
            # Complex ranges with curly quotes
            (f"{left_double}March 2025{right_double}!A2:A91@@{left_double}Summary{right_double}!B1:B10", "'March 2025'!A2:A91@@'Summary'!B1:B10"),
            (f"{left_single}Q1 Data{right_single}!A1@@{left_double}Q2 Results{right_double}!C1:C50", "'Q1 Data'!A1@@'Q2 Results'!C1:C50"),
            ...
            (f"'Normal'!A1@@{left_double}Curly{right_double}!B2@@'Another'!C3", "'Normal'!A1@@'Curly'!B2@@'Another'!C3"),
        ]

        for input_range, expected in curly_quote_cases:
            with self.subTest(input_range=input_range):
                result = self.validator._fix_ranges(input_range)
                self.assertEqual(result, expected, f"Failed to convert curly quotes in: {input_range}")
    ...
    def test_convert_curly_quotes_method(self):
        ...
        test_cases = [
            ...
            (f"{left_double}Mixed{right_double} and {left_double}quotes{right_double}", "'Mixed' and 'quotes'"),
            
            # Single curly quotes  
            (f"{left_single}Left{right_single}", "'Left'"),
            (f"{left_single}Right{right_single}", "'Right'"),
            (f"{left_single}Both{right_single} and {left_single}sides{right_single}", "'Both' and 'sides'"),
            
            # Mixed curly and straight quotes
            (f"'Mixed{right_double} and {left_double}types'", "'Mixed' and 'types'"),
            (f"{left_double}Double' and 'single{right_double}", "'Double' and 'single'"),
            
            # Complex sheet names with curly quotes
            (f"{left_double}March 2025 Data{right_double}!A1:B10", "'March 2025 Data'!A1:B10"),
        ...
        for input_text, expected in test_cases:
            with self.subTest(input_text=input_text):
                ...
                self.assertEqual(result, expected, f"Failed to convert: {repr(input_text)}")
        ...
        for input_range, expected in complex_cases:
            with self.subTest(input_range=input_range):
                ...
                self.assertTrue(validation_result["ok"], 
                              f"Fixed range should be valid: {result}")
