                    ...
                    if hasattr(validator, '_get_spreadsheet'):
                        ...
                        with patch.object(validator, '_get_spreadsheet', return_value=mock_spreadsheet):
                            ...
                            if validator_id == 'different_within_ranges':
                                mock_service = Mock()
                                with patch('urarovite.validators.different_within_ranges.create_sheets_service_from_encoded_creds', return_value=mock_service):
                                    # Mock the batch methods
                                    with patch.object(validator, '_get_sheet_values_batch', return_value={"Sheet1": [["test", "data"]]}):
                                        result = validator.validate(
                                            spreadsheet_source='https://docs.google.com/spreadsheets/d/test_id/edit',
                                            mode=mode,
                                            auth_credentials={'auth_secret': 'eyJ0ZXN0IjogInZhbHVlIn0='},
                                            row=test_row
                                        )
                            # Mock _create_sheets_service for validators that need it
                            elif hasattr(validator, '_create_sheets_service'):
                                mock_service = Mock()
                                with patch.object(validator, '_create_sheets_service', return_value=mock_service):
                                    ...
                                    if hasattr(validator, '_fetch_workbook_effective'):
                                        mock_workbook_data = {"Sheet1": [["test", "data"], ["more", "data"]]}
                                        with patch.object(validator, '_fetch_workbook_effective', return_value=mock_workbook_data):
                                            result = validator.validate(
                                                spreadsheet_source='https://docs.google.com/spreadsheets/d/test_id/edit',
                                                mode=mode,
                                                auth_credentials={'auth_secret': 'eyJ0ZXN0IjogInZhbHVlIn0='},
                                ...
                                mock_workbook = {
                                    "sheets": [{"properties": {"title": "Sheet1"}, "data": [{"rowData": [{"values": [{"userEnteredValue": {"formulaValue": "=NOW()"}}]}
                                    ]}]}]
                                }
                                with patch('urarovite.validators.volatile_formulas.fetch_workbook_with_formulas', return_value=mock_workbook):
                                    result = validator.validate(
                                        spreadsheet_source='https://docs.google.com/spreadsheets/d/test_id/edit',
                                        mode=mode,
                                        auth_credentials={'auth_secret': 'eyJ0ZXN0IjogInZhbHVlIn0='},
