        ...
        def validation_logic(
            ...
            **kwargs
            ...
            if "ranges_str" in kwargs:
                ...
                # No parameters provided - no issues to report
            ...
            for segment in segments:
                if not self._segment_is_quoted(segment):
            ...
            if result.issues_found > 0:
                result.set_automated_log(
                    f"Found {result.issues_found} unquoted sheet names."
                )
            else:
...
def run(
    ...
    try:
        ...
        for segment in segments:
            if not validator._segment_is_quoted(segment):
        ...
        if failing_segments:
            if mode == "flag":
                ...
                result.set_automated_log(f"Found {len(failing_segments)} unquoted sheet names")
            else:
                result.add_fix(len(failing_segments))
                result.set_automated_log(f"Fixed {len(failing_segments)} unquoted sheet names")
        else:
            result.set_automated_log("All sheet names are properly quoted")
