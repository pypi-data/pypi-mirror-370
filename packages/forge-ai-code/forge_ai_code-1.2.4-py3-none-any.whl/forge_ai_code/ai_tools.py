...
class AIToolProcessor:
    ...
    def execute_command(self, command):
        ...
        try:
            # 安全检查 - 禁止危险命令
            dangerous_commands = ['rm -rf', 'del /f', 'format', 'fdisk', 'mkfs', 'shutdown', 'reboot']
            if any(dangerous in command.lower() for dangerous in dangerous_commands):
                return "[ERROR:SECURITY] 禁止执行危险命令 | 命令被安全策略阻止 | 可重试: False"

            # 检查命令是否为空
            if not command.strip():
                return "[ERROR:INVALID_INPUT] 命令不能为空 | 请提供有效的命令 | 可重试: False"

            print(f"{Fore.YELLOW}🔧 执行命令: {command}{Style.RESET_ALL}")
            ...
            if return_code == 0:
                success_msg = f"命令执行成功:\n{output}" if output.strip() else "命令执行成功"
                print(f"{Fore.GREEN}✅ {success_msg}{Style.RESET_ALL}")
                return success_msg
            else:
                error_msg = f"[ERROR:COMMAND_FAILED] 命令执行失败 (返回码: {return_code}) | {output.strip()} | 可重试: True"
                print(f"{Fore.RED}❌ 命令失败 (返回码: {return_code}): {output.strip()}{Style.RESET_ALL}")
                return error_msg

        except subprocess.TimeoutExpired:
            error_msg = "[ERROR:TIMEOUT] 命令执行超时 | 命令运行时间超过30秒 | 可重试: True"
            print(f"{Fore.RED}⏰ 命令执行超时{Style.RESET_ALL}")
            return error_msg
        except FileNotFoundError as e:
            error_msg = f"[ERROR:FILE_NOT_FOUND] 命令或文件不存在 | {str(e)} | 可重试: False"
            print(f"{Fore.RED}❌ 命令不存在: {str(e)}{Style.RESET_ALL}")
            return error_msg
        except PermissionError as e:
            error_msg = f"[ERROR:PERMISSION] 权限不足 | {str(e)} | 可重试: False"
            print(f"{Fore.RED}🔒 权限错误: {str(e)}{Style.RESET_ALL}")
            return error_msg
        except OSError as e:
            error_msg = f"[ERROR:SYSTEM] 系统错误 | {str(e)} | 可重试: True"
            print(f"{Fore.RED}💥 系统错误: {str(e)}{Style.RESET_ALL}")
            return error_msg
        except Exception as e:
            error_msg = f"[ERROR:UNKNOWN] 未知错误 | {str(e)} | 可重试: True"
