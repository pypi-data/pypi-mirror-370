...
// 	                      index_t _index, real_coefficient_t _coefficient)
...
// 		real_coefficient_t coface_coefficient = get_coefficient(entry) * coefficient;
...
void prepare_graph_filtration(Complex& complex, filtered_directed_graph_t& graph,
	...
	if (filtration_algorithm == nullptr) {
		...
		std::cout << "end nontrivial filtration alg" << std::endl;
	}
	...
	std::cout << "end prepare graph filtration" << std::endl;
}
...
class real_directed_flag_complex_computer_t {
	...
	int current_dimension = -1; // Was 0, 2024-01-25
...
public:
	real_directed_flag_complex_computer_t(filtered_directed_graph_t& _graph, filtration_algorithm_t *_filtration_algorithm, unsigned short min_dimension, unsigned short max_dimension)
	    : graph(_graph), filtration_algorithm(_filtration_algorithm),min_dimension(min_dimension), max_dimension(max_dimension),
	      flag_complex(graph), coboundary_matrix(1), coboundary_matrix_offsets(1,0){
...
void real_directed_flag_complex_computer_t::prepare_next_dimension(int dimension) {
...

	{
		if (filtration_algorithm != nullptr) {
			...
			filtration_algorithm->needs_face_filtration();
