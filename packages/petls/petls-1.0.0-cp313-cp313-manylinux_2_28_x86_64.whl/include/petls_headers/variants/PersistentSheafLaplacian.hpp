...
class PersistentSheafLaplacian {
    ...
    public:
         ...
         */
        PersistentSheafLaplacian(std::vector<Eigen::SparseMatrix<storage>> boundaries,
        ...
        // Some build environments seem to struggle to compile with the call to inverse() only in the PSL
        // so for now we will leave out the nonzero_spectra function.
