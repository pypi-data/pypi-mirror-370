...
private:
    ...
    unsigned short min_dim;
	unsigned short max_dim;
	unsigned short top_dim;
...
public:
	real_persistence_computer_t(Complex& _complex) : complex(_complex){}


    void compute_persistent_spectra(unsigned short min_dim,
									unsigned short max_dim,
									std::vector<Eigen::SparseMatrix<int,Eigen::ColMajor>>& boundaries, std::vector<std::vector<double>>& filtrations){
		top_dim = max_dim; // initialized
		compute_coboundaries(min_dim, max_dim);
		sort_coboundaries(min_dim);
        ...
        for (int i = 0; i < (int) sorted_boundaries.size(); i++){
            boundaries.push_back(sorted_boundaries[i].cast<int>());
        }
        for (int i = 0; i < (int) all_filtration_pairs.size(); i++){
            ...
            filtrations.push_back(filtrations_in_dim);
        }
		// stop here

	}

    void compute_coboundaries(unsigned short min_dim, unsigned short max_dim){
		for (auto dimension = min_dim; dimension <= max_dim; ++dimension) {
			...
			if (complex.is_top_dimension() || dimension >= max_dim) {
				top_dim = dimension;
				break;
			}
		}
	...
	void sort_coboundaries(unsigned short min_dim) {
		// set sorted_coboundaries = coboundary matrix ordered by filtration value then index
		for(int dimension = (int) min_dim; dimension < (int) top_dim; dimension++){
    ...
    void set_indices_of_filtered_boundaries(){
		...
		std::vector<std::vector<int>> temp_indices_of_filtered_boundaries(top_dim+1,std::vector<int>(total_filtration.size()));
		for (auto dimension = 0u; dimension <= top_dim; ++dimension) {
