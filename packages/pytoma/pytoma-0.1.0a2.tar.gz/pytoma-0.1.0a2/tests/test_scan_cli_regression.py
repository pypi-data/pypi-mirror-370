...
def test__discover_files_sees_dot(tmp_git_repo: Path, monkeypatch):
    """
    We bypass argparse and test the internal function _discover_files on '.'
    """
    monkeypatch.chdir(tmp_git_repo)
    import pytoma.cli as cli
    files = cli._discover_files([Path(".")])
    assert len(files) >= 3, "discover_files('.') should find .py/.md/.toml"
