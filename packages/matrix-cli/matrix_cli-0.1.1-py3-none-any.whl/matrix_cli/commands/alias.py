...
@app.command("rm")
def remove_alias(alias: str, yes: bool = typer.Option(False, "--yes", "-y", help="Confirm removal")):
    from matrix_sdk.alias import AliasStore
    if not yes and not typer.confirm(f"Are you sure you want to remove alias '{alias}'?"):
        raise typer.Exit(1)
