...
def tool(*args, **kwargs):
    """Enhanced tool decorator that registers for fallback mode and tracks errors"""
    def decorator(func):
        # Add error tracking wrapper
        @auto_track_errors(tool_name=f"mcp_{func.__name__}")
        def error_tracked_func(*fargs, **fkwargs):
            try:
                return func(*fargs, **fkwargs)
            except Exception as e:
                # Error is already tracked by decorator
                # Return user-friendly error message
                error_tracker = get_error_tracker()
                return f"❌ An error occurred: {str(e)}\n\nThis error has been automatically reported for improvement."
        
        # Preserve function metadata
        error_tracked_func.__name__ = func.__name__
        error_tracked_func.__doc__ = func.__doc__
        
        # Register with MCP
        mcp_decorated = original_tool(*args, **kwargs)(error_tracked_func)
        # Register for fallback
        TOOLS[func.__name__] = error_tracked_func
