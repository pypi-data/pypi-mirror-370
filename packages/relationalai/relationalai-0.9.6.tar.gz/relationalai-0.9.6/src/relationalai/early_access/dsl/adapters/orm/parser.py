        ...
        if orm_fact_types:
            for ft in self._single_object_to_list(orm_fact_types):
                ...
                if roles and isinstance(roles, list):
                    for ro in roles:
                        value_range = self._get_nested(ro, "orm:ValueRestriction", "orm:RoleValueConstraint",
                                                        "orm:ValueRanges", "orm:ValueRange")
                        if value_range:
                            values = []
                            # Single values
                            if isinstance(value_range, list):
                                for rvc in value_range:
                                    if rvc.get("@MinValue") != rvc.get("@MaxValue"):
                                        raise ValueError("MinValue and MaxValue should be equal for single value constraints.")
                                    values.append(rvc.get("@MinValue"))
                            # Value ranges - stored in a dictionary
                            else:
