...
class SimpleRoleExpr(RoleExpr):
    ...
    def mandatory(self):
        ...
        if not self._reasoner.one_role(this):
            return False

        roles = self._reasoner.role_siblings(this)
        if len(roles) > 1:
            return False

        return self._reasoner.mandatory(roles[0])
    ...
    def __init__(self, *roles):
        ...
        for r in roles:
            ...
            if matcher is None:
                ...
                if not self._reasoner.compatible(r, matcher):
