...
def replace_appd_protobuf_with_google_if_needed():
    if os.getenv("USE_GOOGLE_PROTOBUF_MSG"):
        import site

        # Step 1: Determine the path to the file
        site_pkg_path = site.getsitepackages()[0]
        file_has_problem = "/appdynamics_bindeps/google/protobuf/internal/builder.py"
        full_path = site_pkg_path + file_has_problem

        try:
            # Step 2: Open the file, read its content, and replace content
            with open(full_path, 'r') as file:
                content = file.read()

            # Step 3: Perform the text replacement
            modified_content = content.replace('from appdynamics_bindeps.google.protobuf import message as _message',
...
replace_appd_protobuf_with_google_if_needed()
