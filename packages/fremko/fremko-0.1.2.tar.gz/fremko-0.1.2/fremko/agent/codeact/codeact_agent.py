...
from fremko.agent.utils import chat_utils
from fremko.agent.utils.executer import SimpleCodeExecutor
from fremko.agent.codeact.prompts import (
    DEFAULT_CODE_ACT_USER_PROMPT,
    DEFAULT_NO_THOUGHTS_PROMPT,
)

from fremko.agent.context.episodic_memory import EpisodicMemory, EpisodicMemoryStep
...
class CodeActAgent(Workflow):
    ...
    async def handle_llm_input(
        ...
        for context in self.required_context:
            if model == "DeepSeek":
                ...
                _, screenshot = await self.tools.take_screenshot()
    ...
    async def _add_final_state_observation(self, ctx: Context) -> None:
        ...
        try:
            ...
            logger.debug("Added final state observation to episodic memory")
