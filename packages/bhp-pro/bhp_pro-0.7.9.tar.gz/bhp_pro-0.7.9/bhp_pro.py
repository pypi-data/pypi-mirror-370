    ...
    def process_target(target, save_flag, output_file, path, isp_domain):
        ...
        for url in [target_http, target_https]:
            ...
            if status_code:
                ...
                # Save if status code is 200 OK, regardless of Connection header
                if status_code in [200, 301, 400, 405, 409, 500]:
    ...
    def mj4():
        ...
        if save_flag:
            if saved_any:
                ...
                print(f"\033[93mNo successful connections (200 OK) or UDP open ports found. Nothing saved.\033[0m")
...
def casper():
    ...
    class CSPProcessor:
        ...
        def process_domain(self, domain, seen_pairs, batch_result, counter, temp_file):
            ...
            for csp_domain in csp_domains:
                ...
                if ip:
                    self.save_pair(csp_domain, ip, seen_pairs, batch_result, counter, temp_file)
                else:
                    # Fallback to root domain
                    ...
                    if root_domain != csp_domain:
                        ...
                        if root_ip:
                            self.save_pair(csp_domain, root_ip, seen_pairs, batch_result, counter, temp_file)
                            self.save_pair(root_domain, root_ip, seen_pairs, batch_result, counter, temp_file)

        def save_pair(self, domain, ip, seen_pairs, batch_result, counter, temp_file):
    ...
    def upload_file(path, max_retries=3):
        ...
        for attempt in range(1, max_retries + 1):
            try:
                print(f"🌐 Uploading file... (Attempt {attempt})")
...
def Android_App_Security_Analyzer():
    ...
    def test_links(links):
        ...
        progress.create_sub_bar('testing', "Current URL", 5)  # 5 stages per URL
        ...
        with ThreadPoolExecutor(max_workers=100) as executor:
            ...
            for future in as_completed(futures):
                ...
                progress.update_main(1)
                progress.update_sub('testing', 5, url=url[:50], status=result.get('status'))
