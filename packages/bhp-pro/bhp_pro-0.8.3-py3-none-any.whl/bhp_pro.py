...
def host_proxy_checker():
    ...
    def get_all_ips(domain):
        """Get ALL IPv4 addresses for a domain using DNS resolution"""
        try:
            ...
            safe_print(f"   ❌ DNS resolution error for {domain}: {e}")
            return []

    def extract_valid_domains(domain):
        """Extract only valid domain and subdomain combinations"""
        # Remove http:// or https:// if present
        domain = re.sub(r'^https?://', '', domain)
        ...
        # Add the main domain (last two parts)
        if len(parts) >= 2:
            main_domain = '.'.join(parts[-2:])
            valid_domains.add(main_domain)
        
        # Add subdomains if they look like proper domains
        if len(parts) > 2:
            # Add all meaningful subdomain combinations
            for i in range(len(parts) - 1):
                subdomain = '.'.join(parts[i:])
                if len(subdomain.split('.')) >= 2:  # Only add if it has at least 2 parts
                    valid_domains.add(subdomain)
        
        return list(valid_domains)

    def get_isp_domains():
        """Prompt user for ISP domains - single, multiple, or from file"""
        safe_print("🌍 ISP Proxy Configuration")
        safe_print("=" * 40)
        
        safe_print("ISP Domain Options:")
        safe_print("1. Enter single ISP domain")
        safe_print("2. Load ISP domains from text file")
        ...
        if choice == "1":
            domain = input("Enter ISP domain (e.g., sports.siber.com, provider.net): ").strip()
            if domain:
                isp_domains = [domain]
        
        elif choice == "2":
            filename = input("Enter path to ISP domains file: ").strip()
            try:
                with open(filename, 'r') as f:
            ...
            domains_input = input("Enter ISP domains (comma-separated): ").strip()
        ...
        if not isp_domains:
            safe_print("❌ No ISP domains provided!")
            return []
        
        # Process each domain to extract valid domains and get ALL IPs
        isp_info_list = []
        for domain in isp_domains:
            try:
                # Extract valid domain combinations
                valid_domains = extract_valid_domains(domain)
                safe_print(f"🔍 Analyzing: {domain}")
                safe_print(f"   → Valid domains: {', '.join(valid_domains)}")
                
                # Get ALL IP addresses for each valid domain
                resolved_ips = {}
                ...
                safe_print(f"❌ Error processing {domain}: {e}")
                continue
    ...
    def test_payload_through_proxy(target_domain, payload, proxy_host, proxy_port):
        """Test payload through ISP proxy"""
        try:
            # Create socket connection to ISP proxy
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            sock.settimeout(15)
            ...
            success_indicators = [
                '200 OK', '101', '302', '301', 'HTTP/', 'websocket',
                'upgrade', 'connection established', 'switching protocols'
            ]
            
            if any(indicator in response_str for indicator in success_indicators):
                return True, response_str[:1000]
    ...
    def prepare_payloads(target_domain):
        """Prepare all payloads for the target domain"""
        prepared_payloads = []
    ...
    def generate_proxy_combos(isp_info):
        """Generate all possible proxy combinations for an ISP"""
        proxy_combos = []
        
        # For each valid domain, test on ports 80 and 443
        ...
        # For each resolved IP (ALL IPs for each domain), test on ports 80 and 443
    ...
    def format_payload_for_http_injector(payload):
        """Format the payload for HTTP Injector by escaping special characters"""
    ...
    def test_single_combination(target_domain, prepared_payloads, proxy_host, proxy_port, isp_info, proxy_type, output_file):
        """Test a single combination of target, payload, and proxy"""
        results = []
        ...
        for payload_index, payload_data in enumerate(prepared_payloads):
            ...
            # Test the payload
            success, response = test_payload_through_proxy(target_domain, payload, proxy_host, proxy_port)
            
            if success:
                safe_print(f"   ✅ SUCCESS: {target_domain} via {proxy_host}:{proxy_port} with payload {payload_index + 1}")
                
                # Format the payload for HTTP Injector
                http_injector_payload = format_payload_for_http_injector(payload)
                
                # Save successful result with full payload details
                result = {
                    ...
                    'valid_domains': isp_info['valid_domains'],
                    'payload_index': payload_index + 1,
                    'user_agent': payload_data['user_agent'],
                    'random_key': payload_data['random_key'],
                    'payload_template': payload_data['template'],
                ...
                safe_print(f"   ❌ FAILED: {target_domain} via {proxy_host}:{proxy_port} with payload {payload_index + 1}")
    ...
    def mj4():
        ...
        # Install dnspython if not available
        try:
            import dns.resolver
        except ImportError:
            safe_print("⚠️ Installing dnspython...")
            import subprocess
            subprocess.run(['pip', 'install', 'dnspython'], check=True)
            import dns.resolver
        ...
        # Get thread count
        try:
            max_threads = int(input("Enter number of threads (default 10): ") or "10")
        except ValueError:
            max_threads = 10
            safe_print("Using default 10 threads")
        
        # Create proxy combos for each ISP
        all_proxy_combos = []
        for isp_info in isp_info_list:
            proxy_combos = generate_proxy_combos(isp_info)
            all_proxy_combos.extend(proxy_combos)
            safe_print(f"   Generated {len(proxy_combos)} combos for {isp_info['original_domain']}")
        
        safe_print(f"\n🚀 Testing {len(target_domains)} targets through {len(all_proxy_combos)} proxy combos")
        safe_print(f"📋 ISP Proxies: {len(isp_info_list)} different ISPs")
        safe_print(f"💣 Payloads: {len(PAYLOADS)} different payloads")
        ...
        with open(output_file, 'w') as f:
            ...
            f.write(f"Total ISPs: {len(isp_info_list)}\n")
            f.write(f"Total Proxy Combos: {len(all_proxy_combos)}\n")
            f.write(f"Total Targets: {len(target_domains)}\n")
            f.write(f"Threads: {max_threads}\n")
            f.write(f"Test Time: {time.ctime()}\n")
            f.write("=" * 60 + "\n\n")
        
        # Test each target domain through each proxy combo with each payload using threading
        with ThreadPoolExecutor(max_workers=max_threads) as executor:
            futures = []
            
            for target_domain in target_domains:
                safe_print(f"\n🔍 Testing target: {target_domain}")
                
                # Prepare payloads for this target
                prepared_payloads = prepare_payloads(target_domain)
                
                for proxy_host, proxy_port, isp_info, proxy_type in all_proxy_combos:
                    safe_print(f"   → Through {proxy_type}: {proxy_host}:{proxy_port} ({isp_info['original_domain']})")
                    ...
                    if results:
                        with open(output_file, 'a') as f:
                            for result in results:
                                ...
                                f.write(f"🔗 PROXY: {result['proxy_host']}:{result['proxy_port']} ({result['proxy_type']})\n")
                                f.write(f"🏢 ORIGINAL ISP: {result['original_isp']}\n")
                                f.write(f"🏢 VALID DOMAINS: {', '.join(result['valid_domains'])}\n")
                                f.write(f"💣 PAYLOAD #: {result['payload_index']}\n")
                                f.write(f"👤 USER AGENT: {result['user_agent']}\n")
                                f.write(f"🔑 RANDOM KEY: {result['random_key']}\n")
                                f.write(f"📋 PAYLOAD TEMPLATE:\n{result['payload_template']}\n")
                                f.write(f"📨 ACTUAL PAYLOAD SENT:\n{result['actual_payload']}\n")
                                f.write(f"📱 PAYLOAD (Copy this):\n\"{result['http_payload']}\",\n")
                                f.write(f"📨 RESPONSE:\n{result['response']}\n")
                                f.write("-" * 60 + "\n\n")
                except Exception as e:
                    safe_print(f"Error processing result: {e}")
        ...
        if successful_tests > 0:
            safe_print(f"\n📊 SUMMARY:")
            safe_print(f"   Successful connections: {successful_tests}")
            safe_print(f"   ISP Proxies tested: {len(isp_info_list)}")
            safe_print(f"   Proxy combinations: {len(all_proxy_combos)}")
            safe_print(f"   Target domains: {len(target_domains)}")
            safe_print(f"   Check {output_file} for detailed results")
