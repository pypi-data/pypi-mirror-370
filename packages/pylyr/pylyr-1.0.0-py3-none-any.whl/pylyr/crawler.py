...
class Crawler:
    ...
    def get_lyrics(self):
        providers = [self._lrclib, self._genius, self._genius_from_search, self._duckduckgo]
    ...
    def _genius_from_search(self) -> str:
        ...
        if not similar:
            if 'the' in self.title.lower():
                logging.info("Trying removing 'the' from song's title in search query")
        ...
        if not similar:
            if 'the' in self.artist.lower():
                logging.info("Trying removing 'the' from song's artist in search query")
