...
def call_builtin(obj: str, meth: str, args: List, env: dict) -> Any:
    ...
    if obj == 'Luffy' and meth == 'stretch':
        ...
        try:
            ...
            raise RuntimeError_("Luffy.stretch expects a number")
    if obj == 'Chopper' and meth == 'heal':
        if len(args) < 2:
            raise RuntimeError_("Chopper.heal needs (value, Type)")
        val, typename = args[0], args[1]
        try:
            if typename == 'Int': return int(float(val)) if isinstance(val, (int, float, str)) else val
            if typename == 'Float': return float(val) if isinstance(val, (int, float, str)) else val
            if typename == 'String': return str(val)
        except (ValueError, TypeError):
            raise RuntimeError_(f"Chopper.heal cannot convert {val} to {typename}")
        return val
    ...
    if obj == 'Robin' and meth == 'clone':
        ...
        try:
            ...
            raise RuntimeError_("Robin.clone expects a number for n")
        return [value] * n
...
class OnePieceInterpreter:
    ...
    def eval_condition(self, expr: str, env: dict) -> bool:
        expr = expr.strip()
        for op in ['==', '!=', '<=', '>=', '<', '>']:
            if op in expr:
                ...
                if isinstance(left_val, str) and isinstance(right_val, str):
                    if op == '==': return left_val == right_val
                    if op == '!=': return left_val != right_val
                raise RuntimeError_(f"Invalid types for comparison: {type(left_val)} {left_val} {op} {type(right_val)} {right_val}")
        return bool(self.eval_expr(expr, env))

    def eval_expr(self, expr: str, env: dict) -> Any:
        expr = expr.strip()
        # Handle expressions with operators first
        if '+' in expr and not (expr.startswith('"') and expr.endswith('"') or expr.startswith("'") and expr.endswith("'")):
            parts = split_expression(expr)
            if len(parts) != 2:
                raise RuntimeError_(f"Invalid addition expression: {expr}")
            left_val = self.eval_expr(parts[0], env)
            right_val = self.eval_expr(parts[1], env)
            try:
                if isinstance(left_val, str) and re.match(r'^-?\d+(\.\d+)?$', left_val):
                    left_val = float(left_val) if '.' in left_val else int(left_val)
                if isinstance(right_val, str) and re.match(r'^-?\d+(\.\d+)?$', right_val):
        ...
        # Check for variables
        if expr in env.get('vars', {}):
            return env['vars'][expr]
        if expr in self.global_env['vars']:
            return self.global_env['vars'][expr]
        ...
        if expr == 'None':
            return None
        # Handle sail calls
        m_sail = re.match(r'^sail\s+([A-Za-z_][A-Za-z0-9_]*)\.([A-Za-z_][A-Za-z0-9_]*)\s*\((.*)\)$', expr)
        if m_sail:
            shipname, fnname, args = m_sail.groups()
            ...
            if not ship:
                raise RuntimeError_(f"Ship {shipname} not found")
            fn = ship['fns'].get(fnname)
            if not fn:
                raise RuntimeError_(f"Function {fnname} not found on ship {shipname}")
            return fn(*avals)
        # Handle method calls
        ...
        # Handle identifiers not in env as literals
        return expr
    ...
    def parse_block(self, lines: List[tuple], start_idx: int, curr_indent: int) -> tuple:
        ...
        if text.startswith('ship '):
            ...
            while i < len(lines) and lines[i][0] > curr_indent:
                node, i = self.parse_block(lines, i, lines[i][0])
                body.append(node)
        ...
        if text.startswith('let '):
            ...
            return (('let', var, expr), start_idx + 1)
        if text.startswith('return '):
            expr = text[len('return '):].strip()
            return (('return', expr), start_idx + 1)
        if text.startswith('sail '):
            call = text[len('sail '):].strip()
            return (('sail', call), start_idx + 1)
        ...
        if text.startswith('while '):
            ...
            cond = m.group(1).strip() if m else None
        ...
        if m:
            ...
            if obj == 'Nami' and i < len(lines) and lines[i][1].startswith('else:'):
                ...
                while j < len(lines) and lines[j][0] > curr_indent:
                    node, j = self.parse_block(lines, j, lines[j][0])
    ...
    def exec_node(self, node: tuple, env: dict) -> Any:
        typ = node[0]
        if typ == 'ship':
            _, name, body = node
            ship = {'fns': {}, 'vars': {}}
                ...
                if b[0] == 'fn':
                    ...
                    def make_fn(fnbody, params):
                        def fn_callable(*args):
                            ...
                            try:
                                for n in fnbody:
                                    self.exec_node(n, local)
                            except ReturnSignal as r:
                                return self.eval_expr(str(r.value), local)
                        return fn_callable
        ...
        if typ == 'sail':
            ...
            if m:
                ...
                if m2:
                    ...
                    return fn(*avals)
                raise RuntimeError_(f"Invalid sail call: {call}")
        if typ == 'for':
        ...
        raise RuntimeError_(f"Unknown node type: {typ}")
