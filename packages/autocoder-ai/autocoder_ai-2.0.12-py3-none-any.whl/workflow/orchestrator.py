...
class WorkflowOrchestrator:
    ...
    def execute_task(self, task_description: str, progress: Progress, task_id: Any, session_id: str = None) -> Dict[str, Any]:
        """Execute the multi-agent workflow"""
        try:
            # Set session ID on all emitters if provided
            if session_id and self.agent_emitters:
                for emitter in self.agent_emitters.values():
    ...
    def _run_planner(self, state: WorkflowState) -> WorkflowState:
        ...
        # Emit status update if we have an emitter
        if 'planner' in self.agent_emitters and state.get('session_id'):
            emitter = self.agent_emitters['planner']
            emitter.set_session(state['session_id'])
            ...
            try:
                loop.run_until_complete(emitter.start_task("Analyzing requirements", 10))
            finally:
                loop.close()
        ...
        try:
            ...
            if 'planner' in self.agent_emitters and state.get('session_id'):
                loop.run_until_complete(
                    self.agent_emitters['planner'].update_progress(50, "Creating task breakdown")
                )
            ...
            if 'planner' in self.agent_emitters and state.get('session_id'):
                loop.run_until_complete(
                    self.agent_emitters['planner'].complete_task("Planning completed")
                )
        finally:
            loop.close()
