...
class CLIWebSocketClient:
    ...
    async def connect(self, session_id: str):
        ...
        try:
            console.print(f"[cyan]Connecting to WebSocket at {ws_url}...[/cyan]")
            
            # Set connection timeout and max message size
            ...
            error_msg = f"Connection timeout - server did not respond within 10 seconds"
            console.print(f"[red]Failed to connect: {error_msg}[/red]")
            raise ConnectionError(error_msg)
        except websockets.exceptions.InvalidURI:
            error_msg = f"Invalid WebSocket URL: {ws_url}"
            ...
            console.print("[yellow]Tip: Make sure the web server is running (use 'autocoder web' to start it)[/yellow]")
            raise
