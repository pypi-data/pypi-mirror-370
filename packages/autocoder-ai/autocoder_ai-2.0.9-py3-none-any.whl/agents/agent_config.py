...
class MCPServerConfig(BaseModel):
    ...
    alwaysAllow: List[str] = Field(default_factory=list)  # Tools to always allow without confirmation
    autoApprove: Optional[List[str]] = None  # Alias for alwaysAllow (some clients use this)
    disabled: bool = False  # Whether the server is disabled
    timeout: int = 60000  # Timeout in milliseconds
    enabled: bool = True  # Backward compatibility
    tools: Optional[List[str]] = None  # List of specific tools to enable
    
    def model_post_init(self, __context):
        """Post-initialization processing"""
        # Handle autoApprove as alias for alwaysAllow
        if self.autoApprove and not self.alwaysAllow:
            self.alwaysAllow = self.autoApprove
        
        # Sync enabled and disabled flags
        if self.disabled:
            self.enabled = False
