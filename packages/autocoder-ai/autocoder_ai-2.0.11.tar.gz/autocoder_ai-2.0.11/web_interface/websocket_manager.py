...
class WebSocketManager:
    ...
    async def leave_session(self, websocket: WebSocket, session_id: str):
        """Remove websocket from a session room"""
    ...
    async def broadcast_to_session(self, session_id: str, message: Dict[str, Any]):
        """Broadcast a message to all clients in a session (alias for send_to_session)"""
        await self.send_to_session(session_id, message)
    
    async def handle_human_response(self, session_id: str, response: str):
        """Handle human response to input requests"""
        # Log the response and notify agents
        await self.send_activity_log(session_id, "human", f"Provided response: {response}", "info")
        await self.send_to_session(session_id, {
    ...
    async def cancel_task(self, session_id: str):
        ...
        await self.send_activity_log(session_id, "system", "Task cancelled by user", "warning")
