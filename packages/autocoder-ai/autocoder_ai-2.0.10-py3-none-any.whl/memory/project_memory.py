...
class ProjectMemory:
    ...
    def __init__(self, db_path: str = None):
        if db_path is None:
            # Use a consistent path in user's home directory
            home_dir = Path.home()
            autocoder_dir = home_dir / '.autocoder'
            autocoder_dir.mkdir(parents=True, exist_ok=True)
            self.db_path = autocoder_dir / 'ai_agent_system.db'
        else:
    ...
    def get_project(self, project_id: str) -> Optional[Dict]:
        """Get project by ID (supports partial ID or name matching)"""
        with self._get_connection() as conn:
            ...
            # If not found, try partial ID match (prefix)
            if not result:
                cursor.execute("""
