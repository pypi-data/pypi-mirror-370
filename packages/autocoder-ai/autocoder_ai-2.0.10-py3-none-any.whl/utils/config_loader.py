"""
Configuration loader for the agent system with external agent configuration support
"""
...
import logging
from .config_merger import ExternalConfigLoader, ConfigMerger
...
class ConfigLoader:
    ...
    def __init__(self, config_path: str = "config.yaml"):
        ...
        self.external_loader: Optional[ExternalConfigLoader] = None
    ...
    def load(self) -> bool:
        ...
        try:
            ...
            # Process external agent configuration files
            self._process_external_configs()
    ...
    def _process_external_configs(self):
        """Process external agent configuration files if specified"""
        try:
            # Initialize external config loader
            self.external_loader = ExternalConfigLoader(self.config)
            
            # Process all agent configurations
            self.config = self.external_loader.process_agent_configs()
            
            # Validate MCP servers if present
            if 'agents' in self.config:
                for agent_name, agent_config in self.config['agents'].items():
                    if not self.external_loader.validate_mcp_servers(agent_config):
                        logger.warning(f"Invalid MCP server configuration for agent {agent_name}")
            
            logger.info("External agent configurations processed successfully")
            
        except Exception as e:
            logger.error(f"Error processing external configurations: {e}")
            # Continue with base configuration if external processing fails
