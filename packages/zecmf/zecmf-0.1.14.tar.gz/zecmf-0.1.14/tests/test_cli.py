...
def test_with_test_app_decorator() -> None:
    """Test that with_test_app decorator creates app with testing config."""
    test_function_called = False

    @with_test_app
    def test_function() -> None:
        nonlocal test_function_called
        test_function_called = True

    with patch("zecmf.app.create_app") as mock_create_app:
        # Create a proper Flask app mock with app context
        mock_app = MagicMock()
        ...
        mock_create_app.assert_called_once_with(config_name="testing")
        # Verify the function was executed
        assert test_function_called


def test_extract_swagger_command_uses_testing_config() -> None:
    """Test that extract-swagger command uses testing configuration."""
    with patch("zecmf.app.create_app") as mock_create_app:
        # Create a proper Flask app mock with app context
        mock_app = MagicMock()
        mock_api = MagicMock()
        ...
        mock_app.extensions = {"restx/api": mock_api}
        mock_context = MagicMock()
        mock_context.__enter__ = MagicMock(return_value=None)
        mock_context.__exit__ = MagicMock(return_value=None)
        mock_app.app_context.return_value = mock_context
        ...
        # We need to patch current_app to return our mock app
        with (
            patch("click.echo"),
            patch("builtins.open", mock_open()),
            patch("zecmf.cli.commands.current_app", mock_app),
        ):
            # Invoke the extract_swagger command
            result = click.testing.CliRunner().invoke(
                extract_swagger, ["--output", "test.json"]
            )
