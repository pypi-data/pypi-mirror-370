...
class FlexiBatch:
    ...
    def to_torch(self, device):
        ...
        if self.truncated is not None:
            self.truncated = torch.from_numpy(self.truncated).float().to(device)
        if self.bootstrap_values is not None:
            self.bootstrap_values = (
                torch.from_numpy(self.bootstrap_values).float().to(device)
            )
    ...
    def __str__(self):
        ...
        s += f"truncated: {self.truncated is not None}\n"
        s += f"bootstrap_values: {self.bootstrap_values is not None}\n"
...
class FlexibleBuffer:
    ...
    def __init__(
        ...
        self.track_truncation = track_truncation
        self._registered_vals = {}
        ...
        if self.track_truncation:
            self.truncated = np.zeros((num_steps), dtype=np.float32)
            self.bootstrap_values = np.zeros((num_steps, n_agents), dtype=np.float32)
        else:
            self.truncated = None
    ...
    def save_transition(
        ...
        truncated: Union[bool, int, float] = False,
        ...
        if self.track_truncation and truncated:
            assert (
                bootstrap_values is not None
            ), "If you want to track truncation and truncated=True you need to pass in bootstrapped values for that step"
        ...
        if self.track_truncation:
            assert (
                self.truncated is not None and self.bootstrap_values is not None
            ), "cant track truncated without an array to store it"
            self.truncated[self.idx] = float(truncated)

            if truncated:
                assert (
                    bootstrap_values is not None
                ), "Cant truncate without supplying a bootstrap val even if it is zero"
                self.bootstrap_values[self.idx] = bootstrap_values
            else:
                self.bootstrap_values[self.idx] = 0.0
    ...
    def print_idx(self, idx):
        print(
            f"obs: {self.obs[idx]} | obs_ {self.obs_[idx]}, action: {self.discrete_actions[idx]}, reward: {self.global_rewards[idx]}, term: {self.terminated[idx]} trunc: {self.truncated[idx] if self.truncated is not None else None}"
    ...
    def G(
        ...
        assert (truncated is None and bootstrap_values is None) or (
            truncated is not None and bootstrap_values is not None
        ), "If you want to use truncation, you need to supply the bootstrap values for steps that were overwritten by the first step in the next episode and zero for non-truncated steps"
        G = torch.zeros_like(rewards).to(rewards.device)
        G[-1] = rewards[-1]
        if terminated[-1] < 0.5:
            G[-1] += gamma * last_value
            # ^ we are still probably going to collect more of this episode but this is a training cutoff

        for i in range(len(rewards) - 2, -1, -1):
            if terminated[i] > 0.1:
                next_value = 0
            elif truncated is not None and truncated[i] > 0.1:
                next_value = gamma * bootstrap_values[i]  # type:ignore
            else:
                next_value = gamma * G[i + 1]
            G[i] = rewards[i] + next_value
        G = G.unsqueeze(-1)
        return G
    ...
    def GAE(
        ...
        for step in reversed(range(num_steps)):
            if step == num_steps - 1:
                next_value = gamma * last_value
            else:
                if terminated[step] > 0.1:
                    next_value = 0.0
                    last_gae_lam = 0.0
                elif truncated is not None and truncated[step] > 0.1:
                    next_value = gamma * bootstrap_values[step]  # type:ignore
                    last_gae_lam = 0.0
                else:
                    next_value = gamma * values[step + 1]

            test_gae_lam = last_gae_lam
            ...
            # print(
            #     f"Step {step} last_gae_lam before: {test_gae_lam}, delta: {delta} last_gae_lam after: {last_gae_lam} advantages: {advantages[step]}"
            # )
        G = advantages + values
            ...
            for i in range(min(k, n_step - step)):
                if i + step == n_step - 1:
                    ...
                    if i == k - 1:
                        ...
                        if terminated[step + i] > 0.1:
                            v += (gamma**i) * rewards[step + i]
                            break
                        elif truncated is not None and truncated[step + i] > 0.1:
                            disc = gamma ** (i + 1)
                            next_val = disc * bootstrap_values[step + i]  # type:ignore
                            v += (gamma**i) * rewards[step + i] + next_val
                            break
                        else:
                            v += (gamma**i) * rewards[step + i]
            G[step] = v
    ...
    def load(path, name):
        ...
        if os.path.exists(fb.path + fb.name + "_truncated.npy"):
            fb.truncated = np.load(fb.path + fb.name + "_truncated.npy")
        else:
            fb.truncated = None

        if os.path.exists(fb.path + fb.name + "_bootstrap_values.npy"):
            fb.bootstrap_values = np.load(fb.path + fb.name + "_bootstrap_values.npy")
        else:
            fb.bootstrap_values = None
        ...
        fb.episode_inds = np.load(
            fb.path + fb.name + "_episode_inds.npy", allow_pickle=True
        ).tolist()
        fb.episode_lens = np.load(
            fb.path + fb.name + "_episode_lens.npy", allow_pickle=True
        ).tolist()
    ...
    def save(fb):
        ...
        if fb.truncated is not None:
            np.save(fb.path + fb.name + "_truncated.npy", fb.truncated)
        if fb.bootstrap_values is not None:
            np.save(fb.path + fb.name + "_bootstrap_values.npy", fb.bootstrap_values)
    ...
    def __str__(self):
        ...
        s += f"terminated: {self.terminated is not None}\n"
        s += f"truncated: {self.truncated is not None}\n"
        s += f"bootstrap_vals: {self.bootstrap_values is not None}\n"
        s += f"memory_weights: {self.memory_weights is not None}\n"
        ...
        s += f"bootstrap_vals: {self.bootstrap_values}\n"
