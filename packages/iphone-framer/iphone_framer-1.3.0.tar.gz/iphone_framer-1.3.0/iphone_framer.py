...
class iPhoneFrameGenerator:
    def __init__(self):
        ...
        # Screenshot and text configuration
        ...
        self.dynamic_island_config = {
            "frame_y_position": 25,
            "screenshot_scale": 0.3,
        }

        # Default styling
        self.default_background_color = (25, 25, 35)
    ...
    def create_rounded_screenshot(
        ...
        if device_size in ["6.9_inch", "6.1_inch"]:
            scale = self.dynamic_island_config["screenshot_scale"]
            island_width = int(self.dynamic_island_dimensions["width"] * scale)
            island_height = int(self.dynamic_island_dimensions["height"] * scale)
            island_radius = int(self.dynamic_island_dimensions["radius"] * scale)
            island_x = (width - island_width) // 2
            island_y = int(self.dynamic_island_dimensions["y_offset"] * scale)
    ...
    def add_dynamic_island(self, draw, frame_width, device_size="6.9_inch"):
        ...
        if device_size in ["6.9_inch", "6.1_inch"]:
            ...
            island_y = self.dynamic_island_config["frame_y_position"]
    ...
    def create_phone_frame(
        ...
        if device_size != "5.5_inch":  # No home indicator on older phones
            ...
            indicator_y = frame_height - self.bezel_thickness - indicator_height - self.frame_config["home_indicator"]["bottom_offset"]
    ...
    def add_promotional_text(self, image, title_text, subtitle_text=""):
        ...
        # Font sizes
        title_font = self.get_font(self.text_config["title_font_size"], "bold")
        subtitle_font = self.get_font(self.text_config["subtitle_font_size"])
        ...
        title_y = self.text_config["title_y_position"]

        font_color = self.text_config["font_color"]
        ...
        if subtitle_text:
            ...
            subtitle_y = title_y + title_height + self.text_config["subtitle_spacing"]
    ...
    def process_screenshot(
        ...
        scale = min(scale_x, scale_y) * self.screenshot_config["scale_factor"]
        ...
        corner_radius = self.screenshot_config["corner_radius"]
        ...
        if final_image.mode != "RGB":
            final_rgb = Image.new("RGB", final_image.size, self.default_background_color)
