...
class OpenAPISpecParser:
    ...
    def _resolve_references(self, spec_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self._spec_dict = spec_dict  # Store reference for resolution
        return self._resolve_refs_recursive(spec_dict)
    ...
    def _resolve_ref(self, ref_path: str) -> Dict[str, Any]:
        ...
        current = self._spec_dict
        for part in path_parts:
            if isinstance(current, dict) and part in current:
                current = current[part]
            else:
                # Reference not found, return original reference
                return {"$ref": ref_path}
        
        # Resolve any nested references in the found object
        return self._resolve_refs_recursive(current)
    ...
    def _extract_parameters(self, operation: Dict[str, Any], path_parameters: List[Dict]) -> List[Parameter]:
        ...
        for param in all_params:
            # Skip parameters that don't have a name (should not happen after $ref resolution)
