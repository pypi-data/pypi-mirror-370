...
class NonTopLevel(Level):
    def __init__(self, name: str, engine: Postgres, attributes=None, key: str = "", fk: str = "",
                 level_member_attr: str = ""):
        ...
        self.alias: str = ""
        if level_member_attr:
            self._attributes.append(level_member_attr)
            lm = Attribute(level_member_attr, engine, self)
            self.level_member_attr = lm
            setattr(self, level_member_attr, lm)
