...
from src.pyCube.session.session import create_session
...
class TestCubeOperators(unittest.TestCase):
    ...
    def test_roll_up_one_level_one_dimension(self):
        ...
        self.assertEqual(len(self.cube.supplier._levels), 4)
        self.assertEqual(len(c.supplier._levels), 3)
        self.assert_level_names_are_equal(self.cube.supplier._levels, get_supplier_level_names(0))
        self.assert_level_names_are_equal(c.supplier._levels, get_supplier_level_names(1))
        self.assertNotIn("supplier_name", list(map(lambda x: x.name, c.supplier._levels)))

    def test_roll_up_two_levels_one_dimension(self):
        c = rollup(self.cube, supplier="supplier_continent")
        self.assertEqual(len(self.cube.supplier._levels), 4)
        self.assertEqual(len(c.supplier._levels), 2)
        self.assert_level_names_are_equal(self.cube.supplier._levels, get_supplier_level_names(0))
        self.assert_level_names_are_equal(c.supplier._levels, get_supplier_level_names(2))
        self.assertNotIn("supplier_name", list(map(lambda x: x.name, c.supplier._levels)))
        self.assertNotIn("supplier_nation", list(map(lambda x: x.name, c.supplier._levels)))
    ...
    def test_roll_up_one_level_two_dimensions(self):
        ...
        self.assertEqual(len(self.cube.product._levels), 3)
        self.assertEqual(len(c.product._levels), 2)
        self.assert_level_names_are_equal(self.cube.product._levels, get_product_level_names(0))
        self.assert_level_names_are_equal(c.product._levels, get_product_level_names(1))
        self.assertNotIn("product_name", list(map(lambda x: x.name, c.product._levels)))
    ...
    def test_roll_up_two_levels_two_dimensions(self):
        ...
        self.assertEqual(len(self.cube.product._levels), 3)
        self.assertEqual(len(c.product._levels), 1)
        self.assert_level_names_are_equal(self.cube.product._levels, get_product_level_names(0))
        self.assert_level_names_are_equal(c.product._levels, get_product_level_names(2))
        self.assertNotIn("product_name", list(map(lambda x: x.name, c.product._levels)))
        self.assertNotIn("product_category", list(map(lambda x: x.name, c.product._levels)))
    ...
    def test_roll_up_no_dimensions(self):
        c = rollup(self.cube)
        self.assertEqual(len(self.cube.supplier._levels), 4)
        self.assertEqual(len(c.supplier._levels), 4)
        self.assertEqual(len(self.cube.product._levels), 3)
        self.assertEqual(len(c.product._levels), 3)
        self.assertEqual(len(self.cube.store._levels), 4)
        self.assertEqual(len(c.store._levels), 4)
        self.assertEqual(len(self.cube.date._levels), 4)
        self.assertEqual(len(c.date._levels), 4)
        self.assert_level_names_are_equal(self.cube.supplier._levels, get_supplier_level_names(0))
        self.assert_level_names_are_equal(c.supplier._levels, get_supplier_level_names(0))
        self.assert_level_names_are_equal(self.cube.product._levels, get_product_level_names(0))
        self.assert_level_names_are_equal(c.product._levels, get_product_level_names(0))
        self.assert_level_names_are_equal(self.cube.store._levels, get_store_level_names(0))
        self.assert_level_names_are_equal(c.store._levels, get_store_level_names(0))
        self.assert_level_names_are_equal(self.cube.date._levels, get_date_level_names(0))
        self.assert_level_names_are_equal(c.date._levels, get_date_level_names(0))
    ...
    def test_drill_down_one_level_one_dimension(self):
        ...
        self.assertEqual(len(self.cube.supplier._levels), 4)
        self.assertEqual(len(c2.supplier._levels), 2)
        self.assert_level_names_are_equal(self.cube.supplier._levels, get_supplier_level_names(0))
        self.assert_level_names_are_equal(c2.supplier._levels, get_supplier_level_names(2))
        self.assertNotIn("supplier_name", list(map(lambda x: x.name, c2.supplier._levels)))
        self.assertNotIn("supplier_nation", list(map(lambda x: x.name, c2.supplier._levels)))

    def test_drill_down_two_levels_one_dimension(self):
        c1 = create_rolled_up_cube(self.cube)
        c2 = drilldown(c1, supplier="supplier_nation")
        self.assertEqual(len(self.cube.supplier._levels), 4)
        self.assertEqual(len(c2.supplier._levels), 3)
        self.assert_level_names_are_equal(self.cube.supplier._levels, get_supplier_level_names(0))
        self.assert_level_names_are_equal(c2.supplier._levels, get_supplier_level_names(1))
        self.assertNotIn("supplier_name", list(map(lambda x: x.name, c2.supplier._levels)))
    ...
    def test_drill_down_one_level_two_dimensions(self):
        ...
        self.assertEqual(len(self.cube.product._levels), 3)
        self.assertEqual(len(c2.product._levels), 2)
        self.assert_level_names_are_equal(self.cube.product._levels, get_product_level_names(0))
        self.assert_level_names_are_equal(c2.product._levels, get_product_level_names(1))
        self.assertNotIn("product_name", list(map(lambda x: x.name, c2.product._levels)))
    ...
    def test_drill_down_two_levels_two_dimensions(self):
        ...
        self.assertEqual(len(self.cube.product._levels), 3)
        self.assertEqual(len(c2.product._levels), 3)
        self.assert_level_names_are_equal(self.cube.product._levels, get_product_level_names(0))
        self.assert_level_names_are_equal(c2.product._levels, get_product_level_names(0))

    def test_drill_down_same_level_ALL_one_dimension(self):
        c1 = create_rolled_up_cube(self.cube)
        c2 = drilldown(c1, supplier="ALL")
        self.assertEqual(len(c1.supplier._levels), 1)
        self.assertEqual(len(c2.supplier._levels), 1)
        self.assert_level_names_are_equal(self.cube.supplier._levels, get_supplier_level_names(0))
        self.assert_level_names_are_equal(c2.supplier._levels, get_supplier_level_names(0))
