...
from code_puppy.command_line.utils import sanitize_error_message
...
def _edit_file(context: RunContext, path: str, diff: str) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            console.print(f"[bold red]Unable to parse diff[/bold red] -- {error_msg}")
            return {
                "success": False,
                "path": file_path,
    ...
    try:
        ...
        error_msg = sanitize_error_message(e)
        console.print("[red]" + error_msg + "[/red]", highlight=False)
        return {
            "success": False,
            "path": file_path,
