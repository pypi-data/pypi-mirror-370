...
def make_code_map(directory: str, ignore_tests: bool = True) -> str:
    """DEPRECATED - Redirects to new paginated codemap.
    
    This function now uses the paginated code map system to prevent token overflow.
    """
    from code_puppy.tools.paginated_code_map import handle_codemap_command
    from code_puppy.tools.tool_logger import log_tool
    
    # Log that old function was called
    log_tool(
        tool_name="make_code_map_redirect",
        input_data={"directory": directory, "ignore_tests": ignore_tests},
        output_data=None,
        metadata={"action": "redirecting_to_paginated"}
    )
    
    # Use new paginated system - return summary view
    return handle_codemap_command(f"~codemap {directory}", directory)
