...
class ModelNameCompleter(Completer):
    """
    A completer that triggers on '/m' (or '~m' for backward compatibility) to show available models from models.json.
    ...
    def __init__(self, trigger: str = "/m"):
        ...
        self.alt_trigger = "~m" if trigger.startswith("/") else None
    ...
    def get_completions(
        ...
        if self.trigger not in text_before_cursor and \
           (not self.alt_trigger or self.alt_trigger not in text_before_cursor):
            return
        
        # Use whichever trigger was found
        actual_trigger = self.trigger
        if self.alt_trigger and self.alt_trigger in text_before_cursor:
            if self.trigger not in text_before_cursor or \
               text_before_cursor.rfind(self.alt_trigger) > text_before_cursor.rfind(self.trigger):
                actual_trigger = self.alt_trigger
        
        symbol_pos = text_before_cursor.rfind(actual_trigger)
        text_after_trigger = text_before_cursor[symbol_pos + len(actual_trigger) :]
...
def update_model_in_input(text: str) -> Optional[str]:
    # If input starts with /m or ~m and a model name, set model and strip it out
    content = text.strip()
    triggers = ["/m", "~m"]  # Support both for backward compatibility
    
    for trigger in triggers:
        if content.startswith(trigger):
            rest = content[len(trigger):].strip()
            for model in load_model_names():
                if rest == model:
                    set_active_model(model)
                    # Remove trigger+model from the input
                    idx = text.find(trigger + model)
                    if idx != -1:
                        new_text = (text[:idx] + text[idx + len(trigger + model) :]).strip()
                        return new_text
    return None
