...
def get_tool_output_limit():
    """
    Returns the maximum character limit for tool outputs,
    or 50000 if unset or misconfigured.
    Configurable by 'tool_output_limit' key.
    """
    val = get_value("tool_output_limit")
    try:
        return max(100, int(val)) if val else 50000
    except (ValueError, TypeError):
        return 50000


def get_smart_truncate():
    """
    Returns whether to use AI-powered smart truncation for large outputs.
    Configurable by 'smart_truncate' key.
    """
    val = get_value("smart_truncate")
    if val is None:
