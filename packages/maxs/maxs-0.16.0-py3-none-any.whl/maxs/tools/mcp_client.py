...
def _validate_connection(
    connection_id: str, check_active: bool = False
) -> dict[str, Any] | None:
    ...
    if check_active and not config.mcp_client:
        return {
            "status": "error",
            "content": [
                {"text": f"Connection '{connection_id}' has no active MCP client"}
            ],
        }
...
def _update_connection_status(
    connection_id: str, is_active: bool, last_error: str | None = None
) -> None:
    ...
    def invoke(self, tool: ToolUse, *args: Any, **kwargs: Any) -> ToolResult:
        ...
        try:
            ...
            if isinstance(result, str):
                ...
                if all(
                    isinstance(item, dict) and ("text" in item or "image" in item)
                    for item in result
                ):
                    ...
                    try:
                        ...
                        content.append(
                            {
                                "text": f"[List with {len(result)} items - serialization failed]"
                            }
                        )
                ...
                if "content" in result:
                    ...
                    try:
                        ...
                        if "error" in result:
                            ...
                            content.append(
                                {
                                    "text": f"[Dictionary result - serialization failed: {e!s}]"
                                }
                            )
...
def _create_transport_callable(config: ConnectionInfo) -> Any:
    ...
    if config.transport == "stdio":
        ...
        return lambda: stdio_client(
            StdioServerParameters(command=config.command, args=args, env=env)
        )
        ...
        raise ValueError(
            f"Unsupported transport: {config.transport}. Supported: stdio, sse, streamable_http"
        )
    ...
    with _CONNECTION_LOCK:
        if connection_id in _connections and _connections[connection_id].is_active:
            return {
                ...
                "content": [
                    {
                        "text": f"Connection '{connection_id}' already exists and is active"
                    }
                ],
            }
...
def _disconnect_from_server(params: dict[str, Any]) -> dict[str, Any]:
    ...
    with _CONNECTION_LOCK:
        ...
        loaded_tools = (
            config.loaded_tool_names.copy() if config.loaded_tool_names else []
        )
            ...
            connections_info.append(
                {
                    ...
                    "tools_count": (
                        config.tool_count if config.tool_count is not None else 0
                    ),
    ...
    try:
        ...
        if not config.mcp_client:
            return {
                "status": "error",
                "content": [
                    {"text": f"Connection '{connection_id}' has no active client"}
                ],
            }
...
def _load_tools_to_agent(params: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        for tool in tools:
            tool_info = {
                "name": tool.tool_name,
                "description": tool.tool_spec.get(
                    "description", f"MCP tool: {tool.tool_name}"
                ),
            ...
            try:
                ...
                logger.info(
                    f"Loaded MCP tool: {wrapper.tool_name} (original: {tool.tool_name})"
                )
