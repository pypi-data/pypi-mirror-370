            ...
            if trigger_keyword.lower() in text.lower():
                try:
                    ...
                    if keyword_index != -1:
                        ...
                        try:
                            # TODO: pass previous transcriptions to use_agent as system_prompt context + parent agent's system prompt
                            response = agent.tool.use_agent(
                                prompt=prompt_text,
                                system_prompt=agent.system_prompt
                                + f"\nReceived voice input matches with {keyword_lower}. Acting on given user voice input. Keep responses concise and conversational. Use speak tool to speak back with user smartly. You will hear my voice echo back, in that case you will decide to speak further or not.",
