...
"""
listen: Background speech listener and Whisper transcriber tool with trigger keyword functionality
...
def _transcriber_worker(stop_event: threading.Event) -> None:
    ...
    while not stop_event.is_set():
        ...
        # Check for trigger keyword and call use_agent if detected
        if (
            trigger_keyword
            and agent
            and text
            ...
            # Case-insensitive check for trigger keyword
            if trigger_keyword.lower() in text.lower():
                try:
                    logger.info(
                        f"🎯 Trigger keyword '{trigger_keyword}' detected in: '{text}'"
                    )
                    ...
                    if keyword_index != -1:
                        # Get text after the trigger keyword
                        prompt_text = text[
                        ...
                        # Call use_agent through the parent agent with record_direct_tool_call=False
                        try:
                            # Build context from previous transcriptions (INCLUDING agent responses so agent can detect its own voice echoes)
                            context_transcripts = []
                            recent_transcripts = STATE["transcript_log"][
                                -100:
                            ]  # Last 100 transcripts for context
                            ...
                            for transcript in recent_transcripts:
                                ...
                                if transcript_text.startswith("[AGENT_RESPONSE]"):
                                    # This is an agent response - include it so agent can detect echoes
                            ...
                            # Build conversation context with FULL history (user + agent responses)
                            conversation_context = ""
                            if context_transcripts:
                                conversation_context = (
                                    f"\n\nRECENT CONVERSATION CONTEXT (including your own responses):\n"
                                    + "\n".join(context_transcripts[-8:])
                                )  # Last 8 messages (user + agent)
                            ...
                            enhanced_system_prompt = (
                                ...
                                f"- You can see your own previous responses in the conversation context below\n"
                                f"- If you detect the current input is just an echo/transcription of your own speech, politely acknowledge or stay silent\n"
                                f"- Look for patterns where the user input matches something you recently said\n"
                                f"- Only respond with new information if the user is asking a genuine new question\n"
                                f"- Focus on the current user request: '{prompt_text}'\n"
                                f"- If the input seems to be your own voice, you can either stay silent or briefly acknowledge"
                                + conversation_context
                            )
                            ...
                            # Optionally log the agent response to transcripts
                            agent_record = {
                                "timestamp": _now_ts(),
                                "wav_path": None,
                                "text": f"[AGENT_RESPONSE] {response.get('content', [{}])[0].get('text', 'No response')}",
                            ...
                            try:
                                with open(log_path, "a", encoding="utf-8") as f:
                                    f.write(
                                        json.dumps(agent_record, ensure_ascii=False)
                                        + "\n"
                                    )
                            except Exception:
                                pass
                            ...
                            logger.error(f"❌ Error calling use_agent: {agent_error}")

                except Exception as trigger_error:
                    logger.error(
                        f"❌ Error processing trigger keyword: {trigger_error}"
                    )
...
def _start_listener(
    ...
    return {
        ...
        "content": [
            {
                "text": f"Listening started (model={model_name}, device_index={device_index}, device='{device_name or 'default'}'{', trigger_keyword=' + trigger_keyword if trigger_keyword else ''}). Saving to: {save_dir}"
            }
        ],
    }
