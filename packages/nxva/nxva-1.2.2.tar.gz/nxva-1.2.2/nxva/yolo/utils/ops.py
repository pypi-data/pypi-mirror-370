...
from ..utils import create_custom_pickle_module
...
#     if version == 'yolov5':
#         num_bbox_conf = 5
#         nc = nc or (prediction.shape[2] - num_bbox_conf)  # e.g. 36
#         nm = prediction.shape[2] - nc - num_bbox_conf  # number of masks
#         mi = num_bbox_conf + nc
#         xywh = prediction[..., :4]
#         if task == 'pose':
...
#         else:
#             xc = prediction[..., 4] > conf_threshold
#             obj_conf = prediction[..., 4:5]  # shape [1, 3087, 1]
#             cls_conf = prediction[..., 5:]  # shape [1, 3087, 36]
#             cls_conf *= obj_conf  # broadcasting，轉為最終 conf score

#             prediction = torch.cat([xywh, cls_conf], dim=-1)  # shape: [1, 3087, 4 + 36] = [1, 3087, 40]
#             prediction[..., :4] = xywh2xyxy(prediction[..., :4])  # xywh to xyxy
#         num_bbox_conf = 4


#     elif version in ['yolo11', 'yolov8', 'yolov5u']:
#         num_bbox_conf = 4
#         prediction = prediction.transpose(-1, -2)
#         nc = nc or (prediction.shape[2] - num_bbox_conf)  # number of classes
#         mi = num_bbox_conf + nc
#         nm = prediction.shape[2] - nc - num_bbox_conf  # number of masks
#         xc = prediction[:, :, 4:mi].amax(-1) > conf_threshold  # candidates
#         prediction[..., :4] = xywh2xyxy(prediction[..., :4])  # xywh to xyxy
#     else:
...
#         if not x.shape[0]:
#             continue
#         if (version == 'yolov5') and (task == 'pose'):
#             box, _, cls, mask = x.split((num_bbox_conf, 1, nc, nm), 1)
#         else:
#             box, cls, mask = x.split((num_bbox_conf, nc, nm), 1)
