...
from .loader import custom_load, create_custom_pickle_module
...
def attempt_load(weights, device=None, replace=None, version=None):
    """
    ...
    if replace is None:
        ...
        pickle_module = create_custom_pickle_module(replace)
        ckpt = custom_load(weights, pickle_module=pickle_module)  # load

    ckpt = ckpt['model'].to(device).float().fuse().eval()  # FP32 model
    return ckpt
