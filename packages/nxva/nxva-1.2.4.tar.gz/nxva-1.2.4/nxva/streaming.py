...
class MultiStreaming(object):
    ...
    def _load_and_validate_config(self, config):
        ...
        self.w = self.cfg.get('parameter', {}).get('WIDTH', 1920)
        self.h = self.cfg.get('parameter', {}).get('HEIGHT', 1080)
        self.gst_enable = self.cfg.get('parameter', {}).get('GSTREAMER', False)
        self.codec = self.cfg.get('parameter', {}).get('CODEC', 'h264')  # global setting for codec
        self.logger.info(f"Streaming global settings: width={self.w}, height={self.h}, gstreamer={self.gst_enable}, codec={self.codec}")
        ...
        for cam in self.cfg['cameras']:
            ...
            if 'rtsp' in cam:
                ...
                cam['rtsp'] = [{'ip': ip, 'user': cam.get('user', None), 'pw': cam.get('pw', None)}]
    ...
    def _generate_rtsp_strings(self):
        ...
        for item_id, cam in self.cameras.items():
            for rtsp in cam['rtsp']:
                ...
                if self.gst_enable:
                    ...
                    if gst_pipeline:
                        rtsp_string = gst_pipeline.format(ip=ip, user=user, pw=pw, codec=cam['codec'])
        ...
        log_string = (
            ...
            + "\n".join(f"{cam['item_id']}: {cam['rtsp_strings']}" for cam in self.cameras.values())
        )
    ...
    def init_cameras(self, cam):
        ...
        for rtsp in cam['rtsp_strings']:
            ...
            if cam['cap'].isOpened():
                ...
                cam['status'] = False
                self.logger.warning(f"Camera {cam['item_id']}: failed to open initially")
        ...
        try:
            ...
            while self.running:
                ...
                if time.time() - reconnect_time > self.reconnect_interval and fail > 10:
                    ...
                    self.logger.warning(f"Camera {cam['item_id']} is not open, try reconnecting")
                    if cap.isOpened():
                        cap.release()
                    del cap
                    gc.collect()
                    ...
                    if cam['rtsp_reconnect']:
                        cap = self._open_camera(cam['rtsp_reconnect'], self.gst_enable)
                        if not cap.isOpened():
                            cam['rtsp_reconnect'] = None
                    else:
                        for rtsp in cam['rtsp_strings']:
