...
class EngineHandler(AutoBackend, exts=['.engine']):
    ...
    def load_model(self):
        ...
        self.img_size = self.config['size']
        self.input_shapes = [(1, 3, self.img_size, self.img_size)]
        return TRTInference(self.config['weights'], self.input_shapes)
...
class TorchHandler(AutoBackend, exts=['.pt', '.pth']):
    ...
    def load_model(self):
        ...
        model = attempt_load(self.config['weights'], device=self.device, replace=replace, version=self.config['version'])
