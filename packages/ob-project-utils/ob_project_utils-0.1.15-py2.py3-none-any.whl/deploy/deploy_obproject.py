...
def deploy_apps():
    ...
    cwd = os.getcwd()
    try:
        for app_dir in os.listdir("deployments"):
            num_apps += 1
...
def register_assets():
    ...
    def register(asset_type, register_func):
        if not os.path.exists(asset_type):
            print(f"No {asset_type} defined, skipping them")
            return
        for asset_dir in os.listdir(asset_type):
...
def discover_flows():
    ...
    for flow_dir in os.listdir("flows"):
