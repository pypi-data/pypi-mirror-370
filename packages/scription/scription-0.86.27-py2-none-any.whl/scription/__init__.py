...
def _run_once(func, args, kwds):
    ...
    def later():
        ...
        if run_once:
            ...
            return cache[0]
        run_once = True
        scription_debug('calling %r' % func)
        result = func(*args, **kwds)
        scription_debug('result: %r' % (result, ))
        cache.append(result)
        return result
...
## optional
def Execute(args, cwd=None, password=None, password_timeout=None, input=None, input_delay=2.5, timeout=None, pty=None, interactive=None, env=None, encoding='utf8', **new_env_vars):
    scription_debug('creating job:', args)
    job = Job(args, cwd=cwd, pty=pty, env=env, **new_env_vars)
    try:
        scription_debug('communicating')
        job.communicate(timeout=timeout, interactive=interactive, password=password, password_timeout=password_timeout, input=input, input_delay=input_delay, encoding=encoding)
