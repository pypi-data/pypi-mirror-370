...
from sympy import Abs, Max, Min, Mod, atan2, pi
...
class Arc(GeometrySet):
    ...
    def bounds(self) -> tuple[Scalar, Scalar, Scalar, Scalar]:
        ...
        step = pi / 2
        angles = [self.start, self.start + self.length]
        direction = 1 if self.length > 0 else -1
        remainder = Mod(self.start, step)
        first = (step - remainder) if direction > 0 else (remainder if remainder != 0 else step)
        k = first
        while k < Abs(self.length):
            angles.append(self.start + direction * k)
            k += step
        a = Symbol("a", real=True)
