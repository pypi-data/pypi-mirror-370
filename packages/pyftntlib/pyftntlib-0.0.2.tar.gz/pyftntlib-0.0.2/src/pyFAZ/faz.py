...
class FAZOAuthTokenError(FAZBaseException):
    """Wrap it as a Base Exception we know it is likely a connection issue and we can't do much past that"""

    def __init__(self, msg=None, *args, **kwargs):
        super(FAZOAuthTokenError, self).__init__(msg=msg, *args, **kwargs)


class FAZLockContext(object):
...
class FortiAnalyzer(object):
    ...
    def __init__(self, host=None, user=None, passwd=None, debug=False, use_ssl=True, verify_ssl=False, timeout=300,
        ...
        self._lock_ctx = FAZLockContext(self)
    ...
    def _revoke_oauth_token(self, url, headers, token):
        ...
        client_id = "FortiAnalyzer" if "fortianalyzer" in self._host else "FortiAnalyzer"
        rev_data = {
            "client_id": client_id,
            "token": token
        }
    ...
    def track_task(self, task_id, sleep_time=3, retrieval_fail_gate=10, timeout=120, delete_task_on_timeout=False, timeout_only=True, zero_percent_timeout=30, task_stale_timeout=60):
        ...
        while track_task:
            try:
            ...
            except FAZConnectionError as err:
                # If the option is enabled in the config to disable connection
                # errors on the track_task function, then catch the FAZConnectionError
                ...
                if self.track_task_disable_connerr:
                    ...
                    code = -99
                    self.req_resp_object.error_msg = "RemoteDisconnect Issue (FAZ BugID: 0703585) occured at " \
    ...
    def login(self):
        ...
        if self.__str__() == "FortiAnalyzer instance connected to {host}.".format(host=self._host):
    ...
    def __str__(self):
        if self.sid is not None:
            return "FortiAnalyzer instance connected to {host}.".format(host=self._host)
