...
class PtWebDiscover():
    ...
    def run(self, args: ArgumentOptions) -> None:
        ...
        if not any([self.args.backups, self.args.backup_all]):
    ...
    def search_for_backup_of_source(self, url: str, ext: str, old_ext: bool, char_only: bool) -> bool:
        ...
        if char_only:
            try:
                ...
                if self.prepare_and_send_request(url + ext, ""):
                    return True
            except:
                return False
        ...
        if old_ext:
            ...
            if Url(url).is_url_dictionary() and not url[:-1] == self.domain_with_protocol:
                if self.prepare_and_send_request(url[:-1] + ext, ""):
                    return True
            else:
                try:
                    patern = r'((https?|ftps?)://[^?#"\'\s]*/[^?#"\'\s]*)\.[?#"\'\s]*' #r'((https?|ftps?):\/\/[^?#"\'\s]*\/[^?#"\'\s]*)\.[?#"\'\s]*'
                    url_without_ext = list(list({result for result in re.findall(patern, url)})[0])[0]
                    if self.prepare_and_send_request(url_without_ext + ext, ""):
                        return True
                except:
                    return False
        return False
...
def get_help():
    return [
        ...
        {"options": [
            ...
            ["-ba", "--backup-all",             "",                 "Find backup of complete website only"],
...
def parse_args() -> ArgumentOptions:
    ...
    # If source is a single item and not full URL, append it to args.url
    if args.source and isinstance(args.source, list) and len(args.source) == 1:
        # Ensure url ends with a slash
        if args.url and not args.url.endswith("/"):
            args.url += "/"
        args.url = f"{args.url}{args.source[0].lstrip('/')}"

    # Merge wordlists into one
    if args.wordlist:
        args.wordlist = helpers.expand_wordlist_patterns(args.wordlist)
