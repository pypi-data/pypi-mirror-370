...
class DacpClient:
    ...
    def download_dataframe_zip(self, dataframe_name: str, token: str, dest_folder_path:str, max_chunksize: Optional[int] = 1024 * 1024 * 5
                             , username: Optional[str] = None , mount_timestamp: Optional[str] = None):
        if not token:
            raise ValueError("token is none")
        if not username :
        ...
        dest_dir = Path(dest_folder_path).parent
        if not dest_dir.exists():
            raise FileNotFoundError(f"目标文件夹 '{dest_dir}' 不存在。请创建该文件夹后再试。")
        ...
        output_file_path = dest_folder_path / "dataset.zip"
        logger.info(f"正在尝试将 '{dataframe_name}' 下载到 '{output_file_path}'。")
        try:
            with ConnectionManager.get_connection() as conn:
                ...
                with open(output_file_path, 'wb') as f:
                    try:
                        ...
                        logger.error("数据获取失败: 数据流中没有数据。")
                        raise ValueError("数据获取失败: 数据流中没有数据。")

                    for chunk in stream_generator:
                        f.write(chunk.body.to_pybytes())

        except Exception as e:
            logger.error(f"下载过程中发生意外错误: {e}")
            raise
        logger.info(f"已成功将 '{dataframe_name}' 下载到 '{output_file_path}'。")
