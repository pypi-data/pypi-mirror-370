...
def populate_ib_paths_for_referral_sources(apps, schema_editor):
    ...
    for source in ReferralSource.objects.filter(
        ...
        hierarchy = IBHierarchy.objects.filter(
            ib_customer_id=source.customer_id
        ).first()
        ...
        if hierarchy:
            ...
            if not hierarchy.parent_ib_customer_id:
                # Single-level IB
                ...
                while current_ib:
                    ib_ids.insert(0, str(current_ib.ib_customer_id))
                    
                    if current_ib.parent_ib_customer_id:
                        current_ib = IBHierarchy.objects.filter(
                            ib_customer_id=current_ib.parent_ib_customer_id
                        ).first()
                    else:
                        current_ib = None
