...
from .errors import (
    ...
    TransportTimeoutError, TransportProtocolError, TransportValidationError,
    TransportRequestError, TransportDuplicateRequestError, 
    TransportConcurrencyLimitError, TransportProcessError
)
...
class StdioTransport(Transport):
    ...
    def __init__(self, command: List[str], request_timeout: int = 60, 
    ...
    async def connect(self) -> None:
        ...
        try:
            logger.info(
                ...
                extra={"command": self.command, "operation": "connect"}
            )
    ...
    async def disconnect(self) -> None:
        ...
        try:
            logger.info(f"Disconnecting from MCP server process PID: {self._process.pid}")
        except ValueError:
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    try:
                        logger.error("MCP server process did not respond to kill signal")
                    except ValueError:
                        # Ignore logging errors during shutdown
                        pass
    ...
    async def _limited_readline(self, stream: asyncio.StreamReader, max_bytes: int) -> bytes:
        ...
        try:
            ...
            raise TransportProtocolError(
                f"Line exceeds stream buffer limit: {e}",
                data=f"Stream limit exceeded"
            )
    ...
    async def _stderr_reader(self):
        ...
        Logs stderr output at debug level for diagnostics.
        """
        ...
        try:
            while self._running:
                try:
                    line_bytes = await self._limited_readline(self._process.stderr, self._max_line_length)
                    if not line_bytes:
                        break  # EOF
        ...
        try:
            ...
            while self._running:
                try:
                    ...
                    line_bytes = await self._limited_readline(self._process.stdout, self._max_line_length)
                    ...
                    if not line_bytes:
                        ...
                        try:
                            ...
                            if hasattr(self._process.wait, '__call__') and not hasattr(self._process.wait, '_mock_name'):
                        ...
                        if self._process.returncode is not None:
                            ...
                            raise TransportConnectionError("Connection closed by MCP server")
                    ...
                    try:
                        ...
                        logger.error(
                            ...
                            extra={"error": str(e), "line_snippet": line[:100] if len(line) > 100 else line, "operation": "message_dispatcher"}
                        )
                        raise TransportProtocolError(
            ...
            if request_id in self._pending_requests:
                ...
                if not future.done():
                    try:
                        ...
                        if self._consecutive_validation_failures >= self._max_consecutive_validation_failures:
                            self._validation_circuit_breaker_until = time.time() + self._validation_backoff_seconds
                            logger.error(
                        ...
                        logger.error(
                            ...
                            extra={"error": str(e), "request_id": request_id, "operation": "route_response", 
                        ...
                        for rid, pending_future in list(self._pending_requests.items()):
                            if not pending_future.done():
                                ...
                                pending_future.set_exception(
                                    TransportRequestError(f"Request {rid} failed due to validation error")
                                )
                                # Retrieve the exception to prevent "unretrieved exception" warnings
                                try:
                        ...
                        self._concurrent_request_count = 0
                        with self._metrics_lock:
                            self._metrics['requests_failed'] += cleared if cleared else 1
                            self._metrics['validation_failures'] += 1  # Track validation failure specifically
                        return
            else:
                logger.warning(
    ...
    async def _route_notification(self, message_dict: dict):
        ...
        try:
            ...
            logger.debug(
                "Notification queued",
                extra={"method": notification.method, "operation": "route_notification"}
            )
        except ValidationError as e:
            logger.error(
    ...
    async def send_message(self, message: MCPRequest) -> None:
        ...
        async with self._request_lock:
            ...
            self._concurrent_request_count += 1  # Track all requests, not just send_and_receive
        ...
        try:
            if self._log_json_content:
                ...
                logger.debug(f"Sending request - method: {message.method}, id: {message.id}")
            with self._metrics_lock:
            ...
            logger.error(
                ...
                extra={"error": str(e), "request_id": message.id, "method": message.method, "operation": "send_message"}
            )
    ...
    async def send_notification(self, notification: MCPNotification) -> None:
        ...
        try:
            ...
            logger.error(
                ...
                extra={"error": str(e), "method": notification.method, "operation": "send_notification"}
            )
        ...
        async with self._request_lock:
            if request_id in self._pending_requests:
                ...
                if not future.done():
                    ...
                    logger.debug(f"Request {request_id} already completed, removed from tracking")
                return True
            return False
        ...
        try:
            ...
            try:
                if self._log_json_content:
                    ...
                    logger.debug(f"Sending request - method: {request.method}, id: {request.id}")
                with self._metrics_lock:
                    self._metrics['requests_sent'] += 1
                self._process.stdin.write(json_data.encode('utf-8'))
                await self._process.stdin.drain()
    ...
    async def get_next_notification(self) -> MCPNotification:
        ...
        Raises:
            TransportDisconnectedError: If not connected or transport stopped
            TransportTimeoutError: If timeout waiting for notification
        """
        if not self.is_connected():
            raise TransportDisconnectedError("get_next_notification")
        
        try:
            # Wait for notification with timeout to allow checking _running
            ...
            if not self._running:
                raise TransportDisconnectedError("get_next_notification")
            # Wrap in TransportTimeoutError for consistency
            raise TransportTimeoutError(
                "Timeout waiting for notification",
                timeout=1.0,
                operation="get_next_notification"
            ) from e
