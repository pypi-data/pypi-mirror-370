...
from watchgate.utils.namespacing import parse_namespaced_name
...
class ServerManager:
    ...
    async def connect_all(self) -> Tuple[int, int]:
        ...
        if successful == 0:
            logger.warning(f"No upstream servers connected successfully: {error_summary}")
        elif failed > 0:
    ...
    async def _connect_server(self, conn: ServerConnection) -> bool:
        ...
        try:
            ...
            if response.result is not None:
                ...
                logger.info(f"Successfully connected to {server_desc} ({server_name} v{server_version})")
                
                return True
            else:
                raise Exception(f"Invalid initialize response: {response.error or 'No result'}")
                
        except Exception as e:
            conn.status = "disconnected"
            conn.error = str(e)
            logger.error(f"Failed to connect to {server_desc}: {e}")
            return False
