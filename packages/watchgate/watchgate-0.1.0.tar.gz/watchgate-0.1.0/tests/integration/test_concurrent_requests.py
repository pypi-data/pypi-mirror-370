...
Tests for Watchgate's ability to handle multiple simultaneous MCP requests
...
proxy:
  ...
  upstreams:
    - name: concurrent-server
      command: "python {real_mcp_server_script}"
  timeouts:
    ...
    _global:
      - policy: "tool_allowlist"
        ...
        config:
          mode: "allowlist"
          tools: 
            concurrent-server: ["concurrent_tool", "test_tool", "memory_tool"]
...
class TestConcurrentRequests:
    ...
    @pytest.mark.asyncio
    async def test_basic_concurrent_requests_10(self, real_concurrent_test_config, temp_work_directory):
        ...
        proxy = MCPProxy(config, config_loader.config_directory, stdio_server=mock_stdio_server)
        await proxy.start()
        ...
        try:
            ...
            for i, response in enumerate(responses):
                assert not isinstance(response, Exception), f"Request {i} failed with: {response}"
            
            # Verify responses have unique IDs matching requests
            response_ids = {resp.id for resp in responses if hasattr(resp, 'id')}
            request_ids = {req.id for req in requests}
            assert response_ids == request_ids, f"Response IDs {response_ids} should match request IDs {request_ids}"
            ...
            assert total_time < 3.0, f"Concurrent processing too slow: {total_time}s (expected < 3.0s)"
            ...
            with open(audit_file, 'r') as f:
                ...
                assert request_count >= 10, f"Expected at least 10 audit entries, found {request_count}"
            
        finally:
            await proxy.stop()
    
    @pytest.mark.asyncio
    async def test_medium_concurrent_requests_50(self, real_concurrent_test_config, temp_work_directory):
        ...
        try:
            ...
            assert total_time < 10.0, f"50 concurrent requests took too long: {total_time}s"
    ...
    @pytest.mark.asyncio
    async def test_high_concurrent_requests_100(self, real_concurrent_test_config, temp_work_directory):
        ...
        try:
            ...
            min_success = 90
            assert success_count >= min_success, f"Only {success_count}/100 requests succeeded (expected >= {min_success})"
            ...
            assert total_time < 15.0, f"100 concurrent requests took too long: {total_time}s"
            
        finally:
            await proxy.stop()
    
    @pytest.mark.asyncio
    async def test_request_response_ordering(self, real_concurrent_test_config, temp_work_directory):
        ...
        try:
            ...
            for req, resp in zip(requests, responses):
                assert resp.id == req.id, f"Response ID {resp.id} doesn't match request ID {req.id}"
                
                # Verify response contains expected data from request
                if hasattr(resp, 'result') and resp.result:
                    assert isinstance(resp.result, dict), "Response result should be a dictionary"
    ...
    @pytest.mark.asyncio
    async def test_plugin_state_isolation(self, real_concurrent_test_config, temp_work_directory):
    ...
    @pytest.mark.asyncio
    async def test_resource_cleanup_after_requests(self, real_concurrent_test_config, temp_work_directory):
        ...
        try:
            ...
            for i, resp in enumerate(responses):
                assert not isinstance(resp, Exception), f"Cleanup test request {i} failed: {resp}"
            ...
            assert task_increase < 50, f"Too many tasks remain after requests: {task_increase} new tasks"
            ...
            assert not isinstance(verify_response, Exception), "System not responsive after cleanup"
            
        finally:
            await proxy.stop()
    
    @pytest.mark.asyncio
    async def test_error_scenarios_under_load(self, real_concurrent_test_config, temp_work_directory):
        ...
        try:
            ...
            for i, (req, resp) in enumerate(zip(requests, responses)):
                if req.id.startswith("valid_req_"):
                    ...
                    if isinstance(resp, Exception) or (hasattr(resp, 'error') and resp.error):
                        expected_failures += 1
                    else:
                        unexpected_failures += 1
            
            # Verify error handling worked correctly
            assert valid_successes >= 8, f"Too many valid requests failed: {valid_successes}/10"
            assert expected_failures >= 6, f"Invalid requests should have failed: {expected_failures}/8"
            assert unexpected_failures <= 2, f"Too many unexpected failures: {unexpected_failures}"
            ...
            test_request = MCPRequest(
                ...
                id="post_error_test",
                method="tools/call", 
                params={"name": "concurrent-server__test_tool", "arguments": {"test": "recovery"}}
            )
            
            recovery_response = await proxy.handle_request(test_request)
            assert not isinstance(recovery_response, Exception), "System should recover after error scenarios"
            
        finally:
            await proxy.stop()

    @pytest.mark.asyncio 
    async def test_concurrent_request_limit(self, real_concurrent_test_config, temp_work_directory):
        ...
        try:
            ...
            for i in range(50):  # Well below the 100 limit
                request = MCPRequest(
                    ...
                    method="tools/call",
                    id=f"limit_test_{i}",
                    params={"name": "concurrent-server__test_tool", "arguments": {"index": i}}
                )
            ...
            successes = sum(1 for r in results if not isinstance(r, Exception) and (not hasattr(r, 'error') or r.error is None))
            failures = sum(1 for r in results if isinstance(r, Exception) or (hasattr(r, 'error') and r.error is not None))
