...
from watchgate.plugins import SecurityPlugin, AuditingPlugin, PolicyDecision
...
def plugin_manager(security_plugin, auditing_plugin):
    ...
    plugins_config = {
        "security": [{"name": "test_security", "enabled": True, "config": {}}],
        "auditing": [{"name": "test_auditing", "enabled": True, "config": {}}]
    }
...
def mock_transport():
    ...
    transport.send_and_receive = AsyncMock()  # Changed to send_and_receive for concurrency
...
class TestMCPProxyIntegration:
    ...
    @pytest.mark.asyncio
    async def test_successful_request_processing_pipeline(self, mock_config, plugin_manager, mock_server_manager, mock_transport):
        ...
        assert auditing_plugin.request_log[0]["method"] == "test_method"
        assert auditing_plugin.request_log[0]["decision"]["allowed"] is True
        
        assert len(auditing_plugin.response_log) == 1
        assert auditing_plugin.response_log[0]["id"] == "test-1"
    
    @pytest.mark.asyncio
    async def test_security_policy_blocking(self, mock_config, plugin_manager, mock_server_manager, mock_transport):
        ...
        auditing_plugin = plugin_manager.auditing_plugins[0]
        assert len(auditing_plugin.request_log) == 1
        assert auditing_plugin.request_log[0]["decision"]["allowed"] is False
    
    @pytest.mark.asyncio
    async def test_malicious_content_blocking(self, mock_config, plugin_manager, mock_server_manager, mock_transport):
    ...
    @pytest.mark.asyncio
    async def test_upstream_server_error_handling(self, mock_config, plugin_manager, mock_server_manager, mock_transport):
    ...
    @pytest.mark.asyncio
    async def test_plugin_failure_isolation(self, mock_config, plugin_manager, mock_server_manager, mock_transport):
        ...
        original_check = security_plugin.check_request
        security_plugin.check_request = Mock(side_effect=Exception("Plugin error"))
        ...
        security_plugin.check_request = original_check
    
    @pytest.mark.asyncio
    async def test_concurrent_request_processing(self, mock_config, plugin_manager, mock_server_manager, mock_transport):
    ...
    @pytest.mark.asyncio
    async def test_request_statistics_tracking(self, mock_config, plugin_manager, mock_server_manager, mock_transport):
    ...
    @pytest.mark.asyncio
    async def test_malformed_request_handling(self, mock_config, plugin_manager, mock_server_manager, mock_transport):
    ...
    async def test_plugin_configuration_integration(self):
        ...
        # Even with no plugins configured, it should return a valid decision
        decision = await plugin_manager.process_request(test_request)
        
        # Verify we get a real PolicyDecision object (not a mock)
        ...
        # Verify the decision contains real upstream-aware metadata
        assert "upstream" in decision.metadata
        assert "plugin_count" in decision.metadata
        assert isinstance(decision.metadata["plugin_count"], int)
        ...
        response_decision = await plugin_manager.process_response(test_request, test_response)
        assert response_decision is not None
        assert isinstance(response_decision.allowed, bool)
        ...
        notification_decision = await plugin_manager.process_notification(test_notification)
        assert notification_decision is not None
        assert isinstance(notification_decision.allowed, bool)
        
        # Cleanup real plugin manager
        await plugin_manager.cleanup()
    
    @pytest.mark.asyncio
    async def test_graceful_shutdown_with_pending_requests(self, mock_config, plugin_manager, mock_server_manager, mock_transport):
