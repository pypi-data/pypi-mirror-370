...
This module tests the complete notification flow through Watchgate,
...
class TestNotificationFlowIntegration:
    """Integration tests for notification flow through Watchgate."""
    ...
    async def test_server_to_client_notification_flow(self, proxy_config):
        ...
        # Create proxy
        proxy = MCPProxy(proxy_config, server_manager=mock_server_manager, stdio_server=mock_stdio_server)
        proxy._is_running = True
    ...
    async def test_bidirectional_notification_flow(self, proxy_config):
        ...
        mock_stdio_server.write_notification = AsyncMock(side_effect=capture_client_notification)
        mock_stdio_server.is_running = MagicMock(return_value=True)
        ...
        for i in range(5):
            try:
                notif = await asyncio.wait_for(upstream_mock.receive_client_to_server_notification(), timeout=0.1)
        ...
        for i in range(5):
            assert f"Client message {i}" in client_to_server_notifications[i].params["data"]
            assert f"Server message {i}" in server_to_client_notifications[i].params["data"]
        ...
        try:
            while True:
                notif = await asyncio.wait_for(
                    upstream_mock.receive_notification(), 
                    timeout=0.1
                )
