...
class TestNotificationArchitecture:
    ...
    async def test_concurrent_requests_and_notifications(self, proxy_config, mock_process):
        ...
        with patch('asyncio.create_subprocess_exec', return_value=mock_process):
            ...
            messages = [
                {"jsonrpc": "2.0", "method": "notification/test", "params": {"msg": "notif-1"}},
                {"jsonrpc": "2.0", "id": "req-1", "result": {"data": "response-1"}},
                {"jsonrpc": "2.0", "method": "notification/test", "params": {"msg": "notif-2"}},
                {"jsonrpc": "2.0", "id": "req-2", "result": {"data": "response-2"}},
                {"jsonrpc": "2.0", "method": "notification/test", "params": {"msg": "notif-3"}},
            ]
    ...
    async def test_notification_delivery_under_load(self, proxy_config, mock_process):
        ...
        with patch('asyncio.create_subprocess_exec', return_value=mock_process):
            ...
            for i in range(10):
                messages.append({"jsonrpc": "2.0", "id": f"req-{i}", "result": {"data": f"response-{i}"}})
            
            # Add notifications
            for i in range(5):
                messages.append({"jsonrpc": "2.0", "method": "notification/load", "params": {"id": i}})
            ...
            requests = [MCPRequest(jsonrpc="2.0", method="test", id=f"req-{i}") for i in range(10)]
            for req in requests:
    ...
    async def test_error_recovery_in_dispatcher(self, proxy_config, mock_process):
        ...
        with patch('asyncio.create_subprocess_exec', return_value=mock_process):
            ...
            valid_response = {"jsonrpc": "2.0", "id": "req-1", "result": {"data": "response-1"}}
