...
from watchgate.config import ConfigLoader, ProxyConfig, UpstreamConfig, TimeoutConfig
from watchgate.transport import StdioTransport
...
"""
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.yaml', delete=False) as f:
            ...
            transport = StdioTransport(
                command=config.upstreams[0].command
            )
...
"""
    ...
    def test_configuration_without_timeouts_section(self):
        ...
        config_content = """
# Watchgate MCP Gateway Configuration
proxy:
  transport: stdio
  upstreams:
    - name: "my_mcp_server"
...
"""
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.yaml', delete=False) as f:
            ...
            assert config.transport == "stdio"
            assert config.upstreams[0].command == ["python", "-m", "my_mcp_server", "--verbose"]
            assert config.upstreams[0].restart_on_failure is True
...
class TestCommandFormatIntegration:
    """Test integration between string/array command formats and transport layer."""
    
    def setup_method(self):
        """Set up test fixtures."""
        self.loader = ConfigLoader()
    
    def test_string_command_format_integration(self):
        """Test that string command format works end-to-end with transport."""
        yaml_content = """
proxy:
  transport: stdio
  upstreams:
...
"""
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.yaml', delete=False) as f:
            ...
            # Load configuration with string command
            config = self.loader.load_from_file(Path(f.name))
            
            # Verify command was parsed correctly
            assert config.upstreams[0].command == ["python", "-m", "test_server"]
            
            # Verify transport can be created with parsed command
            transport = StdioTransport(command=config.upstreams[0].command)
            assert transport is not None
    
    def test_array_command_format_integration(self):
        """Test that array command format continues to work with transport."""
        yaml_content = """
proxy:
  transport: stdio
  upstreams:
...
"""
    ...
    def test_complex_string_command_integration(self):
        """Test complex string command with quotes works with transport."""
        yaml_content = """
proxy:
  transport: stdio
  upstreams:
...
"""
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.yaml', delete=False) as f:
            ...
            # Load configuration with complex string command
            config = self.loader.load_from_file(Path(f.name))
            
            # Verify command was parsed correctly with proper argument separation
            expected_command = [
                "python", 
    ...
    def test_both_formats_produce_identical_transport_behavior(self):
        """Test that string and array formats produce identical results."""
        string_yaml = """
proxy:
  transport: stdio
  upstreams:
    - name: "test_server"
      command: "python -m test_server --port 8080"
  timeouts:
    connection_timeout: 30
    request_timeout: 60
"""
...
"""
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.yaml', delete=False) as f:
            f.write(string_yaml)
            f.flush()
            string_config = self.loader.load_from_file(Path(f.name))
        
        # Load array format
        with tempfile.NamedTemporaryFile(mode='w', suffix='.yaml', delete=False) as f:
            f.write(array_yaml)
            f.flush()
            array_config = self.loader.load_from_file(Path(f.name))
        
        # Verify both produce identical command arrays
        assert string_config.upstreams[0].command == array_config.upstreams[0].command
        assert string_config.upstreams[0].command == ["python", "-m", "test_server", "--port", "8080"]
        
        # Verify both can create equivalent transports
        string_transport = StdioTransport(command=string_config.upstreams[0].command)
        array_transport = StdioTransport(command=array_config.upstreams[0].command)
        
        assert string_transport is not None
        assert array_transport is not None
