...
class TestComplexConfiguration:
    ...
    def test_deeply_nested_plugin_config(self, temp_config_file, config_loader_no_validation):
...
"""
        ...
        assert plugin_config["level1"]["level2"]["level3"]["level4"]["deep_value"] == "nested_data"
        assert plugin_config["level1"]["level2"]["level3"]["level4"]["deep_array"] == [1, 2, 3]
        assert plugin_config["level1"]["level2"]["level3"]["level4"]["deep_dict"]["key2"] == 42
        assert len(plugin_config["level1"]["level2"]["level3"]["level3_array"]) == 2
    
    def test_mixed_types_in_nested_structures(self, temp_config_file, config_loader_no_validation):
...
"""
    ...
    @patch.dict(os.environ, {"AG_PROXY_UPSTREAM_COMMAND": '["override", "command"]',
                             "AG_PLUGINS_SECURITY_0_PRIORITY": "99",
                             "AG_PLUGINS_SECURITY_0_CONFIG_NESTED_VALUE": "env_override"})
    def test_environment_variable_overrides(self, temp_config_file, config_loader_no_validation):
...
"""
        ...
        with pytest.raises(AssertionError):
            assert config.plugins.security["_global"][0].config["nested"]["value"] == "env_override"
    
    def test_relative_paths_in_nested_configs(self, temp_config_file, config_loader_no_validation):
...
"""
        ...
        expected_path = (config_dir / "logs" / "watchgate.log").resolve()
        ...
        assert plugin_config["paths"]["templates"] == "./templates"
        assert plugin_config["paths"]["reports"] == "~/reports"
    
    def test_empty_configuration_sections(self, temp_config_file, config_loader_no_validation):
...
"""
    ...
    def test_null_vs_missing_values(self, temp_config_file, config_loader_no_validation):
...
"""
    ...
    def test_unicode_in_configuration(self, temp_config_file, config_loader_no_validation):
...
"""
    ...
    def test_very_large_configuration(self, temp_config_file, config_loader_no_validation):
        """Test handling of very large configuration files."""
        # Create a large config with many plugins
        plugins_section = []
        for i in range(100):
            plugins_section.append(f"""
      - policy: "plugin_{i}"
        priority: {i % 100}
        config:
            index: {i}
...
"""
    ...
    def test_circular_path_references(self, temp_config_file, config_loader_no_validation):
...
"""
    ...
    def test_arrays_within_nested_structures(self, temp_config_file, config_loader_no_validation):
...
"""
        ...
        assert plugin_config["nested"]["deep"]["deeper_array"][0]["metadata"]["tags"] == ["tag1", "tag2"]
