...
from watchgate.plugins.interfaces import SecurityPlugin, AuditingPlugin, PathResolvablePlugin
...
class MockPathResolvableSecurityPlugin(SecurityPlugin, PathResolvablePlugin):
    ...
    async def check_request(self, request, server_name: Optional[str] = None):
        ...
        return PolicyDecision(allowed=True, reason="Mock plugin allows all")
...
class MockNonPathResolvablePlugin(SecurityPlugin):
    ...
    async def check_request(self, request, server_name: Optional[str] = None):
        from watchgate.plugins.interfaces import PolicyDecision
        return PolicyDecision(allowed=True, reason="Non-path plugin allows all")
...
class TestPluginManagerPathResolution:
    ...
    def test_calls_set_config_directory_on_path_resolvable_security_plugins(self):
        ...
        plugins_config = {
            "security": {
                "_global": [
                    {"policy": "mock_path_resolvable", "enabled": True, "config": {}}
                ]
            }
        }
        ...
        with patch.object(PluginManager, '_discover_policies') as mock_discover:
            ...
            mock_discover.return_value = {"mock_path_resolvable": MockPathResolvableSecurityPlugin}
    ...
    def test_calls_set_config_directory_on_path_resolvable_auditing_plugins(self):
        ...
        with patch.object(PluginManager, '_discover_policies') as mock_discover:
            # Mock discovery to return our test plugin
            mock_discover.return_value = {"mock_path_resolvable": MockPathResolvableAuditingPlugin}
    ...
    def test_skips_set_config_directory_on_non_path_resolvable_plugins(self):
        ...
        plugins_config = {
            "security": {
                "_global": [
                    {"policy": "mock_non_path", "enabled": True, "config": {}}
                ]
            }
        }
    ...
    def test_removes_config_directory_from_plugin_config_dict(self):
        ...
        plugins_config = {
            "security": {
                "_global": [
                    {"policy": "mock_path_resolvable", "enabled": True, "config": {"some_setting": "value"}}
                ]
            }
        }
    ...
    def test_validates_path_resolvable_plugins_during_loading(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(PluginManager, '_discover_policies') as mock_discover:
                ...
                mock_discover.return_value = {"mock_path_resolvable": FailingValidationPlugin}
                
                manager = PluginManager(plugins_config, config_directory)
                manager._load_upstream_scoped_security_plugins(plugins_config["security"])
                
                # Plugin loading should fail and be recorded as a failure
                assert len(manager.security_plugins) == 0  # No plugins loaded
                assert manager.has_load_failures()
                failures = manager.get_load_failures()
                assert len(failures) == 1
                assert failures[0]["type"] == "security"
                assert failures[0]["policy"] == "mock_path_resolvable"
                assert "path validation failed" in failures[0]["error"]
