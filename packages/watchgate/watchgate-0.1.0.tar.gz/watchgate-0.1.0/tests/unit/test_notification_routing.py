...
from watchgate.config.models import (
    ProxyConfig, UpstreamConfig, PluginsConfig, 
    PluginConfig, TimeoutConfig
)
from watchgate.protocol.messages import MCPNotification, MCPRequest
...
class TestNotificationRouting:
    ...
    def mock_server_manager(self):
        ...
        mock_server_manager.connections = {
            "server1": conn1,
            "server2": conn2
        }
        mock_server_manager.get_connection = Mock(side_effect=lambda name: 
            mock_server_manager.connections.get(name))
    ...
    def mock_plugin_manager(self):
        ...
        mock_plugin_manager.process_notification.return_value = PolicyDecision(
            allowed=True,
    ...
    @pytest.mark.asyncio
    async def test_cancellation_notification_routing(self, basic_config, mock_stdio_server, mock_plugin_manager):
        ...
        proxy = MCPProxy(basic_config, server_manager=mock_server_manager, stdio_server=mock_stdio_server)
        ...
        conn1.transport.send_notification.assert_called_once_with(cancellation_notification)
    ...
    @pytest.mark.asyncio
    async def test_cancellation_notification_unknown_request(self, basic_config, mock_stdio_server, mock_plugin_manager):
    ...
    @pytest.mark.asyncio
    async def test_initialization_notification_broadcast(self, basic_config, mock_stdio_server, mock_plugin_manager):
    ...
    @pytest.mark.asyncio
    async def test_initialization_notification_partial_failure(self, basic_config, mock_stdio_server, mock_plugin_manager):
    ...
    @pytest.mark.asyncio
    async def test_progress_notification_forwarded_to_default_server(self, basic_config, mock_stdio_server, mock_plugin_manager):
    ...
    @pytest.mark.asyncio
    async def test_other_notification_forwarded_to_default_server(self, basic_config, mock_stdio_server, mock_plugin_manager):
    ...
    @pytest.mark.asyncio
    async def test_request_tracking_infrastructure(self, basic_config, mock_stdio_server, mock_plugin_manager):
    ...
    @pytest.mark.asyncio
    async def test_cancellation_notification_disconnected_server(self, basic_config, mock_stdio_server, mock_plugin_manager):
    ...
    @pytest.mark.asyncio
    async def test_initialization_notification_no_connections(self, basic_config, mock_stdio_server, mock_plugin_manager):
    ...
    @pytest.mark.asyncio
    async def test_notification_routing_with_plugin_blocking(self, basic_config, mock_stdio_server):
    ...
    @pytest.mark.asyncio
    async def test_notification_routing_with_plugin_modification(self, basic_config, mock_stdio_server, mock_plugin_manager):
    ...
    @pytest.mark.asyncio
    async def test_initialization_notification_partial_failure_logging(self, basic_config, mock_stdio_server, mock_plugin_manager, caplog):
        ...
        # Make conn2 fail to send
        conn2.transport.send_notification.side_effect = Exception("Server connection lost")
    ...
    @pytest.mark.asyncio
    async def test_plugin_manager_exception_during_notification_processing(self, basic_config, mock_stdio_server, caplog):
        ...
        mock_plugin_manager.process_notification.side_effect = Exception("Plugin processing failed")
        proxy._plugin_manager = mock_plugin_manager
    ...
    @pytest.mark.asyncio
    async def test_server_to_client_notification_transparent_forwarding(self, basic_config, mock_stdio_server, mock_plugin_manager):
        ...
        server_notification = NotificationScenarios.progress_notification("long_operation", 75)
        
        # Test the processing logic that _listen_server_notifications uses
        from watchgate.plugins.interfaces import PolicyDecision
        mock_plugin_manager.process_notification.return_value = PolicyDecision(
            allowed=True, reason="Server notification allowed"
        )
        
        # Simulate what _listen_server_notifications does when it receives a notification
        decision = await proxy._plugin_manager.process_notification(server_notification)
        await proxy._plugin_manager.log_notification(server_notification, decision)
        ...
        assert mock_stdio_server.notifications_sent[0].method == "notifications/progress"
        assert mock_stdio_server.notifications_sent[0].params == server_notification.params
    
    @pytest.mark.asyncio
    async def test_cancellation_notification_with_plugin_exception(self, basic_config, mock_stdio_server, caplog):
        ...
        mock_plugin_manager = AsyncMock()
        mock_plugin_manager.process_notification.side_effect = Exception("Plugin system error")
        proxy._plugin_manager = mock_plugin_manager
