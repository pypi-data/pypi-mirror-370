...
class TestLoggingConfig:
    ...
    def test_default_config(self):
        ...
        assert logging_config.format == "%(asctime)s [%(levelname)s] %(name)s: %(message)s"
    ...
    def test_file_logging_config(self):
        ...
        logging_config = LoggingConfig(
            level="DEBUG",
            handlers=["file"],
            file_path=Path("logs/watchgate.log")
        )
        
        assert logging_config.level == "DEBUG"
        assert logging_config.handlers == ["file"]
        assert logging_config.file_path == Path("logs/watchgate.log")
    ...
    def test_file_handler_without_path(self):
        ...
        with pytest.raises(ValueError, match="file_path is required when using file handler"):
