...
def test_server_names_cannot_contain_separator():
    ...
    with pytest.raises(ValueError, match="cannot contain '__'"):
        ProxyConfig(
            transport="stdio",
            upstreams=[
                UpstreamConfig(name="fs__bad", command=["cmd"])
            ],
            timeouts=TimeoutConfig()
        )
...
def test_single_server_with_name_required():
    ...
    config = ProxyConfig(
        transport="stdio",
        upstreams=[
            UpstreamConfig(name="fs", command=["cmd"])
        ],
        timeouts=TimeoutConfig()
    )
...
def test_at_least_one_upstream_required():
    ...
    with pytest.raises(ValueError, match="At least one upstream server must be configured"):
        ProxyConfig(
            transport="stdio",
            upstreams=[],
            timeouts=TimeoutConfig()
