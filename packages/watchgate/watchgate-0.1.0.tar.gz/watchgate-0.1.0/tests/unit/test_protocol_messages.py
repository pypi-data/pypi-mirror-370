...
from watchgate.protocol.messages import (
    MCPRequest, MCPResponse, MCPError, MCPNotification,
    MessageSender, SenderContext
)
...
class TestMCPRequest:
    ...
    def test_request_required_fields(self):
        ...
        request = MCPRequest(jsonrpc="2.0", method="initialize", id="test")
        
        # Should have all required fields
        assert hasattr(request, 'jsonrpc')
...
class TestMCPNotification:
    ...
    def test_notification_required_fields(self):
        """Test that MCPNotification validates required fields."""
...
class TestMessageTypesIntegration:
    ...
    def test_different_metadata_structures(self):
        ...
        assert all(hasattr(ctx.sender_context, 'sender') for ctx in [request, response, notification])
        assert all(hasattr(ctx.sender_context, 'metadata') for ctx in [request, response, notification])
