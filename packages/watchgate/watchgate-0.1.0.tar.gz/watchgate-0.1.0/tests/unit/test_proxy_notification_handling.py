...
class TestProxyNotificationHandling:
    ...
    @pytest.mark.asyncio
    async def test_handle_notification_basic(self, basic_config, mock_stdio_server, mock_transport, mock_server_manager):
    ...
    @pytest.mark.asyncio
    async def test_handle_notification_with_plugin_processing(self, basic_config, mock_stdio_server, mock_transport, mock_server_manager):
        ...
        notification = NotificationScenarios.log_message_notification("info", "Test message")
        await proxy.handle_notification(notification)
    ...
    @pytest.mark.asyncio
    async def test_handle_notification_blocked_by_plugin(self, basic_config, mock_stdio_server, mock_transport, mock_server_manager):
        ...
        mock_plugin_manager.process_notification.return_value = PolicyDecision(
            allowed=False,
            reason="Notification contains sensitive data"
        )
        mock_plugin_manager.log_notification = AsyncMock()
        proxy._plugin_manager = mock_plugin_manager
        ...
        # Create and handle notification
        notification = NotificationScenarios.log_message_notification("error", "Password: secret123")
        await proxy.handle_notification(notification)
    ...
    @pytest.mark.asyncio
    async def test_handle_notification_modified_by_plugin(self, basic_config, mock_stdio_server, mock_transport, mock_server_manager):
        """Test notification modified by security plugin."""
        proxy = MCPProxy(basic_config, server_manager=mock_server_manager, stdio_server=mock_stdio_server)
        proxy._is_running = True
        
        # Create modified notification
        modified_notification = NotificationScenarios.log_message_notification("error", "[REDACTED]")
        
        # Mock plugin manager to modify notification
        mock_plugin_manager = AsyncMock(spec=PluginManager)
        mock_plugin_manager.process_notification.return_value = PolicyDecision(
            allowed=True,
            reason="Notification sanitized",
        ...
        original_notification = NotificationScenarios.log_message_notification("error", "Password: secret123")
    ...
    @pytest.mark.asyncio
    async def test_handle_notification_error_handling(self, basic_config, mock_stdio_server, mock_transport, mock_server_manager):
    ...
    @pytest.mark.asyncio
    async def test_listen_for_upstream_notifications(self, basic_config, mock_stdio_server):
        ...
        assert mock_stdio_server.notifications_sent[1].method == "notifications/tools/list_changed"
        
        await notification_mock.disconnect()
        
    @pytest.mark.asyncio
    async def test_upstream_notification_error_recovery(self, basic_config, mock_stdio_server):
        ...
        async def process_with_error(notification):
            nonlocal call_count
            call_count += 1
            if call_count == 1:
        ...
        await notification_mock.send_server_notification(
            NotificationScenarios.progress_notification("op2", 100)  # This should succeed
        )
