...
from watchgate.protocol.errors import MCPErrorCodes, create_error_response, create_error_response_dict, create_error_dict


class TestMCPErrorCodes:
    """Test the MCPErrorCodes class with standard JSON-RPC and Watchgate-specific codes."""
    ...
    def test_watchgate_specific_error_codes(self):
        """Test that Watchgate-specific error codes are defined correctly."""
        # Watchgate-specific error codes (in -32000 to -32099 range)
        assert MCPErrorCodes.POLICY_VIOLATION == -32000
    ...
    def test_error_code_ranges(self):
        ...
        custom_codes = [
            MCPErrorCodes.POLICY_VIOLATION,
...
class TestStandardErrorScenarios:
    ...
    def test_policy_violation_scenario(self):
        """Test Watchgate policy violation error."""
        response = create_error_response_dict(
            request_id="req-1",
            code=MCPErrorCodes.POLICY_VIOLATION,
            message="Policy violation: Operation not allowed",
            data={
                "policy": "file_access",
                "action": "read",
                "resource": "/etc/passwd"
        ...
        assert response["error"]["code"] == MCPErrorCodes.POLICY_VIOLATION
        assert response["error"]["data"]["policy"] == "file_access"
...
class TestSerializeError:
    ...
    def test_serialize_mcp_error_object(self):
        ...
        from watchgate.protocol.messages import MCPError
        from watchgate.protocol.errors import serialize_error
    ...
    def test_serialize_error_invalid_type_raises(self):
        ...
        with pytest.raises(TypeError, match="Error must be MCPError object or dictionary"):
