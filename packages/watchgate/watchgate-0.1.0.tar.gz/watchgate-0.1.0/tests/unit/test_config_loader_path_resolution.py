...
class TestConfigLoaderPathResolution:
    ...
    def test_config_loader_stores_config_directory(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            config_file = temp_path / "watchgate.yaml"
    ...
    def test_config_loader_resolves_relative_config_file_path(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            loader = ConfigLoader()
            relative_path = Path("./watchgate.yaml")
            ...
            try:
                os.chdir(temp_dir)
                config = loader.load_from_file(relative_path)
    ...
    def test_config_loader_passes_config_directory_to_logging_config(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            expected_path = temp_path / "logs" / "watchgate.log"
                        ...
                        "_global": [
                            {
                                ...
                                "enabled": True,
                                "config": {
                                    "output_file": str(temp_path / "test_audit_activity.log"),  # Use absolute path
                                    "format": "json"
                                }
                            }
                        ]
    ...
    def test_config_loader_handles_home_directory_paths(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Create watchgate/logs directory in home to satisfy path validation
            home_watchgate_logs_dir = Path.home() / "watchgate" / "logs"
            home_watchgate_logs_dir.mkdir(parents=True, exist_ok=True)
            ...
            try:
                ...
                config_data = {
                    ...
                    "logging": {
                        "level": "INFO",
                        "handlers": ["file"],
                        "file_path": "~/watchgate/logs/watchgate.log"  # Home directory path
                    }
                }
                ...
                expected = Path.home() / "watchgate" / "logs" / "watchgate.log"
                assert config.logging.file_path == expected
            finally:
                # Clean up the logs directory we created
                try:
                    home_watchgate_logs_dir.rmdir()
                    (Path.home() / "watchgate").rmdir()
                except OSError:
    ...
    def test_config_loader_preserves_absolute_paths(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            abs_log_dir.mkdir()
            absolute_log_path = str(abs_log_dir / "watchgate.log")
...
class TestConfigLoaderPathResolutionEdgeCases:
    ...
    def test_config_directory_with_symlinks(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                config_file = link_config_dir / "watchgate.yaml"
    ...
    def test_config_directory_with_unicode_paths(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            config_file = unicode_config_dir / "watchgate.yaml"
