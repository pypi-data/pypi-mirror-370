...
from .messages import MCPRequest, MCPResponse, MCPNotification, RequestID, SenderContext, MessageSender
from .errors import MCPErrorCodes, create_error_response
...
class ValidationError(Exception):
    ...
    """
    def __init__(self, message: str, error_type: Optional[Union[ValidationErrorType, str]] = None, cause: Optional[Exception] = None):
...
class MessageValidator:
    ...
    METHOD_NAME_PATTERN = re.compile(r'^[^\x00-\x1F\x7F\s]+[^\x00-\x1F\x7F]*[^\x00-\x1F\x7F\s]+$|^[^\x00-\x1F\x7F\s]+$')
    ...
    def determine_message_type(self, data: dict) -> str:
        ...
        if not isinstance(data, dict):
            raise ValidationError("Message must be a dictionary", error_type=ValidationErrorType.INVALID_STRUCTURE)
        ...
        raise ValidationError("Unable to determine message type", error_type=ValidationErrorType.UNKNOWN_TYPE)
    ...
    def validate_request(self, data: dict) -> MCPRequest:
        ...
        if data is None or not isinstance(data, dict):
            raise ValidationError("Invalid message data", error_type=ValidationErrorType.INVALID_STRUCTURE)
        ...
        if params is not None and not isinstance(params, (dict, list)):
            raise ValidationError("Request params must be an object, array, or omitted", error_type=ValidationErrorType.INVALID_PARAMS)
        
        # Validate and populate sender context if present
        validated_data = dict(data)
        if 'sender_context' in data:
            validated_data['sender_context'] = self._validate_sender_context(data['sender_context'])
    ...
    def validate_response(self, data: dict) -> MCPResponse:
        ...
        if response_id is None:
            ...
            if error_code not in (MCPErrorCodes.PARSE_ERROR, MCPErrorCodes.INVALID_REQUEST):
                raise ValidationError(
    ...
    def validate_notification(self, data: dict) -> MCPNotification:
        ...
        if 'id' in data:
            raise ValidationError("Notification must not have an id field", error_type=ValidationErrorType.INVALID_NOTIFICATION)
        
        # Optional params validation - must be dict or array or omitted
        params = data.get('params')
        if params is not None and not isinstance(params, (dict, list)):
            raise ValidationError("Notification params must be an object, array, or omitted", error_type=ValidationErrorType.INVALID_PARAMS)
    ...
    def validate_batch(self, data: Any) -> List[Union[MCPRequest, MCPNotification]]:
        ...
        if not isinstance(data, list):
            raise ValidationError("Batch must be an array", error_type=ValidationErrorType.INVALID_BATCH)
        
        if len(data) == 0:
            raise ValidationError("Batch cannot be empty", error_type=ValidationErrorType.INVALID_BATCH)
        ...
        try:
            ...
            raise ValidationError(f"Invalid JSON in batch: {e}", error_type=ValidationErrorType.JSON_ERROR, cause=e)
        
        results = []
        for i, item in enumerate(data):
            if not isinstance(item, dict):
                raise ValidationError(f"Batch item {i} must be an object", error_type=ValidationErrorType.INVALID_BATCH)
            ...
            try:
                ...
                if "exceeds maximum" not in str(e):
                    raise ValidationError(f"Batch item {i} is not JSON serializable: {e}", error_type=ValidationErrorType.JSON_ERROR, cause=e)
                raise
            ...
            try:
                ...
                raise ValidationError(f"Batch item {i}: {e}", error_type=ValidationErrorType.INVALID_BATCH, cause=e)
    ...
    def validate_json_string(self, json_str: str) -> dict:
        ...
        try:
            ...
            raise ValidationError(f"Invalid JSON: {e}", error_type=ValidationErrorType.PARSE_ERROR, cause=e)
    ...
    def _validate_required_fields(self, data: dict, required_fields: list) -> None:
        ...
        for field in required_fields:
            if field not in data:
                raise ValidationError(f"Missing required field: {field}", error_type=ValidationErrorType.MISSING_FIELD)
    ...
    def _validate_jsonrpc_version(self, version: Any) -> None:
        ...
        """
        if version != "2.0":
            raise ValidationError("Invalid jsonrpc version (must be '2.0')", error_type=ValidationErrorType.INVALID_VERSION)
    ...
    def _validate_method_name(self, method: Any) -> None:
        ...
        if not isinstance(method, str):
            raise ValidationError("Method must be a string", error_type=ValidationErrorType.INVALID_METHOD)
        if not method:
            raise ValidationError("Method cannot be empty", error_type=ValidationErrorType.INVALID_METHOD)
    ...
    def _validate_error_object(self, error: Any) -> None:
        ...
        if not isinstance(error, dict):
            raise ValidationError("Error must be an object", error_type=ValidationErrorType.INVALID_ERROR)
        
        # Required fields
        if 'code' not in error:
            raise ValidationError("Error object missing 'code' field", error_type=ValidationErrorType.INVALID_ERROR)
        if 'message' not in error:
            raise ValidationError("Error object missing 'message' field", error_type=ValidationErrorType.INVALID_ERROR)
        
        # Type validation
        if not isinstance(error['code'], int):
            raise ValidationError("Error code must be an integer", error_type=ValidationErrorType.INVALID_ERROR)
        if not isinstance(error['message'], str):
            raise ValidationError("Error message must be a string", error_type=ValidationErrorType.INVALID_ERROR)
    ...
    def _validate_sender_context(self, data: Any) -> Optional[SenderContext]:
        ...
        # Parse sender enum
        sender = MessageSender.CLIENT if sender_str == 'client' else MessageSender.SERVER
