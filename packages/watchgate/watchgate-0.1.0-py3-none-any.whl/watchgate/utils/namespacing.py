"""Utility functions for handling tool namespacing in Watchgate.
...
def extract_server_context(request_method: str, request_params: Optional[Dict]) -> Optional[str]:
    """Extract server context from MCP request parameters.
    
    Args:
        request_method: The MCP method (e.g., "tools/call", "resources/read")
        request_params: The request parameters dictionary
        
    Returns:
        Server name if extractable from namespaced parameters, None otherwise
    """
    if not request_params:
        return None
        
    # Extract server name based on request method
    if request_method == "tools/call" and "name" in request_params:
        server_name, _ = parse_namespaced_name(request_params["name"])
        return server_name
    elif request_method == "resources/read" and "uri" in request_params:
        server_name, _ = parse_namespaced_name(request_params["uri"])
        return server_name
    elif request_method == "prompts/get" and "name" in request_params:
        server_name, _ = parse_namespaced_name(request_params["name"])
        return server_name
...
def create_denamespaced_request_params(request_method: str, request_params: Dict) -> Dict:
    """Create request parameters with de-namespaced names for upstream servers.
    
    Args:
        request_method: The MCP method
        request_params: The original request parameters
        
    Returns:
        New parameters dictionary with de-namespaced names
    """
    denamespaced_params = request_params.copy()
    
    if request_method == "tools/call" and "name" in denamespaced_params:
        _, tool_name = parse_namespaced_name(denamespaced_params["name"])
        denamespaced_params["name"] = tool_name
    elif request_method == "resources/read" and "uri" in denamespaced_params:
        _, resource_uri = parse_namespaced_name(denamespaced_params["uri"])
        denamespaced_params["uri"] = resource_uri
    elif request_method == "prompts/get" and "name" in denamespaced_params:
        _, prompt_name = parse_namespaced_name(denamespaced_params["name"])
        denamespaced_params["name"] = prompt_name
        
    return denamespaced_params