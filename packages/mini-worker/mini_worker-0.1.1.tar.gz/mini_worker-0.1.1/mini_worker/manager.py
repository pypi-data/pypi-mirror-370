...
class MiniWorkerManager:
    ...
    def start_worker_with_params(self, worker_name: str, parameters: Dict[str, Any]):
        ...
        Includes robust race condition handling and automatic cleanup.
        ...
        # Check if worker is running with automatic stale cleanup
        # The enhanced is_worker_running will clean up stale PID files automatically
        if self.is_worker_running(worker_name):
            # Double-check after a brief moment to handle race conditions
            import time
            time.sleep(0.1)
            if self.is_worker_running(worker_name):
