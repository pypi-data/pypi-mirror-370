...
class BaseMiniWorker(ABC):
    ...
    def write_pid_file(self):
        """Write process ID to file for monitoring with atomic operation"""
        ...
        try:
            ...
            self.logger.debug(f"PID file written atomically: {pid_file}")
    ...
    def remove_pid_file(self):
        """Remove PID file on shutdown with proper verification"""
        pid_file = os.path.join(self.stats_dir, f"{self.worker_id}.pid")
        try:
            if os.path.exists(pid_file):
                # Verify this is actually our PID file before removing
                try:
                    ...
                    if stored_pid == current_pid:
                        ...
                        self.logger.warning(f"PID file contains different PID ({stored_pid} vs {current_pid}), not removing")
                except (ValueError, IOError):
                    # Corrupted PID file, safe to remove
                    os.unlink(pid_file)
                    self.logger.debug(f"Corrupted PID file removed: {pid_file}")
        except Exception as e:
