...
class SmartFollowUpService:
    ...
    def get_follow_up_strategy_info_for_cot(workflow_link_state: SmartFollowUpState, strategy: FollowUpStrategy) -> str:
        """Get the descriptive title of the follow up strategy"""
        return f"🤖 La estrategia de seguimiento es: {strategy.name} | Se ejecutaron {workflow_link_state.consecutive_count}/{strategy.maximum_consecutive_follow_ups} veces consecutivas y en total se han enviado {workflow_link_state.total_followups_sent}/{strategy.maximum_follow_ups_to_be_executed} seguimientos. \n El próximo seguimiento es el #{workflow_link_state.consecutive_count + 1} - configurado para {strategy.get_interval_for_followup(workflow_link_state.consecutive_count + 1)/60} horas. Se hará a las {workflow_link_state.next_call.astimezone(ZoneInfo('UTC')).strftime('%H:%M')} (UTC-0) | {workflow_link_state.next_call.astimezone(ZoneInfo('UTC')).strftime('%H:%M')} (UTC-3)"
