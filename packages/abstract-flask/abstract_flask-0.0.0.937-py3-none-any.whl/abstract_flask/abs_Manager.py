...
class AbsManager(metaclass=SingletonMeta):
    def __init__(self, base_path=None,override=False):
        if not hasattr(self, 'initialized'):
            self.initialized = True
            if base_path and isinstance(base_path,str) and isfile(base_path):
                self.base_path = base_path or os.getcwd()
            elif base_path and isinstance(base_path,dict):
                for dir_desig,path in base_path.items():
        if override and             
    ...
    def get_data_dir(self):
        return self._make_dir(os.path.join(self.get_base_path(), 'data'))
