...
from os.path import join,dirname,basename,splitext,abspath,isfile,isdir,exists
...
import os
@singleton
class AbsManagerDynamic:
    def __init__(self, base_path=None,directories={},override=False):
        if not hasattr(self, 'initialized'):  # This ensures the initialization happens only once.
            self.initialized = True
            self.base_path = base_path or os.getcwd()
            self.override_call_registry={}
            self.directory_map = {}  # To keep track of directories initialized by users with override.
        
        elif override == True:
            caller_path = os.getcwd()
            for each in self.override_call_registry:
                if caller_path == each:
                    return 
            base_path=basepath or caller_path
            override_caller_name = f"override_{len(self.override_call_registry)}"
            self.override_call_registry[override_caller_name]={"caller_path":caller_path,"base_path":base_path}
            self.directory_map[override_caller_name]=directory_map
        
    def check_caller_id():
        caller_path = os.getcwd()
        override_caller_recursive = None
        for overide_caller,values in self.override_call_registry.items():
             curr_override_caller_path = values['caller_path']
             if curr_override_caller_path in caller_path:
                 if override_caller_recursive == None:
                     override_caller_recursive=overide_caller
                 else:
                     len(self.override_call_registry[override_caller_recursive]['caller_path'])<len(curr_override_caller_path)
                     override_caller_recursive=overide_caller
        
        if override_caller_recursive == None:
            print('no override caller found')
            return None
        return self.override_call_registry[override_caller_recursive]['base_path']
    ...
    def get_base_path(self):
        base_path = check_caller_id()
        if base_path:
            self.base_path = base_path
        return self.base_path

    def get_converts_dir(self):
        return self.add_directory('converts', join(self.get_base_path(), 'converts'))

    def get_users_dir(self):
        return self.add_directory('users', join(self.get_base_path(), 'users'))

    def get_data_dir(self):
        return self.add_directory('data', join(self.get_base_path(), 'data'))

    def get_uploads_dir(self):
        return self.add_directory('uploads', join(self.get_base_path(), 'uploads'))

    def get_downloads_dir(self):
        return self.add_directory('downloads', join(self.get_base_path(), 'downloads'))

    # User-specific directories
    def get_user_dir(self, user, sub_path=None):
        user_dir_key = f"user_{user}"
        user_base_dir = self.add_directory(user_dir_key, join(self.get_users_dir(), user))
        if sub_path:
            user_dir_key = f"{user_dir_key}_{sub_path}"
            return self.add_directory(user_dir_key, join(user_base_dir, sub_path))
        return user_base_dir

    def check_user_dir(self, user, sub_path=None):
        user_dir = self.get_user_dir(user, sub_path)
        return isdir(user_dir)

    def make_user_dir(self, user, sub_path=None):
        """Ensure the user directory and any sub-directories exist."""
        return self.get_user_dir(user, sub_path)

    # File path generation (without creating directories)
    def get_file_path(self, directory, file_name):
        """Generate a full path for a file within a directory."""
