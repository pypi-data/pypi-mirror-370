        ...
        if username and password:
            ...
            try:
                ...
                if r and Config().get('app.login.cache-creds', True):
                    Idp.cache_credentials(env_f, okta, username, password)
        elif username and Config().get('app.login.cache-username', True):
            Idp.cache_credentials(env_f, okta, username)

        return None

    def cache_credentials(env_f: str, okta: str, username: str, password: str = None):
        updated = []
            ...
            with open(env_f, 'r') as file:
                try:
                    ...
                    for l in file_content.split('\n'):
                        ...
                        if key == f'IDP_USERNAME':
                            value = username
                        elif key == f'IDP_PASSWORD' and not KubeContext.in_cluster():
