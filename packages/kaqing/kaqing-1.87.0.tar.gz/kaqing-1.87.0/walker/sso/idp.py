...
class Idp:
    ...
    def with_creds(app_host: str, idp_uri: str, forced: bool, username: str = None) -> IdpLogin:
        ...
        def update_authn(user2: str = None):
            if user2:
                idp_uri = SsoConfig().find_idp_uri(user2, app_host, app_host)
                idp = urlparse(idp_uri).hostname
        ...
        authn: Authenticator = update_authn(username)
        if not authn:
            return None
        ...
        while not username or Idp.ctrl_c_entered:
            ...
            if default_user:
                ...
                # idp = urlparse(idp_uri).hostname
                authn = update_authn(default_user)
