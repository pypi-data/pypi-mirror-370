...
from scontoolkit.models.slg.models import Asset
...
class CKANCatalogService(ICatalogService):
    ...
    def __retrieve_datasets__(self):
    ...
    async def list_datasets(self, db: MongoStorage) -> list[Dataset]:
        ckan_dsets = self.__retrieve_datasets__()
    ...
    async def list_assets(self,  db: MongoStorage) -> list[Dataset]:
        ckan_dsets = self.__retrieve_datasets__()
        exposed = await db.get_offers_for_datasets([item['id'] for item in ckan_dsets])
        assets = [
            Asset(
                id = item['id'],
                name = item['name'],
                offer_id = exposed[item['id']] if item['id'] in exposed.keys() else None,
                description=None,
