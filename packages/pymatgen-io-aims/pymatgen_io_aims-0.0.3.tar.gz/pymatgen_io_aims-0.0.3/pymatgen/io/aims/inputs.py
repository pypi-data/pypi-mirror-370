...
class AimsGeometryIn(MSONable):
    ...
    def from_file(cls, filepath: str | Path) -> Self:
        ...
        Args:
            filepath (str | Path): The path to the input file
    ...
    def from_dict(cls, dct: dict[str, Any]) -> Self:
        ...
        Args:
            dct (dict[str, Any]): The MontyEncoded dictionary of the AimsGeometryIn
...
class AimsControlIn(MSONable):
    ...
    def get_content(
        ...
        if (
            ...
            warn(
                "Removing spin from parameters since no spin information "
                "is in the structure",
                RuntimeWarning,
                stacklevel=1,
            )
        ...
        species_defaults_map = self._parameters.get(
        ...
        for label, el in elements_map.items():
            if isinstance(basis_set, dict):
                ...
                if basis_name is None:
                    raise ValueError(
                        f"Basis set not found for specie {label} "
                        "(represented by element {el})"
                    )
            else:
            ...
            paths_to_try = [
                (Path(species_dir) / basis_name / species_file_name)
                .expanduser()
                .as_posix(),
                (Path(species_dir) / "defaults_2020" / basis_name / species_file_name)
                .expanduser()
                .as_posix(),
            ]
            for path in paths_to_try:
                ...
                if os.path.isfile(zip_path):
                    with zopen(zip_path, mode="rt") as file:
        ...
        content += control_in.get_content(
            geometry=geometry, verbose_header=verbose_header
        )
        return content
    ...
    def write_file(
        ...
        if isinstance(structure, Structure) and (
            "k_grid" not in self._parameters
            and "k_grid_density" not in self._parameters
        ):
