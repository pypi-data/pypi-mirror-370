...
try:
    ...
    TG4PERFETTO_AVAILABLE = True
except ImportError:
    TG4PERFETTO_AVAILABLE = False
    # 创建一个简单的替代类
    class TraceGenerator:
        def __init__(self):
            self.events = []
        
        def trace_begin(self, name, category="default"):
            pass
        
        def trace_end(self, name, category="default"):
            pass
        
        def trace_instant(self, name, category="default"):
            pass
        
        def write_trace(self, filename):
            print(f"Warning: tg4perfetto not available. Trace not saved to {filename}")
            print("Install tg4perfetto with: pip install tg4perfetto")
