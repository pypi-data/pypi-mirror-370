...
def cim_compute(A: Any, B: Any, 
    ...
    # Check if inputs are torch tensors first
    is_torch_tensor_A = TORCH_AVAILABLE and hasattr(A, 'device') and str(type(A)).find('torch') != -1
    is_torch_tensor_B = TORCH_AVAILABLE and hasattr(B, 'device') and str(type(B)).find('torch') != -1
    
    if is_torch_tensor_A and is_torch_tensor_B:
        # Both A and B are torch tensors
        if operation == "matmul":
    ...
    if NUMPY_AVAILABLE and TORCH_AVAILABLE:
        ...
        try:
            ...
            logger.debug(f"Torch conversion failed: {e}, falling back to numpy")
            # Fall back to numpy if torch conversion fails
            pass
