...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="YICA-Yirage Performance Monitor - Benchmark and Monitor Tool",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  yica-benchmark --monitor --duration 60
  yica-benchmark --benchmark matmul --size 1024
  yica-benchmark --profile model.onnx --output report.json
  yica-benchmark --compare yica cuda --operation attention
        """
    )
    
    parser.add_argument('--version', action='version', version='YICA-Yirage Performance Monitor v1.0.6')
    ...
    monitor_parser.add_argument('--duration', type=int, default=30, help='Monitoring duration in seconds')
    monitor_parser.add_argument('--interval', type=float, default=1.0, help='Sampling interval in seconds')
    monitor_parser.add_argument('--output', help='Output file for monitoring data')
    
    # 基准测试命令
    benchmark_parser = subparsers.add_parser('benchmark', help='Run performance benchmarks')
    benchmark_parser.add_argument('operation', choices=['matmul', 'attention', 'mlp', 'all'],
                                help='Operation to benchmark')
    benchmark_parser.add_argument('--size', type=int, default=512, help='Problem size')
    ...
    benchmark_parser.add_argument('--output', help='Output benchmark results file')
    
    # 性能分析命令
    profile_parser = subparsers.add_parser('profile', help='Profile model performance')
    profile_parser.add_argument('model', help='Model file to profile')
    profile_parser.add_argument('--backend', default='auto', help='Backend to use')
    profile_parser.add_argument('--output', help='Output profile report')
    
    # 比较命令
    compare_parser = subparsers.add_parser('compare', help='Compare different backends')
    compare_parser.add_argument('backends', nargs='+', help='Backends to compare')
    compare_parser.add_argument('--operation', default='matmul', help='Operation to compare')
    compare_parser.add_argument('--size', type=int, default=512, help='Problem size')
    
    args = parser.parse_args()
    
    print("📊 YICA-Yirage Performance Monitor v1.0.6")
    print("=" * 50)
    ...
    try:
        if args.command == 'monitor':
            print(f"🔍 Starting system monitoring for {args.duration}s...")
            # 这里应该实现实际的监控逻辑
            ...
            # 这里应该实现实际的基准测试逻辑
            print(f"✅ Benchmark completed for {args.operation}")
            ...
            print(f"⚖️ Comparing backends: {', '.join(args.backends)}")
            # 这里应该实现实际的比较逻辑
