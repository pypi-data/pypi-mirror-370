                        ...
                        for mdv_idx, var in enumerate(mdv):
                            ...
                            for sub_idx, sub in enumerate(var):
                                if sub.tag == "ItemGroupDef":
                                    ...
                                    for child2_idx, child2 in enumerate(sub):
                                        ns_match = re.match(r"^\{(.+)\}", child2.tag)
                                        child2_ns = ns_match.group(1) if ns_match else ""
                                        tag_clean = re.sub(r'^\{.*\}', '', child2.tag)
                                        oid_value = child2.attrib.get("OID", "")
                                        ig_def_list.append({
                                            "key": f"study{child_idx}_mdv{mdv_idx}_ig{sub_idx}_child{child2_idx}_{tag_clean}",
                                            "value": oid_value,
                                            "ns": child2_ns 
                                        })
                                        for attr_key, attr_value in child2.attrib.items():
                                            ig_def_list.append({
                                                "key": f"study{child_idx}_mdv{mdv_idx}_ig{sub_idx}_child{child2_idx}_{attr_key}",
                                                "value": attr_value,
                                                "ns": ""
                                            })
