...
class TabDefine(QWidget):
    ...
    def tab_button_3_f(self):
        ...
        self.folder_file_df = pd.DataFrame()

        # Save tab_sd_meta_df
        file_sd_meta = os.path.join(file_path_base, "log_sd_meta.csv")
        try:
            self.tab_sd_meta_df.to_csv(file_sd_meta, index=False)
            mtbp3cd.gui.util_show_message(self.message_list, f"sd_meta_df saved: {file_sd_meta}", status="s")
        except Exception as e:
            mtbp3cd.gui.util_show_message(self.message_list, f"Failed to export tab_sd_meta_df: {e}", status="f")

        # Save tab_sd_igdef_df
        file_sd_igdef = os.path.join(file_path_base, "log_sd_igdef.csv")
        try:
            self.tab_sd_igdef_df.to_csv(file_sd_igdef, index=False)
            mtbp3cd.gui.util_show_message(self.message_list, f"sd_igdef_df saved: {file_sd_igdef}", status="s")
        except Exception as e:
            mtbp3cd.gui.util_show_message(self.message_list, f"Failed to export tab_sd_igdef_df: {e}", status="f")

        # Save tab_sd_igdef_oid (list)
        file_sd_igdef_oid = os.path.join(file_path_base, "log_sd_igdef_oid.txt")
        try:
            with open(file_sd_igdef_oid, "w", encoding="utf-8") as f:
                for item in self.tab_sd_igdef_oid:
                    f.write(str(item) + "\n")
            mtbp3cd.gui.util_show_message(self.message_list, f"sd_igdef_oid saved: {file_sd_igdef_oid}", status="s")
        except Exception as e:
            mtbp3cd.gui.util_show_message(self.message_list, f"Failed to export sd_igdef_oid: {e}", status="f")

        # Save tab_sd_igdef_oid_desc
        file_sd_igdef_oid_desc = os.path.join(file_path_base, "log_sd_igdef_oid_desc.csv")
        try:
            self.tab_sd_igdef_oid_desc.to_csv(file_sd_igdef_oid_desc, index=False)
            mtbp3cd.gui.util_show_message(self.message_list, f"sd_igdef_oid_desc saved: {file_sd_igdef_oid_desc}", status="s")
        except Exception as e:
            mtbp3cd.gui.util_show_message(self.message_list, f"Failed to export sd_igdef_oid_desc: {e}", status="f")

        # Save tab_ad_meta_df
        file_meta = os.path.join(file_path_base, "log_ad_meta.csv")
        try:
            self.tab_ad_meta_df.to_csv(file_meta, index=False)
            mtbp3cd.gui.util_show_message(self.message_list, f"ad_meta_df saved: {file_meta}", status="s")
        except Exception as e:
            mtbp3cd.gui.util_show_message(self.message_list, f"Failed to export tab_ad_meta_df: {e}", status="f")

        # Save tab_ad_igdef_df
        file_igdef = os.path.join(file_path_base, "log_ad_igdef.csv")
        try:
            self.tab_ad_igdef_df.to_csv(file_igdef, index=False)
            mtbp3cd.gui.util_show_message(self.message_list, f"ad_igdef_df saved: {file_igdef}", status="s")
        except Exception as e:
            mtbp3cd.gui.util_show_message(self.message_list, f"Failed to export tab_ad_igdef_df: {e}", status="f")

        # Save tab_ad_igdef_oid (list)
        file_igdef_oid = os.path.join(file_path_base, "log_ad_igdef_oid.txt")
        try:
            with open(file_igdef_oid, "w", encoding="utf-8") as f:
                for item in self.tab_ad_igdef_oid:
                    f.write(str(item) + "\n")
            mtbp3cd.gui.util_show_message(self.message_list, f"ad_igdef_oid saved: {file_igdef_oid}", status="s")
        except Exception as e:
            mtbp3cd.gui.util_show_message(self.message_list, f"Failed to export ad_igdef_oid: {e}", status="f")

        # Save tab_ad_igdef_oid_desc
        file_igdef_oid_desc = os.path.join(file_path_base, "log_ad_igdef_oid_desc.csv")
        try:
            self.tab_ad_igdef_oid_desc.to_csv(file_igdef_oid_desc, index=False)
            mtbp3cd.gui.util_show_message(self.message_list, f"ad_igdef_oid_desc saved: {file_igdef_oid_desc}", status="s")
        except Exception as e:
