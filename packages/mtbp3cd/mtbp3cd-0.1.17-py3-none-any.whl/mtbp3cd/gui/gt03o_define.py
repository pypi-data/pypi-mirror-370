...
class TabDefine(QWidget):
    def __init__(self, _p):
        ...
        self.tabs_sd.addTab(self.tab_sd_tree, "Dataset")
        ...
        self.tabs_ad.addTab(self.tab_ad_tree, "Dataset")
    ...
    def util_get_define_meta_info(folder):
                ...
                for child_idx, child in enumerate(root):
                    ...
                    if mdv is not None:
                        ...
                        for ig_idx, ig in enumerate(itemgroupdef_list):
                            ...
                            ig_oid = ig.attrib.get("OID", "")
                            ig_def_oid_list.append(ig_oid)
                            ig_def_list.append({
                            ...
                            for attr_key, attr_value in ig.attrib.items():
                                ig_def_list.append({
                                    "key": f"study{child_idx}_mdv{mdv_idx}_itemgroupdef{ig_idx}_{attr_key}",
                                    "value": attr_value,
                                    "ns": ig_ns
                                })
                            ...
                            if desc is not None:
                                ...
                                if translate_text is not None and translate_text.text is not None:
                                    ig_def_list.append({
                                        "key": f"study{child_idx}_mdv{mdv_idx}_itemgroupdef{ig_idx}_desc",
                            ...
                            for itemref in itemref_list:
                                ...
                                if item_oid:
                                    if mandatory == "Yes":
                                        ig_def_oid_list.append(f"{item_oid} (Mandatory)")
                                    else:
                                        ig_def_oid_list.append(item_oid)j
        ...
        return meta_info, igdef_info, ig_def_oid_list
    ...
    def tab_button_1_f(self):
        ...
        if folder:
            ...
            self.tab_sd_meta_df, self.tab_sd_igdef_df, self.tab_sd_igdef_oid = self.util_get_define_meta_info(folder)
    ...
    def tab_button_2_f(self):
        ...
        if folder:
            ...
            lsr1 = LsrTree(self.tab_ad_igdef_oid, outfmt="tree", infmt='dotspace')
            lsr1_str = lsr1.list_files() 
