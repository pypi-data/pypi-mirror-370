...
class TabDefine(QWidget):
    def __init__(self, _p):
        ...
        self.tab_sd_igdef_table.setHorizontalHeaderLabels(["Id", "File Name", "Description"])
        ...
        self.tab_ad_igdef_table.setHorizontalHeaderLabels(["Id", "File Name", "Description"])
    ...
    def util_get_define_meta_info(folder):
                ...
                for child_idx, child in enumerate(root):
                    ...
                    if mdv is not None:
                        ...
                        for item_idx, itemdef in enumerate(itemdef_list):
                            ...
                            if item_oid:
                                ig_def_oid_list.append(f"{ig_attr_oid}.{item_oid}")
                                ig_def_oid_list_desc.append(f"({data_type},{length}); [desc] {description}")
        ...
        return meta_info, igdef_info, ig_def_oid_list, ig_def_oid_list_desc
    ...
    def tab_button_1_f(self):
        ...
        if folder:
            ...
            for row_idx, row in self.tab_sd_meta_df.iterrows():
                ...
                self.tab_sd_meta_table.setItem(row_idx, 0, key_item)
                self.tab_sd_meta_table.setItem(row_idx, 1, value_item)
            ...
            for row_idx, row in self.tab_sd_igdef_df.iterrows():
                key_item = QTableWidgetItem(str(row["key"]))
                value_item = QTableWidgetItem(str(row["value"]))
                ns_item = QTableWidgetItem(str(row["description"]))
                self.tab_sd_igdef_table.insertRow(row_idx)
                self.tab_sd_igdef_table.setItem(row_idx, 0, key_item)
                self.tab_sd_igdef_table.setItem(row_idx, 1, value_item)
                self.tab_sd_igdef_table.setItem(row_idx, 2, ns_item)
            ...
            if self.tab_sd_igdef_oid:
                ...
                list0 = []
                list1 = []
                for item in lsr1_str:
                    parts = item.split("_idx_")
                    list1.append(parts[-1])
                    list0.append(''.join(parts[:-1]))
                combined_list = []
                for item, idx_str in zip(list0, list1):
                    try:
                        desc = self.tab_sd_igdef_oid_desc[int(idx_str)]
                    except (IndexError, ValueError):
                        desc = ""
                    combined_list.append(item+str(desc))

                self.tab_sd_tree_str.clear()
                width = len(str(len(combined_list)))
                self.tab_sd_tree_str.addItems([f"{str(idx+1).zfill(width)}: {str(item)}" for idx, item in enumerate(combined_list)])
            else:
                self.tab_sd_tree_str.clear()
    ...
    def tab_button_2_f(self):
        ...
        if folder:
            ...
            self.tab_ad_meta_df, self.tab_ad_igdef_df, self.tab_ad_igdef_oid = self.util_get_define_meta_info(folder)
            ...
            if self.tab_ad_igdef_oid:
                ...
                for item, idx_str in zip(list0, list1):
                    try:
                        desc = self.tab_ad_igdef_oid_desc[int(idx_str)]
                    except (IndexError, ValueError):
                        desc = ""
                    combined_list.append(item+str(desc))
                ...
                width = len(str(len(combined_list)))
                self.tab_ad_tree_str.addItems([f"{str(idx+1).zfill(width)}: {str(item)}" for idx, item in enumerate(combined_list)])
            else:
                self.tab_ad_tree_str.clear()
