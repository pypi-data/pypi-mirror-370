...
def q(p):
    """Devuelve la ruta entre comillas dobles, lista para archivos .in."""
    return f'"{os.fspath(p)}"'
...
def plot_hydrograms(obs_dir, model_dir, wells_dir, scale, date_ini, format_date='%d/%m/%Y', lang='EN'):
    ...
    # Model files
    gsf_file = dis_file = hds_file = None
    ...
    if not all([gsf_file, dis_file, hds_file]):
        raise FileNotFoundError("No se encontraron .gsf/.dis/.hds esperados en model_dir")
    
    # Rutas completas
    gsf_path = Path(model_dir) / gsf_file
    dis_path = Path(model_dir) / dis_file
    hds_path = Path(model_dir) / hds_file
    wells_path = Path(wells_dir)
            
    ## run quadfac to obtain factors
    quadfac = 'usg_quadfac.in'
    with open(quadfac, 'w', newline='\r\n', encoding='utf-8') as g:
        g.write(q(gsf_path) + '\r\n')
        g.write('n\r\n')
        g.write('0\r\n')
        g.write(q(dis_path) + '\r\n')
        g.write(q(wells_path) + '\r\n')
        g.write(q(wells_path) + '\r\n')
        g.write('factors.dat\r\n')
        g.write('1\r\n')

    with open(quadfac, 'rb') as fin:
        subprocess.run([get_executable_path('usgquadfac.exe')], stdin=fin, check=True)
    
    # run mod2smp to obtain simulated values
    ins_file = 'mod2smp.in'
    with open(ins_file, 'w', newline='\r\n', encoding='utf-8') as f:
        f.write('factors.dat\r\n')
        f.write(q(wells_path) + '\r\n')
        f.write('g\r\n')
        f.write('y\r\n')
        f.write(q(hds_path) + '\r\n')
        f.write(str(999999) + '\r\n')
        f.write(str(5000) + '\r\n')
        f.write('d\r\n')
        f.write(str(date_ini) + '\r\n')
        f.write('00:00:00\r\n')
        f.write('niveles_simulados.smp\r\n')

    with open(ins_file, 'rb') as fin:
        subprocess.run([get_executable_path('usgmod2smp.exe')], stdin=fin, check=True)
