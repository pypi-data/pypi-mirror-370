...
class DataManager:
    ...
    def _add_data_generic(
        ...
        if self.data.get(data_type) is None:
            self.data[data_type] = validated_input.data
    ...
    def _add_static(
        ...
        if self.data.get(DataType.STATIC) is None:
            self.data[DataType.STATIC] = validated.data
            ...
            # 데이터 결합
            self.data[DataType.STATIC] = np.concatenate([existing, data])
...
if __name__ == "__main__":
    ...
    dm = DataManager()
    dm.add_stock("open", cf.get_df("price_open"))
    dm.add_stock("high", cf.get_df("price_high"))
    dm.add_stock("low", cf.get_df("price_low"))
    dm.add_stock("close", cf.get_df("price_close"))
    dm.add_stock("volume", cf.get_df("volume_sum"))
    dm.info()
