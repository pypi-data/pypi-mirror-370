...
import numpy as np
import torch
from elastic_models.transformers import AutoModelForCausalLM as EMAutoModelForCausalLM
...
class Model(BaseModel):
    ...
    def __init__(self):
        ...
        if model_size == "eager":
            model_class = HFAutoModelForCausalLM
        else:
            model_init_params.update({"mode": model_size})
            model_class = EMAutoModelForCausalLM
    ...
    def forward(self, input_batch: dict[str, list[np.ndarray[Any, Any]]]) -> list[Any]:
        ...
        # Format messages for chat template processing
        chat_messages_batch = [
            [
                {
                    "role": "system",
        ...
        for sample_idx, input_sequence in enumerate(tokenized_inputs):
        ...
        # Prepare tensors for model execution
        tokenized_inputs = tokenized_inputs.to(self.device)
        attention_mask = (tokenized_inputs != self.tokenizer.pad_token_id).long()
        ...
        input_sequence_lengths = [len(seq) for seq in tokenized_inputs]
        results = []
