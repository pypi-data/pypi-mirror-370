...
class LineRepository:
    ...
    @staticmethod
    def _parse_image_hash_from_md(content: str) -> Optional[str]:
        """
    ...
    async def save_lines(self, doc_id: UUID, lines: list[Any]):
        """
        Жестко-однократная запись массива строк для документа:
        - если строки для doc_id уже существуют -> ошибка (не перезаписываем);
        - строгая опора на 'position' (line_no игнорируем);
        - защита от дублей позиции;
        - удаляем дубли по line_id в image-details на всякий случай;
        - подстраховка: если нет image_hash, пытаемся достать из ![](hash.png), иначе пропускаем изображение.
        """
        if not lines:
            return
        ...
        async with AsyncUnitOfWork(self._session_factory) as uow:
            try:
                # 0) Запрещаем повторный импорт массива строк
                ...
                # 1) Проверяем позиции (только position, без line_no)
                positions = []
                for ln in lines:
                    pos = getattr(ln, "position", None)
                    if pos is None:
                        raise DatabaseError("Line is missing 'position'.")
                    positions.append(int(pos))

                dup_positions = [p for p, c in Counter(positions).items() if c > 1]
                if dup_positions:
                    raise DatabaseError(f"Duplicate positions in input: {dup_positions[:10]}")

                # 2) Вставляем ядро (raw_lines)
                core_values = []
                for ln in lines:
                    block_type = self._detect_block_type(ln) or "text"
                    content = getattr(ln, "content", "") or ""
                    core_values.append(
                        {
                            "doc_id": doc_id,
                            "position": int(getattr(ln, "position")),
                ...
                for ln in lines:
                    ...
                    geometry = self._geometry_dict(ln)  # соберет из polygon/bbox
                    hierarchy = getattr(ln, "hierarchy", None)
                    if any(x is not None for x in (page_idx, block_id, geometry, hierarchy)):
                    ...
                    if self._is_image_line(block_type, ln):
                        ...
                        if not image_hash:
                            # пробуем достать из контента (![](hash.png))
                            image_hash = self._parse_image_hash_from_md(getattr(ln, "content", ""))
                        if not image_hash:
                            # нет hash – пропускаем, чтобы не падать на NOT NULL
                            continue

                        status = getattr(ln, "status", None) or "pending"
                        ...
                        if has_object_key_col:
                            row["object_key"] = getattr(ln, "object_key", None)
                        img_details_values.append(row)
                        ...
                        audio_details_values.append(
                            {
                                ...
                                "emo_primary": getattr(ln, "emo_primary", None),
                                "emo_scores": getattr(ln, "emo_scores", None),
                            }
                        )

                # 3a) Вставка doc-details пачкой
                await self._doc_repo.bulk_insert_in_session(uow.session, doc_details_values)

                # 3b) Вставка image-details пачкой с защитой от дублей в одном вызове
                if img_details_values:
                    # дедуп по line_id (на всякий случай)
                    dedup = {}
                    for d in img_details_values:
                        dedup[d["line_id"]] = d
                    img_details_values = list(dedup.values())

                    # безопасная вставка: если вдруг совпадет line_id – игнорируем дубль
                    stmt_img = (
                        pg_insert(ImageLineORM)
                        .values(img_details_values)
                        .on_conflict_do_nothing(index_elements=[ImageLineORM.line_id])
                    )
                    await uow.session.execute(stmt_img)

                # 3c) Вставка audio-details (обычная)
