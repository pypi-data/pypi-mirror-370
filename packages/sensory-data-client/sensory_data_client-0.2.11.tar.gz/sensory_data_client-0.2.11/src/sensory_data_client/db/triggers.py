...
BEGIN
    ...
    SELECT sf.extension INTO v_doc_extension
    FROM documents d
    JOIN stored_files sf ON sf.id = d.stored_file_id
    WHERE d.id = NEW.doc_id;
    
    v_doc_extension := COALESCE(v_doc_extension, 'unknown');

    -- Хеш изображения = имя файла без расширения (для дедупликации в рамках документа)
    v_image_hash := regexp_replace(v_filename, '\\.[^.]+$', '', 'g');
    -- Собираем полный путь к объекту в MinIO
    v_object_key := v_doc_extension || '/' || replace(NEW.doc_id::text, '-', '') || '/images/' || v_filename;

    -- UPSERT в lines_image. Конфликт по line_id, так как это PK.
    -- Это гарантирует, что для одной строки-картинки будет только одна запись с деталями.
    INSERT INTO lines_image (line_id, doc_id, object_key, filename, image_hash, status, attempts)
    VALUES (NEW.id, NEW.doc_id, v_object_key, v_filename, v_image_hash, 'pending', 0)
    ON CONFLICT (line_id)
    DO UPDATE SET
        ...
        updated_at = now(),
        -- Сбрасываем 'failed' задачи для повторной попытки при обновлении строки
        status = CASE WHEN lines_image.status = 'failed' THEN 'pending' ELSE lines_image.status END;

    -- Отправляем уведомление диспетчеру с ID строки, которая и является ID задачи
    PERFORM pg_notify('image_jobs', json_build_object('image_id', NEW.id::text)::text);
