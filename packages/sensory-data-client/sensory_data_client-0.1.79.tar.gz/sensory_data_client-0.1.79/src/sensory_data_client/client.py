...
class DataClient:
    ...
    def __init__(
        ...
        audio_meta_repo: AudioRepository | None = None,
...
######################## AUDIO
    async def save_audio_sentences(self, doc_id: UUID, sentences: list[AudioSentenceIn]) -> int:
        if not self.audio_meta_repo:
            raise NotImplementedError("AudioMetaRepository is not configured.")
        return await self.audio_meta_repo.replace_audio_sentences_with_meta(doc_id, sentences)