...
"""
MGQuant - a Algorithm Trading System
"""

import pkgutil

from mgquant.__main__ import cli
from mgquant.api.api_base import export_as_api
...
def load_ipython_extension(ipython):
    """call by ipython"""
    from mgquant.__main__ import inject_mod_commands
    inject_mod_commands()

    ipython.register_magic_function(run_ipython_cell, 'line_cell', 'mgquant')
...
def run(config, source_code=None):
    # [Deprecated]
    from mgquant.utils.config import parse_config
    from mgquant import main
...
def run_ipython_cell(line, cell=None):
    from mgquant.__main__ import run
    from mgquant.utils.py2 import clear_all_cached_functions
...
def run_func(**kwargs):
    from mgquant.utils import dummy_func
    from mgquant.utils.py2 import clear_all_cached_functions
    from mgquant.utils.config import parse_config
    from mgquant import main
